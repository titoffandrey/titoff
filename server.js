...
app.post('/api/analytics/ping', (req, res) => {
  ...
  //
  /* Кроме «человек ещё здесь» этот ping несёт три замера: глубину прокрутки,
   * нажатия по ссылкам наружу и скорость страницы. Своего маршрута у них нет
   * намеренно — лишний запрос на посетителя стоит дороже, чем три поля в уже
   * уходящем теле.
   *
   * Проверять их здесь незачем и НЕЛЬЗЯ ЗАБЫВАТЬ ПРОБРАСЫВАТЬ: модель сама
   * зажимает глубину в 0–100, отбрасывает невозможные замеры скорости и
   * собирает подпись нажатия из хоста. А вот поле, забытое в этой строке,
   * молча не доезжает — счётчик остаётся пустым, и по виду это неотличимо от
   * «никто не нажимал». Так и случилось при первой сборке.
   */
  if (id && !metricsSkipped(req)) {
    metrics.heartbeat({
      id, path: req.body.path, scroll: req.body.scroll, clicks: req.body.clicks, speed: req.body.speed,
      context: metrics.context(req, clientIp(req), cloudflareTrusted(req))
    });
  }
 ...
 * правку адреса.
 *
 * `total` определяет, есть ли товары для доставки. Тариф от суммы товаров
 * не зависит; настоящую сумму и цену доставки /api/order считает сам.
 */
app.post('/api/delivery/quote', (req, res) => {
  ...
  // Без выбранных товаров нет ни цены, ни срока доставки. Это штатное
  // состояние оформления, а не ошибка адреса.
  if (!Number.isFinite(goods) || goods <= 0) {
    return res.json({ ok: true, valid: false, error: '', prices: null, days: null });
  }
  ...
  const q = SHIP.quoteAll(address, goods);
 ...
 *
 * Обе двери повтора ниже — и «то же нажатие» по ключу идемпотентности, и «тот же
 * заказ» по составу — берегут покупателя от второго списания за одно и то же.
 * Заказ, по которому платить уже нельзя, беречь не от чего, а вернуть его вместо
 * нового значит запереть покупателя намертво: полчаса вышли, он собирает ту же
 * корзину заново — и раз за разом попадает на «Оформите заказ заново», сутки
 * напролёт, пока живёт окно повтора. Ровно это и приехало с боевой витрины.
 *
 * Правило одно и живёт в `lib/render.js` рядом со сроком оплаты: страница оплаты
 * и оформление обязаны одинаково понимать, что заказ закрыт, — иначе одно из них
 * будет отправлять покупателя туда, где другое его не принимает.
 */
const orderClosedForBuyer = order => R.payClosed(order, Date.now(), db.isOrderArchived(order));
...
function chatOrders(chat, limit) {
  ...
  return db.visibleOrders()
    ...
    .filter(o => (visitorId && o.visitorId === visitorId)
      || (phone && o.phone === phone)
      || (!o.visitorId && ip && o.clientIp === ip))
    .slice(0, limit || 10);
}
 ...
 *     реплика была бы тем же перебором;
 *   - разбирает номер тот же модуль, что и поле оформления, поэтому «8 996…» и
 *     «+7 996…» — это один и тот же номер и здесь, и в заказе.
 *
 * ПО НОМЕРУ ЗАКАЗА поиска нет намеренно: он шестизначный, то есть перебирается
 * целиком, и по этой же причине с витрины когда-то убрали форму отслеживания по
 ...
 *
 * 3. ПОКУПАТЕЛЮ ОНО ДОСТУПНО СРАЗУ ПОСЛЕ СОЗДАНИЯ (`visible: true`). До начала
 *    маршрута отслеживание показывает «Ожидает отправки». Менеджер при
 *    необходимости может скрыть его галочкой в форме отправления.
 *
...
app.get('/admin/analytics', (req, res) => {
   ...
   *
   * СТРАНЫ В АДРЕСЕ НЕТ — берём ту, что смотрели в прошлый раз. Отчёт по одной
   * стране открывают неделями подряд, и выбирать её заново на каждом заходе в
   * раздел значит делать руками то, что панель помнит сама. «Весь мир» приходит
   * при этом явным `geo=world` (как `geo=Worldwide` у Trends): пустой параметр
   * из адреса выпадает вовсе, и выбор мира был бы неотличим от обычного
   * открытия раздела — панель возвращала бы вчерашнюю страну поверх него.
   */
  const asked = req.query.geo;
  const report = metrics.snapshot({ days: req.query.days, geo: asked === undefined ? db.metricGeo() : asked });
  // Запоминаем код, УЖЕ нормализованный моделью: своей проверки кода в маршруте
  // быть не должно, иначе их станет две.
  db.rememberMetricGeo(report.geo);
  res.send(A.analyticsPage(settings(), db, report, req.query.reg, {
