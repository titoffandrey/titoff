...
say "Код в $PROJECT"
if [ -n "$REPO" ] && [ ! -d "$PROJECT/.git" ] && [ -n "$(ls -A "$PROJECT" 2>/dev/null)" ]; then
  # Каталог с кодом есть, а репозитория в нём нет — так выглядит сервер, который
  # выкатывали заливкой дерева (обычный путь). `git clone` в непустой каталог
  # падает со своей невнятной ошибкой, поэтому объясняем прямо: либо выкатывать
  # тем же способом, что и раньше, либо осознанно убрать каталог.
  echo "ОШИБКА: $PROJECT уже содержит код, но не является git-репозиторием." >&2
  echo "  Сервер выкатывали заливкой рабочего дерева — повторите так же:" >&2
  echo "    ./deploy/install.sh <алиас> <домен>        (без git-url)" >&2
  echo "  Либо уберите каталог руками, если правда нужен clone." >&2
  exit 1
