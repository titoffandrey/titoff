...
const source = path.join(root, 'vendor', 'bklit');
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'store-bklit-build-'));
...
try {
  ...
  if (verify) {
    for (const filename of ['bklit-charts.js', 'bklit-charts.css', 'bklit-charts.LICENSE.txt']) {
