...
//
// Значение бывает и СПИСКОМ: одно слово площадки у разных товаров означает
// разные наши цвета — «белый» у AirPods это «Белый», а у iPhone 15 Pro Max
// «Белый титан». Берётся первый из списка, который у товара есть; порядок
// внутри списка значим только там, где у товара есть оба имени.
const COLOR_ALIASES = {
  'silver': 'Серебристый',
  ...
  // iPhone 15 Pro и Pro Max — тоже титан, но синий бывает только у них.
  // Продавцы пишут корпус как попало: «титан», «серый», «тёмно-синий» — всё
  // это у 15-й серии один из четырёх титанов. «Тёмно-синий» у 17 Pro — это
  // «Глубокий синий», поэтому здесь список: берётся то, что есть у товара.
  'blue titanium': 'Синий титан',
  'титан': 'Натуральный титан',
  'серый': 'Натуральный титан',
  'темно-синий': ['Синий титан', 'Глубокий синий'],
  ...
  // MacBook Air. Цвета те же, что у часов SE и AirPods Max, только площадка
  // называет их по-английски.
  'midnight': 'Полуночный',
  'starlight': 'Сияющая звезда',
  // Apple Watch Series 11. У алюминия площадка пишет цвет полностью, у титана —
  // одним словом («Gold», «Natural», «Slate»), а титан дописан уже в нашем
  // названии. Британское «Grey» встречается наравне с «Gray».
  'rose gold': 'Розовое золото',
  'jet black': 'Глянцевый чёрный',
  'space gray': 'Серый космос',
  'space grey': 'Серый космос',
  'natural': 'Натуральный титан',
  'gold': 'Золотой титан',
  'slate': 'Сланцевый титан',
  'светло-розовый': 'Румяный',
  'желтый': 'Цитрусовый',
  'горчичный': 'Цитрусовый',
  // «Синий» и «чёрный» — у Neo это Индиго и Полуночный, у 15 Pro Max — титаны.
  'синий': ['Индиго', 'Синий титан'],
  ...
  'черный': ['Полуночный', 'Чёрный титан'],
...
function ourColorName(sourceColor, configColor) {
  ...
  // которого магазин не продаёт, хуже, чем оставить исходный. Из списка
  // кандидатов побеждает первый, который у товара есть.
  const candidates = Array.isArray(mapped) ? mapped : (mapped ? [mapped] : []);
  const hit = candidates.find(m => ourColors.some(c => sameColor(c, m)));
  if (hit) return hit;
