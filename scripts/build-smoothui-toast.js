...
const source = path.join(root, 'vendor', 'smoothui');
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'store-smoothui-build-'));
...
try {
  ...
  if (verify) {
    for (const filename of ['smoothui-toast.js', 'smoothui-toast.css', 'smoothui-toast.LICENSE.txt']) {
