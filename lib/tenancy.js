...
function viewFor(product, site, rating) {
  ...
  const v = {
    ...
    // Представление собирается по полям поимённо, поэтому подпись наличия надо
    // переносить явно — иначе она есть в хранилище, но на витрину не доезжает.
    stockLevel: product.stockLevel || 'in',
    imageColors: product.imageColors || {},
    colors: product.colors || [],
    // forChoice — «доступно только при таком выборе в другой группе» (8 ТБ бывает
    // лишь с M5 Max). Представление пересобирает варианты, поэтому привязку надо
    // переносить явно: иначе она есть в хранилище, но на витрину не доезжает.
    storages: (product.storages || []).map(s => Object.assign(
      { label: s.label, add: round(Number(s.add || 0) * m), inStock: s.inStock !== false },
      s.forChoice ? { forChoice: s.forChoice } : null)),
