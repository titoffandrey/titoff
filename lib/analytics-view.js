...
function lineChart(points, options) {
  const o = options || {};
  const data = { labels: (points || []).map(p => String(p.note || p.label || '')), series: [{ name: 'Посетители', data: (points || []).map(p => p.future ? null : Math.max(0, Number(p.value) || 0)) }] };
  const id = 'metric-chart-' + createHash('sha256').update(o.aria || 'traffic').digest('hex').slice(0, 12);
  return `<div class="ta ta-analytics-chart" id="${id}" data-live-key="${id}" data-ta-chart="area" data-ta-config="${esc(JSON.stringify(data))}" role="group" aria-label="${esc(o.aria || 'График посещаемости')}"><div class="ta-chart-canvas" data-ta-chart-canvas data-live-key="${id}-canvas"></div><div class="ta-chart-fallback">${lineChartFallback(points, options)}</div></div>`;
}

function lineChartFallback(points, options) {
  const o = options || {};
...
function statCard(item) {
  return TA.render('metric', { ICON: item.icon ? CI.icon(item.icon) : '', LABEL: esc(item.label), VALUE: item.value + (item.trend || ''), DETAIL: item.note || '' }).replace('ta ta-metric', 'ta ta-metric metric-card' + (item.live ? ' metric-live' : '') + (item.idle ? ' is-idle' : ''));
}
...
function dashboard(snapshot, options) {
  ...
  <div class="metric-grid-secondary" id="metric-sales">
    ${funnelPanel(s)}
