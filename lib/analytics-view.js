 ...
 *
 * Цена инлайна — 51 КБ разметки (11 КБ по проводу), и она платится осознанно:
...
function russiaMap(items) {
  ...
  const rows = top.length ? top.map((row, i) => `<div class="rm-rank-row">
    <span class="rm-rank-num">${i + 1}</span><b>${esc(row[0])}</b><span>${n(row[1])}</span>
  ...
  return `<div class="ru-map">
    ...
    <div class="rm-ranking">${rows}${top.length ? `<div class="rm-rank-foot">Субъекты: 1–${top.length} из ${ranking.length}</div>` : ''}</div>
  </div>`;
}
...
function dashboard(snapshot, options) {
  ...
  <section class="a-panel metric-panel metric-russia trends-panel"><div class="metric-panel-head trends-head"><div><h2>Посетители по регионам <span class="trends-help" title="Уникальные посетители по субъектам РФ · зарубежные заходы в карту не попадают">?</span></h2></div><span class="trends-scope">Субъекты РФ</span></div>${russiaMap(s.regions || [])}</section>
