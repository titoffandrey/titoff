...
function bars(items, className) {
  ...
  return items.length ? `<div class="metric-bars ${className || ''}">${items.map(x => `<div class="metric-bar-row">
    <div class="metric-bar-label"><span>${esc(x.label)}</span><b>${n(x.value)}</b></div>
...
function dashboard(snapshot, options) {
  ...
  <div class="metric-grid-tertiary">
    <section class="a-panel metric-panel"><div class="metric-panel-head"><div><span class="metric-kicker">География</span><h2>Города</h2></div></div>${bars(snapshot.locations || [])}</section>
