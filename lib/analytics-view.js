...
const { ONLINE_MS, VISITORS_PER_PAGE, BOUNCE_SECONDS, sessionsOf } = require('./analytics');
...
// Переводчик названий регионов — тот же, что пишет их в карточку посетителя:
// старые карточки хранят латиницу, и сводить её с картой должен он, а не вторая
// таблица написаний рядом.
...
function dayLabel(date, opts) {
  // Это уже календарный день магазина, а не момент времени. Сдвиг +03:00 с
  // последующим форматированием на UTC-сервере превращал 27 июля в 26 июля.
  const d = new Date(String(date) + 'T00:00:00Z');
  if (Number.isNaN(d.getTime())) return String(date || '');
  return d.toLocaleDateString('ru-RU', Object.assign({ day: '2-digit', month: 'short' }, opts || {}, { timeZone: 'UTC' }));
}
...
function monthLabel(date) {
  const d = new Date(String(date) + 'T00:00:00Z');
  if (Number.isNaN(d.getTime())) return String(date || '');
  return d.getUTCMonth() === 0 ? String(d.getUTCFullYear()) : d.toLocaleDateString('ru-RU', { month: 'long', timeZone: 'UTC' });
}
...
function bars(items, options) {
  const o = options || {};
  // `note` — строка под полосой: у страницы это глубина прокрутки, у ступени
  // воронки — доля от предыдущей. Пустая не рисуется вовсе, поэтому ряды без
  // неё выглядят ровно так же, как раньше.
  const list = (items || []).map(x => ({ label: String(x.label == null ? '' : x.label), value: Number(x.value) || 0, note: x.note ? String(x.note) : '' }));
  ...
  return `<div class="metric-bars${iconOf ? ' has-ico' : ''}${o.className ? ' ' + o.className : ''}">${list.map(x => {
    ...
    return `<div class="metric-bar-row" style="--metric-share:${Math.max(0, Math.min(100, x.value / max * 100)).toFixed(1)}%">
    ...
    <div class="metric-bar-track"><i style="width:${Math.max(0, Math.min(100, x.value / max * 100)).toFixed(1)}%"></i></div>
    ${x.note ? `<small class="metric-bar-note">${esc(x.note)}</small>` : ''}
  </div>`;
  }).join('')}</div>`;
}
...
function lineChart(points, options) {
  const o = options || {};
  // Будущие интервалы остаются на оси времени, но не становятся нулевыми
  // значениями. Незаконченный интервал показываем отдельно пунктиром.
  const list = (points || []).map(p => ({
    label: String(p.label == null ? '' : p.label), value: Math.max(0, Number(p.value) || 0),
    note: String(p.note || ''), future: !!p.future, partial: !!p.partial
  }));
  const available = list.filter(p => !p.future);
  if (!available.some(p => p.value > 0)) return `<p class="metric-empty">${esc(o.empty || 'График появится после первых посещений')}</p>`;
  const max = niceMax(Math.max(...available.map(p => p.value)));
  const last = list.length - 1;
  const x = i => list.length === 1 ? 50 : i / last * 100;
  const y = value => 100 - value / max * 100;
  const pts = list.map((p, i) => ({ x: x(i), y: y(p.value), ...p })).filter(p => !p.future);
  const completed = pts.filter(p => !p.partial);
  const line = smoothPath(completed);
  const partial = pts.some(p => p.partial) ? smoothPath(pts.slice(Math.max(0, completed.length - 1))) : '';
  const area = completed.length > 1 ? line + ` L${completed.at(-1).x.toFixed(2)},100 L${completed[0].x.toFixed(2)},100 Z` : '';
  ...
  const hits = list.map((p, i) => {
    ...
    const edge = x(i) <= 25 ? ' at-start' : (x(i) >= 75 ? ' at-end' : '');
    const note = (p.note || p.label) + (p.partial ? ' · данные неполные' : '');
    return `<button type="button" class="mc-hit" aria-label="${esc(note)}: ${n(p.value)} ${plural(p.value, 'посетитель', 'посетителя', 'посетителей')}" style="left:${before.toFixed(2)}%;width:${(after - before).toFixed(2)}%">
      <i class="mc-guide" style="left:${local.toFixed(2)}%"></i>
      <i class="mc-dot" style="left:${local.toFixed(2)}%;top:${y(p.value).toFixed(2)}%"></i>
      <span class="mc-tip${edge}" style="left:${local.toFixed(2)}%;top:${y(p.value).toFixed(2)}%"><b>${esc(note)}</b><span><i></i><em>Посетители</em><strong>${n(p.value)}</strong></span></span>
    </button>`;
  }).join('');
  let shown = '';
  const marks = list.map((p, i) => {
    if (i % step !== 0 && i !== last || p.label === shown) return '';
    shown = p.label;
    const edge = i === 0 ? ' at-start' : (i === last ? ' at-end' : '');
    return `<span class="mc-x${edge}" style="left:${x(i).toFixed(2)}%">${esc(p.label)}</span>`;
  }).join('');
  const peak = pts.reduce((a, p) => p.value > a.value ? p : a, pts[0]);
  const peakMark = list.length > 2 ? `<span class="mc-peak" style="left:${peak.x.toFixed(2)}%;top:${peak.y.toFixed(2)}%">${n(peak.value)}</span>` : '';
  const current = pts.at(-1);
  const currentMark = current && (current.partial || pts.length === 1)
    ? `<span class="mc-current" style="left:${current.x.toFixed(2)}%;top:${current.y.toFixed(2)}%" aria-hidden="true"></span>` : '';
  const axis = [];
  for (let i = AXIS_STEPS; i >= 0; i--) axis.push(max / AXIS_STEPS * i);
  return `<div class="metric-chart" role="group" aria-label="${esc(o.aria || 'График посещаемости')}">
    ...
    <div class="mc-plot">
      <svg class="mc-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true" focusable="false">
        ${area ? `<path class="mc-area" d="${area}"/>` : ''}
        <path class="mc-line" d="${line}" fill="none" stroke="#7868d8" stroke-width="2.4" stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke"/>
        ${partial ? `<path class="mc-line is-partial" d="${partial}" fill="none" stroke="#7868d8" stroke-width="2.4" stroke-dasharray="4 5" stroke-linecap="round" vector-effect="non-scaling-stroke"/>` : ''}
      </svg>
      ${peakMark}${currentMark}${hits}
    </div>
    <div class="mc-marks">${marks}</div>
    <div class="mc-legend"><span><i></i>Посетители</span>${partial ? '<span class="mc-legend-partial"><i></i>Неполный интервал</span>' : ''}</div>
  </div>`;
}
...
/* «Весь мир» уезжает в адрес ЯВНЫМ словом, а не пустым параметром: пустые
 * значения `href()` выбрасывает вовсе, и ссылка на мир выглядела бы как обычное
 * открытие раздела. Отличить их обязан маршрут — он помнит последнюю выбранную
 * страну и без явного слова возвращал бы её поверх только что выбранного мира.
 *
 * Слово то же, что у Trends (`geo=Worldwide`), и моделью оно читается как мир
 * само собой: код страны там ровно две буквы, всё остальное — «весь мир».
 */
const WORLD = 'world';
 ...
 */
function shapesFor(map, items, code) {
  const byKey = new Map();
  ...
  for (const item of items || []) {
    ...
    /* Второй заход — через переводчик геобазы. Карточки, записанные до того,
     * как он выучил очередное написание, хранят регион ЛАТИНИЦЕЙ («Chuvashia»,
     * «Primorskiy (Maritime) Kray»), а у карты России латинских имён нет вовсе:
     * её контуры собраны из другого источника и подписаны по-русски. На боевых
     * данных так мимо карты проходило шесть субъектов. */
    const hit = byKey.get(GEO.matchKey(label))
      || byKey.get(GEO.matchKey(regionMapName(GEOIP.regionRu(label, code))));
...
function mapStage(shapes, options) {
   ...
   *
   * `data-live-key` называет, ЧЬЯ это карта. Приближённый кадр принадлежит
   * человеку, и живое обновление его не трогает (`ownedByUser` в admin-live.js),
   * — но у другой страны карта другая, и оставленный от прежней кадр показал бы
   * её кусок неизвестно где. С ключом узел не переписывается, а заменяется
   * целиком: сменил страну — получил её карту в её же исходном кадре. */
  return `<div class="gm-stage" data-map${o.key ? ` data-live-key="${esc(o.key)}"` : ''} data-home="${esc(o.viewBox)}">
...
function worldBoard(s, o) {
  ...
  return {
    stage: mapStage(shapes, { key: 'gm-world', viewBox: world.viewBox, aria: 'Посетители по странам мира' }),
...
function countryBoard(s, o) {
  ...
  const parsed = map ? shapesFor(map, s.regions || [], code) : null;
  ...
  if (map && (parsed.values.size || parsed.extra.length)) {
    ...
    return {
      stage: mapStage(shapes, { key: 'gm-' + code, viewBox: map.viewBox, aria: 'Посетители по регионам: ' + GEO.countryName(code) }),
  ...
  if (map) {
    ...
    return {
      stage: mapStage(shapes, { key: 'gm-' + code, viewBox: map.viewBox, aria: 'Посетители: ' + GEO.countryName(code) }),
  ...
  return {
    stage: mapStage(shapes, { key: 'gm-near-' + code, viewBox, aria: 'Посетители: ' + GEO.countryName(code) }),
    rank: ranking(cities, cityRank),
    note: 'страна'
  };
}
...
function trend(now, before, options) {
  ...
  if (!diff) return '';
  const up = diff > 0;
  const positive = o.lowerIsBetter ? !up : up;
  return `<span class="metric-trend ${positive ? 'is-up' : 'is-down'}" title="${esc((o.title || 'к прошлому периоду'))}">${up ? '↑' : '↓'} ${Math.abs(diff)}%</span>`;
}

// Тот же расчёт, что у `trend()`, но числом — для плашки графиков Bklit.
function trendValue(now, before) {
  const a = Number(now) || 0; const b = Number(before) || 0;
...
/* ---------------------------- Острова графиков Bklit -------------------------
 *
 * Графики «Метрики» рисуют исходные компоненты Bklit UI (vendor/bklit, собраны
 * в public/bklit-charts.*). Сервер отдаёт им данные атрибутом `data-bk-props`, а
 * внутрь острова кладёт прежнюю разметку — она остаётся там, где скрипт не
 * загрузился, и её же читают тесты. Подписи точек считает сервер: у Bklit даты
 * зашиты в en-US, а панель русская и время у неё московское.
 */
function island(kind, props, fallback, options) {
  const o = options || {};
  return `<${o.tag || 'div'} class="bk-island${o.className ? ' ' + o.className : ''}" data-bk="${esc(kind)}" data-bk-props="${esc(JSON.stringify(props))}"${o.key ? ` data-live-key="${esc(o.key)}"` : ''}><div class="bk-fallback">${fallback}</div></${o.tag || 'div'}>`;
}

// Плитка сводки: значок, подпись, число и строка пояснения под ним.
function statCard(item) {
  const inner = `<span>${item.icon ? `<i class="metric-card-ico">${CI.icon(item.icon)}</i>` : ''}${esc(item.label)}</span>
    <strong>${item.live ? '<i></i>' : ''}${item.value}${item.trend || ''}</strong>
    <small>${item.note || ''}</small>`;
  const cls = `metric-card${item.live ? ' metric-live' : ''}${item.idle ? ' is-idle' : ''}`;
  if (!item.bk) return `<article class="${cls}">
    ${inner}
  </article>`;
  return island('stat', Object.assign({ title: item.label, note: item.plain || '' }, item.bk), inner, { tag: 'article', className: cls, key: 'bk-stat-' + item.icon });
}
...
/* Подпись глубины прокрутки под строкой страницы.
 *
 * Отвечает на вопрос, ради которого чаще всего и включают вебвизор: долистывают
 * ли до кнопки покупки. Замеров может не быть вовсе — страницу открывали до
 * появления счётчика, — и тогда строка остаётся без подписи, а не получает
 * честный на вид ноль.
 */
function scrolled(pages) {
  return (pages || []).map(row => (row && row.scroll
    ? Object.assign({}, row, { note: 'долистывают до ' + pct(row.scroll) })
    : row));
}

/* ------------------------------- Путь к заказу -------------------------------
 *
 * Воронка отвечает на единственный вопрос, ради которого метрику магазина
 * открывают каждый день: где теряются покупатели. Данные для неё лежали и
 * раньше — ступени считаются по тем же адресам, что и просмотры, — а показать
 * их было негде, и отчёт заканчивался на «сколько было народу».
 *
 * У каждой ступени доля от ПРЕДЫДУЩЕЙ: воронку читают именно так — «из тех, кто
 * открыл карточку, до оформления дошёл каждый пятый». Доля от заходов у нижних
 * ступеней быстро становится неразличимо малой и на вопрос «где обрыв» не
 * отвечает вовсе.
 *
 * Рядом с заголовком стоят отказы. Это ровно те заходы, что оборвались, не начав
 * воронку, и объяснять её без них значило бы прятать половину ответа.
 */
function funnelPanel(s) {
  /* Подпись ступени называет и переход, и ПОТЕРЮ. Вопрос к воронке задают
   * именно про потерю — «сколько отвалилось и где», — а раньше её приходилось
   * вычитать в уме из соседних чисел. */
  const rows = (s.funnel || []).map(row => Object.assign({}, row, {
    note: row.step == null ? ''
      : 'дошли ' + pct(row.step) + ' с прошлого шага' + (row.lost ? ' · ушли здесь ' + n(row.lost) : '')
  }));
  // Заходов не было — показывать нечего, и пустая воронка из четырёх нулей
  // читалась бы как поломка отчёта, а не как тихий день.
  if (!rows.length || !rows[0].value) return '';
  const prev = s.prev || {};
  const prevRate = prev.visits ? Math.round((Number(prev.bounces) || 0) / prev.visits * 1000) / 10 : 0;
  /* Что такое отказ, сказано подсказкой, а не строкой под воронкой: определение
   * читают один раз, а видят при каждом заходе в отчёт. Тот же приём, что у
   * вопросительного знака рядом с заголовком графика. */
  const bounce = `отказов ${pct(s.bounceRate)}${prevRate ? ' ' + trend(s.bounceRate, prevRate, { title: 'к прошлому периоду', lowerIsBetter: true }) : ''}`;
  let hint = `Отказ — заход, где открыли одну страницу и ушли, не задержавшись и на ${BOUNCE_SECONDS} секунд`;
  /* Счётчик ступеней моложе счётчика заходов, и на длинном периоде в числителе
   * стоят двое суток, а в знаменателе тридцать. Панель говорит об этом прямо:
   * иначе она уверенно называет потерю, которой не было («ушли здесь 9233»).
   * Само пройдёт, когда счётчик догонит срок хранения. */
  const since = s.funnelSince ? `<small class="metric-note">Ступени считаются с ${esc(R.formatDate(Date.parse(s.funnelSince + 'T00:00:00Z')))} — до этого дня в отчёте только заходы.</small>` : '';
  return `<section class="a-panel metric-panel metric-funnel"><div class="metric-panel-head"><div><span class="metric-kicker">Продажи</span><h2>Путь к заказу</h2></div><span title="${esc(hint)}">${bounce}</span></div>
    ${island('funnel', {
    stages: rows.map(row => ({ label: String(row.label), value: Number(row.value) || 0, note: row.note || '' }))
  }, bars(rows, { of: rows[0].value }), { key: 'bk-funnel' })}
    ${since}
  </section>`;
}
 ...
 * СТРАНИЦЕ. Разница денежная: «387 открыли товар, до оформления дошли 63» не
 * говорит, оборвалось это на карточке конкретного Mac, на выборе доставки или
 * на странице оплаты, — а чинят каждое по-своему.
 *
 * У ряда три числа, и каждое отвечает на свой вопрос:
 *   · сколько ВИЗИТОВ здесь оборвалось — сколько раз страница была последней;
 *   · доля выходов — из скольких её открытий это случилось. Одного счётчика
 *     мало: главную открывают все, и она будет первой всегда просто по объёму;
 *   · сколько из них НЕ ВЕРНУЛИСЬ — про кого уже можно так сказать.
 *
 * Про «не вернулись» блок говорит прямо, а не молчит: у ушедших меньше суток
 * назад ответа ещё нет, и они считаются отдельно, а не записываются в
 * вернувшихся. За «Сегодня» таких почти все — это правда о сегодняшнем дне, а
 * не поломка отчёта, и подпись обязана это сказать.
 */
function exitsPanel(s, products) {
  const x = s.exits || {};
  const rows = x.rows || [];
  if (!rows.length) return '';
  const list = rows.map(row => Object.assign({}, row, {
    note: [
      row.rate == null ? '' : 'выход с ' + pct(row.rate) + ' открытий',
      row.gone ? 'не вернулись ' + n(row.gone) : ''
    ].filter(Boolean).join(' · ')
  }));
  /* Подпись у заголовка — про ВЕСЬ период, а не про верхнюю строку: сумма
   * «не вернулись» по показанным двенадцати строкам меньше настоящей, а
   * подписывать ею блок значило бы занизить потерю. */
  const head = x.lost
    ? n(x.lost) + ' ' + plural(x.lost, 'не вернулся', 'не вернулись', 'не вернулись')
    : (x.fresh ? 'ушли только что — судить рано' : '');
  const hint = 'Не вернулся — тот, чей последний заход кончился здесь и кто с тех пор сутки не заходил.'
    + (x.fresh ? ' Ещё ' + n(x.fresh) + ' ушли меньше суток назад, про них говорить рано.' : '');
  return `<section class="a-panel metric-panel metric-exits"><div class="metric-panel-head"><div><span class="metric-kicker">Продажи</span><h2>Где обрывается визит</h2></div><span title="${esc(hint)}">${esc(head)}</span></div>
    ${bars(list, { of: x.visits, icon: 'page', labelOf: p => pageLabel(p, products || {}) })}
  </section>`;
}

/* ------------------------------- Куда уходят ---------------------------------
 *
 * Витрина ведёт наружу ровно в мессенджеры и к контактам: Telegram и WhatsApp
 * стоят и в меню, и в подвале, и на «О компании», — а сколько по ним нажимают,
 * не знал никто. У Метрики это `trackLinks`.
 *
 * Значков у списка нет ни одного, и это осознанно: свой знак есть у Telegram, а
 * у «Звонка» и «Почты» пришлось бы ставить общий — глиф у одной строки из
 * четырёх читался бы как поломка ряда. То же правило, что у категорий в меню
 * витрины.
 *
 * Панель стоит отдельной строкой, а не половиной пары: она появляется только
 * при нажатиях, и в двухколоночной сетке одинокая карточка оставляла бы рядом
 * полосу пустоты во всю ширину экрана.
 */
function outboundPanel(rows) {
  const list = (rows || []).filter(row => row && row.value);
  if (!list.length) return '';
  const total = list.reduce((sum, row) => sum + row.value, 0);
  return `<section class="a-panel metric-panel metric-outbound"><div class="metric-panel-head"><div><span class="metric-kicker">Контакты</span><h2>Куда уходят</h2></div><span>сколько раз нажимали</span></div>${bars(list, { of: total })}</section>`;
}

/* --------------------------- Скорость у покупателей --------------------------
 *
 * Весь проект вылизан под скорость — спрайты, снятие комментариев на отдаче,
 * уменьшенные копии фото, — а измерений с настоящих телефонов не было ни одного:
 * PageSpeed меряет лабораторию, а не покупателя из Ноябрьска на 4G.
 *
 * У каждого показателя стоят СРАЗУ И среднее, И разбивка «хорошо / терпимо /
 * плохо» по официальным порогам Web Vitals. Одного среднего мало: оно прячет
 * хвост, а хвост и есть та часть покупателей, у которых витрина открывается
 * медленно. Одних долей тоже мало — по ним не сказать, насколько медленно.
 *
 * Полоса трёхцветная, и цвета взяты уже принятые в проекте: зелёный подписи
 * наличия, оранжевый очереди модерации, красный плашки акции. Своей палитры
 * ради одного блока заводить незачем.
 */
const SPEED_VIEW = {
  lcp: {
    label: 'Отрисовка главного',
    hint: 'Largest Contentful Paint — через сколько покупатель видит главное на странице. Хорошо — до 2,5 секунды',
    format: value => (value / 1000).toFixed(1).replace('.', ',') + ' с'
  },
  cls: {
    label: 'Скачки вёрстки',
    hint: 'Cumulative Layout Shift — насколько содержимое прыгает, пока страница грузится. Хорошо — до 0,1',
    format: value => (value / 1000).toFixed(2).replace('.', ',')
  },
  ttfb: {
    label: 'Ответ сервера',
    hint: 'Time to First Byte — сколько ждут первый байт ответа. Хорошо — до 0,8 секунды',
    format: value => n(value) + ' мс'
  }
};

function speedPanel(rows) {
  const list = (rows || []).filter(row => row && SPEED_VIEW[row.name] && row.count);
  // Замеров нет вовсе — панели нет: три строки нулей у магазина, который ещё не
  // открывали ни в одном браузере, читались бы как поломка.
  if (!list.length) return '';
  const measured = Math.max(...list.map(row => Number(row.count) || 0));
  const body = list.map(row => {
    const view = SPEED_VIEW[row.name];
    const total = Math.max(1, row.good + row.ok + row.poor);
    const part = value => Math.round((value / total) * 1000) / 10;
    const seg = (value, cls) => (value ? `<i class="${cls}" style="width:${part(value)}%"></i>` : '');
    return `<div class="speed-row">
      <div class="speed-label"><span title="${esc(view.hint)}">${esc(view.label)}</span><b>${esc(view.format(row.average))}</b></div>
      <div class="speed-track">${seg(row.good, 'is-good')}${seg(row.ok, 'is-ok')}${seg(row.poor, 'is-poor')}</div>
      <small>хорошо у ${esc(pct(part(row.good)))} · ${n(row.count)} ${plural(row.count, 'замер', 'замера', 'замеров')}</small>
    </div>`;
  }).join('');
  return `<section class="a-panel metric-panel metric-speed"><div class="metric-panel-head"><div><span class="metric-kicker">Скорость</span><h2>Как грузится у покупателей</h2></div><span>${n(measured)} ${plural(measured, 'замер', 'замера', 'замеров')}</span></div>
    ${island('speed', {
    measured,
    rows: list.map(row => ({ label: SPEED_VIEW[row.name].label, good: Number(row.good) || 0, total: (Number(row.good) || 0) + (Number(row.ok) || 0) + (Number(row.poor) || 0), avg: SPEED_VIEW[row.name].format(row.average) }))
  }, `<div class="speed-rows">${body}</div>`, { key: 'bk-speed' })}
  </section>`;
}

/* ---------------- Блокировки и исключения: список под раскрытием -------------
 *
 * Заводят их в карточке посетителя — там видно, кого закрывают. Здесь список
 * нужен для обратного: СНЯТЬ правило. Карточку вытесняет срок хранения (365
 * дней) и потолок в 10 000 записей, а правило живёт своим файлом и переживает
 * её — без этого списка разблокировать человека было бы уже нечем.
 *
 * Разметка — та же `<details class="set">`, что у ботов и у разделов настроек:
 * правило «редкое прячем под раскрытие» в панели одно. Свёрнутая строка сама
 * называет числа, поэтому раскрывать её ради ответа «а есть ли блокировки?» не
 * нужно.
 */
const RULE_KINDS = {
  block: { one: 'заблокирован', many: 'Заблокировано' },
  skip: { one: 'не учитывается в метрике', many: 'Не учитываются' }
};

function ruleSignals(rule) {
  const chip = (label, value, title) => `<span class="rule-sig"${title ? ` title="${esc(title)}"` : ''}>${esc(label)}<b>${esc(value)}</b></span>`;
  const out = [];
  // Метка — до восьми знаков, как в строке посетителя: полные 32 знака в списке
  // ничего не добавляют, а строку разрывают.
  if (rule.ids.length) out.push(chip('метка', rule.ids.map(id => id.slice(0, 8)).join(', '), 'Cookie метрики: точна до браузера'));
  if (rule.ips.length) out.push(chip('адрес', rule.ips.join(', '), 'За одним адресом бывает целая квартира или офис'));
  if (rule.prints.length) out.push(chip('устройство', rule.prints.join(', '), 'Отпечаток: экран, платформа, часовой пояс, язык и техника'));
  return out.join('');
}

function rulesFold(rules, options) {
  const o = options || {};
  const list = Array.isArray(rules) ? rules : [];
  const counts = { block: 0, skip: 0 };
  for (const rule of list) counts[rule.kind]++;
  const note = list.length
    ? Object.entries(counts).filter(([, count]) => count).map(([kind, count]) => `${RULE_KINDS[kind].many}: ${n(count)}`).join(' · ')
    : 'никого не блокировали и не исключали';
  const rows = list.map(rule => {
    const hits = rule.hits && rule.hits.count
      ? `${n(rule.hits.count)} ${plural(rule.hits.count, 'срабатывание', 'срабатывания', 'срабатываний')} · ${esc(ago(rule.hits.at, Date.now()))}`
      : 'после перезапуска не срабатывало';
    return `<li class="rule-row is-${esc(rule.kind)}">
      <div class="rule-main"><b>${esc(rule.note || 'Без подписи')}</b><span class="rule-kind">${esc(RULE_KINDS[rule.kind].one)} · с ${esc(dateTime(rule.createdAt))}</span><span class="rule-sigs">${ruleSignals(rule)}</span></div>
      <div class="rule-side"><span class="rule-hits">${hits}</span>
        <form method="post" action="${esc(o.base || '/admin/analytics')}/rules/${esc(rule.id)}/delete" data-confirm="Снять правило «${esc(rule.note || rule.id)}»?"><button class="btn btn-sm">Снять</button></form>
      </div></li>`;
  }).join('');
  return `<details class="set metric-rules">
    <summary>
      <span class="set-ico">${CI.icon('ban')}</span>
      <span class="set-cap"><h3>Блокировки и исключения</h3><span class="set-note">${esc(note)}</span></span>
      <svg class="set-chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="m9.5 5.5 7 6.5-7 6.5"/></svg>
    </summary>
    <div class="set-body">
      ${rows ? `<ul class="rule-list">${rows}</ul>` : '<p class="metric-empty">Заводятся они в карточке посетителя: там видно, кого именно закрывают — город, техника, страницы и заказы.</p>'}
    </div>
  </details>`;
}
...
function dashboard(snapshot, options) {
  ...
  const snapshotTime = Number(s.generatedAt) || Date.now();
  const moscowNow = new Date(snapshotTime + 3 * 3600000);
  const currentHour = moscowNow.getUTCHours();
  const currentDate = moscowNow.toISOString().slice(0, 10);
  const chart = byHour
    ? lineChart((s.hourly || []).map(h => ({
      label: String(h.hour).padStart(2, '0'),
      value: h.visitors,
      note: String(h.hour).padStart(2, '0') + ':00',
      future: Number(h.hour) > currentHour,
      partial: Number(h.hour) === currentHour
    ...
    : lineChart((byWeek ? s.weekly : (s.daily || [])).map(d => ({
      ...
      value: d.visitors,
      future: d.date > currentDate,
      partial: byWeek ? d.date <= currentDate && d.endDate >= currentDate : d.date === currentDate,
      note: byWeek
  ...
  /* Тот же ряд — для графика Bklit. Будущие часы уходят отдельно: у Bklit они
   * продолжают ось, но не рисуются нулями; незаконченный интервал — пунктиром. */
  const trafficRows = byHour
    ? (s.hourly || []).map(h => {
      const hh = String(h.hour).padStart(2, '0') + ':00';
      return { d: currentDate, h: Number(h.hour), v: Number(h.visitors) || 0, s: hh, l: 'Сегодня, ' + hh, future: Number(h.hour) > currentHour, partial: Number(h.hour) === currentHour };
    })
    : (byWeek ? s.weekly : (s.daily || [])).map(d => ({
      d: d.date, v: Number(d.visitors) || 0,
      s: byWeek ? monthLabel(d.date) : dayLabel(d.date),
      l: byWeek ? dayLabel(d.date, { month: 'long' }) + ' — ' + dayLabel(d.endDate, { month: 'long' }) : dayLabel(d.date, { month: 'long' }),
      future: d.date > currentDate,
      partial: byWeek ? d.date <= currentDate && d.endDate >= currentDate : d.date === currentDate
    }));
  const trafficPoint = r => ({ d: r.d, h: r.h, v: r.v, s: r.s, l: r.l + (r.partial ? ' · данные неполные' : '') });
  const trafficNow = trafficRows.filter(r => !r.future);
  const traffic = trafficNow.length > 1 && trafficNow.some(r => r.v > 0)
    ? island('traffic', {
      k: chartScale,
      series: 'Посетители',
      ticks: byHour ? 7 : byWeek ? 4 : (days > 14 ? 6 : 7),
      partial: trafficNow.some(r => r.partial),
      points: trafficNow.map(trafficPoint),
      // Ось дня кончается полуночью следующих суток («24:00»): 24 часа делятся
      // на шесть ровных шагов по 4 часа, а 23 — ни на что.
      future: trafficRows.filter(r => r.future).map(r => ({ d: r.d, h: r.h, s: r.s, l: r.l }))
        .concat(byHour ? [{ d: currentDate, h: 24, s: '24:00', l: 'Сегодня, 24:00' }] : [])
    }, chart, { key: 'bk-traffic' })
    : chart;

  /* Искорки у плиток — только у периода длиннее суток: у «Сегодня» по часам
   * есть одни посетители, а плитки одного ряда обязаны быть одной высоты. */
  const sparkRows = days > 1 ? (s.daily || []).filter(d => d.date <= currentDate) : [];
  const spark = value => sparkRows.length > 1
    ? sparkRows.map(d => ({ d: d.date, v: value(d), s: dayLabel(d.date), l: dayLabel(d.date, { month: 'long' }) }))
    : undefined;
  const trendTitle = 'к ' + was;
  ...
  const cards = [
    {
      ...
      bk: { value: Number(s.online) || 0, live: true }
    },
    {
      ...
      bk: { value: Number(s.unique) || 0, trend: trendValue(s.unique, prev.visitors), trendTitle, spark: spark(d => Number(d.visitors) || 0) }
    },
    {
      ...
      bk: { value: Number(s.averageSeconds) || 0, format: 'duration', trend: trendValue(s.averageSeconds, prevAverage), trendTitle, spark: spark(d => d.visits ? Math.round((Number(d.activeSeconds) || 0) / d.visits) : 0) }
    },
    {
      ...
      bk: { value: Number(s.orders) || 0, trend: trendValue(s.orders, prev.orders), trendTitle, spark: spark(d => Number(d.orders) || 0) }
    }
  ].map(card => Object.assign(card, { plain: card.note })).map(statCard).join('');
  ...
  /* Страну несёт КАЖДАЯ ссылка отчёта, и мир в ней назван словом. Раздел помнит
   * последнюю выбранную страну, а «geo» с пустым значением из адреса выпадает —
   * то есть ссылка на другой период с мировой карты вернула бы вчерашнюю страну.
   * Явное слово оставляет догадки одному входу в раздел из меню. */
  const link = patch => href(base, { days, geo: geo || WORLD }, patch);
  ...
  const geoSelect = gSelect({
    ...
    groups: [
      { items: [{ label: 'Весь мир', href: link({ geo: WORLD, reg: '' }), active: !geo, note: n(s.unique) }] },
   ...
   */
  return `<div class="metric-dashboard" id="metric-overview">
  <div class="metric-toolbar">${daysSelect}<span class="metric-toolbar-note">Время — Москва</span><a class="metric-more-link" href="${visitorsHref}">${CI.icon('users')}Все посетители${s.visitorsTotal ? ' · ' + n(s.visitorsTotal) : ''}</a></div>
  <nav class="metric-section-nav" aria-label="Разделы отчёта">
    <a href="#metric-overview">Обзор</a><a href="#metric-sales">Продажи</a><a href="#metric-audience">Аудитория</a><a href="#metric-people">Посетители</a>
  </nav>
  <div class="metric-summary">${cards}</div>
  <section class="a-panel metric-panel metric-traffic trends-panel"><div class="metric-panel-head trends-head"><div><h2>Динамика посещаемости <span class="trends-help" title="Уникальные посетители за выбранный период">?</span></h2></div><span>${esc(chartScale)}</span></div>${traffic}<p class="metric-chart-caption">${byHour ? 'Текущий час ещё идёт. Будущие часы на графике не учитываются.' : byWeek ? 'Текущая неделя ещё идёт — её итоговое значение изменится.' : 'Текущий день ещё идёт — его итоговое значение изменится.'}</p></section>
  <div class="metric-grid-secondary">
    <section class="a-panel metric-panel"><div class="metric-panel-head"><h2>Откуда приходят</h2><span>посетители · доля</span></div>${bars(s.sources, { icon: 'source', of: s.unique })}</section>
    <section class="a-panel metric-panel"><div class="metric-panel-head"><h2>Популярные страницы</h2><span>открытия · доля</span></div>${bars(scrolled(s.pages), { icon: 'page', of: s.pageViews, labelOf: p => pageLabel(p, products) })}</section>
  </div>
  <div id="metric-sales">
    ${/* Воронка — во всю строку: у Bklit она лежит горизонтально, и в половине
       сетки четыре ступени сжимали подписи до наезда друг на друга. */''}
    ${funnelPanel(s)}
    ${exitsPanel(s, products)}
  </div>
  <div id="metric-audience">
    ...
    <div class="metric-grid-secondary">
      <section class="a-panel metric-panel"><div class="metric-panel-head"><h2>Устройства</h2><span>посетители · доля</span></div>${bars(s.devices, { icon: 'device', of: s.unique, className: 'metric-device-bars' })}</section>
      <section class="a-panel metric-panel"><div class="metric-panel-head"><h2>Города</h2><span>посетители · доля</span></div>${bars(s.locations || [], { icon: 'place', of: s.unique, className: 'metric-location-bars' })}</section>
    </div>
    <div class="metric-grid-secondary">
      <section class="a-panel metric-panel"><div class="metric-panel-head"><h2>Браузеры</h2><span>посетители · доля</span></div>${bars(s.browsers || [], { icon: 'browser', of: s.unique })}</section>
      <section class="a-panel metric-panel"><div class="metric-panel-head"><h2>Операционные системы</h2><span>посетители · доля</span></div>${bars(s.systems || [], { icon: 'system', of: s.unique })}</section>
    </div>
  </div>
  <section class="a-panel metric-panel metric-visitors" id="metric-people"><div class="metric-panel-head"><h2>Последние посетители</h2><a class="link" href="${visitorsHref}">Все посетители →</a></div>
    ${visitorsTable(s.visitors || [], { now: s.generatedAt, products, visitorBase: options.visitorBase, ordersHref: options.ordersHref })}
  </section>
  ${outboundPanel(s.outbound)}
  ${speedPanel(s.speed)}
  ${s.campaigns && s.campaigns.length ? `<section class="a-panel metric-panel metric-campaigns"><div class="metric-panel-head"><h2>UTM-кампании</h2></div>${bars(s.campaigns, { icon: 'source' })}</section>` : ''}
  ${botsFold(s.bots || {})}
  ${rulesFold(options.rules, { base: options.base })}
  </div>`;
}
...
/* ------------------- Карточка: доступ к витрине и учёт в метрике -------------
 *
 * Две кнопки стоят ОТДЕЛЬНОЙ ПАНЕЛЬЮ ВНИЗУ карточки, а не в её шапке. Причина
 * простая: у каждой есть цена, и её надо назвать словами — блок закрывает
 * витрину целиком, исключение стирает карточку. В шапке рядом с «Написать в
 * чат» они читались бы как ещё одно обычное действие, а промах здесь стоит
 * дороже.
 *
 * Наверху при этом остаётся отметка (`ruleMark`): состояние надо видеть, не
 * прокручивая страницу до конца.
 *
 * Ни строчки скрипта: две обычные формы, подтверждение — общий `data-confirm`
 * панели. Галочка адреса — родная и видимая, снята по умолчанию: за одним
 * адресом сидит целая квартира, и закрывать её заодно владелец должен решить
 * сам, а не обнаружить потом по жалобе покупателя.
 */
function ruleMark(rules) {
  const r = rules || {};
  const marks = [];
  if (r.block) marks.push('<span class="rule-mark is-block">Заблокирован</span>');
  if (r.skip) marks.push('<span class="rule-mark is-skip">Не учитывается в метрике</span>');
  return marks.join('');
}

function accessPanel(v, options) {
  const o = options || {};
  const rules = o.rules || {};
  const base = (o.ruleBase || '') + encodeURIComponent(o.key || v.id || '');
  const row = (kind, title, on, note, action) => `<div class="access-row is-${kind}${on ? ' is-on' : ''}">
    <div><b>${esc(title)}</b><span>${esc(note)}</span></div>
    <form method="post" action="${esc(base)}/${kind}"${on ? '' : ` data-confirm="${esc(action.ask)}"`}>
      ${on ? '<input type="hidden" name="off" value="1">' : ''}
      ${on ? '' : `<label class="access-ip"><input type="checkbox" name="ip" value="1"${v.ip && v.ip !== '?' ? '' : ' disabled'}>и по адресу ${esc(v.ip || 'не определён')}</label>`}
      <button class="btn btn-sm${on ? '' : ' btn-danger'}">${esc(action.label)}</button>
    </form>
  </div>`;
  return `<section class="a-panel metric-panel visitor-access">
    <div class="metric-panel-head"><div><span class="metric-kicker">Решения владельца</span><h2>Доступ и учёт</h2></div></div>
    ${row('block', 'Доступ к витрине', !!rules.block,
    rules.block ? 'Витрина ему не отдаётся вовсе — ни страницы, ни картинки, ни ответа корзины.' : 'Закрыть витрину целиком: ни страниц, ни картинок, ни оформления заказа.',
    { label: rules.block ? 'Разблокировать' : 'Заблокировать', ask: 'Закрыть витрину для этого посетителя?' })}
    ${row('skip', 'Учёт в метрике', !!rules.skip,
    rules.skip ? 'В отчёте его нет: ни онлайна, ни визитов, ни заявок.' : 'Убрать из отчёта — так исключают себя и менеджеров, чтобы не путать их с покупателями. Карточка при этом удаляется.',
    { label: rules.skip ? 'Учитывать снова' : 'Не учитывать', ask: 'Исключить из метрики? Карточка и его метки в отчёте будут удалены.' })}
  </section>`;
}
...
function visitorPage(visitor, options) {
  ...
  const visits = sessions.map((s, i) => {
    ...
    /* Строка просмотра отвечает на четыре вопроса подряд: когда, что открыл,
     * сколько пробыл и ДОКУДА ДОЛИСТАЛ. Последние два — одного рода, поэтому
     * стоят одной ячейкой в столбик, как адрес под названием страницы слева.
     *
     * Глубина прокрутки писалась в просмотр (`hit.d`) с самого начала, но не
     * показывалась нигде: на бою к этому дню накопилось 1767 записей, и все
     * впустую. Вопрос у неё свой и денежный — «дошёл ли он до кнопки покупки
     * или закрыл на первом экране», — и суточная сводка на него не отвечает:
     * там среднее по всем, а тут конкретный человек с конкретной заявкой. */
    const steps = s.hits.map(h => {
      const depth = Math.min(100, Math.max(0, Math.round(Number(h.d) || 0)));
      const out = (Array.isArray(h.o) ? h.o : []).slice(0, 3);
      return `<li>
      <span class="visit-time">${esc(clock(h.t))}</span>
      <span class="visit-page">${ICON_OF.page(h.p)}${esc(pageLabel(h.p, products))}<i>${esc(h.p || '')}</i>${
        out.length ? `<em class="visit-out">Ушёл в ${esc(out.join(' · '))}</em>` : ''}</span>
      <span class="visit-sec">${h.s ? esc(duration(h.s)) : '—'}${
        depth ? `<i title="Долистал до ${depth}% страницы">${depth}%</i>` : ''}</span></li>`;
    }).join('');
...
/* `gSelect` наружу — ради кнопки периода на «Обзоре» и в «Заказах». Своя вторая
 * пилюля с тем же смыслом разъехалась бы с этой на первой правке, а увидеть это
 * можно было бы только глазами: две страницы панели читались бы как два разных
 * раздела. */
module.exports = { dashboard, visitorsPage, visitorPage, visitorMissing, pageLabel, ago, gSelect };
