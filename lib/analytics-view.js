...
function funnelPanel(s) {
  ...
  const bounce = `отказов ${pct(s.bounceRate)}${prevRate ? ' ' + trend(s.bounceRate, prevRate, { title: 'к прошлому периоду' }) : ''}`;
...
function dashboard(snapshot, options) {
  ...
  <div class="metric-grid-secondary">
    <section class="a-panel metric-panel"><div class="metric-panel-head"><div><span class="metric-kicker">Контент</span><h2>Популярные страницы</h2></div><span>сколько раз открывали</span></div>${bars(scrolled(s.pages), { icon: 'page', of: s.pageViews, labelOf: p => pageLabel(p, products) })}</section>
