...
const RUSSIA_MAP = require('./russia-map-data');
const esc = R.esc;
 ...
 * перевозчиков. Сперва они лежали отдельным `public/russia-regions.svg`, а
 * страница ссылалась на него через `<use href="/static/…svg#id">` — красиво по
 ...
 *
 * Цена инлайна — 65 КБ разметки (13 КБ по проводу), и она платится осознанно:
 * геометрия для этого пересобрана под ту точность, которая ВИДНА. Допуск
 * упрощения задавался в градусах и выходил примерно в 0,06 единицы viewBox,
 * тогда как карта показывается шириной 300–900 px — то есть файл был раз в
 * тридцать точнее, чем может показать экран.
 *
 * Сами контуры лежат в КОНИЧЕСКОЙ проекции Альберса — той, в которой Россию
 * рисуют в атласах. Плоская сетка, стоявшая здесь раньше, давала верные
 * контуры и НЕПОХОЖУЮ страну: меридианы у неё не сходятся к полюсу, поэтому
 * северный берег выходил прямой линией, а Таймыр с Чукоткой раздувались вдвое.
 *
 * Источник контуров и команда пересборки — в scripts/build-russia-map.js.
 */
...
function russiaMap(items, options) {
  const o = options || {};
  const values = {};
  for (const item of items || []) {
    const name = regionMapName(item && item.label);
    if (name) values[name] = (Number(values[name]) || 0) + (Number(item && item.value) || 0);
  }
  const ranking = Object.entries(values).sort((a, b) => b[1] - a[1]);
  const max = Math.max(1, ...ranking.map(row => row[1]));
  /* Ступени заливки считаются от самого посещаемого субъекта, У КОТОРОГО ЕСТЬ
   * КОНТУР, а не от вершины всего рейтинга. Свободные данные Click That 'Hood
   * собраны до 2014 года, и Крыма с Севастополем в них нет вовсе: посетители
   * оттуда честно стоят в рейтинге справа, но на карте их не нарисовать. Считай
   * мы шкалу по общей вершине — приехавший из Крыма первым местом обесцветил бы
   * всю карту разом. */
  const mapMax = Math.max(1, ...RUSSIA_MAP.regions.map(region => Number(values[region.name]) || 0));
  /* У контура нет ни `aria-label`, ни фокуса, и это не забывчивость: их
   * восемьдесят три, а читающему с экрана они дали бы восемьдесят три остановки
   * табуляции посреди отчёта. Числа он берёт из рейтинга справа — обычным
   * текстом. Ровно то же правило действует у точек графика. */
  const paths = RUSSIA_MAP.regions.map(region => {
    const value = Number(values[region.name]) || 0;
    const level = value ? Math.max(1, Math.min(5, Math.ceil((value / mapMax) * 5))) : 0;
    return `<path class="rm-region heat-${level}" d="${region.d}" data-region="${esc(region.name)}" data-value="${n(value)}" data-unit="${plural(value, 'посетитель', 'посетителя', 'посетителей')}"></path>`;
  }).join('');
  /* Мусор в адресе приводится к первой странице, а номер зажимается — то же
   * правило, что у листалки отзывов: адрес правят руками, и «страница 99»
   * обязана показать последнюю, а не пустоту. */
  const pages = Math.max(1, Math.ceil(ranking.length / REGIONS_PER_PAGE));
  const page = Math.min(pages, Math.max(1, Math.floor(Number(o.page)) || 1));
  const start = (page - 1) * REGIONS_PER_PAGE;
  const top = ranking.slice(start, start + REGIONS_PER_PAGE);
  /* Длина полосы считается от вершины ВСЕГО рейтинга, а не от вершины страницы:
   * иначе на второй странице первая же строка растянулась бы во всю ширину и
   * прочиталась бы как самый посещаемый субъект. */
  const rows = top.length ? top.map((row, i) => `<div class="rm-rank-row">
    <span class="rm-rank-num">${start + i + 1}</span><b>${esc(row[0])}</b><span>${n(row[1])}</span>
    <i><em style="width:${Math.max(4, Math.round((row[1] / max) * 100))}%"></em></i>
  </div>`).join('') : '<p class="metric-empty rm-empty">Карта заполнится, когда определятся регионы посетителей.</p>';
   ...
   * экране не место. */
  const base = esc(o.base || '');
  const arrow = (dir, num, label) => (num >= 1 && num <= pages)
    ? `<a class="rm-page" href="${base}${num}#regions" rel="${dir}" aria-label="${label}">${R.PAGER_CHEVRON[dir]}</a>`
    : `<span class="rm-page rm-page-off" aria-hidden="true">${R.PAGER_CHEVRON[dir]}</span>`;
  const nav = pages > 1 ? arrow('prev', page - 1, 'Предыдущие субъекты') : '';
  const navNext = pages > 1 ? arrow('next', page + 1, 'Следующие субъекты') : '';
  const foot = top.length
    ? `<div class="rm-rank-foot">${nav}<span>Субъекты: ${start + 1}–${start + top.length} из ${ranking.length}</span>${navNext}</div>`
    : '';
  return `<div class="ru-map">
    <div class="rm-map-stage">
      <svg class="rm-map-svg" viewBox="${RUSSIA_MAP.viewBox}" role="img" aria-label="Посетители по субъектам России">${paths}</svg>
      <div class="rm-tip" hidden><b></b><span></span></div>
    </div>
    <div class="rm-ranking">${rows}${foot}</div>
  </div>`;
}
...
function dashboard(snapshot, options) {
  ...
  const ranges = [1, 7, 30, 365].map(d => `<a href="${esc(options.rangeBase || '')}${d}" class="metric-range${days === d ? ' active' : ''}">${d === 1 ? 'Сегодня' : (d === 365 ? '12 мес.' : d + ' дней')}</a>`).join('');
  /* База листания рейтинга регионов собирается из того же адреса, что и кнопки
   * периода: второй копии маршрута метрики здесь быть не должно. Смена периода
   * при этом возвращает рейтинг на первую страницу — другой период, другой
   * рейтинг, и «страница 9» в нём означала бы уже не тех соседей. */
  const regionBase = `${options.rangeBase || '?days='}${days}&reg=`;
  ...
  <section class="a-panel metric-panel metric-russia trends-panel" id="regions"><div class="metric-panel-head trends-head"><div><h2>Посетители по регионам <span class="trends-help" title="Уникальные посетители по субъектам РФ · зарубежные заходы в карту не попадают">?</span></h2></div><span class="trends-scope">Субъекты РФ</span></div>${russiaMap(s.regions || [], { page: options.regionPage, base: regionBase })}</section>
