...
function dashboard(snapshot, options) {
  ...
  const visitors = snapshot.visitors.map(v => {
    ...
    return `<tr>
      <td>${open(`<div class="metric-visitor"><span class="metric-status${online ? ' online' : ''}"></span><div><b>${online ? 'Сейчас на сайте' : ago(v.lastSeen, snapshot.generatedAt)}</b><small>${esc(String(v.id || '').slice(0, 8))}</small></div></div>`)}</td>
      <td><b>${flagMark(v)}${esc(location(v))}</b><small>${open(esc(v.ip || 'IP не определён'), 'metric-ip')}${v.isp ? ' · ' + esc(v.isp) : ''}</small></td>
      <td>${deviceMarks(v) || `<b>${esc(device(v))}</b>`}<small>${esc(technical(v) || v.platform || 'Технические данные не переданы')}</small><small>Источник: ${esc(v.source || 'Прямой заход')}${v.utmCampaign ? ' · UTM: ' + esc(v.utmCampaign) : ''}</small></td>
  ...
  return `<div class="metric-toolbar"><div class="metric-ranges">${ranges}</div><div class="metric-updated">Обновлено ${new Date(snapshot.generatedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</div><button class="metric-refresh" type="button" onclick="location.reload()">Обновить</button></div>
