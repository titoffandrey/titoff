...
function resolve(view, item, settings, promo, opts) {
   ...
   * корзины тоже. Ручной код «скидка товара» даёт её и без автоматического
   * кода; для этого передаём уже проверенное состояние выбора покупателя. */
  const pct = PROMO.pctFor(view, settings, promo);
