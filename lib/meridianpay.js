...
// Проверка реквизита общая с CrocoPAY: правило «номер, которого не бывает, до
// покупателя не доходит» обязано быть одним на обе кассы.
...
/*
 * А ВОТ СТОЛЬКО ЭТОТ СПИСОК ИДЁТ НА САМОМ ДЕЛЕ.
 *
 * Замер с боевого сервера (24 августа 2026): `/payment-gateways` отвечает
 * 8,4–8,7 с — 317 КБ и 906 банков, и быстрее не бывает (три замера подряд).
 * В отведённые списку четыре секунды он не укладывается НИКОГДА, поэтому живой
 * список MeridianPay не доезжал ни разу: и страница оплаты, и настройки честно
 * ждали свои четыре секунды и оставались без её способов. Со стороны это
 * выглядело так, будто вторая касса ничего не умеет, — а в панели она при этом
 * значилась работающей.
 *
 * Лечится не удлинением ожидания (эти секунды платит покупатель на самом
 * нетерпеливом шаге), а тем же приёмом, что у пунктов OZON: запрос уходит со
 * своим длинным сроком и живёт сам по себе, а вызывающий ждёт его не дольше
 * OPTIONS_TIMEOUT и довольствуется тем, что уже лежит в кэше. Не дождались
 * сейчас — список приедет к следующему открытию страницы.
 */
...
/* `detail_type` кассы -> вид реквизита, которым его знает весь остальной проект
 * (`kind` в lib/pay-methods.js). Проверяем по ФАКТИЧЕСКОМУ типу из ответа, а не
 * по запрошенному: у ПСБ на запрос НСПК законно приходит `qr`, и проверять его
 * правилами карты было бы неверно. */
const KIND = { card: 'card', phone: 'phone', sim: 'phone', nspk: 'qr', qr: 'qr' };
...
/* Запрос списка — ОДИН на всех и со своим длинным сроком.
 *
 * Страницу оплаты и настройки открывают одновременно, а список у кассы один и
 * стоит восемь секунд: без этой склейки два открытия подряд означали бы два
 * восьмисекундных запроса за одним и тем же ответом.
 */
let _inflight = null;
function gatewaysJob(settings, key) {
  if (_inflight && _inflight.key === key) return _inflight.job;
  const entry = { key, job: null };
  _inflight = entry;
  entry.job = (async () => {
    try {
      const r = await api(settings, '/payment-gateways', null, LIST_TIMEOUT);
      if (r.ok) {
      ...
      // Снимаем отметку в любом случае: оставленная после падения, она навсегда
      // отдавала бы всем один и тот же провалившийся ответ.
...
// Подождать ответ, но не дольше отведённого. Таймер не держит процесс (`unref`),
// поэтому брошенное ожидание ничего не стоит, а сам запрос продолжает идти.
function waitAtMost(job, ms) {
  return new Promise(resolve => {
    let done = false;
    const finish = value => { if (!done) { done = true; clearTimeout(timer); resolve(value); } };
    const timer = setTimeout(() => finish(null), ms);
    if (timer.unref) timer.unref();
    job.then(finish, () => finish(null));
  });
}

async function availableOptions(settings) {
  const key = trimmed(settings && settings.meridianpayApiKey);
  const shape = (live, extra) => Object.assign({
    ok: true, currencies: live.currencies, byCurrency: live.byCurrency,
    limits: live.limits, options: live.byCurrency[CURRENCY] || []
  }, extra || {});
  const mine = _gateways.live && _gateways.key === key ? _gateways.live : null;
  if (mine && Date.now() - _gateways.at < GATEWAYS_TTL) return shape(mine, { cached: true });
  const job = gatewaysJob(settings, key);
  // Кэш есть, но протух — отдаём его немедленно, а обновление идёт своим ходом.
  // Список банков меняется раз в жизнь, и ждать ради него восемь секунд, имея
  // на руках вчерашний ответ, незачем.
  if (mine) return shape(mine, { cached: true, stale: true });
  const r = await waitAtMost(job, OPTIONS_TIMEOUT);
  // Не дождались — это НЕ отказ кассы: запрос жив и заполнит кэш к следующему
  // открытию страницы. Так и говорим (`pending`), чтобы панель не писала «касса
  // не отвечает» про кассу, которая просто медленная.
  if (!r) return { ok: false, error: 'timeout', pending: true };
  if (!r.ok) return r;
  return shape(_gateways.live || parseGateways(r.data));
}
...
async function createInvoice(settings, params) {
  ...
  // У банковского способа важен не только вид реквизита, но и конкретный банк.
  // Иначе запрос «СБП Т-Банк» мог принять телефон Альфа-Банка: detail_type у них
  // одинаковый, но это уже другой выбранный покупателем маршрут.
  if (route.gateway && view.gateway !== route.gateway) {
    console.error('meridianpay order: касса вернула шлюз', view.gateway || '(пусто)', 'вместо', route.gateway);
    return { ok: false, error: 'method_mismatch', invoice: view };
  }
  ...
  /*
   * Последняя граница — сам реквизит, и заведена она по живым заказам.
   *
   * 25 августа 2026 на мелких чеках касса трижды подряд вернула номера, которых
   * не бывает: десятизначный «7775553535», двенадцатизначный «987777777777» и
   * счёт на «Testov T.T.» — то есть её пул на такие суммы отдаёт тестовые
   * записи. Показать это покупателю нельзя: заплатить по такому номеру
   * невозможно, а если выдумка совпала с чьим-то настоящим номером, деньги уйдут
   * постороннему. Fail-closed: уходим на другую кассу, сделку отменяем (это
   * делает вызывающий), реквизит в лог не пишем.
   */
  const problem = PAY.requisiteProblem(KIND[view.method] || '', view.requisite, !route.transgran);
  if (problem) {
    console.error('meridianpay order: реквизит не похож на настоящий —', problem,
      '| тип', view.method, '| способ', method);
...
function matchesInvoice(expected, actual) {
  ...
  const issuedType = trimmed(expected.actualMethod);
  // POST уже мог доказать подмену типа. GET не всегда повторяет detail_type, и
  // отсутствие поля не должно стирать ранее замеченный факт.
  if (route && issuedType && !(ACCEPT[route.detail] || [route.detail]).includes(issuedType)) {
    return { ok: false, reason: 'method' };
  }
  if (route && Object.prototype.hasOwnProperty.call(expected, 'actualMethod')
    && !issuedType && !type) return { ok: false, reason: 'method' };
  if (route && type && !(ACCEPT[route.detail] || [route.detail]).includes(type)) {
    return { ok: false, reason: 'method' };
  }
  // То же правило для способов, привязанных к конкретному банку, и для
  // российского/трансграничного маршрута. Эти поля GET может не вернуть, поэтому
  // проверяем сохранённый ответ POST.
  const issuedGateway = trimmed(expected.actualGateway);
  if (route && route.gateway && issuedGateway && issuedGateway !== route.gateway) {
    return { ok: false, reason: 'method' };
  }
  const issuedRegion = trimmed(expected.actualRegion);
  if (route && !route.transgran && issuedRegion && !/росс/i.test(issuedRegion)) {
    return { ok: false, reason: 'method' };
  }
  return { ok: true };
}
