...
function invoiceView(raw) {
  ...
  return {
    ...
    // Название банка — то, что покупатель увидит рядом с реквизитом.
    bank: pick(data, ['payment_gateway']) || pick(detail, ['region']),
...
async function availableOptions(settings) {
  ...
  const r = await api(settings, '/payment-gateways');
  if (!r.ok) return r;
...
module.exports = {
  ...
  name: 'MeridianPay',
  verifyCallback,
  API, CURRENCY, MINOR, MAP, ACCEPT,
  configured, enabled, supports, validInvoiceId, validMerchantId, validExternalId,
  createInvoice, invoice, cancel, availableOptions, parseGateways, forgetMethods,
  matchesInvoice, sameStartRequest, retryableStart, invoiceView, stateOf, expiryMs,
  integrityHint,
