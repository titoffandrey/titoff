...
function addMessage(chat, role, text, opts) {
  ...
  const message = { role, text: body, at: Math.max(Date.now(), previous + 1), by: clean(opts && opts.by, 80) };
...
/* Подписи смены собеседника. Лежат здесь, а не у вызывающих: сменить его можно
 * из четырёх мест (реплика в теме Telegram, кнопка в теме, команда, ответ из
 * панели), и четыре копии текста разъехались бы на первой же правке — а
 * покупатель увидел бы, что «менеджер подключился» и «оператор на связи» это
 * почему-то разные события. */
const MODE_NOTES = {
  operator: 'Менеджер подключился к разговору',
  ai: 'Отвечает консультант магазина',
  closed: 'Разговор завершён. Если понадобится — напишите снова.'
};

function setMode(chat, mode, opts) {
  if (!chat || !['ai', 'operator', 'closed'].includes(mode)) return chat;
  if (chat.mode === mode) return chat;
  const first = chat.mode === 'ai' && !chat.messages.some(m => m.role !== 'system');
  chat.mode = mode;
  chat.lastAt = Math.max(Date.now(), (Number(chat.lastAt) || 0) + 1);
  markDirty();
  if (opts && opts.silent) return chat;
  /* Серая строка в ленте — единственное, что говорит покупателю о смене
   * собеседника. Раньше это была ещё и подпись в шапке, но её видно только
   * пока смотришь наверх, а событие происходит внизу, где идёт разговор.
   *
   * В пустом диалоге её не пишем: «менеджер подключился» первой строкой, до
   * единого вопроса, — это не событие, а недоразумение. */
  if (!first && MODE_NOTES[mode]) say(chat, 'system', MODE_NOTES[mode]);
  // Шапка и поле ввода тоже знают о смене: у завершённого разговора поля нет.
...
module.exports = {
  MAX_MESSAGES, MAX_TEXT, RETENTION_DAYS, MODE_NOTES,
