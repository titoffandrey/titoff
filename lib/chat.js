...
function unreadCount() {
  let n = 0;
  for (const chat of chats.values()) {
    if (chat.mode === 'closed') continue;
    const last = lastFrom(chat, 'user');
    if (last && last > (chat.receipt.storeRead || 0)) n++;
  }
  return n;
}
...
function markStore(chat, kind) {
  return bump(chat, kind === 'read' ? 'storeRead' : 'storeGot', chat ? lastFrom(chat, 'user') : 0);
}
...
module.exports = {
  ...
  get, byTopicId, byVisitorId, list, count, activeCount, unreadCount, onlineVisitorIds,
