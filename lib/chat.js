...
// Ссылка в реплике — тот же общий разборщик, что у витрины, панели и Telegram.
// Нужен он здесь ровно в одном месте: чтобы в цитату уехало название товара, а
// не скобки с адресом (см. `replyTo`).
...
const MAX_PHOTOS = 3;
/* Сколько раз в одном диалоге можно назвать номер, по которому заказов нет.
 * Пять — это опечатка, второй телефон и ещё три попытки вспомнить, но уже не
 * перебор чужих номеров. */
const MAX_PHONE_TRIES = 5;
/* Сколько текста запоминает цитата — та реплика, на которую отвечают. Она
 * отвечает на один вопрос: «о чём это». Полный текст лежит выше в самой ленте,
 * а длинная цитата в пузыре читалась бы как вторая реплика. */
const REPLY_CUT = 160;
...
// Первый вопрос, который консультант уступил открытому администратору, ждёт
// меньше уже начатого менеджером разговора: человек ещё не получил ни ответа.
const ADMIN_FIRST_WAIT_MIN = 2;
...
function init(dataDir, opts) {
  ...
  load();
  // Срок хранения не должен зависеть от того, идут ли сегодня разговоры.
  startSweep();
}
  ...
  const normalizedMessages = messages.filter(m => m && ROLES.includes(m.role))
    .slice(-MAX_MESSAGES).map(m => {
      ...
      const row = {
        ...
        at: messageAt,
        /* Имя Ксении обновляем и в старых диалогах. Иначе после переименования
         * новая реплика шла бы от Ксении, а предыдущая в той же ленте — от
         * прежнего консультанта. Имя менеджера сохраняем снимком: его история
         * к переименованию ИИ не относится. */
        by: m.role === 'ai' ? SPEAKERS.ai : clean(m.by, 80)
      };
      if (m.orderEvent) row.orderEvent = clean(m.orderEvent, 100);
      ...
      // Цитата — тоже необязательное поле: отвечают на конкретную реплику редко.
      const reply = replyView(m.reply);
      if (reply) row.reply = reply;
      return row;
    });
  const createdAt = Number(item.at) || Date.now();
  const storeRead = stamp(item.receipt && item.receipt.storeRead);
  /* До появления отдельной отметки менеджера счётчик панели жил на
   * `receipt.storeRead`. Старым диалогам подставляем прежнее значение: иначе
   * после обновления вся история за 90 дней внезапно стала бы «новой».
   * У новых записей обоих полей нет — отсчёт честно начинается с нуля. */
  const managerRead = Object.prototype.hasOwnProperty.call(item, 'managerRead')
    ? stamp(item.managerRead) : storeRead;
  return {
    id: item.id,
    at: createdAt,
    ...
    /* Номер телефона, которым покупатель опознал свои заказы, — в E.164, как в
     * самой заявке. Метка посетителя связывает диалог с заказом сама, но только
     * пока человек пишет из того же браузера: с другого телефона, из другого
     * браузера или после чистки cookie метка другая, и заказ переставал
     * находиться вовсе. Номер это чинит — он у покупателя один и тот же.
     *
     * Поля нет — покупатель номера не называл: у всех диалогов до появления
     * опознания так и есть, и заказы у них по-прежнему подбирает метка. */
    phone: clean(item.phone, 32),
    /* Сколько раз названный номер не нашёл ни одного заказа. Считаем, потому
     * что телефон — не секрет: без предела чат стал бы способом перебирать
     * чужие номера и узнавать, кто здесь покупал. */
    phoneTries: Math.max(0, Math.floor(Number(item.phoneTries)) || 0),
    ...
    /* Докуда сообщения покупателя видел именно МЕНЕДЖЕР. Ответ ИИ сюда не
     * относится: он двигает `receipt.storeRead` ради галочек покупателя, но
     * уведомление в панели обязано ждать человека. */
    managerRead,
...
// Возвращает, убрала ли она хоть что-то: по этому ответу сторожевой таймер ниже
// решает, надо ли переписывать файл.
function cleanup(force) {
  const now = Date.now();
  if (!force && now - lastCleanup < CLEANUP_MS) return false;
  lastCleanup = now;
...
/* Уборка обязана идти и на ТИХОМ магазине.
 *
 * Она вызывается из `flush()`, а `flush()` выходит первой строкой при `!dirty` —
 * то есть на магазине, где сегодня никто не писал, переписка старше
 * `RETENTION_DAYS` не удалялась вовсе, и снимки покупателей вместе с ней. А
 * срок хранения — это обещание из политики конфиденциальности, и держаться оно
 * не должно на том, пришёл ли сегодня новый вопрос.
 *
 * Поэтому свой сторожевой таймер. Он ничего не стоит: `cleanup()` выходит по
 * `lastCleanup` мгновенно, а метит к записи, только если правда что-то убрала.
 */
let sweepTimer = null;
function startSweep() {
  if (sweepTimer || !dataFile) return;
  sweepTimer = setInterval(() => { if (cleanup()) markDirty(); }, CLEANUP_MS);
...
/* Сколько диалогов ждут прочтения МЕНЕДЖЕРОМ — счётчик в шапке панели.
 *
 * Ответ ИИ вопрос покупателя читает, но менеджером от этого не становится.
 * Поэтому галочки покупателя живут на `receipt.storeRead`, а уведомление панели
 * — на отдельном `managerRead` и гаснет только после человеческого просмотра.
 */
 ...
 * это число не ноль), — и в этом весь смысл: строка и шапка обязаны указывать
 * на одни разговоры. `chat.unread` рядом — НЕ про то же самое: он про
 * непрочитанное У ПОКУПАТЕЛЯ и висит значком на его кнопке.
 */
function storeUnread(chat) {
  if (!chat || chat.mode === 'closed') return 0;
  const read = Number(chat.managerRead) || 0;
  let n = 0;
...
/* Номер, которым покупатель опознал свои заказы.
 *
 * Хранится один и последний названный: второй номер — это второй телефон того
 * же человека, и показать надо заказы по нему. Прежний при этом не теряется
 * молча — по нему заказы уже названы в переписке выше.
 */
function setPhone(chat, phone) {
  const value = clean(phone, 32);
  if (!chat || !value || chat.phone === value) return false;
  chat.phone = value;
  markDirty();
  return true;
}

/* Названный номер не нашёл ни одного заказа. Считаем такие попытки: телефон —
 * не секрет, и без предела чат превратился бы в способ перебирать чужие номера.
 * Предел на ДИАЛОГ, а не на реплику: новый диалог с той же сессии и адреса
 * упирается в общий `floodLimited`, который считает и то, и другое. */
function phoneMiss(chat) {
  if (!chat) return;
  chat.phoneTries = (Number(chat.phoneTries) || 0) + 1;
  markDirty();
}

// Сколько попыток опознания осталось. Ноль — больше не ищем и не отвечаем на
// названный номер ничем, кроме обычного «передам менеджеру».
function phoneTriesLeft(chat) {
  return chat ? Math.max(0, MAX_PHONE_TRIES - (Number(chat.phoneTries) || 0)) : 0;
}
...
function addMessage(chat, role, text, opts) {
  ...
  if (opts && opts.orderEvent) message.orderEvent = clean(opts.orderEvent, 100);
  if (photos.length) message.photos = photos;
  // На какую реплику это ответ. Снимком, а не ссылкой на оригинал — см.
  // `replyTo()` ниже.
  const reply = replyView(opts && opts.reply);
 ...
 *
 * Цитата хранится СНИМКОМ (`{at, role, by, text}`), а не ссылкой на оригинал, и
 * это не экономия на поиске.
 *
 * Оригинал живёт своей жизнью: его правят и удаляют из панели, и покупатель
 * этого не видит — таково главное правило правки (см. `editMessage`). Резолви мы
 * цитату на лету, удалённая реплика превратила бы её в «сообщение удалено» прямо
 * у него в окне, то есть рассказала бы ровно то, что мы прячем. Со снимком
 * цитата остаётся такой, какой её видел человек в момент ответа.
 *
 * Текст режется до `REPLY_CUT`: цитата отвечает на «о чём это», а полная реплика
 * лежит выше в той же ленте.
 */
...
// Чужой объект (из файла или из тела запроса) в цитату не попадает: роль
// сверяется со списком, время — число, текст режется и чистится, как реплика.
function replyView(value) {
  if (!value || typeof value !== 'object') return null;
  const at = stamp(value.at);
  const text = clean(cutTo(value.text, REPLY_CUT), REPLY_CUT);
  if (!at || !text) return null;
  return { at, role: ROLES.includes(value.role) ? value.role : 'user', by: clean(value.by, 80), text };
}

// Снимок по времени реплики: панель присылает только его, всё остальное берётся
// из самой переписки — подменить автора или текст цитаты запросом нельзя.
function replyTo(chat, at) {
  const i = findMessage(chat, at);
  if (i < 0) return null;
  const m = chat.messages[i];
  const shots = photosOf(m).length;
  /* Реплика бывает без слов — из одних снимков. Пустая цитата не сохранится
   * вовсе (`replyView` отбросит), а «📷 2 фото» отвечает на тот же вопрос.
   *
   * Ссылка консультанта уходит в цитату ОДНИМ НАЗВАНИЕМ (общий разборщик, тот
   * же, что у витрины и Telegram): скобки с адресом в двух строках цитаты
   * читались бы как вылезшая наружу разметка, а обрезка по `REPLY_CUT` вдобавок
   * рвала бы их посередине. */
  const body = m.text
    ? LINKS.parts(m.text).map(p => (p.text != null ? p.text : p.label)).join('')
    : (shots ? '📷 ' + shots + ' фото' : '');
  return replyView({ at: m.at, role: m.role, by: m.by, text: body });
}
...
/* Менеджер действительно увидел переписку: открыл её в панели либо ответил
 * или выполнил команду в теме Telegram. ИИ эту отметку не трогает никогда.
 *
 * Как и галочки, это watermark по времени последней реплики покупателя. Повтор
 * ничего не пишет — иначе живое обновление открытого диалога обновляло бы само
 * себя по кругу. */
function markManager(chat, at) {
  const edge = stamp(at) || (chat ? lastFrom(chat, 'user') : 0);
  if (!chat || !(edge > (Number(chat.managerRead) || 0))) return false;
  chat.managerRead = edge;
  markDirty();
  return true;
}
...
module.exports = {
  MAX_MESSAGES, MAX_TEXT, MAX_PHOTOS, REPLY_CUT, RETENTION_DAYS, SPEAKERS, speakerOf, photosOf, cleanPhotos,
  replyTo,
  TAKEOVER_DEFAULT_MIN, TAKEOVER_MAX_MIN, ADMIN_FIRST_WAIT_MIN, takeoverMinutes,
  ...
  MAX_PHONE_TRIES, setPhone, phoneMiss, phoneTriesLeft,
  addMessage, editMessage, dropMessage, say, setMode, setTopic, markRead, messagesFor,
  markManager, markStore, markUser, receipt,
