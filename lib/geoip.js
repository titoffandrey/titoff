...
// Регионы: в базе они пишутся по-разному («Moscow», «Moscow Oblast», «St.-Petersburg»,
// «Sverdlovsk Oblast»), поэтому ключ нормализуется, а хвост «oblast/krai/republic»
// переводится общим правилом — перечислять все восемьдесят пять руками незачем.
const REGION_RU = {
  moscow: 'Москва', 'moscow oblast': 'Московская область', 'st petersburg': 'Санкт-Петербург',
  'leningrad oblast': 'Ленинградская область', 'krasnodar krai': 'Краснодарский край',
  'sverdlovsk oblast': 'Свердловская область', 'novosibirsk oblast': 'Новосибирская область',
  'nizhny novgorod oblast': 'Нижегородская область', tatarstan: 'Татарстан',
  'republic of tatarstan': 'Татарстан', bashkortostan: 'Башкортостан',
  'republic of bashkortostan': 'Башкортостан', 'chelyabinsk oblast': 'Челябинская область',
  'samara oblast': 'Самарская область', 'rostov oblast': 'Ростовская область',
  'perm krai': 'Пермский край', 'krasnoyarsk krai': 'Красноярский край',
  'voronezh oblast': 'Воронежская область', 'volgograd oblast': 'Волгоградская область',
  'saratov oblast': 'Саратовская область', 'tyumen oblast': 'Тюменская область',
  'irkutsk oblast': 'Иркутская область', 'omsk oblast': 'Омская область',
  'kemerovo oblast': 'Кемеровская область', 'primorsky krai': 'Приморский край',
  'khabarovsk krai': 'Хабаровский край', 'stavropol krai': 'Ставропольский край',
  'altai krai': 'Алтайский край', 'kaliningrad oblast': 'Калининградская область',
  'crimea': 'Крым', 'republic of crimea': 'Крым', 'sevastopol': 'Севастополь'
};
...
function regionRu(name, code) {
  const raw = String(name || '').trim();
  if (!raw) return '';
  const found = REGION_RU[key(raw)];
  if (found) return found;
  if (code !== 'RU') return raw;
  // Общее правило для остальных: «Kirov Oblast» → «Кировская область» перевести
  // нечем (склонение не выводится из английского), поэтому переводим только тип
  // региона, а название оставляем как есть: «Kirov (область)» было бы хуже
  // читаемого «Kirov Oblast». Оставляем строку целиком.
  return raw;
}
