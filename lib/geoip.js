 ...
 *      выдумывать перевод.
 *
 * Города переводятся СЛОВАРЁМ (`city-names-ru.txt.gz` рядом с этим файлом), а не
 * только руками. Ручная таблица ниже покрывала три сотни крупных городов, и
 * этого хватало ровно до первого посёлка: на боевых данных латиницей оставалась
 * КАЖДАЯ ПЯТАЯ карточка — «Sysert'», «Druzhba», «Kuznechikha», а заодно
 * «Stockholm» и «Frankfurt am Main». Дописывать их руками — работа без конца:
 * в базе сто шестьдесят тысяч мест. Словарь собран из GeoNames (свободная
 * лицензия CC BY 4.0) скриптом `scripts/build-city-names.js`.
 *
...
/* Таблицы БЕЗ прототипа: ключ приходит из названия места, а у обычного объекта
 * `CITY_RU['constructor']` отвечает функцией — и «перевод» уехал бы в панель
 * куском кода. Та же грабля, из-за которой поля формы разбираются через
 * `hasOwnProperty` (см. `own()` в `lib/server-lib.js`). */
const CITY_RU = Object.assign(Object.create(null), {
...
const REGION_CORE = Object.assign(Object.create(null), {
  ...
  tula: 'Тульская область', yaroslavl: 'Ярославская область', jaroslavl: 'Ярославская область', arkhangelsk: 'Архангельская область',
  ...
  // DB-IP пишет «Primorskiy (Maritime) Kray»: скобочный хвост `key()` снимает
  // сам, а вот «primorskiy» через «и» в таблице не было — и целый край
  // оставался латиницей и мимо карты.
  primorskiy: 'Приморский край', 'primorsk|kray': 'Приморский край',
  ...
  chuvashiya: 'Чувашия', chuvash: 'Чувашия', chuvashia: 'Чувашия',
  'mariy el': 'Марий Эл', 'mari el': 'Марий Эл',
  ...
  // Через дефис и через тире геобаза пишет одно и то же место, а `key()`
  // оставляет от них пробел: без этой строки Северная Осетия не находится вовсе.
  ...
  crimea: 'Крым', krym: 'Крым', sevastopol: 'Севастополь', sebastopol: 'Севастополь',
...
/* ---------------------------------------------------------------------------
 * Словарь городов: «страна + латинское имя» → русское имя.
 *
 * Собран из GeoNames (`scripts/build-city-names.js`) и лежит рядом сжатым:
 * 1,2 МБ в репозитории против пяти распакованных. Ищется бинарным поиском по
 * буферу — тем же приёмом, что и сама база: сто с лишним тысяч строк объектом
 * стоили бы вчетверо больше памяти, а нужна из них одна.
 *
 * КЛЮЧ «МЯГКИЙ», и это не изящество, а необходимость: одно и то же место
 * записано в двух базах разной транслитерацией. DB-IP пишет «Vyshny Volochyok»,
 * GeoNames — «Vyshniy Volochëk»; «Beryozovo» против «Berëzovo», «Schelkovo»
 * против «Shchyolkovo», «Himki» против «Khimki». Свёртка приводит обе записи к
 * одному виду, и на боевых данных она добирает ещё три десятка городов, которые
 * точным сравнением не находились вовсе.
 *
 * Порядок правил значим: «yo»/«ye» сводятся к «е» ДО того, как «y» станет «i»
 * (иначе «Ozyory» и «Ozëry» разойдутся), «shch» — до схлопывания повторов.
 */
function softKey(value) {
  return key(value)
    .replace(/yo|jo/g, 'e').replace(/ye/g, 'e')
    .replace(/shch|sch/g, 'sh')
    .replace(/kh/g, 'h')
    .replace(/w/g, 'v')
    .replace(/[yj]/g, 'i')
    // Удвоенные согласные транслитерация ставит по-разному («Novorossiysk» и
    // «Novorosiysk»), а схлопывание одинаково у обеих сторон сравнения.
    .replace(/(.)\1+/g, '$1');
}

const NAMES_FILE = path.join(__dirname, 'city-names-ru.txt.gz');
// undefined — ещё не читали, null — файла нет (магазин работает как раньше,
// только по ручной таблице), иначе { buf, at }.
let names;
...
function dict() {
  ...
  names = null;
  try {
    const buf = zlib.gunzipSync(fs.readFileSync(NAMES_FILE));
    const at = [];
    let i = 0;
    while (i < buf.length) {
      at.push(i);
    ...
    if (at.length) names = { buf, at: Uint32Array.from(at) };
  } catch (e) {}
  return names;
}

// Строки в файле отсортированы по ключу байтово — генератор и поиск обязаны
// сравнивать их одинаково, поэтому ключ читается как latin1 (он ASCII).
function fromDict(k) {
  const d = dict();
  if (!d) return '';
  let low = 0, high = d.at.length - 1;
  while (low <= high) {
    const mid = (low + high) >> 1;
    const start = d.at[mid];
    const tab = d.buf.indexOf(9, start);
    if (tab < 0) return '';
    const cur = d.buf.toString('latin1', start, tab);
    if (cur === k) {
      let end = d.buf.indexOf(10, tab);
      if (end < 0) end = d.buf.length;
      return d.buf.toString('utf8', tab + 1, end);
    }
    if (cur < k) low = mid + 1; else high = mid - 1;
  }
  return '';
}
...
/* Город по-русски.
 *
 * Ручная таблица идёт ПЕРВОЙ: она правится руками и означает «здесь мы решили
 * иначе, чем словарь» — автоматический источник не вправе отменять такое
 * решение. Словарь вторым, и только при известной стране: «Moscow» бывает и в
 * Айдахо. Не нашлось ни там, ни там — оставляем латиницу: выдумать перевод
 * нельзя, а «Yekaterinburg» в панели всё-таки лучше, чем «Город не определён».
 */
function cityRu(name, code) {
  const raw = String(name || '').trim();
  if (!raw) return '';
  const manual = CITY_RU[key(raw)];
  if (manual) return manual;
  const cc = String(code || '').trim().toUpperCase();
  if (/^[A-Z]{2}$/.test(cc)) {
    const hit = fromDict(cc + '|' + softKey(raw));
    if (hit) return hit;
  }
  return raw;
}
...
module.exports = {
  FILE, MAGIC, HEADER, REC, NAMES_FILE,
  lookup, info, ipToInt, close, cityRu, regionRu,
  // Ключ словаря считает ровно этот модуль — генератор берёт его отсюда же,
  // иначе собранный файл однажды перестанет находиться при поиске.
  softKey, cityNames: dict
};
