...
function setOrderShipment(id, shipment) {
  ...
  const order = list.find(o => o.id === id);
  if (!order) return { ok: false, reason: 'not_found' };
  const next = TRACK.normalize(shipment);
  ...
  if (order.shipment && Number(order.shipment.createdAt) > 0) next.createdAt = Number(order.shipment.createdAt);
  order.shipment = next;
  saveOrders(list);
...
/* Заказ по НОМЕРУ — то, что покупатель вводит на странице отслеживания.
 *
 * Номер случайный и шестизначный (см. `nextOrderNumber`), но всё же короткий,
 * поэтому здесь он только находит заказ: что именно показывать по нему, решает
 * маршрут витрины, и персональных данных там нет. Сравниваем по цифрам: у
 * заявок, оформленных до перехода на случайные номера, номер записан с
 * префиксом «ORD-», а покупатель видит и вводит его без префикса.
 */
function getOrderByNumber(number) {
  const digits = String(number == null ? '' : number).replace(/\D+/g, '');
  if (!digits) return null;
  return getOrders().find(o => String(o.number || '').replace(/\D+/g, '') === digits) || null;
}
...
module.exports = {
  ...
  getOrderByNumber, setOrderShipment, clearOrderShipment,
