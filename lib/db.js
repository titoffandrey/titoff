...
function createOrder(data) {
  ...
  const order = {
    ...
    clientIp: (data.clientIp || '').slice(0, 80),
    clientCity: (data.clientCity || '').slice(0, 100),
    clientRegion: (data.clientRegion || '').slice(0, 120),
    clientCountry: (data.clientCountry || '').slice(0, 100),
    clientIsp: (data.clientIsp || '').slice(0, 140),
    clientDevice: (data.clientDevice || '').slice(0, 40),
    clientModel: (data.clientModel || '').slice(0, 80),
    clientOs: (data.clientOs || '').slice(0, 80),
    clientBrowser: (data.clientBrowser || '').slice(0, 80),
    clientSource: (data.clientSource || '').slice(0, 120),
    status: 'new',
    createdAt: Date.now()
  };
  list.unshift(order);
