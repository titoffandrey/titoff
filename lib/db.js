 ...
 * фильтрованный список, отданный в saveOrders, стёр бы черновики заодно.
 */
function visibleOrders() { return getOrders(); }
...
function deleteOrder(id) {
  ...
  // Счёт, webhook и повторная сверка обязаны иметь неизменяемую запись заказа.
  // Финансовую историю не удаляем даже у failed/expired: платёж может прийти
  // поздно. Удаление остаётся только для заявок, где касса не запускалась.
  if (order.payment) return false;
  saveOrders(list.filter(o => o.id !== id));
  return true;
}
...
function canDiscardDraftOrder(order) {
  return !!order && order.status === 'new' && order.draft === true && order.payment === null;
}
