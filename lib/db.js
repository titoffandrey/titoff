 ...
 * Ответить на него без метки нечем: ручной статус заявки с панелей снят (см.
 * «Почему оплата не подменяет статус заказа»), и «новым» заказ остаётся навсегда.
 *
...
function createOrder(data) {
  ...
  const order = {
    ...
    status: 'new',
    // Доставка и оплата — два независимых процесса. Касса меняет только
    // `payment.status`, а этот журнал ведёт работу магазина с заказом.
    statusHistory: [{ status: 'new', at: createdAt, by: 'system' }],
...
// Операционный статус не подменяет состояние кассы: оплаченный заказ может
// одновременно быть «собирается», а отменённая доставка не делает платёж
// возвращённым. История нужна, чтобы смена статуса в панели не была бесследной.
const ORDER_STATUSES = ['new', 'confirmed', 'processing', 'shipped', 'done', 'cancelled'];
function setOrderStatus(id, status, by) {
  if (!ORDER_STATUSES.includes(status)) return null;
  const list = getOrders();
  const o = list.find(x => x.id === id);
  if (o && o.status !== status) {
    o.status = status;
    const history = Array.isArray(o.statusHistory) ? o.statusHistory.slice(-49) : [];
    history.push({ status, at: Date.now(), by: textValue(by, 40) || 'admin' });
...
module.exports = {
  ...
  updateOrderClient, setOrderStatus, ORDER_STATUSES, archiveOrder, restoreOrder, deleteOrder, purgeOrder, purgeArchivedOrders,
