...
/* Стереть ВЕСЬ архив разом.
 *
 * Архив копится сам: брошенные черновики и тестовые заявки удаляют из рабочего
 * списка, и дальше они лежат там навсегда. Чистить их по одному — это два
 * нажатия с подтверждением на каждую, и на полусотне записей за этим просто
 * перестают следить.
 *
 * Берёт ровно то же, что и `purgeOrder()`, — только заархивированное. Рабочий
 * список не трогается ни при каких условиях: это единственная защита от «одним
 * нажатием снёс все заказы», и держится она на том же `isOrderArchived()`.
 *
 * `protected` считает записи, которые остались именно из-за финансовой истории.
 */
...
module.exports = {
  ...
  updateOrderClient, setOrderPaidManually, setOrderVoided, archiveOrder, restoreOrder, deleteOrder, purgeOrder, purgeArchivedOrders,
