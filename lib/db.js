...
function setOrderPaidManually(id, paid, by) {
  ...
  if (order.payment && (order.payment.status === 'paid' || order.payment.status === 'mismatch')) {
...
// `secondaryColor` в списке больше нет: цветовая настройка у магазина одна, и
// переносить с домена нечего — второй цвет не читается нигде.
const SITE_BRAND_FIELDS = ['storeName', 'tagline', 'accentColor', 'currency', 'currencyPosition',
  'contactTelegram', 'contactWhatsApp', 'contactWhatsAppMessage', 'contactPhone', 'footerNote',
  'legalOperator', 'legalInn', 'legalOgrn', 'legalDetails', 'legalAddress', 'privacyEmail',
...
function applySiteView(product, multiplier, override) {
  ...
  /* `fromLegacy` отдаёт ЦЕНУ ПРОДАЖИ (столько платили на прежней витрине), а в
   * нынешней модели `price` — цена БЕЗ скидки: её и восстанавливаем обратным
   * ходом. Без этого шага переехавший товар продавался бы по цене, которая уже
   * была со скидкой, и промоакция срезала бы её второй раз. */
  const sold = manual ? round(manualPrice) : scale(legacy.price);
  const percent = manual ? 0 : legacy.percent;
  p.price = percent ? DISCOUNT.compareFor(sold, percent) || sold : sold;
  p.discountPercent = percent;
  delete p.oldPrice; delete p.hotDeal; delete p.hotDealPrice; delete p.hotDealUntil;
  /* Доплаты едут тем же путём, что и цена: сперва множитель домена, потом
   * обратный ход по проценту. Второй шаг обязателен — скидка режет ВСЮ сумму
   * сборки, и доплата, оставшаяся «со скидкой внутри», сделала бы дорогую
   * конфигурацию дешевле, чем она стоила на прежней витрине. */
  const add = n => (percent ? DISCOUNT.compareFor(scale(n), percent) || scale(n) : scale(n));
  if (multiplier !== 1 || percent) {
    p.storages = (product.storages || []).map(s => Object.assign({}, s, { add: add(s.add) }));
    p.bands = (product.bands || []).map(g => Object.assign({}, g, {
      sizes: (g.sizes || []).map(x => Object.assign({}, x, { add: add(x.add) })),
      options: (g.options || []).map(o => Object.assign({}, o, { add: add(o.add) }))
    }));
    p.options = (product.options || []).map(g => Object.assign({}, g, {
      values: (g.values || []).map(v => Object.assign({}, v, { add: add(v.add) }))
    }));
  }
  return p;
}
...
function ensureSeeded() {
  ...
  if (!exists('products')) {
    ...
    /* Метка модели цен. Каталог засеян из `catalog.js`, а он уже отдаёт цены БЕЗ
     * скидки, — значит разовый `scripts/full-price.js` этому магазину не нужен
     * и, запущенный по ошибке, поднял бы ценник второй раз. Отличить
     * переведённый каталог от старого по самим числам нельзя: разница нигде в
     * них не записана. Ставится она вместе с КАТАЛОГОМ, а не с настройками:
     * метка про товары, и у магазина со старым каталогом (настройки могли
     * потеряться) она означала бы неправду. */
    saveSettings({ fullPriceAt: Date.now() });
  }
...
module.exports = {
  DATA_DIR, UPLOAD_DIR, PRODUCT_IMAGE_MAX, newId, ensureSeeded, normHost, deleteUploadIfUnused, uploadIsUsed,
