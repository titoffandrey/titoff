...
// Только внутренние отметки доставки сообщений; из тела запроса не читаются.
function setOrderChatFollowup(id, patch) {
  const list = getOrders();
  const order = list.find(item => item.id === id);
  if (!order) return null;
  order.chatFollowup = Object.assign({}, order.chatFollowup, patch);
  saveOrders(list);
  return order;
}
...
function ensureSeeded() {
  ...
  // Товары: наполняем каталог только если файла нет. Каталог из
  ...
  // Ни первый запуск, ни восстановление каталога не публикуют демо-отзывы
  // и не перезаписывают уже сохранённые отзывы покупателей.
  if (!exists('reviews')) saveReviews([]);
