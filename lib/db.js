...
function orderFee(value, total) {
  ...
  const quote = FEE.quote(value.baseAmount, value.percent);
  if (!quote || quote.amount !== value.amount || quote.total !== total) return null;
  return { provider: 'platega', method: 'SBP_ONLINE', baseAmount: quote.baseAmount, amount: quote.amount, percent: quote.percent };
}
