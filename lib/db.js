...
function defaultSettings() {
  return {
    ...
    crocopayCurrency: 'RUB',   // базовая валюта кассы (ISO 4217)
    adminUsername: 'admin',
...
function createOrder(data) {
  ...
  const order = {
    ...
    // Адрес доставки, подсказки берутся из dadata.ru.
...
//
// Состояния: 'pending' — ссылка выдана, ждём вебхук; 'paid' — оплата подтверждена;
// 'mismatch' — вебхук пришёл с подписью, но сумма меньше ожидаемой, поэтому
// оплаченным заказ не считается и его должен посмотреть человек.
const PAYMENT_STATES = ['pending', 'paid', 'mismatch'];

// Начать оплату: записать ожидаемую сумму и одноразовый token, по которому потом
// узнаем вебхук именно этого заказа.
function startOrderPayment(id, data) {
  const list = getOrders();
  const order = list.find(x => x.id === id);
  if (!order) return null;
  // Уже подтверждённую оплату второй ссылкой не сбрасываем: иначе повторное
  // нажатие «Оплатить» обнулило бы token, и вебхук по прежней ссылке отвергся бы.
  if (order.payment && order.payment.status === 'paid') return order;
  order.payment = {
    provider: textValue(data && data.provider, 40) || 'crocopay',
    status: 'pending',
    token: /^[a-f0-9]{32}$/.test(String((data && data.token) || '')) ? data.token : null,
    amount: priceValue(data && data.amount, 0),          // ожидаем столько минимальных единиц
...
// Закрыть оплату по вебхуку. Идемпотентно: платёжка вправе повторить вызов, а
// второй раз писать заказ и уведомлять менеджера незачем — отсюда `changed`.
// Расхождение по сумме до 'paid' дорасти может (пришёл верный вебхук после
// неверного), обратно — нет.
...
module.exports = {
  ...
  getOrders, getOrder, createOrder, updateOrderClient, ordersForSite, setOrderStatus, deleteOrder,
  startOrderPayment, settleOrderPayment,
