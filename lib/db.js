...
function purgeOrder(id) {
  ...
  const order = list[i];
  if (!isOrderArchived(order)) return { ok: false, reason: 'not_archived', order };
  // Платёжная попытка — финансовый журнал, даже если касса не успела вернуть id
  // счёта. Стирать её нельзя: поздний callback/ручная сверка должны по-прежнему
  // находить заказ, а история отказов нужна для разбора с провайдером.
  if (order.payment) return {
    ok: false, reason: 'financial_history', order,
    hadInvoice: orderHasSettleableAttempt(order)
  };
  list.splice(i, 1);
  saveOrders(list);
