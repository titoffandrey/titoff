 ...
 *
 * `hasFunds` возвращается вызывающему, чтобы он мог предупредить о заказе, по
 * которому деньги ещё могут прийти; запрещать здесь нечего — решение владельца.
 */
 ...
 *
 * `hadInvoice` считает, у скольких из стёртых был выставлен счёт: запрета нет —
 * решение владельца, — но панель обязана сказать это до нажатия, а не после.
 */
function purgeArchivedOrders() {
  const list = getOrders();
  const doomed = list.filter(o => isOrderArchived(o));
  if (!doomed.length) return { ok: true, removed: 0, hadInvoice: 0 };
  saveOrders(list.filter(o => !isOrderArchived(o)));
  return {
    ok: true,
    removed: doomed.length,
    hadInvoice: doomed.filter(o => orderHasSettleableAttempt(o)).length
  };
}
...
module.exports = {
  ...
  updateOrderClient, setOrderStatus, archiveOrder, restoreOrder, deleteOrder, purgeOrder, purgeArchivedOrders,
