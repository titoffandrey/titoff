...
function deleteProduct(id) {
  ...
  // файлы фото, если они больше нигде не используются
  if (gone && gone.images) {
    const stillUsed = new Set();
    for (const p of getProducts()) for (const f of (p.images || [])) stillUsed.add(f);
    for (const f of gone.images) {
      if (stillUsed.has(f) || path.basename(f) !== f) continue;
...
function createReview(data) {
  ...
  const review = {
    ...
    photos: data.photos || [],
    aspects: data.aspects || null,   // {delivery, service, price} — оценки по аспектам 1..5
