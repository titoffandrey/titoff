...
function deleteReview(id) {
  ...
  if (gone) {
    for (const f of (gone.photos || [])) deleteUploadIfUnused(f);
    for (const f of (gone.videos || [])) deleteUploadIfUnused(f);
    if (gone.poster) deleteUploadIfUnused(gone.poster);
  }
}
...
function deleteReviewsForProduct(productId) {
  ...
  for (const rv of gone) {
    for (const f of (rv.photos || [])) deleteUploadIfUnused(f);
    for (const f of (rv.videos || [])) deleteUploadIfUnused(f);
    if (rv.poster) deleteUploadIfUnused(rv.poster);
  }
  return gone.length;
}
...
module.exports = {
  ...
  createReview, setReviewStatus, deleteReview, deleteReviewsForProduct, pendingReviewCount,
  saveReviews,
