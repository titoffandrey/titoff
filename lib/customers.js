...
function update(id, data) {
  ...
  const customer = patch(id, c => {
    ...
    if (d.email !== undefined && email !== String(c.email || '')) {
      c.email = email;
      // Ссылка восстановления была отправлена на ПРЕЖНИЙ адрес. После его
      // смены (или удаления) тот ящик больше не должен открывать кабинет.
      delete c.resetToken; delete c.resetUntil;
    }
