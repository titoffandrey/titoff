 ...
 */
/* Отбор по строке поиска. Нужного пункта может не быть в пятёрке ближайших —
 * тогда покупатель ищет его по улице. Сравниваем тем же разбором строки, что и
 * везде: «ул. Малышева» и «малышева» обязаны находить одно и то же.
 *
 * Ищем по всей строке пункта (город и улица с домом), а не только по улице:
 * человек из пригорода ищет и по названию соседнего города тоже.
 */
function matchesQuery(p, q) {
  if (!q) return true;
  return cityKey(shortOf(p)).includes(q);
}
...
function nearest(carrier, opts) {
  ...
  const all = index().byCarrier.get(key) || [];
  if (!all.length) return [];
  const q = cityKey(o.q || '');
  const list = q ? all.filter(p => matchesQuery(p, q)) : all;
  ...
  const inCity = pointsInCity(key, o.address);
  const city = q ? inCity.filter(p => matchesQuery(p, q)) : inCity;
...
function shape(p, km) {
  return {
    ...
    // Откуда точка. У данных OpenStreetMap лицензия требует указать авторство,
    // и витрина подписывает список — но только когда в нём есть такие точки.
    osm: p.source === 'osm',
...
/* Ближайший пункт как он лежит в базе — с регионом и городом, без подписей для
 * витрины. Нужен разбору точек OZON из OpenStreetMap (lib/pickup-osm.js): там
 * региона нет вовсе, а у нас он уже есть у каждого пункта СДЭК.
 */
function nearestRaw(carrier, lat, lon, maxKm) {
  if (!isCoord(lat, lon)) return null;
  const limit = Number(maxKm) > 0 ? Number(maxKm) : MAX_KM;
  const list = index().byCarrier.get(String(carrier || '')) || [];
  const dLat = limit / 111;
  const dLon = limit / Math.max(1, 111 * Math.cos(rad(lat)));
  ...
  return bestKm <= limit ? best : null;
}

module.exports = {
  FILE, VERSION, LIMIT, MAX_KM,
  load, save, empty, has, stats, staleNote,
  nearest, findPoint, nearestRaw, shape, addressOf, shortOf,
