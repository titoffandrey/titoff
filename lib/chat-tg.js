...
// Разбор ссылок консультанта — общий с витриной (`public/chat-links.js`).
...
function headline(chat) {
  ...
  /* Заказы собеседника и что с их оплатой.
   *
   * Половина разговоров про них и есть («оплатил, а статус прежний», «счёт не
   * открывается»), и без этой строки дежурный переспрашивает номер заказа у
   * человека, который сидит на сайте с открытой страницей оплаты. Строку
   * собирает сервер (`deps.ordersLine`): мост о хранилище заказов не знает
   * вовсе и знать не должен. */
  const orders = deps && typeof deps.ordersLine === 'function' ? String(deps.ordersLine(chat) || '') : '';
  if (orders) rows.push('🧾 ' + esc(orders));
  rows.push('🆔 <code>' + esc(chat.id.slice(0, 8)) + '</code>');
  // Диалог, начатый менеджером из панели, — не «новый диалог на сайте»: там
  // покупатель написал сам, здесь ему написали первым, и дежурному это первое,
  // что нужно понять о теме.
  const head = chat.startedBy === 'operator'
    ? '💬 <b>Написали покупателю из панели</b>' : '💬 <b>Новый диалог на сайте</b>';
  return head + '\n' + rows.join('\n')
    + '\n\nОтветьте в этой теме — покупатель увидит сообщение в окне чата, и ИИ замолчит.';
}

/* Кнопка под шапкой ровно одна, и она про то, кто ведёт разговор дальше.
 *
 * «Завершить» отсюда убрана вместе с самим завершением: переписка с покупателем
 * не кончается никогда — он вправе написать через неделю, и «завершённый»
 * разговор означал бы, что кто-то за него решил, что говорить больше не о чем.
 * Возвращать бота руками тоже нужно всё реже: не ответив пять минут, менеджер
 * отдаёт разговор консультанту сам (см. armTakeover в server.js). */
...
/* Диалоги, для которых тема прямо сейчас создаётся. Создание идёт до секунды, а
 * покупатель за это время успевает написать второй раз — без этой карты второй
 * вызов увидел бы пустой `topicId` и завёл ВТОРУЮ тему на тот же разговор. */
const opening = new Map();

/* Доставить реплику покупателя менеджеру. Одна дверь на весь мост, и это
 * важно: раньше маршрут сам решал, звать `openTopic` или `relayUser`, — и на
 * ПЕРВОМ сообщении звал только первое. В Telegram уходила шапка «Новый диалог
 * на сайте», а сам вопрос не уходил вовсе: на сайте он есть, у менеджера его
 * нет. Поймано на боевой витрине.
 *
 * Теперь порядок здесь и нарушить его нельзя: сперва тема (если её ещё нет),
 * потом сама реплика — всегда.
 */
async function deliverUser(chat, settings, text) {
  if (!configured(settings)) return false;
  if (!chat.topicId) {
    let job = opening.get(chat.id);
    if (!job) {
      job = openTopic(chat, settings, text).finally(() => opening.delete(chat.id));
      opening.set(chat.id, job);
    }
    // Тема не завелась — реплика всё равно уйдёт, только в общий чат: молчание
    // хуже неудобства (см. про запасной путь в шапке файла).
    await job.catch(() => {});
  }
  relayUser(chat, text);
  return true;
 ...
 *
 * Ссылка разворачивается в «Название (/адрес)» (`LINKS.plain`): в окне
 * покупателя скобки становятся ссылкой, а в Telegram читать их некому. */
function relayAi(chat, text) {
  enqueue(chat.topicId || '', Number(chat.topicId) || 0, '🤖 <i>' + esc(LINKS.plain(text)) + '</i>');
}

// Служебная строка в тему: диалог завершён, ИИ вернули, ИИ не ответил. Идёт
// только менеджеру — покупателю о смене собеседника говорит своя строка в ленте.
function relaySystem(chat, text) {
  enqueue(chat.topicId || '', Number(chat.topicId) || 0, 'ℹ️ <b>' + esc(text) + '</b>');
}

/* Реплика оператора, написанная из ПАНЕЛИ.
 *
 * Дверь та же, что у `deliverUser`, и по той же причине: разговор, начатый
 * менеджером, темы в Telegram ещё не имеет, а вести его дальше он будет именно
 * оттуда — там уведомления. Без темы первое сообщение ушло бы в никуда, а ответ
 * покупателя приехал бы в общий чат отдельной строкой без всякой связи с ним.
 *
 * Тема заводится тем же способом и через ту же карту `opening`, что и у
 * покупателя: два быстрых сообщения подряд не должны создать две темы.
 */
async function deliverOperator(chat, settings, text) {
  if (!configured(settings)) return false;
  if (!chat.topicId) {
    let job = opening.get(chat.id);
    if (!job) {
  ...
  enqueue(chat.topicId || '', Number(chat.topicId) || 0, '💬 <b>Из панели:</b> ' + esc(text));
  return true;
}
...
/* Команды в теме. Их две, и обе — про то, кто ведёт разговор:
 ...
 *
 * `/close` снята вместе с самим завершением диалога (см. `controls` выше).
 */
...
async function handleCallback(query) {
  ...
  if (action === 'ai') {
    ...
    await TG.answerCallback(token, query.id, 'ИИ снова отвечает');
  }
}
...
module.exports = {
  ...
  openTopic, deliverUser, deliverOperator, relayUser, relayAi, relaySystem, controls
};
