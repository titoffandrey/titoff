...
async function fetchTile(tile) {
  ...
  try {
    ...
    // Overpass сообщает о timeout и частичном результате через remark даже
    // при HTTP 200. Это не «в городе нет пунктов»: такой ответ не имеет права
    // стирать прежнюю плитку и откладывать следующую проверку на две недели.
    if (!data || !Array.isArray(data.elements)) throw new Error('Overpass: неверный формат ответа');
    if (String(data.remark || '').trim()) throw new Error('Overpass: неполный результат');
...
async function refreshTile(tile) {
  ...
  // Радиус запроса захватывает соседние плитки. Их уже известные пункты
  // заменяем по коду, иначе каждое обновление дописывало копии в список ПВЗ.
  const refreshed = new Set(points.map(p => p.code));
  const keep = base.points.filter(p => p.carrier !== 'ozon'
    || (tileFor(p.lat, p.lon).key !== tile.key && !refreshed.has(p.code)));
