...
function productForm(db, product) {
  ...
  const colorsText = (p.colors || []).map(c => `${c.name}|${c.hex || '#cccccc'}${c.inStock === false ? '|нет' : ''}`).join('\n');
  const storagesText = (p.storages || []).map(s => `${s.label}|${s.add || 0}${s.inStock === false ? '|нет' : ''}`).join('\n');
  ...
  const chip = src => `<div class="img-chip${src === mainSrc ? ' is-main' : ''}" data-src="${esc(src)}" draggable="true">
    ...
    <div class="img-chip-controls">
      ...
      <select class="img-color" name="imgcolor:${esc(src)}" aria-label="Цвет фотографии">${colorOpts(ic[src] || '')}</select>
  ...
  <form class="a-form product-form" data-product-form method="post" action="${isEdit ? '/owner/products/' + product.id : '/owner/products'}" enctype="multipart/form-data">
    ...
    <div class="field"><label>Характеристики</label><textarea class="specs-input" name="specs" rows="12">${esc(p.specs || '')}</textarea></div>
    <div class="a-panel product-options"><h2>Варианты товара</h2>
    ...
    ${isEdit ? `<div id="photo-manager" data-product="${esc(product.id)}" data-order="${esc(JSON.stringify(p.images || []))}">
      ...
      ${colorUploads}
    </div>` : `${uploadField('Добавить общие фото', 'name="images"')}
    ${colorUploads}`}
