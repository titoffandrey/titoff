...
function productForm(db, product) {
  ...
  const colorUploads = isEdit
    ...
    <p class="muted small">Эти фото автоматически привяжутся к цвету: на витрине они показываются при выборе этого цвета. Общие фото видны всегда. Поля появляются под каждый добавленный цвет — после выбора файлов нажмите «Сохранить».</p>
  </div>`
