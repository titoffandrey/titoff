...
function productForm(db, product, opts) {
  ...
  const bandsText = draft ? String(draft.bands || '') : (p.bands || []).map(g =>
    ...
    + (g.options || []).map(o => `- ${o.name} | ${o.hex || '#cccccc'} | ${o.add || 0}${o.inStock === false ? ' | нет' : ''}`).join('\n')
  ).join('\n');
  ...
  // Группы снимков: общие, по цвету корпуса и по каждой вариации ремешка
  const groupForBand = (g, o) => {
    const key = `${g.name}|${o.name}`;
    const list = (p.images || []).filter(src => (ib[src] || '') === key);
    return `<div class="img-group" data-band="${esc(key)}"${list.length ? '' : ' hidden'}>
      <div class="img-group-head"><span class="swatch" style="background:${R.cssColor(o.hex, '#cccccc')}"></span>${esc(g.name + ' · ' + o.name)}<span class="img-group-count"></span></div>
      <div class="img-chips">${list.map(chip).join('')}</div>
    </div>`;
  };
  const existing = [groupFor('', '')]
    .concat((p.colors || []).map(c => groupFor(c.name, c.hex)))
    .concat((p.bands || []).map(g => (g.options || []).map(o => groupForBand(g, o)).join('')))
    .join('');
  ...
  <div class="field" id="band-uploads" style="display:none">
    <label>Фото для конкретного ремешка</label>
