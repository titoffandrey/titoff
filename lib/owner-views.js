...
const OWNER_VISITOR_BASE = '/owner/analytics/visitor/';
...
function ordersList(db, flash, page) {
  ...
  const rows = slice.items.map(o => {
    // Возврат на ту же страницу: после удаления на 7-й странице владелец не
    // должен оказываться в начале списка.
    const back = `<input type="hidden" name="page" value="${slice.page}">`;
    return `<tr id="order-${esc(o.id)}">
      <td class="o-num"><b>${esc(R.orderNo(o.number))}</b><span class="muted small">${R.formatDate(o.createdAt)}</span>
        <span class="muted small">${esc(o.siteName || (sites[o.siteId] && sites[o.siteId].storeName) || '—')}</span></td>
      <td class="o-client">${R.orderClient(o, { metricsBase: OWNER_VISITOR_BASE, money: { currency: '₽' } })}</td>
      <td class="o-items">${R.orderItems(o)}</td>
      <td class="o-sum"><b>${R.money(o.total, { currency: '₽' })}</b>${R.paymentBadge(o)}</td>
      <td class="o-act"><form method="post" action="/owner/orders/${o.id}/delete" data-confirm="Удалить заказ?">${back}<button class="btn btn-sm btn-danger" aria-label="Удалить заказ">✕</button></form></td></tr>`;
  }).join('') || `<tr><td colspan="5" class="muted">Заказов нет.</td></tr>`;
  return layout({ active: 'orders', title: 'Все заказы', pendingCount: db.pendingReviewCount(), flash,
    body: `<div class="a-panel"><table class="a-table a-orders"><thead><tr><th>Заказ</th><th>Клиент</th><th>Состав</th><th>Сумма</th><th></th></tr></thead><tbody>${rows}</tbody></table>${pager}</div>` });
}
...
function analyticsPage(db, snapshot, selectedSiteId) {
  ...
  const body = AV.dashboard(snapshot, { products, siteSelect, rangeBase: '/owner/analytics?days=', ordersHref: '/owner/orders', visitorBase: OWNER_VISITOR_BASE }).replace(/(\/owner\/analytics\?days=\d+)/g, '$1' + siteQuery);
...
function visitorPage(db, visitor, opts) {
  opts = opts || {};
  const products = {}; db.getProducts().forEach(p => { products[p.id] = p.name; });
  const body = visitor
    ? AV.visitorPage(visitor, {
      products, backHref: '/owner/analytics', ordersHref: '/owner/orders', visitorBase: OWNER_VISITOR_BASE,
      orders: opts.orders || [], alsoOnIp: opts.alsoOnIp || []
    })
    : AV.visitorMissing(opts.key || '', { backHref: '/owner/analytics' });
  return layout({ active: 'analytics', title: 'Посетитель', pendingCount: db.pendingReviewCount(), wide: true, body });
}
...
function settingsPage(settings, db, flash, flashType) {
  ...
  const shownMethods = Array.isArray(settings.payMethods) ? settings.payMethods : PAY.DEFAULT_IDS;
...
module.exports = { loginPage, dashboard, productsList, productForm, reviewsList, addReviewForm, sitesList, siteForm, ordersList, analyticsPage, visitorPage, settingsPage };
