...
function productForm(db, product) {
  ...
  const p = product || { name: '', category: '', price: '', oldPrice: '', badge: '', inStock: true, shortDesc: '', description: '', specs: '', images: [], hotDeal: false, hotDealPrice: '', hotDealUntil: null, colors: [], storages: [] };
  const colorsText = (p.colors || []).map(c => `${c.name}|${c.hex || '#cccccc'}`).join('\n');
  const storagesText = (p.storages || []).map(s => `${s.label}|${s.add || 0}`).join('\n');
  const cats = db.categories();
  const ic = p.imageColors || {};
  const colorOpts = cur => `<option value="">— общее —</option>` + (p.colors || []).map(c =>
    `<option value="${esc(c.name)}" ${ic_sel(cur, c.name)}>${esc(c.name)}</option>`).join('');
  function ic_sel(cur, name) { return cur === name ? 'selected' : ''; }
  const mainSrc = (p.images || [])[0] || '';
  const chip = src => `<div class="img-chip${src === mainSrc ? ' is-main' : ''}" data-src="${esc(src)}">
    ...
    <select class="img-color" name="imgcolor:${esc(src)}">${colorOpts(ic[src] || '')}</select>
  </div>`;
  // Фото сгруппированы: сначала «общие», затем по каждому цвету товара
  const groupHead = (name, hex) => name
    ? `<div class="img-group-head"><span class="swatch" style="background:${esc(hex || '#ccc')}"></span>${esc(name)}<span class="img-group-count"></span></div>`
  ...
  const groupFor = (name, hex) => {
    const list = (p.images || []).filter(src => (ic[src] || '') === name);
    return `<div class="img-group" data-color="${esc(name)}"${list.length ? '' : ' hidden'}>
      ${groupHead(name, hex)}
      <div class="img-chips">${list.map(chip).join('')}</div>
    </div>`;
  };
  const existing = [groupFor('', '')].concat((p.colors || []).map(c => groupFor(c.name, c.hex))).join('');
  const colorUploads = isEdit
    ...
    <p class="muted small">Эти фото автоматически привяжутся к цвету: на витрине они показываются при выборе этого цвета. Общие фото видны всегда. Загрузка начинается сразу после выбора файлов.</p>
  </div>`
    : `<p class="muted small">Привязать фото к цветам можно после создания товара (на странице редактирования).</p>`;
  const body = `
  <form class="a-form" method="post" action="${isEdit ? '/owner/products/' + product.id : '/owner/products'}" enctype="multipart/form-data">
    ...
    <div class="a-panel"><h2>Варианты товара</h2>
      <div class="a-form-grid">
        <div class="field"><label>Цвета товара</label>
          ...
          <p class="muted small">Кружок слева — выбор оттенка (или впишите #hex), поле рядом — название. Цвета добавляются и удаляются кнопками, всё обновляется сразу.</p>
        </div>
        <div class="field"><label>Память / конфигурации</label>
          ...
          <p class="muted small">Слева метка (например «256 ГБ»), справа — полная цена этого варианта в ₽. Вариант с ценой, равной базовой, будет базовым. На сайтах с множителем цены масштабируются.</p>
        </div>
      </div>
    </div>
    ${isEdit ? `<div id="photo-manager" data-product="${esc(product.id)}">
      <div class="field" id="img-chips-wrap"${(p.images || []).length ? '' : ' hidden'}><label>Текущие фото</label>
        <p class="muted small">Наведите на фото: ★ — сделать главным, × — удалить. Ниже каждого снимка — цвет, к которому он привязан; при смене фото сразу переезжает в нужную группу.</p>
        <div class="img-groups" id="img-chips">${existing}</div>
      </div>
      <div class="field"><label>Добавить общие фото</label><input type="file" accept="image/*" multiple data-auto><p class="muted small">Файлы загружаются сразу после выбора — нажимать «Сохранить» для этого не нужно.</p></div>
      ${colorUploads}
    </div>` : `<div class="field"><label>Добавить общие фото</label><input type="file" name="images" accept="image/*" multiple></div>
    ${colorUploads}`}
