...
function productForm(db, product) {
  ...
  const existing = (p.images || []).map(src => `<div class="img-chip">
    <img src="/uploads/${esc(src)}" alt="">
    <select class="img-color" name="imgcolor:${esc(src)}">${colorOpts(ic[src] || '')}</select>
    <label class="img-remove"><input type="checkbox" name="removeImages" value="${esc(src)}"> удалить</label>
  </div>`).join('');
  const colorUploads = (isEdit && (p.colors || []).length) ? `<div class="field"><label>Фото для конкретного цвета</label>
    <div class="a-form-grid">${p.colors.map((c, i) =>
      `<div class="field"><label><span class="swatch" style="background:${esc(c.hex || '#ccc')}"></span> ${esc(c.name)}</label><input type="file" name="imagesColor_${i}" accept="image/*" multiple></div>`
    ).join('')}</div>
    <p class="muted small">Эти фото автоматически привяжутся к цвету: на витрине они показываются при выборе этого цвета. Фото «— общее —» видны всегда.</p>
  </div>` : (isEdit ? '' : `<p class="muted small">Привязать фото к цветам можно после создания товара (на странице редактирования).</p>`);
  const body = `
  <form class="a-form" method="post" action="${isEdit ? '/owner/products/' + product.id : '/owner/products'}" enctype="multipart/form-data">
    ...
    <div class="a-panel"><h2>Варианты товара</h2>
      <div class="a-form-grid">
        <div class="field"><label>Цвета (по строке «Название|#hex»)</label><textarea name="colors" rows="5" placeholder="Титановый чёрный|#3b3b3d&#10;Титановый белый|#f2f1ec">${esc(colorsText)}</textarea></div>
        <div class="field"><label>Память (по строке «Метка|доплата к базовой цене»)</label><textarea name="storages" rows="5" placeholder="256 ГБ|0&#10;512 ГБ|20000&#10;1 ТБ|50000">${esc(storagesText)}</textarea></div>
      </div>
      <p class="muted small">Первый объём памяти с доплатой 0 — базовый (равен базовой цене). Доплаты масштабируются множителем сайта.</p>
    </div>
    ${existing ? `<div class="field"><label>Текущие фото (у каждого можно выбрать цвет)</label><div class="img-chips">${existing}</div></div>` : ''}
    <div class="field"><label>Добавить общие фото</label><input type="file" name="images" accept="image/*" multiple></div>
    ${colorUploads}
