...
const { CURRENCIES: CROCO_CURRENCIES } = require('./crocopay');
const esc = R.esc;
  ...
  const body = `<form class="a-form" method="post" action="/owner/settings">
    ...
    <div class="a-panel"><h2>Онлайн-оплата (CrocoPAY)</h2>
      ...
      <div class="a-form-grid">
        ...
        <div class="field"><label>Базовая валюта кассы</label>
          <select name="crocopayCurrency">${CROCO_CURRENCIES.map(c => `<option value="${c}"${(settings.crocopayCurrency || 'RUB') === c ? ' selected' : ''}>${c}</option>`).join('')}</select></div>
      </div>
      ...
      Схема — Express: покупатель платит на форме CrocoPAY, реквизиты карт через наш сервер не проходят. Ключи хранятся только на сервере и на витрину не попадают.</p>
      ...
      Оплату подтверждает <b>вебхук</b> платёжки, поэтому возврат покупателя с формы ещё не значит «оплачено» — состояние видно в списке заказов.</p>
