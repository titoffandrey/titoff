...
function productForm(db, product, opts) {
  ...
  // Если снимок привязан к ремешку, в списке отмечаем именно ремешок: корпус
  // у такого фото задаётся переключателем в блоке загрузки, а не этим списком.
  const colorOpts = (cur, curBand) => `<option value="">— общее —</option>`
    + (p.colors || []).map(c => `<option value="${esc(c.name)}" ${curBand ? '' : ic_sel(cur, c.name)}>${esc(c.name)}</option>`).join('')
