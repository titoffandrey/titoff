...
function productForm(db, product, opts) {
  ...
  const p = product || { name: '', category: '', price: '', oldPrice: '', badge: '', inStock: true, shortDesc: '', description: '', specs: '', images: [], hotDeal: false, hotDealPrice: '', hotDealUntil: null, colors: [], storages: [], bands: [], options: [] };
  ...
  // «?Группа=знач1, знач2» — привязка к выбору в другой группе (8 ТБ только с
  // M5 Max). Своего поля в редакторе у неё нет, но в текст она обязана попасть:
  // иначе форма стёрла бы её при первом же сохранении товара.
  const choiceTail = (x) => x && x.forChoice
    ? ' | ?' + Object.keys(x.forChoice).map(k => k + '=' + (x.forChoice[k] || []).join(', ')).join('; ')
    : '';
  ...
  const optionsText = draft ? String(draft.options || '') : (p.options || []).map(g =>
    ...
    + (g.values || []).map(v => `- ${v.label} | ${v.add || 0}${v.inStock === false ? ' | нет' : ''}${(v.forStorage || []).length ? ' | @' + v.forStorage.join(', ') : ''}${choiceTail(v)}`).join('\n')
  ).join('\n');
