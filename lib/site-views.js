...
function ordersList(db, site, flash, page) {
  ...
  const rows = slice.items.map(o => {
    ...
    const sel = `<form method="post" action="/admin/orders/${o.id}/status" class="inline-form">${back}<select name="status" onchange="this.form.submit()">${statuses.map(x => `<option value="${x}" ${o.status === x ? 'selected' : ''}>${label[x]}</option>`).join('')}</select></form>`;
    return `<tr id="order-${esc(o.id)}"><td><b>${esc(R.orderNo(o.number))}</b><br><span class="muted small">${R.formatDate(o.createdAt)}</span></td>
      <td>${esc(o.customerName || '—')}<br><span class="muted">${esc(o.contact)}</span>${orderNote(o)}${clientMeta(o)}</td>
      <td class="small">${items}</td><td><b>${R.money(o.total, ss)}</b>${R.paymentBadge(o)}</td><td>${sel}</td>
      <td><form method="post" action="/admin/orders/${o.id}/delete" data-confirm="Удалить заказ?">${back}<button class="btn btn-sm btn-danger">✕</button></form></td></tr>`;
  }).join('') || `<tr><td colspan="6" class="muted">Заказов пока нет.</td></tr>`;
  return layout(site, { active: 'orders', title: 'Заказы', flash, body: `<div class="a-panel"><table class="a-table"><thead><tr><th>Заказ</th><th>Клиент</th><th>Состав</th><th>Сумма</th><th>Статус</th><th></th></tr></thead><tbody>${rows}</tbody></table>${pager}</div>` });
}
