...
function layout(site, opts) {
  ...
  ].map(([href, label, key]) => `<a href="${href}" class="a-nav-item${opts.active === key ? ' active' : ''}"${opts.active === key ? ' aria-current="page"' : ''}>${R.adminIcon(key)}<span>${label}</span></a>`).join('');
  return `<!DOCTYPE html><html lang="ru"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(site.storeName)} · админка</title>
<link rel="stylesheet" href="/static/styles.css?v=${R.assetV('styles.css')}"><style>:root{--accent:${R.cssColor(site.accentColor)}}</style>
...
function ordersList(db, site, flash, page) {
  ...
  const rows = slice.items.map(o => {
    ...
    return `<tr id="order-${esc(o.id)}">
      ...
      <td class="o-client">${R.orderClient(o, { metricsBase: SITE_VISITOR_BASE, money: ss })}</td>
      <td class="o-items">${R.orderItems(o)}</td>
      <td class="o-state">${R.orderStatus(o)}</td>
      <td class="o-pay">${R.orderPayMethod(o)}</td>
      <td class="o-sum"><b>${R.money(o.total, ss)}</b></td>
  ...
  }).join('') || `<tr><td colspan="7" class="muted">Заказов пока нет.</td></tr>`;
  return layout(site, { active: 'orders', title: 'Заказы', flash, body: `<div class="a-panel">${R.editSwitch('orders-edit')}<table class="a-table a-orders"><thead><tr><th>Заказ</th><th>Клиент</th><th>Состав</th><th>Статус</th><th>Оплата</th><th class="o-sum">Сумма</th><th class="o-act"><span class="sr-only">Удалить</span></th></tr></thead><tbody>${rows}</tbody></table>${pager}</div>` });
}
