...
// разрешает только уменьшение. Сильнее этого не растягиваем — пойдёт мыло.
 ...
 */
// Содержимое упирается в кадр по длинной стороне — после вписывания в 92 % это
// верно почти для всех снимков, поэтому решают именно углы. Требовать заполнения
// по ОБЕИМ сторонам нельзя: вертикальный крупный план задней панели занимает всю
// высоту, но лишь две трети ширины — и такой кадр проскакивал мимо проверки.
...
async function inspectCrop(bin, input, size) {
  ...
  if (!frame) return { frame: null, corners: [], cropped: false };
  const fills = Math.max(frame.w, frame.h) / size >= FILL_RATIO;
  const corners = fills ? await cornerMeans(bin, input, frame) : [];
  return { frame, corners, cropped: looksCropped(frame, corners, size) };
}
...
module.exports = { optimizeToWebp, optimizeMany, detectBin, squareTransformArgs, targetContentSize, contentBox, validImageOrder,
  contentFrame, cornerMeans, looksCropped, inspectCrop,
