...
const LIMITS = ['-limit', 'memory', '256MiB', '-limit', 'map', '512MiB', '-limit', 'thread', '1'];
...
async function measure(bin, input, ops) {
  try {
    const { stdout } = await execFileP(bin, [input].concat(LIMITS, ops || [], ['-format', '%wx%h', 'info:']), { timeout: 12000 });
    const m = String(stdout).trim().match(/^(\d+)x(\d+)/);
    return m ? { w: Number(m[1]), h: Number(m[2]) } : null;
  } catch (e) { return null; }
}
...
async function optimizeToWebp(dir, filename, maxSize, opts) {
  ...
  let args = [input].concat(LIMITS, ['-auto-orient', '-strip']);
  if (opts.square) {
  ...
  args = args.concat(['-define', 'webp:method=4', '-quality', '82', outPath]);
  try {
