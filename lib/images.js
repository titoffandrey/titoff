...
// opts.upscale === false: мелкий товар не увеличивать (повторная обработка готового кадра).
...
// upscale:false — мелкий товар не увеличивать. Так идёт повторная обработка уже
// приведённого к кадру файла (refit): решение об увеличении принято при загрузке
// по исходному разрешению, и пересматривать его по перекодированным 1200 px
// значило бы растянуть мелкий исходник второй раз поверх первого — размылить.
function targetContentSize(box, maxSize, opts) {
  ...
  if (opts && opts.upscale === false) return longest;
...
async function optimizeToWebp(dir, filename, maxSize, opts) {
  ...
  if (opts.square) {
    ...
    const fit = targetContentSize(box, maxSize, { upscale: opts.upscale });
  ...
  try {
    ...
    if (opts.square && opts.trim !== false) return optimizeToWebp(dir, filename, maxSize, { square: true, trim: false, upscale: opts.upscale });
 ...
 */
/* Нижняя ступень 200 — для плиток категорий на главной (19 сентября 2026,
 * второй отчёт PageSpeed: «улучшите загрузку изображений — 35 КиБ», и почти
 * всё это девять плиток). На телефоне плитка стоит в три колонки и её снимок —
 * около 100 CSS-пикселей: 170 точек при DPR 1.75 (устройство PageSpeed) и
 * 200 при DPR 2, а самой мелкой копией была 320 — вдвое больше по площади у
 * каждой из девяти. DPR 3 берёт 320, как и брал. Карточкам каталога ступень
 * не мешает: их `sizes` просит не меньше 300 точек. */
const CARD_SUFFIX = '-c';
const CARD_SIZES = [200, 320, 480, 640];
...
// Плитке редактора (92 CSS-px) достаточно самой маленькой готовой копии.
// Старые загрузки без копий остаются доступны по исходному имени.
function previewName(dir, file) {
  const card = cardSources(dir, file);
  return card ? card.sizes[0].name : file;
}
...
module.exports = { montage, optimizeToWebp, optimizeMany, detectBin, squareTransformArgs, targetContentSize, contentBox, imageSize, validImageOrder,
  ...
  makeCards, cardName, cardSources, previewName, derivedNames, isDerived, uploadFiles, CARD_SIZES, CARD_SUFFIX,
