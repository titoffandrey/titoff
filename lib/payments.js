...
async function availableOptions(settings) {
  ...
  const answers = await Promise.all(list.map(async p => {
    try {
      ...
      return r && r.ok ? { provider: p, live: r } : { provider: p, live: null };
    } catch (e) { return { provider: p, live: null }; }
  }));
  const answered = answers.filter(a => a.live);
  if (!answered.length) return null;
  const byCurrency = {};
  const byProvider = {};
  for (const { provider: p, live } of answers) {
    byProvider[p.id] = live || null;
    if (!live) continue;
  ...
  return { currencies, byCurrency, byProvider, options: byCurrency[PAY.BASE] || [] };
}
 ...
 */
function chainFor(settings, live, methodId, currency) {
 ...
 */
const CODE_PRIORITY = ['no_requisite', 'timeout', 'amount', 'method_unavailable',
  'route_mismatch', 'invoice_invalid', 'provider_error'];
...
module.exports = {
  ...
  providerIds, provider, nameOf, ordered, enabledProviders, configuredProviders,
  enabled, configured, payable, limitError, limits, limitFor,
  availableOptions, chainFor, forgetMethods, sameStartRequest, requestIdFor,
  CODE_PRIORITY, summaryErrorCode,
