...
// Комиссия входит в итог ДО оформления. Только один заранее выбранный маршрут:
// при выборе разных касс нельзя приписывать всем тариф конкретного сервиса.
function checkoutFee(settings, baseAmount) {
  const methods = offeredMethods(settings);
  if (methods.length !== 1 || methods[0].id !== 'SBP_ONLINE' || methods[0].provider !== 'platega') return null;
  const quote = FEE.quote(baseAmount, settings.plategaFeePercent == null ? 0 : settings.plategaFeePercent);
  return quote ? { provider: 'platega', method: 'SBP_ONLINE', ...quote } : null;
}
...
module.exports = {
  ...
  ownRequisites, ownEnabled, mode, offeredMethods, checkoutFee,
