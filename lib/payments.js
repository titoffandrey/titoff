 ...
 */
/* Третья касса — Альфа-Банк — устроена иначе двух первых, и это стоит держать в
 * голове, читая всё ниже. CrocoPAY и MeridianPay — P2P: у них конечный пул чужих
 * реквизитов, отказ «нет свободной карты» штатный, и вторая касса заводилась
 * ровно затем, чтобы его пережить. У Альфы эквайринг: реквизитом приходит ссылка
 * на её собственную платёжную страницу, пула нет, деньги идут на счёт магазина.
 * Поэтому она умеет ровно один способ (`CARD_ONLINE`) и только рубли — очередь
 * это учитывает сама, через `supports()` и `acceptsAmount()`.
 *
 * Пятая — Solutiones: на заказ выдаёт ссылку СБП (`qr.nspk.ru`), способ у неё
 * один (`SBP_ONLINE`) и только рубли; пула реквизитов нет, комиссию удерживает
 * из выплаты магазину, сумму покупателя не меняет (см. lib/solutiones.js). */
const PROVIDERS = [CROCO, MERIDIAN, ALFA, PLATEGA, SOLUTIONES];
...
/* ПОТОЛОК КАССЫ БОЛЬШЕ НЕ ГАСИТ ТОВАРЫ — заказ дороже него уходит другим путём.
 *
 * Прежде пределы касс были пределами витрины: товар дороже 250 000 ₽ при
 * включённой кассе становился «Нет в наличии», и это было терпимо ровно
 * потому, что таких товаров было три. С Platega потолок стал 20 000 ₽ на
 * платёж (лимит СБП кассы, см. lib/platega.js), и то же правило погасило бы
 * весь каталог iPhone на обоих сайтах. Владелец о такой цене не просил.
 *
 * Поэтому режим витрины теперь считается ПО СУММЕ ЗАКАЗА (`modeFor`): пока
 * заказ помещается в кассу — оплата на витрине через неё; дороже — по своим
 * реквизитам, если они заполнены, иначе заявкой менеджеру, как у магазина
 * без касс. Потолка на витрине при этом нет вовсе: у режимов «свои
 * реквизиты» и «заявки» его не было никогда, и у их сочетания с кассой ему
 * взяться неоткуда. `limits()` остаётся ради вызывающих (разметка товара,
 * плавающие цены, витринный скрипт), но отдаёт «предела нет»; границы самой
 * кассы спрашивают `payable()` и `cashboxMax()`.
 */
function limits() { return { min: 0, max: 0 }; }
function limitFor() { return ''; }

/* Потолок ОДНОЙ кассы: её собственный (`p.maxTotal`, есть у Platega) в
 * границах общих настроек. Кассы без своего потолка живут по `payMaxTotal`. */
function providerMax(p, settings) {
  const max = boundsOf(settings).max;
  const own = p && typeof p.maxTotal === 'function' ? p.maxTotal(settings) : 0;
  // Ноль у кассы — «своего потолка нет» (Solutiones по решению владельца).
  return own > 0 ? Math.min(max, own) : max;
}
// До какой суммы заказ вообще может уйти в кассу — самый высокий потолок среди
// включённых. Ноль — касс нет. Витрина берёт число атрибутом, чтобы подписать
// кнопку по сумме («Оплатить» до потолка, «Оформить заказ» дороже).
function cashboxMax(settings) {
  const providers = enabledProviders(settings);
  return providers.length ? Math.max(...providers.map(p => providerMax(p, settings))) : 0;
}
/* Сумма, которую увидит КАССА. У Platega комиссия включена в цену: касса
 * получает округлённый вниз итог (`FEE.checkout`, скидка не больше рубля), и
 * потолок 20 000 ₽ «вместе с комиссией» сравнивается именно с ним. Ноль —
 * такой заказ в эту кассу не уходит (итог не собрался). */
function providerTotal(p, settings, total) {
  if (!p || p.id !== PLATEGA.id) return Number(total) || 0;
  const quote = checkoutFee(settings, total);
  return quote && quote.paymentTotal > 0 ? quote.paymentTotal : 0;
}
/* Возьмётся ли ХОТЬ ОДНА касса за заказ на эту сумму — без единого запроса в
 * сеть. Касса считается только та, чей способ правда предложен покупателю
 * (`offeredMethods`): включённая касса без отмеченного способа заказ не
 * обслужит, и обещать ей оплату нельзя. */
function cashboxAccepts(settings, total) {
  const sum = Number(total);
  if (!enabled(settings) || !Number.isFinite(sum) || sum <= 0) return false;
  const offered = offeredMethods(settings).map(m => m.id);
  return enabledProviders(settings).some(p => {
    // Способ может обслуживать несколько касс. Имя первой нужно для подписи
    // в интерфейсе, но её отказ по сумме не выключает резервную кассу.
    if (!offered.some(method => p.supports(method))) return false;
    const amount = providerTotal(p, settings, sum);
    return amount > 0 && payable(amount, settings)
      && (!p.acceptsAmount || p.acceptsAmount(amount, PAY.BASE, null, settings));
  });
}
/* Чем витрина принимает ЭТОТ заказ. Одно место на весь проект — от него
 * зависят черновик, страница оплаты, подпись кнопки и факты консультанта:
 *   cashbox — заказ помещается в кассу, покупатель платит на витрине;
 *   own     — дороже кассы (или касс нет), но заполнены свои реквизиты;
 *   request — платить на сайте нечем, заявку разбирает менеджер.
 * Без суммы отвечает про витрину в целом (`mode`). */
function modeFor(settings, total) {
  if (total === undefined) return mode(settings);
  if (cashboxAccepts(settings, total)) return 'cashbox';
  return ownRequisites(settings).ready ? 'own' : 'request';
}
// Куда уходит заказ, который в кассу не помещается. Для витрины и панели.
function fallbackMode(settings) { return ownRequisites(settings).ready ? 'own' : 'request'; }

/*
 * Какие способы оплаты покупатель ПРАВДА получит — без единого запроса в сеть.
 *
 * Галочек владельца мало: они остаются стоять и у способов ВЫКЛЮЧЕННЫХ касс. На
 * боевой витрине так и было — отмечены «СБП», «Перевод на карту» и «Карта», при
 * том что включена одна касса и умеет она только карту. Поэтому отмеченное
 * пересекается с тем, что умеют ВКЛЮЧЁННЫЕ кассы (`supports` — чистая функция,
 * никакой сети). Живой список (что включено у кассы прямо сейчас) остаётся
 * делом `payContext()` в server.js: отсеет он что-то — `/api/pay/start` откажет.
 *
 * Каждому способу приписывается первая по очереди касса, которая его умеет, —
 * та же, что первой получит запрос (`chainFor`). По ней подписывается способ
 * (`PAY.describe(id, provider)`): у Альфы `CARD_ONLINE` — это «СБП».
 *
 * Спрашивают это двое, и обязаны получать один ответ: оформление (вести ли
 * покупателя на страницу банка сразу, `directPayMethod`) и консультант в чате
 * (какими словами описать оплату). Второй расчёт того же списка разошёлся бы с
 * первым молча.
 */
function offeredMethods(settings) {
  const s = settings || {};
  if (mode(s) !== 'cashbox') return [];
  const own = Array.isArray(s.payMethods) ? s.payMethods : PAY.DEFAULT_IDS;
  const providers = enabledProviders(s);
  const list = [];
  for (const raw of own) {
    const id = String(raw == null ? '' : raw).trim();
    if (!id || list.some(m => m.id === id)) continue;
    const p = providers.find(x => x.supports(id));
    if (p) list.push({ id, provider: p.id });
  }
  return list;
}

// Расходы Platega включены в прежние цены. Снимок нужен и при выборе касс:
// уменьшенная база применяется только при фактической оплате через Platega.
function checkoutFee(settings, totalAmount) {
  const methods = offeredMethods(settings);
  if (!methods.some(m => m.id === 'ONLINE_PAYMENT' && m.provider === 'platega')) return null;
  const quote = FEE.checkout(totalAmount, settings.plategaFeePercent == null ? 0 : settings.plategaFeePercent);
  return quote ? { provider: 'platega', method: 'ONLINE_PAYMENT', ...quote } : null;
}
...
async function availableOptions(settings) {
  ...
  for (const { provider: p, answer } of answers) {
    ...
    status[p.id] = {
      ...
      stale: !!answer.stale,
      verified: answer.verified !== false,
...
const HEALTH_STATES = [
  ...
  // У Альфы то же самое приходит по-русски: «Доступ запрещен» на неверный
  // платёжный токен.
  ['auth', /доступ\s+запрещ|invalid access token|verify the client|unauthor|forbidden|access denied|invalid client|http_40[13]/i],
  // Не дошли вовсе: сеть, таймаут, чужая пятисотка. Сюда же несовпавший пин
  // сертификата — до банка мы в этом случае не доходим вовсе.
  ['down', /timeout|abort|fetch failed|network|socket|econn|enotfound|getaddrinfo|tls_pin|pin_mismatch|http_5\d\d/i]
];
function healthState(entry) {
  if (!entry) return 'unknown';
  if (entry.ok) return entry.verified === false ? 'unverified' : 'ok';
...
function chainFor(settings, live, methodId, currency, amount) {
  ...
  return enabledProviders(settings).filter(p => {
    ...
    if (amount !== undefined && p.acceptsAmount && !p.acceptsAmount(amount, cur, own, settings)) return false;
    if (!own) return true;
...
module.exports = {
  ...
  ownRequisites, ownEnabled, mode, modeFor, fallbackMode, cashboxAccepts, cashboxMax, providerMax, providerTotal,
  offeredMethods, checkoutFee,
