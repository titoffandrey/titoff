...
function chatList(settings, db, flash, page) {
  ...
  const rows = slice.items.map(c => {
    ...
    const who = last ? (last.role === 'user' ? 'Покупатель' : last.role === 'operator' ? (last.by || 'Менеджер') : last.role === 'ai' ? 'Консультант' : '') : '';
...
function chatPage(settings, db, chat, flash) {
  const rows = chat.messages.map(m => {
    const who = m.role === 'user' ? (chat.name || 'Покупатель')
      : m.role === 'operator' ? (m.by || 'Менеджер')
        : m.role === 'ai' ? 'Консультант' : '';
