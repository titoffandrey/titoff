...
function layout(settings, opts) {
  ...
  // общей карточкой. Ошибки остаются до явного закрытия.
...
function flashKind(text, type) {
  ...
  if (/не удалось|ошибк|не найден|не отправлен|не сохран|не разобрал|укажите|некоррект|недопустим|нет связи|отказ|неверн/.test(value)) return 'error';
  if (/выключен|задержк|ист[её]к|просрочен|лимит|пропущен/.test(value)) return 'warning';
...
function flashNote(text, type) {
  ...
  return `<div class="a-flash ${kind === 'error' ? 'err' : 'ok'}"${kind === 'error' ? ' data-flash' : ''}`
    + ` data-store-toast data-toast-type="${kind}" data-toast-duration="${kind === 'error' ? 0 : 6000}" role="${kind === 'error' ? 'alert' : 'status'}">${esc(text)}</div>`;
}
...
function noteCard(kind, data) {
  ...
  return `<div class="a-note-${esc(kind)}" data-note data-store-toast data-toast-type="info" data-toast-duration="12000"`
...
function productForm(settings, db, product, opts) {
  ...
  <form class="a-form product-form" data-product-form method="post" action="${isEdit ? '/admin/products/' + encodeURIComponent(product.id) : '/admin/products'}" enctype="multipart/form-data">
    ${errors.length ? `<div class="form-alert" role="alert" data-store-toast data-toast-type="error" data-toast-duration="0" data-toast-id="admin-product-errors" data-toast-message="${esc('Не сохранено. ' + errors.map(e => e.text).join('; '))}"><b>Не сохранено.</b> ${errors.length === 1 ? 'Поправьте одно поле:' : 'Поправьте поля:'}
...
function payMethodsSection(shown, live) {
  ...
  return `<h3 class="a-subhead">Способы оплаты на витрине</h3>
      ...
      ${shown.length ? '' : '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-duration="0" data-toast-id="admin-config-payment-methods" data-toast-message="Не отмечен ни один способ — оплатить на витрине будет нечем.">Не отмечен ни один способ — оплатить на витрине будет нечем.</p>'}`;
}
...
function ownPayFields(settings, s) {
  ...
  return `<details class="pay-fold"${open}>
      ...
      ${own.on && !own.ready
        ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-duration="0" data-toast-id="admin-config-own-payment" data-toast-message="Нужны имя получателя и хотя бы один реквизит — карта или телефон.">Нужны имя получателя и хотя бы один реквизит — карта или телефон.</p>' : ''}
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const sections = [
    ...
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-duration="0" data-toast-id="admin-config-crocopay" data-toast-message="CrocoPAY: Касса включена, но ключи не заданы — реквизиты она не выдаст.">Касса включена, но ключи не заданы — реквизиты она не выдаст.</p>' : ''}
      <div class="a-form-grid">
    ...
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-duration="0" data-toast-id="admin-config-meridianpay" data-toast-message="MeridianPay: Касса включена, но ключ или Merchant ID не заданы — реквизиты она не выдаст.">Касса включена, но ключ или Merchant ID не заданы — реквизиты она не выдаст.</p>' : ''}
      <div class="a-form-grid">
