...
<link rel="stylesheet" href="/static/admin.css?v=${R.assetV('admin.css')}">
<link rel="stylesheet" href="/static/tailadmin.css?v=${R.assetV('tailadmin.css')}">
<link rel="stylesheet" href="/static/smoothui-toast.css?v=${R.assetV('smoothui-toast.css')}">${shellStyle(settings)}</head>
...
function dashboard(settings, db, pulse, opts) {
  ...
  const recent = orders.slice(0, RECENT_ORDERS).map(o => {
    ...
    return TA.render('orderRow', {
      ID: 'recent-' + esc(o.id),
      PRODUCT: `<a class="ta-product-cell" href="/admin/orders#order-${esc(o.id)}"><span class="ta-product-symbol">${R.adminIcon('products')}</span><span><b>${esc(name)}</b><small>${esc(R.orderNo(o.number))} · ${R.formatDate(o.createdAt)}</small></span></a>`,
      CUSTOMER: esc(o.customerName || o.contact || 'Без контакта'), TOTAL: R.money(o.total, settings), STATUS: R.orderStatus(o)
    });
...
function ordersList(settings, db, flash, page, edit, filters, opts) {
  ...
  const rows = slice.items.map(o => {
    ...
    return TA.render('orderFullRow', {
      ...
      PRODUCT: `<div class="ta-product-cell"><span class="ta-product-symbol">${R.adminIcon('products')}</span><span><b>${esc(firstItem.name || firstItem.title || 'Заказ')}</b><small>${esc(R.orderNo(o.number))} · ${R.formatDate(o.createdAt)}${itemCount > 1 ? ' · товаров: ' + itemCount : ''}</small></span></div>`,
  ...
  return layout(settings, { active: 'orders', title: 'Заказы', counts: navCounts(settings, db), flash, live: 'orders',
    ...
    body: '<div class="orders-page">' + periodBar
      ...
      })}${TA.render('ordersTable', { ROWS: rows })}<noscript><style>body.admin .ta-order-dialog{display:block;position:static;width:auto;max-height:none;margin:16px 0}body.admin .ta-order-open{display:none}</style></noscript>${pager}</div>`
      + summaryDetails
