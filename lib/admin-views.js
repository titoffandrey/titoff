...
const TA = require('./tailadmin');
const AC = require('./admin-charts');
...
// Личный кабинет и почта — ради строки состояния раздела настроек.
...
// Геоназвания приходят из DB-IP латиницей. Новые карточки метрика переводит
// при записи, но старые диалоги уже хранят исходные строки — список обязан
// локализовать и их, иначе рядом оказываются «Белгород» и «Amsterdam».
...
function layout(settings, opts) {
  ...
  const nav = [
    ...
    return TA.navItem(href, label, R.adminIcon(key), badge, opts.active === key);
  }).join('');
  // На компьютере разделы постоянно видны слева; на телефоне тот же список
  // открывается чекбоксом. Он остаётся вне live-блоков, чтобы обновление
  // счётчиков не закрывало меню. Его состояние не включает desktop sidebar.
  const menuBtn = `<label class="a-menu-btn" for="a-menu" title="Разделы" role="button" tabindex="0" aria-label="Открыть меню" aria-expanded="false" aria-controls="a-menu-navigation">`
    + TA.toggleIcon
  ...
  const menu = `<input type="checkbox" id="a-menu" class="a-menu-check" aria-label="Разделы">
    <div class="a-menu-wrap">
      ...
      <div class="a-menu-panel" data-live-part="menu">${TA.render('sidebar', {
        BRAND: `<div class="a-menu-brand"><a class="a-brand" href="/admin"><span class="a-brand-icon">${R.adminIcon('products')}</span><span class="a-brand-name">${esc(settings.storeName || 'Магазин')}<small>Панель управления</small></span></a><label class="a-menu-close" for="a-menu" role="button" tabindex="0" aria-label="Закрыть меню">×</label></div>`,
        NAV: nav + `<li><span class="a-menu-sep" aria-hidden="true"></span></li>` + TA.navItem('/', 'Открыть витрину', R.adminIcon('sites'), '', false, true)
      })}</div>
    </div>`;
  ...
  // Сервер оставляет читаемый текст без JavaScript; SmoothUI заменяет его
  // общей карточкой с исходным временем показа SmoothUI.
  const flashText = String(opts.flash == null ? '' : opts.flash);
  const flashNoteHtml = flashText ? flashNote(flashText, opts.flashType) : '';
  const flashErr = !!flashText && flashKind(flashText, opts.flashType) === 'error';
...
${TA.themeBoot}
<link rel="icon" href="/static/favicon.ico?v=${R.assetV('favicon.ico')}">
${/* Стили панели — отдельным файлом ПОСЛЕ общего: витрина его не грузит, а
     каскад между ними тот же, что был внутри одного styles.css (правило панели
     всегда шло после общего). См. шапку public/admin.css. */''}
<link rel="stylesheet" href="/static/styles.css?v=${R.assetV('styles.css')}">
<link rel="stylesheet" href="/static/admin.css?v=${R.assetV('admin.css')}">
<link rel="stylesheet" href="/static/tailadmin.css?v=${R.assetV('tailadmin.css')}">
<link rel="stylesheet" href="/static/smoothui-toast.css?v=${R.assetV('smoothui-toast.css')}">${shellStyle(settings)}
${/* Без JS карточка заказа раскрыта прямо в строке, кнопка «Открыть» не нужна.
   Правило стоит ЗДЕСЬ, в <head>, а не рядом с таблицей: внутри живого блока
   `<noscript>` при подмене разметки (DOMParser разбирает его как элементы)
   становился настоящим <style>, и все закрытые dialog раскрывались в таблице. */''}
<noscript><style>body.admin .ta-order-dialog{display:block;position:static;width:auto;max-height:none;margin:16px 0}body.admin .ta-order-open{display:none}</style></noscript>${opts.active === 'orders' ? `<link rel="stylesheet" href="/static/admin-orders.css?v=${R.assetV('admin-orders.css')}">` : ''}${opts.active === 'analytics' ? `<link rel="stylesheet" href="/static/admin-metrics.css?v=${R.assetV('admin-metrics.css')}">` : ''}</head>
<body class="admin${opts.app ? ' a-app' : ''}"${live ? ` data-live="${esc(live)}"` : ''}>
<div class="a-shell">
  <main class="a-main">
    ${menu}
    <div class="a-topbar" data-live-part="topbar">${TA.render('header', {
      CONTENT: menuBtn + TA.render('headerBrand', { BRAND: esc(settings.storeName || 'Магазин') }) + TA.render('actionsToggle', {}) + TA.render('headerSearch', {}),
      ACTIONS: liveMark + TA.themeToggle + TA.render('notificationMenu', { COUNT: pending || '', ITEMS: `<h3>Уведомления</h3><a href="/admin/orders">Новые заказы <b>${badges.orders}</b></a><a href="/admin/reviews">Отзывы на модерации <b>${badges.reviews}</b></a><a href="/admin/chat">Непрочитанные сообщения <b>${badges.chat}</b></a>` }) + TA.render('profileMenu', { INITIAL: esc(String(settings.adminUsername || 'A').slice(0,1).toUpperCase()), NAME: esc(settings.adminUsername || 'Администратор') })
    })}</div>
    ${flashErr ? flashNoteHtml : ''}
    <div class="a-content${opts.wide ? ' a-content-wide' : ''}" data-live-part="content">${TA.render('breadcrumb', { TITLE: esc(opts.title || ''), ACTIONS: (opts.active === 'orders' && Number.isFinite(opts.ordersPaidCount) ? `<a class="of-paid-count" href="${esc(opts.ordersPaidHref || '/admin/orders?pay=ok')}">Оплатили (${opts.ordersPaidCount})</a>` : '') + (opts.actions ? `<div class="a-topbar-acts">${opts.actions}</div>` : '') })}${accessibleBody}</div>
  </main>
</div>
${live || (flashNoteHtml && !flashErr) ? `<div id="a-notes">${flashErr ? '' : flashNoteHtml}</div>` : ''}
...
${/* ГДЕ СТОЯЛИ — ТУДА И ВОЗВРАЩАЕМСЯ.
   Действие в панели — это POST и та же страница заново, поэтому браузер ставит
   её на начало: разобрав заявку внизу списка, менеджер оказывался наверху и
   искал своё место сам. Отметка живёт в sessionStorage ровно одну навигацию и
   помнит ПУТЬ: заявка со списка возвращает на список (туда и надо), а форма
   товара уводит в каталог — это другая страница, и её начало правильное.
   Скрипт ЗДЕСЬ, а не в admin-ui.js, по единственной причине: вернуться надо до
   первой отрисовки, а отложенный файл успевает показать страницу сверху и
   дёрнуть её на глазах. Обе половины рядом, чтобы не разъехались.
   Отменённую подтверждением отправку не запоминаем (`defaultPrevented`), а
   страницу с ошибкой не возвращаем на место вовсе: объяснение стоит наверху.
   Высоту документа перед прокруткой СПРАШИВАЕМ, и не для проверки: чтение
   `scrollHeight` заставляет браузер посчитать раскладку. Без него он вправе
   считать, что страница ещё нулевой высоты, — и обрезать прокрутку до нуля,
   молча. Повтор по `DOMContentLoaded` — на случай, когда так и вышло; на уже
   прокрученной странице он ничего не делает и человеку под руку не лезет. */''}
<script>(function(){var K='a-scroll',v;document.addEventListener('submit',function(e){if(e.defaultPrevented)return;try{sessionStorage.setItem(K,location.pathname+'|'+Math.round(window.pageYOffset||0)+'|'+Date.now())}catch(x){}});try{v=sessionStorage.getItem(K);sessionStorage.removeItem(K)}catch(x){return}if(!v||document.querySelector('[data-flash]'))return;var p=v.split('|'),y=+p[1];if(p[0]!==location.pathname||!(y>0)||!(Date.now()-p[2]<30000))return;function go(){if(!window.pageYOffset&&document.documentElement.scrollHeight)window.scrollTo(0,y)}go();document.addEventListener('DOMContentLoaded',go);})();</script>
<script src="/static/smoothui-toast.js?v=${R.assetV('smoothui-toast.js')}" defer></script>
<script src="/static/mobile-shell.js?v=${R.assetV('mobile-shell.js')}" defer></script>
<script src="/static/tailadmin-ui.js?v=${R.assetV('tailadmin-ui.js')}" defer></script>
${accessibleBody.includes('data-ta-chart=') ? `<script src="/static/tailadmin-charts.js?v=${R.assetV('tailadmin-charts.js')}" defer></script>` : ''}
<script src="/static/admin-ui.js?v=${R.assetV('admin-ui.js')}" defer></script>
<script src="/static/admin-presence.js?v=${R.assetV('admin-presence.js')}" defer></script>
 ...
 * Сервер передаёт текст, тип и адрес. Отображает их исходный компонент
 * SmoothUI BasicToast; запасной HTML остаётся читаемым без JavaScript.
 */
const NOTE_TEXT_MAX = 90;

function flashKind(text, type) {
  const aliases = { err: 'error', ok: 'success', error: 'error', success: 'success', warning: 'warning', info: 'info' };
  const value = String(text || '').toLowerCase();
  // Просьба заполнить или выбрать — подсказка, даже если старый маршрут
  // передал её общим полем ошибки формы.
  if ((!type || type === 'err' || type === 'error') && /^(?:сначала\s+)?(?:укажите|выберите|введите|напишите|добавьте|прикрепите|отметьте|подтвердите)(?:\s|$)/.test(value)) return 'info';
  if (aliases[type]) return aliases[type];
  if (/не удалось|ошибк|не отправлен|не сохран|нет связи|отказ/.test(value)) return 'error';
  if (/не найден|не разобрал|некоррект|недопустим|неверн|выключен|задержк|ист[её]к|просрочен|лимит|пропущен/.test(value)) return 'warning';
  if (/жд[её]т|ожида|проверя|обрабатыва/.test(value)) return 'info';
  if (/сохран|создан|добавлен|удал[её]н|измен[её]н|отправлен|снят|очищен|восстановлен|оплачен|готово|исключ[её]н|смен[её]н/.test(value)) return 'success';
  return 'info';
}

function flashNote(text, type) {
  const kind = flashKind(text, type);
  return `<div class="a-flash ${kind === 'error' ? 'err' : 'ok'}"${kind === 'error' ? ' data-flash' : ''}`
    + ` data-store-toast data-toast-type="${kind}" role="${kind === 'error' ? 'alert' : 'status'}">${esc(text)}</div>`;
}
...
function noteCard(kind, data) {
  const message = [data.title, data.text].filter(Boolean).join(' · ');
  return `<div class="a-note-${esc(kind)}" data-note data-store-toast data-toast-type="info"`
    + ` data-toast-message="${esc(message)}" data-toast-href="${esc(data.href)}">`
    + `<a class="a-note-link" href="${esc(data.href)}">${esc(message)}</a></div>`;
}
...
${/* Без JS карточка заказа раскрыта прямо в строке, кнопка «Открыть» не нужна.
   ...
   `<noscript>` при подмене разметки (DOMParser разбирает его как элементы)
   становился настоящим <style>, и все закрытые dialog раскрывались в таблице. */''}
<noscript><style>body.admin .ta-order-dialog{display:block;position:static;width:auto;max-height:none;margin:16px 0}body.admin .ta-order-open{display:none}</style></noscript></head>
<body class="admin login-body">
${TA.render('signin', { STORE: esc(settings.storeName || 'Магазин'), ERROR: error ? flashNote(error, 'error') : '' })}
<script src="/static/tailadmin-ui.js?v=${R.assetV('tailadmin-ui.js')}" defer></script>
<script src="/static/smoothui-toast.js?v=${R.assetV('smoothui-toast.js')}" defer></script>
</body></html>`;
}
...
/* Кнопка периода — ТА ЖЕ, что над отчётом метрики (`gSelect` в
 * lib/analytics-view.js): белая пилюля с календарём, меню под ней, выбранный
 * пункт на зелёной плашке с галочкой. Своя вторая такая же кнопка означала бы
 * два вида одного и того же выбора в соседних разделах панели.
 *
 * Пункты — обычные ссылки, состояние держит `<details>`: без скриптов период
 * выбирается как раньше, а живое обновление `open` не трогает вовсе.
 */
function periodSelect(period, link) {
  const days = R.orderPeriod(period);
  return AV.gSelect({
    className: 'gs-days', icon: CI.icon('calendar'), title: 'Период заказов',
    label: R.orderPeriodLabel(days),
    groups: [{ items: R.ORDER_PERIODS.map(([d, label]) => ({ label, href: link(d), active: days === d })) }]
  });
}

/* Снимок товара в строке заказа — как в исходной таблице TailAdmin, где у
 * каждой строки стоит картинка. Позиция заказа хранит только id, поэтому фото
 * берётся у живой карточки каталога; товар удалили или фото нет — остаётся
 * прежний значок, а не пустой квадрат. */
function orderItemPhoto(db, item) {
  const product = item && item.id && typeof db.getProduct === 'function' ? db.getProduct(item.id) : null;
  if (product && product.images && product.images.length) {
    return R.imageMarkup(product, 0, { small: true, dir: db.UPLOAD_DIR });
  }
  return `<span class="ta-product-symbol">${R.adminIcon('products')}</span>`;
}

function dashboard(settings, db, pulse, opts) {
  const live = pulse || {};
  /* Период — охват всей страницы, а не ещё один отбор: и сводка, и последние
   * заявки обязаны говорить про один и тот же срок. */
  const period = R.orderPeriod(opts && opts.period);
  const visible = db.visibleOrders();
  const orders = R.ordersInPeriod(visible, period);
  ...
  const financialOrders = R.ordersInPeriod(
    typeof db.getOrders === 'function' ? db.getOrders() : visible, period);
  const stats = R.orderStats(orders, undefined, financialOrders);
  const periodControl = periodSelect(period, d => R.esc('/admin' + (d ? '?period=' + d : '')));
  const data = AC.salesSeries(financialOrders, period);
  const countries = (live.countries || []).filter(c => Number(c.value) > 0);
  const countryTotal = countries.reduce((sum, c) => sum + Number(c.value), 0);
  const geography = TA.render('map', {
    DATA: esc(JSON.stringify(countries.map(c => ({ code: c.code, name: CI.countryName(c.code) || 'Страна не определена', value: c.value })))),
    COUNTRIES: countries.slice(0, 3).map(c => TA.render('countryRow', {
      FLAG: CI.flag('', c.code), NAME: esc(CI.countryName(c.code) || 'Не определено'),
      COUNT: Number(c.value).toLocaleString('ru-RU') + ' ' + R.pluralRu(c.value, 'посетитель', 'посетителя', 'посетителей'),
      PERCENT: Math.round(c.value / countryTotal * 100)
    })).join('') || '<p class="text-sm text-gray-500 dark:text-gray-400">Сегодня посетителей пока нет</p>'
  });
  const recent = orders.slice(0, RECENT_ORDERS).map(o => {
    const item = (o.items || [])[0] || {};
    const name = item.name || item.title || R.orderNo(o.number);
    return TA.render('orderRow', {
      ID: 'recent-' + esc(o.id),
      PRODUCT: `<a class="ta-product-cell" href="/admin/orders#order-${esc(o.id)}">${orderItemPhoto(db, item)}<span><b>${esc(name)}</b><small>${esc(R.orderNo(o.number))} · ${R.formatDate(o.createdAt)}</small></span></a>`,
      // В сводке — только плашка состояния, как бейдж в оригинале: приписки
      // («отметил менеджер · 22.09 в 12:00») делали столбец шире карточки.
      CUSTOMER: esc(o.customerName || o.contact || 'Без контакта'), TOTAL: R.money(o.total, settings), STATUS: R.orderStatus(o, undefined, { short: true })
    });
  }).join('') || `<tr><td colspan="4" class="ta-empty">${period ? 'За выбранный период заказов нет' : 'Заказов пока нет'}</td></tr>`;
  const body = TA.render('dashboard', {
    METRICS: `<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">${TA.render('metric', { ICON: R.adminIcon('orders'), VALUE: stats.count, LABEL: 'Заказы', DETAIL: R.orderPeriodLabel(period) })}${TA.render('metric', { ICON: R.adminIcon('analytics'), VALUE: R.money(stats.revenue, settings), LABEL: 'Выручка', DETAIL: 'Оплаченных заказов: ' + stats.paid })}</div>`,
    BAR: TA.chart('barChart', 'sales-by-period', { labels: data.labels, series: [{ name: 'Заказы', data: data.count }] }, { TITLE: data.monthly ? 'Заказы по месяцам' : 'Заказы по дням', CONTROLS: '' }),
    RADIAL: TA.chart('radialChart', 'paid-ratio', { series: [stats.financialCount ? Math.round(stats.paid / stats.financialCount * 1000) / 10 : 0] }, { TITLE: 'Оплата заказов', SUBTITLE: 'Доля оплаченных заказов за выбранный период', DESCRIPTION: stats.financialCount ? 'Оплачено ' + stats.paid + ' из ' + stats.financialCount + ' заказов' : 'Заказов пока нет', COUNT: stats.financialCount, PAID: stats.paid, REVENUE: R.money(stats.revenue, settings) }),
    AREA: TA.chart('areaChart', 'sales-statistics', { labels: data.labels, series: [{ name: 'Сумма заказов', data: data.total }, { name: 'Оплачено', data: data.revenue }], currency: true }, { TITLE: 'Статистика продаж', SUBTITLE: !period ? 'Сумма заказов и выручка · последние 12 месяцев' : 'Сумма заказов и выручка за выбранный период', CONTROLS: periodControl }),
    ACTIVITY: geography + `<div class="ta-activity">${TA.render('metric', { ICON: R.adminIcon('analytics'), VALUE: Number(live.online) || 0, LABEL: 'Сейчас на сайте', DETAIL: live.visitors ? 'сегодня ' + Number(live.visitors).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visitors, 'посетитель', 'посетителя', 'посетителей') + ' · ' + Number(live.visits || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visits, 'заход', 'захода', 'заходов') : 'сегодня заходов ещё не было' })}<a href="/admin/reviews">${TA.render('metric', { ICON: R.adminIcon('reviews'), VALUE: pending, LABEL: 'Отзывов на модерации', DETAIL: pending ? 'ждут ответа' : 'очередь разобрана' })}</a><a class="ta-history-link" href="/admin/analytics/visitors">История посещений за год →</a></div>`,
    RECENT: TA.render('recentOrders', { TITLE: 'Последние заказы', ROWS: recent })
  });
...
function productsList(settings, db, flash) {
  ...
  const rows = list.map((p, i) => {
    ...
    return `<tr data-id="${esc(p.id)}" draggable="true"${hidden ? ' class="row-off"' : ''}>
      ...
      ${/* Цена без скидки — то, что стоит в карточке товара. Сколько платит
           покупатель, зависит от промоакции, и в списке из полусотни строк это
           было бы вторым числом в каждой. */''}
  ...
  return layout(settings, { active: 'products', title: 'Каталог', counts: navCounts(settings, db),
    ...
    flash, body: TA.render('tableFrame', { TABLE: `<table class="a-table a-table-sortable"><thead><tr><th><span class="sr-only">Порядок</span></th><th></th><th>Товар</th><th>Цена без скидки</th><th></th><th></th></tr></thead>
    <tbody id="product-order" data-order="${esc(JSON.stringify(list.map(p => p.id)))}">${rows}</tbody></table>` }) + `
...
function productForm(settings, db, product, opts) {
  ...
  const chip = src => `<div class="img-chip${src === mainSrc ? ' is-main' : ''}" data-src="${esc(src)}" draggable="true"${caseOf(src) ? ` data-case="${esc(caseOf(src))}"` : ''}>
    <div class="img-chip-media"${caseOf(src) ? ` data-case="${esc(caseOf(src))}"` : ''}><img data-src="/uploads/${esc(IMG.previewName(db.UPLOAD_DIR, src))}" loading="lazy" decoding="async" width="92" height="92" alt="">
  ...
  const groupHead = (name, hex) => name
    ? `<summary class="img-group-head"><span class="swatch" style="background:${R.cssColor(hex, '#cccccc')}"></span>${esc(name)}<span class="img-group-count"></span></summary>`
    : `<summary class="img-group-head"><span class="swatch swatch-any"></span>Общие фото<span class="img-group-count"></span></summary>`;
  const groupFor = (name, hex) => {
    ...
    return `<details class="img-group" data-color="${esc(name)}"${list.length ? '' : ' hidden'}>
      ${groupHead(name, hex)}
  ...
  const groupForBand = (g, o, caseName, caseHex) => {
    ...
    return `<details class="img-group" data-band="${esc(key)}" data-case="${esc(caseName)}"${list.length ? '' : ' hidden'}>
      <summary class="img-group-head"><span class="swatch" style="background:${R.cssColor(o.hex, '#cccccc')}"></span>${title}<span class="img-group-count"></span></summary>
      <div class="img-chips">${list.map(chip).join('')}</div>
    </details>`;
  };
  ...
  // Поле в обёртке (цена с «₽», скидка с «%») `accessibleFields()` не видит:
  // между подписью и полем стоит <div>. Такой подписи `for` ставится здесь,
  // иначе скринридер читал «поле ввода, число» без названия.
  const field = (name, label, control, hint) => {
    let labelFor = '';
    const single = (control.match(/<(?:input|select|textarea)\b/gi) || []).length === 1 && !/<label\b/i.test(control);
    const inner = !single || /^\s*<(?:input|select|textarea)\b/i.test(control) ? null : control.match(/<(input|select|textarea)\b([^>]*)>/i);
    if (inner) {
      const own = inner[2].match(/\bid="([^"]+)"/);
      const id = own ? own[1] : `pf-${name}`;
      if (!own) control = control.replace(inner[0], `<${inner[1]}${inner[2]} id="${id}">`);
      labelFor = ` for="${id}"`;
    }
    return fieldHtml(name, `<label${labelFor}>${label}</label>`, control, hint);
  };
  const fieldHtml = (name, labelTag, control, hint) => `<div class="field${errFor(name) ? ' has-error' : ''}">
    ${labelTag}${control}
  ...
  <form class="a-form product-form" data-product-form method="post" action="${isEdit ? '/admin/products/' + encodeURIComponent(product.id) : '/admin/products'}" enctype="multipart/form-data">
    ${errors.length ? `<div class="form-alert" role="alert" data-store-toast data-toast-type="warning" data-toast-id="admin-product-errors" data-toast-message="${esc('Не сохранено. ' + errors.map(e => e.text).join('; '))}"><b>Не сохранено.</b> ${errors.length === 1 ? 'Поправьте одно поле:' : 'Поправьте поля:'}
    ...
    ${section('Цена', 'В поле — цена БЕЗ скидки. Скидку срезает промоакция: один процент на все конфигурации.', `
      <div class="a-form-grid">
        ${/* `required` и `min="1"` сняты: у карточки с галочкой «Не показывать
             цену» цены может не быть вовсе, и браузер иначе не давал бы сохранить
             даже правку описания. Пустое поле у обычного товара ловит сервер и
             возвращает форму с ошибкой — как и всё остальное в ней. */''}
        ${field('price', 'Цена без скидки', `<div class="price-field"><input name="price" type="number" min="0" step="1" value="${esc(val('price', p.price))}" placeholder="89990"><span>₽</span></div>`, 'Столько стоит базовая сборка, когда промоакции нет')}
        ${field('discountPercent', 'Скидка промоакции', `<div class="price-field"><input name="discountPercent" id="discount-pct" type="number" min="0" max="${D.MAX_PCT}" step="1" value="${esc(val('discountPercent', pct || ''))}" placeholder="0"><span>%</span></div>`, `0 — скидки нет. Больше ${D.MAX_PCT}% не бывает`)}
      </div>
      <p class="price-preview" id="price-preview" hidden></p>
      <label class="switch-row"><input type="checkbox" name="hidePrice" ${checked('hidePrice', p.hidePrice === true) ? 'checked' : ''}><span>Не показывать цену на витрине</span></label>
      <p class="field-hint">На месте ценника встанет «Цена уточняется» — и в каталоге, и на странице товара. Для того, что уже объявлено, но прайса на него ещё нет: фотографии, характеристики и варианты покупатель видит, а суммы нет. <b>Купить такой товар нельзя</b>, и это не оговорка: корзина считает цену сама и показала бы ровно ту сумму, которую карточка скрыла. Поисковику цена тоже не отдаётся.</p>
      ...
      <p class="field-hint">Процент один на весь товар, поэтому у дорогой сборки выгода в рублях больше, а «−${esc(String(pct || 10))}%» на витрине везде одинаковые. Цена со скидкой считается как «цена × (1 − процент)» и отдельным числом не хранится — иначе она разошлась бы с ценой при первой же её правке. Скидку даёт промоакция: <a class="link" href="/admin/promo">выключите её</a> — и покупатель платит цену из поля выше.</p>
    `)}
    ...
    ${section('Варианты', 'Цвета и конфигурации, между которыми покупатель выбирает на странице товара. Снятая галочка «в наличии» оставляет вариант видимым, но недоступным.', `
      ...
      <div class="field option-field">
        ...
        <p class="field-hint">Цена — полная и БЕЗ скидки: сколько стоит товар с этим значением, когда промоакции нет. «Только для конфигураций» ограничивает значение частью вариантов памяти — как нанотекстурное стекло у iPad Pro.</p>
      ...
      <div class="field band-field">
        ...
        <p class="field-hint">Цена — полная и БЕЗ скидки: сколько стоят часы с этим ремешком, когда промоакции нет. Фото вариации выбирается в списке фотографий ниже.</p>
    ...
    ${section('Фотографии', isEdit ? 'Первое фото — главное. Перетаскиванием меняется порядок.' : 'Фото можно добавить и после создания товара.', isEdit ? `
      <div id="photo-manager" data-product="${esc(product.id)}" data-order="${esc(JSON.stringify(p.images || []))}">
        <div class="field" id="img-chips-wrap"${(p.images || []).length ? '' : ' hidden'}><label>Текущие фото</label>
          <p class="field-hint">Откройте нужную группу, чтобы посмотреть и изменить фото.</p>
...
const REVIEW_TABS = [['pending', 'На модерации'], ['seen', 'Прочитанные'], ['approved', 'Опубликованные'], ['all', 'Все']];
/* Состояния в форме правки. Подписи здесь СВОИ и отвечают на другой вопрос:
 * вкладка называет набор («Прочитанные»), а селект — что станет с этим отзывом,
 * поэтому у прочитанного прямо сказано, кто его увидит. Список закрытый и
 * повторяет `db.REVIEW_STATES`; порядок — от «виден всем» к «виден одному». */
const REVIEW_STATE_OPTIONS = [
  ['approved', 'Опубликован — виден всем'],
  ['seen', 'Прочитан — виден только автору'],
  ['pending', 'На модерации — виден только автору']
];
...
/* Вкладка «На модерации» показывает РОВНО ждущих решения.
 *
 * Раньше здесь стояло «всё, что не опубликовано», и с появлением прочитанных
 * (`seen`) это значило бы, что разобранный отзыв возвращается в очередь на
 * следующий же день — то есть кнопка «Прочитано» не делала бы ничего. Каждая
 * вкладка теперь называет одно состояние, а «Все» остаётся общей лентой.
 */
function filterByStatus(list, status) {
  if (status === 'pending') return list.filter(r => r.status !== 'approved' && r.status !== 'seen');
  if (status === 'seen') return list.filter(r => r.status === 'seen');
...
function reviewRow(rv, product, back, opts) {
  opts = opts || {};
  const seen = rv.status === 'seen';
  const waiting = rv.status !== 'approved' && !seen;      // ждёт решения
  const pending = rv.status !== 'approved';               // не на витрине: и ждущий, и прочитанный
  const fields = backFields(back);
  const form = (path, label, cls) =>
    `<form method="post" action="/admin/reviews/${esc(rv.id)}/${path}">${fields}<button class="btn btn-sm${cls ? ' ' + cls : ''}">${label}</button></form>`;
  /* Действий у неопубликованного отзыва ДВА, и второе важнее первого.
   *
   * «Прочитано» — обычный конец разбора: отзыв остаётся у автора, на витрину не
   * идёт, из очереди уходит. До него разобрать очередь можно было только
   * «Одобрить» (показать всем) или «Удалить» (стереть насовсем, вместе с тем,
   * что видел автор) — и разбирали удалением.
   *
   * У опубликованного — «Снять с витрины»: он возвращается в очередь. Убрать
   * неудачный отзыв со страницы товара нужно по-прежнему, а удаление для этого
   * слишком грубо — отзыв покупателя восстановить неоткуда.
   */
  const act = pending
    ? form('approve', 'Одобрить', 'btn-primary') + (seen ? '' : form('seen', 'Прочитано'))
    : form('hide', 'Снять с витрины');
  ...
  /* Тёплая подсветка — только у того, что ЖДЁТ решения. Прочитанный отзыв
   * разобран, и красить его как несделанную работу нельзя; но и молчать о нём
   * тоже: он не на витрине, и это надо видеть, не открывая вкладку. Поэтому у
   * него своя спокойная плашка. */
  return `<article class="rv-row${waiting ? ' is-pending' : ''}${seen ? ' is-seen' : ''}" id="rv-${esc(rv.id)}">
    <div class="rv-main">
      <div class="rv-head">
        ${R.stars(rv.rating)}
        <b class="rv-who">${esc(rv.author)}</b>
        <span class="rv-when muted">${R.formatDate(rv.createdAt)}</span>
        ${waiting ? '<span class="rv-tag">На модерации</span>' : ''}
        ${seen ? '<span class="rv-tag rv-tag-seen">Прочитано · не на витрине</span>' : ''}
        ${where}
      </div>
...
function productReviews(settings, db, product, statusFilter, flash, page, sort, media) {
  ...
  const waiting = filterByStatus(all, 'pending').length;   // ровно те, что ждут решения
...
function reviewForm(settings, db, review, opts) {
  ...
  const status = REVIEW_STATE_OPTIONS.some(([k]) => k === rv.status) ? rv.status : 'approved';
      ...
      <div class="rv-form-col">
        <div class="a-form-grid">
          ...
          <div class="field"><label>Состояние</label><select name="status">
            ${REVIEW_STATE_OPTIONS.map(([key, label]) =>
              `<option value="${key}"${status === key ? ' selected' : ''}>${label}</option>`).join('')}
          </select></div>
...
function promoSummary(settings) {
  /* Выключенная промоакция снимает с витрины ВЕСЬ язык скидки — плашку,
   * зачёркнутую цену и процент (`PROMO.pctFor`), — а цены остаются теми же:
   * `price` и есть цена со скидкой. Строка обязана сказать это прямо: раньше
   * она обещала, что «скидки товаров остаются», и это стало неправдой. */
  if (!settings.promoOn) return ['', 'выключена — на витрине базовые цены: ни плашек, ни зачёркнутых сумм, поля промокода на оформлении тоже нет'];
  const list = PROMO.codes(settings);
  if (!list.some(c => c.on)) return ['err', 'включена, но ни один код не работает — витрина показывает базовые цены без скидок'];
...
function promoRow(entry, settings, used) {
  // Отметка «по умолчанию» — про ВЫБОР владельца, а не про то, работает ли
  // промоакция сейчас: иначе у выключенной она пропадала бы со строки кода.
  const isDefault = (PROMO.savedDefault(settings) || {}).code === entry.code;
  const orders = used || 0;
...
function promoPage(settings, db, opts) {
   ...
   * его тут не было вовсе, и владелец, поставивший коду свой процент, видел
   * список из одного «Не применять»: со стороны это неотличимо от сломанного
   * раздела, а настройка при этом молча слетала. Причину даёт `defaultBlock()`
   * — та же функция, которой объясняется отказ при сохранении: два места
   * разъехались бы на первой правке.
   *
   * Выбрать погашенный пункт браузер не даст, так что серверная проверка
   * остаётся страховкой, а не единственной дверью. */
  /* В селекте стоит СОХРАНЁННЫЙ выбор, а не применённый: при выключенной
   * промоакции применённого нет вовсе, и форма показывала бы «Не применять» —
   * а сохранение стирало бы выбор владельца (так он и пропал на бою). */
  const current = (PROMO.savedDefault(settings) || {}).code || '';
  const options = ['<option value="">Не применять — цены как в каталоге</option>']
    .concat(list.map(c => {
      const why = PROMO.defaultBlock(c);
      return `<option value="${esc(c.code)}"${why ? ' disabled' : ''}${!why && c.code === current ? ' selected' : ''}>`
        + `${esc(c.code)}${why ? ' — ' + esc(why) : ''}</option>`;
    }))
    .join('');
  ...
  const body = `<div class="a-panel promo-top">
    <form method="post" action="/admin/promo">
      ...
      ${/* Галочка называется «Промоакция», а не «Промокоды»: она включает не
           только поле кода на оформлении, но и сам ценник со скидкой — плашку,
           зачёркнутую цену и процент. Прежняя подпись обещала меньше, чем
           галочка делает, а цена такой ошибки — ценник всей витрины. */''}
      <label class="switch-row"><input type="checkbox" name="promoOn"${settings.promoOn ? ' checked' : ''}><span>Промоакция работает: скидки на витрине и промокоды на оформлении</span></label>
      <div class="promo-fields">
...
// единственного потока, то есть замершая витрина для всех посетителей.
function ordersList(settings, db, flash, page, edit, filters, opts) {
  const now = Date.now();
  ...
  const editing = !!edit;
  const f = filters || {};
  /* Период — это ОХВАТ страницы, а не ещё одно поле поиска: он одинаково задаёт
   * и список, и сводку под ним, поэтому и стоит отдельной кнопкой перед формой
   * отбора, а не внутри неё. Тот же выбор и та же кнопка, что над отчётом
   * метрики. */
  const period = R.orderPeriod(f.period);
  const visible = db.visibleOrders();
  const viewOrders = R.ordersInPeriod(visible, period, now);
  const q = String(f.q || '').trim().slice(0, 100);
  const payFilter = ['ok', 'wait', 'idle', 'warn', 'off', 'none', 'draft'].includes(String(f.pay || ''))
    ? String(f.pay) : '';
  ...
  const searchText = order => {
    ...
    return [order.number, order.customerName, order.firstName, order.lastName,
      ...
      order.clientCity, GEOIP.cityRu(order.clientCity, CI.countryCode(order.clientCountry, order.clientCountryCode)),
      ...itemText, ...paymentText].filter(Boolean).join(' ').toLocaleLowerCase('ru-RU');
  };
  const matchingOrders = viewOrders.filter(o => {
    if (query) {
  ...
  // Фильтр и плашка используют одно состояние на один момент времени.
  // Просроченный черновик уже не относится к «Способ не выбран».
  const orders = matchingOrders.filter(o => !payFilter || R.orderTone(o, now) === payFilter);
  const paidCount = matchingOrders.filter(o => R.orderTone(o, now) === 'ok').length;
   ...
   */
  const listHref = (n, patch) => {
    const p = patch || {};
    const days = p.period === undefined ? period : p.period;
    const queryValue = p.q === undefined ? q : p.q;
    const payValue = p.pay === undefined ? payFilter : p.pay;
    const params = [];
    if (n > 1) params.push('page=' + n);
    if (editing) params.push('edit=1');
    if (queryValue) params.push('q=' + encodeURIComponent(queryValue));
    if (payValue) params.push('pay=' + encodeURIComponent(payValue));
    if (days) params.push('period=' + days);
  ...
  const rows = slice.items.map(o => {
    ...
    const displayPayment = R.payDisplay(o.payment, now);
    const checkable = R.payLive(displayPayment, now) ? displayPayment
      : attempts.slice().reverse().find(a => a && a.invoiceId && a.status !== 'paid' && a.status !== 'refunded');
    ...
    const method = o.payment
      ? `<div class="o-payment-method">${R.orderPayMethod(o, now)}</div>` : '';
    ...
    const settled = o.payment && ['paid', 'mismatch', 'refunded'].includes(o.payment.status);
    ...
    /* Товарный чек — у оплаченного заказа. Открывается в новой вкладке: список
     * заказов на месте панели закрылся бы, а чек чаще всего печатают и
     * возвращаются к разбору заявок. Условие показа то же, что у покупателя на
     * странице оплаты (`receiptLink` в lib/render.js): документ о покупке имеет
     * смысл, когда деньги правда пришли. */
    const receipt = !(o.payment && o.payment.status === 'refunded') && ((o.payment && o.payment.status === 'paid') || o.manualPaid)
      ? `<a class="btn btn-sm o-receipt" href="/receipt/${esc(o.id)}" target="_blank" rel="noopener">Товарный чек</a>` : '';
    ...
    const phone = R.phoneText(o.phone);
    const name = String(o.customerName || [o.firstName, o.lastName].filter(Boolean).join(' ') || '').trim() || 'Имя не указано';
    // Полное местоположение клиента не подменяет отдельный адрес доставки.
    const countryCode = CI.countryCode(o.clientCountry, o.clientCountryCode);
    const country = CI.countryName(countryCode) || String(o.clientCountry || '').trim();
    const place = [GEOIP.cityRu(o.clientCity, countryCode), GEOIP.regionRu(o.clientRegion, countryCode), country]
      .map(value => String(value || '').trim()).filter(Boolean).filter((value, i, list) => list.indexOf(value) === i).join(', ');
    const flag = CI.flag(country, countryCode);
    const deviceText = [o.clientModel || o.clientDevice, o.clientOs, o.clientBrowser].filter(Boolean).join(' · ');
    const visitorHref = R.visitorHref(o, VISITOR_BASE);
    const deviceIcon = CI.icon(CI.deviceKey(o.clientDevice, o.clientModel));
    const device = o.clientDevice || o.clientModel
      ? (visitorHref
        ? `<a class="of-device" href="${esc(visitorHref)}" aria-label="Карточка посетителя: ${esc(deviceText)}" title="${esc(deviceText)}">${deviceIcon}</a>`
        : `<span class="of-device" role="img" aria-label="${esc(deviceText)}" title="${esc(deviceText)}">${deviceIcon}</span>`) : '';
    const tel = R.orderTelHref(o), telegram = R.orderTelegramHref(o, settings, opts), wa = R.orderWaHref(o, settings, opts);
    const callIcon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 3H4a1 1 0 0 0-1 1c0 9.4 7.6 17 17 17a1 1 0 0 0 1-1v-3l-5-2-2 2a14 14 0 0 1-7-7l2-2-2-5Z"/></svg>';
    const chatIcon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 11.5a8.5 8.5 0 0 1-8.5 8.5H4l-1 1V11.5a8.5 8.5 0 1 1 18 0Z"/><path d="M8 10h8M8 14h5"/></svg>';
    const chat = /^[a-f0-9]{32}$/.test(String(o.visitorId || ''))
      ? `<a class="of-contact of-chat" href="${esc(CHAT_TO_BASE + encodeURIComponent(o.visitorId))}" aria-label="Чат на сайте с клиентом: ${esc(name)}">${chatIcon}<span>Чат на сайте</span></a>` : '';
    const contactButtons = [
      tel ? `<a class="of-contact of-call" href="${esc(tel)}" aria-label="Позвонить ${esc(name)}: ${esc(phone)}">${callIcon}<span>Позвонить</span></a>` : '',
      telegram ? `<a class="of-contact of-telegram" href="${esc(telegram)}" target="_blank" rel="noopener noreferrer" aria-label="Написать ${esc(name)} в Telegram по заказу ${esc(R.orderNo(o.number))}">${R.tgMark()}<span>Telegram</span></a>` : '',
      wa ? `<a class="of-contact of-whatsapp" href="${esc(wa)}" target="_blank" rel="noopener noreferrer" aria-label="Написать ${esc(name)} в WhatsApp по заказу ${esc(R.orderNo(o.number))}">${R.waMark()}<span>WhatsApp</span></a>` : '',
      chat
    ].filter(Boolean).join('');
    const contacts = [phone, String(o.contact || '').trim()].filter(Boolean);
    const items = Array.isArray(o.items) ? o.items : [];
    const itemCount = items.length;
    const bank = R.orderBankMark(o, now);
    // Банковская диагностика относится к показанному счёту. Отказ другой
    // попытки не должен противоречить действующему статусу оплаты.
    const statusOrder = o.payment ? { ...o, payment: { ...o.payment, note: '' } } : o;
    const paymentNote = displayPayment && displayPayment.note
      ? `<p class="of-bank-note"><span>Ответ платёжного сервиса</span>${esc(displayPayment.note)}</p>` : '';
    const paymentActions = method + paymentNote + `<div class="of-payment-actions">${reconcile}${manual}${receipt}</div>`;
    const paymentDetails = method || paymentNote || reconcile || manual || receipt
      ? `<details class="of-payment-details"><summary>Реквизиты и действия</summary><div>${paymentActions}</div></details>` : '';
    const detail = `<article class="of-order" aria-labelledby="client-${esc(o.id)}">
        <div class="of-customer"><h2 id="client-${esc(o.id)}">${esc(name)}${device}</h2>
          <div class="of-order-meta"><b>${esc(R.orderNo(o.number))}</b><span>${R.formatDate(o.createdAt)}${o.createdAt ? ' · ' + R.mskTime(o.createdAt) : ''}</span></div>
          <div class="of-city${place ? '' : ' is-unknown'}" title="Местоположение по IP">${flag ? `<span class="of-flag" aria-hidden="true">${flag}</span>` : CI.icon('pin')}<span>${esc(place || 'Город не определён')}</span></div>
          <div class="of-contact-values">${contacts.map(text => `<span>${esc(text)}</span>`).join('') || '<span>Контакты не указаны</span>'}</div>
        </div>
        <section class="of-items"><h3 class="of-label">${itemCount === 1 ? 'Товар' : `Товары · ${itemCount}`}</h3>${R.orderItems(o, { all: true })}
          <div class="of-delivery"><div class="of-delivery-heading"><h3 class="of-label">Доставка</h3>${shipMark(o)}</div>${R.orderDelivery(o, { money: settings }) || '<p class="muted">Данные доставки не указаны</p>'}</div>
        </section>
        <div class="of-finance"><div class="o-sum of-sum"><span class="sr-only">Сумма заказа</span><b>${R.money(o.total, settings)}</b>${R.orderPromo(o, settings)}</div>
          <div class="o-payment of-payment"><span class="sr-only">Статус оплаты</span><div class="o-payment-main"><div class="o-payment-state">${R.orderStatus(statusOrder, now)}</div></div>${bank ? `<div class="of-bank">${bank}</div>` : ''}${paymentDetails}</div>
        </div>
        <div class="of-contacts">${contactButtons ? `<nav class="of-contact-actions" aria-label="Связаться с ${esc(name)}">${contactButtons}</nav>` : ''}</div>
        <div class="o-act"><div class="o-acts">${remove}</div></div>
      </article>`;
    const firstItem = items[0] || {};
    return TA.render('orderFullRow', {
      ID: 'order-' + esc(o.id), ROWCLASS: R.orderRowClass(o, now),
      PRODUCT: `<div class="ta-product-cell">${orderItemPhoto(db, firstItem)}<span><b>${esc(firstItem.name || firstItem.title || 'Заказ')}</b><small>${esc(R.orderNo(o.number))} · ${R.formatDate(o.createdAt)}${itemCount > 1 ? ' · товаров: ' + itemCount : ''}</small></span></div>`,
      CUSTOMER: `<span class="ta-customer-cell"><b>${esc(name)}</b><small>${esc(phone || o.contact || '')}</small></span>`,
      TOTAL: R.money(o.total, settings), STATUS: R.orderStatus(statusOrder, now),
      ACTIONS: `<button class="btn btn-sm btn-ghost ta-order-open" type="button" data-ta-dialog="detail-${esc(o.id)}" aria-label="Открыть заказ ${esc(R.orderNo(o.number))}">Открыть</button><dialog id="detail-${esc(o.id)}" class="ta ta-order-dialog ${TA.dialogClass}" aria-labelledby="client-${esc(o.id)}"><button class="ta-dialog-close" type="button" data-ta-close aria-label="Закрыть заказ">×</button>${detail}</dialog>`
    });
  }).join('') || `<tr><td colspan="5" class="muted o-empty ta-empty">${q || payFilter
    ? 'По вашему запросу заказов нет.'
    : (period ? 'За выбранный период заказов нет.' : 'Заказов пока нет.')}</td></tr>`;
  const payOptions = [
    ['', 'Любой статус'], ['ok', 'Оплачено'], ['wait', 'Ждём оплату'], ['idle', 'Оплата не завершена'],
    ['warn', 'Требует сверки'], ['off', 'Не оплачено'], ['none', 'Без онлайн-оплаты'],
    ['draft', 'Способ не выбран']
  ];
  const paymentIcon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="3"/><path d="M3 10h18M7 15h3"/></svg>';
  const paymentSelect = AV.gSelect({
    className: 'gs-payment', icon: paymentIcon, title: 'Статус оплаты',
    label: (payOptions.find(([value]) => value === payFilter) || payOptions[0])[1],
    groups: [{ items: payOptions.map(([value, label]) => ({
      label, href: esc(listHref(1, { pay: value })), active: value === payFilter
    })) }]
  });
  // Поиск независим от фильтров: отправка и очистка текста сохраняют период
  // и оплату; смена фильтра сохраняет уже применённый поисковый запрос.
  const filterBar = `<form class="o-filters" method="get" action="/admin/orders">
    ${editing ? '<input type="hidden" name="edit" value="1">' : ''}
    ${period ? `<input type="hidden" name="period" value="${period}">` : ''}
    ${payFilter ? `<input type="hidden" name="pay" value="${payFilter}">` : ''}
    <label>${CI.icon('search')}<span class="sr-only">Поиск заказа</span><input type="search" name="q" value="${esc(q)}" placeholder="Имя, город, телефон или номер заказа"></label>
    <button class="btn btn-sm" type="submit">Найти</button>
    ${q ? `<a class="link small" href="${esc(listHref(1, { q: '' }))}">Сбросить</a>` : ''}
  </form>`;
   ...
   */
  const stats = R.orderStats(viewOrders, now, R.ordersInPeriod(
    typeof db.getOrders === 'function' ? db.getOrders() : visible, period, now));
  ...
  /* Общая сумма стоит В ШАПКЕ СПИСКА, а не только в свёрнутой сводке под ним:
   * «на сколько заказали» — первый вопрос к любому отбору, и отвечать на него
   * нажатием на свёртку значит не отвечать вовсе.
   *
   * Считается она по НАЙДЕННЫМ заявкам, то есть по тем самым строкам, что ниже:
   * иначе «Найдено 3 из 12» и сумма рядом были бы про разные наборы заказов.
   * Сводка под таблицей отвечает на другой вопрос — сколько из этого оплачено,
   * — и считается по всему периоду, как считалась раньше по всему списку. */
  const shownSum = R.ordersTotal(orders);
  const periodBar = `<div class="of-toolbar"><div class="a-period">
    <div class="of-filter"><span class="of-filter-label">Период</span>${periodSelect(period, d => esc(listHref(1, { period: d })))}</div>
    <div class="of-filter"><span class="of-filter-label">Оплата</span>${paymentSelect}</div>
    ${period || payFilter ? `<a class="of-filter-reset" href="${esc(listHref(1, { period: 0, pay: '' }))}">Сбросить фильтры</a>` : ''}
    </div><div class="of-search"><span class="of-filter-label">Поиск заказа</span>${filterBar}</div></div>`;
  return layout(settings, { active: 'orders', title: 'Заказы', counts: navCounts(settings, db), flash, live: 'orders',
    ordersPaidCount: paidCount, ordersPaidHref: listHref(1, { pay: 'ok' }),
    // Рабочий сценарий идёт первым: период, поиск и список. Общая сумма стоит
    // в шапке списка — за ней в свёртку лазить не приходится; а разбивка по
    // состояниям оплаты осталась свёрнутой под таблицей: на этой странице
    // разбирают конкретные заказы, и постоянно она нужна на «Обзоре». Счётчики
    // и там, и там считаются по всему списку, а не по показанной странице.
    body: '<div class="orders-page">' + periodBar
      + `<div class="a-panel o-list-panel">${R.editSwitch('orders-edit', {
        on: editing, title: 'Список заказов', note: resultNote, sum: R.money(shownSum, settings)
      })}${TA.render('ordersTable', { ROWS: rows })}${pager}</div>`
      + summaryDetails
...
function shipmentPage(settings, db, order, opts) {
  ...
  const params = `<div class="a-panel">
      ...
      <div class="field field-check ship-visible"><label>
        ...
        <span class="muted small">Новые отправления видны покупателю сразу. Снимите галочку, если нужно скрыть отслеживание.</span>
      </div>
    </div>`;
  ...
  return layout(settings, {
    ...
    body: (state && state.delayed
      ? flashNote('Покупателю сейчас показывается сообщение о задержке.', 'warning') : '') + body
  });
}
...
function chatPlaceParts(chat) {
  ...
  const raw = c.city || c.country
    ? [c.city, c.country]
    : String((chat && chat.city) || '').split(',');
  const countryRaw = String(raw[1] || '').trim();
  const countryCode = CI.countryCode(countryRaw, c.countryCode);
  // Код страны словарю обязателен: «Moscow» бывает и в Айдахо. Считаем его
  // РАНЬШЕ города — иначе перевод остаётся на одной ручной таблице, и старые
  // диалоги так и стоят с «Amsterdam» рядом с «Белгородом».
  const city = GEOIP.cityRu(raw[0], countryCode);
  const country = CI.countryName(countryCode) || countryRaw;
  return [city, country].map(p => String(p || '').trim()).filter(Boolean);
}
...
function chatMarks(chat, href) {
  const c = (chat && chat.client) || {};
  const place = chatPlaceParts(chat);
  const marks = R.clientMarks({
    place: place[0], country: place[1] || c.country, countryCode: c.countryCode,
 ...
 * и на фокусе, а на телефоне стоит открыто (см. `.chat-edit` в admin.css):
...
const CHAT_ICONS = {
  ...
  /* Загнутая стрелка «ответить» — тот же знак, каким это действие подписано в
     ...
     (`.chat-item::after` в admin.css): фигура одна, поэтому и смахивание, и
...
function chatPage(settings, db, chat, flash, orders, sent, replyTo) {
   ...
   * компьютере (см. `.a-app` в `admin.css`), поэтому стрелка «назад» живёт
  ...
  // Запасной текст уведомления находится в разговоре: общей шапки здесь нет.
  return layout(settings, {
...
function chatNewPage(settings, db, found, flash) {
  ...
  const off = CHAT.visible(settings) ? '' :
    flashNote('Чат на витрине выключен — покупатель не увидит сообщение. Включите его в «Настройках».', 'warning');
...
function payLogResult(a) {
  ...
  if (a.status === 'refunded') {
    return `<span class="pay-log-tag is-fail">возврат платежа</span>${a.note ? `<span class="pay-log-why">${esc(a.note)}</span>` : ''}`;
  }
  ...
  if (['expired', 'cancelled', 'failed'].includes(a.status)) {
    /* Приписка кассы стоит и здесь, а не только у «нужна проверка»: отчёт
     * открывают ровно с вопросом «почему не платят», и «истёк срок ввода
     * данных» отвечает на него, а голое «не оплачен» — нет. */
    return `<span class="pay-log-tag is-fail">${a.status === 'expired' ? 'истёк' : (a.status === 'cancelled' ? 'отменён' : 'не оплачен')}</span>`
      + (a.note ? `<span class="pay-log-why">${esc(a.note)}</span>` : '');
  }
...
function payLogRow(a, settings) {
  ...
  return `<tr class="pay-log-row${bad ? ' is-bad' : ''}">
    ...
    <td data-l="Способ">${esc(PAY.describe(a.method, a.provider).name)}${gave
...
function analyticsPage(settings, db, snapshot, regionPage, opts) {
  opts = opts || {};
  const products = {}; db.getProducts().forEach(p => { products[p.id] = p.name; });
  // Период и выбранная страна живут в одном адресе, поэтому представлению
  // достаётся сам путь, а параметры оно собирает само — второй сборки адреса
  // метрики в проекте быть не должно.
  const body = AV.dashboard(snapshot, {
    products, regionPage, base: '/admin/analytics', ordersHref: '/admin/orders',
    visitorBase: VISITOR_BASE, visitorsHref: VISITORS_BASE,
    // Блокировки и исключения приходят готовым списком: панель ничего о
    // правилах не решает, она их показывает.
    rules: opts.rules || []
  });
  ...
  return layout(settings, { active: 'analytics', title: 'Метрика', counts: navCounts(settings, db), wide: true, live: 'analytics orders', flash: opts.flash, body });
}
...
function visitorPage(settings, db, visitor, opts) {
  ...
  const body = visitor
    ? AV.visitorPage(visitor, {
      ...
      /* Стоит ли на нём блок и считает ли его метрика — приходит готовым
       * состоянием из `lookupVisitor`. Ключ отдельно от id карточки: по адресу
       * её открывают и тогда, когда метки у заявки не было вовсе. */
      rules: opts.rules || {}, key: opts.key || (visitor && visitor.id) || '', ruleBase: VISITOR_BASE,
  ...
  return layout(settings, { active: 'analytics', title: 'Посетитель', counts: navCounts(settings, db), wide: true, live: 'analytics orders', flash: opts.flash, body });
}
...
function payMethodRows(shown, live) {
  ...
  return ids.map(id => {
    const providers = live && live.byProvider
    ...
    const m = PAY.describe(id, providers.length === 1 ? providers[0] : '');
    ...
    const via = providers.map(pid => PAYMENTS.nameOf(pid));
    return {
...
function payMethodsSection(shown, live) {
  ...
  return `<h3 class="a-subhead">Способы оплаты на витрине</h3>
      ...
      ${shown.length ? '' : '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-payment-methods" data-toast-message="Не отмечен ни один способ — оплатить на витрине будет нечем.">Не отмечен ни один способ — оплатить на витрине будет нечем.</p>'}`;
}
...
function payModeNote(settings, health) {
  ...
  if (!asked.length) {
    const crew = on.length > 1
      ? `Включены ${on.length === 2 ? 'обе кассы' : 'кассы'}: ${esc(on.map(p => p.name).join(' и '))}.`
      : `Включена одна касса — ${esc(on[0].name)}, подстраховки нет.`;
    return `<p class="pay-mode is-on"><b>Сейчас: оплата на витрине.</b> ${crew}${payCeilingNote(settings)}</p>`;
  }
  ...
  // что чинить не нужно.
  // Статический список способов не проверяет ключи и сетевой доступ.
  const unverified = asked.filter(r => r.state === 'unverified');
  if (unverified.length) {
    const confirmed = live.length ? `На связи ${names(live)}. ` : '';
    return `<p class="pay-mode is-on"><b>Сейчас: оплата на витрине.</b> ${confirmed}${names(unverified)}: ключи заданы, связь ещё не проверена.${payCeilingNote(settings)}</p>`;
  }
  ...
  if (!live.length) {
    return slowOnly
      ...
      ${on.length > 1 ? (on.length === 2 ? 'Молчат обе' : 'Кассы не отвечают') : 'Молчит ' + esc(on[0].name)} — покупатель реквизитов не получит.</p>`;
  }
  const crew = live.length > 1
    ? `На связи ${live.length === 2 ? 'обе кассы' : 'кассы'}: ${names(live)}.`
    : on.length > 1
      ? (slowOnly
        ? `На связи ${names(live)}, ${on.length === 2 ? 'вторая ещё не ответила' : 'остальные ещё не ответили'}.`
        : `На связи только ${names(live)} — ${on.length === 2 ? 'вторая не отвечает' : 'остальные не отвечают'}.`)
      : `Работает одна касса — ${names(live)}, подстраховки нет.`;
  return `<p class="pay-mode is-on"><b>Сейчас: оплата на витрине.</b> ${crew}${payCeilingNote(settings)}</p>`;
}

/* Потолок кассы ниже общего предела — это видно покупателю: заказ дороже
 * уходит не в кассу. Владелец обязан знать этот порог и путь заказа за ним,
 * не открывая настроек Platega. Потолок общего предела не называем: он и
 * так стоит полем ниже. */
function payCeilingNote(settings) {
  const max = PAYMENTS.cashboxMax(settings);
  if (!max || max >= PAYMENTS.boundsOf(settings).max) return '';
  const where = PAYMENTS.fallbackMode(settings) === 'own'
    ? 'переводом по своим реквизитам' : 'заявкой, менеджер связывается сам';
  return ` В кассу уходят заказы до ${esc(R.moneyText(max, settings))}, дороже — ${where}.`;
}
...
function payProviderSwitches(settings, checked) {
  ...
  return `<div class="pay-switches">
        ...
        ${row('platega', 'Platega', 'plategaEnabled', !!settings.plategaEnabled)}
        ${row('alfabank', 'Альфа-Банк', 'alfabankEnabled', !!settings.alfabankEnabled)}
        ${row('solutiones', 'Solutiones', 'solutionesEnabled', !!settings.solutionesEnabled)}
      </div>`;
}
...
function ownPayFields(settings, s) {
  ...
  /* При включённой кассе свои реквизиты не спят: на них уходит заказ, который
   * кассе не по размеру (см. `PAYMENTS.modeFor`). Прежнее «включатся, когда
   * выключите кассы» стало неправдой ровно с потолком Platega в 20 000 ₽. */
  const cashboxMax = PAYMENTS.cashboxMax(settings);
  const state = active ? 'принимаем перевод на них'
    : own.on && !own.ready ? 'не хватает реквизитов'
      : own.on && own.ready && cashboxMax ? `на них уходят заказы дороже ${R.moneyText(cashboxMax, settings)}`
  ...
  return `<details class="pay-fold"${open}>
      ...
      ${own.on && !own.ready
        ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-own-payment" data-toast-message="Нужны имя получателя и хотя бы один реквизит — карта или телефон.">Нужны имя получателя и хотя бы один реквизит — карта или телефон.</p>' : ''}
...
const PAY_STATE_TEXT = {
  ...
  unverified: ['warn', 'ключи заданы · связь ещё не проверена'],
 ...
 */
function payProviderFold(row, stat, body, flagged) {
  ...
  // Работающую держать открытой незачем — ключи это секреты. И когда форма
  // вернулась с ошибкой ПРО ЭТУ кассу (`flagged`): поле с ошибкой обязано быть
  // на виду, а не под свёрткой, до которой ещё надо догадаться.
  const open = flagged || (row.on && (!row.ready || row.state === 'auth'));
...
/* НОВЫЕ ПРАВИЛА ПРОЕКТА, КОТОРЫХ НЕТ В ИНСТРУКЦИИ ВЛАДЕЛЬЦА.
 *
 * После первого сохранения поле инструкции считается полным, и проект в него
 * больше не вмешивается — иначе правка владельца затиралась бы обновлением. Из
 * этого следовало, что новое правило до работающего магазина не доезжает вовсе
 * и владелец о нём не знает: так случилось с правилами про цену сборки и про
 * названия цветов — в коде они появились, в панели их не было.
 *
 * Блок показывает недостающее ЦЕЛИКОМ, а не одной строкой «есть обновления»:
 * решать, нужно ли правило, можно только прочитав его. Кнопка дописывает их в
 * конец поля, и дальше это обычный текст владельца — правится и удаляется.
 */
function newRulesBlock(settings) {
  const missing = PROMPT.missingRules(settings);
  /* Отозванные правила показываются той же плашкой: строка, от которой проект
   * отказался (она оказалась неправдой хотя бы на одном сайте), иначе живёт в
   * сохранённой инструкции вечно — `chatPromptComplete` запрещает проекту
   * трогать поле. Кнопка одна на оба действия: владелец читает списки и решает. */
  const stale = PROMPT.staleRules(settings);
  if (!missing.length && !stale.length) return '';
  const label = missing.length && stale.length ? 'Дописать новые и убрать устаревшие'
    : stale.length ? 'Убрать их из инструкции' : 'Дописать их в инструкцию';
  return `<div class="a-newrules">
        ${missing.length ? `<b>В проекте появились правила, которых нет в вашей инструкции — ${missing.length}</b>
        <ul>${missing.map(r => `<li>${esc(r)}</li>`).join('')}</ul>` : ''}
        ${stale.length ? `<b>Правила, от которых проект отказался, — ${stale.length}: они устарели и могут спорить с условиями магазина</b>
        <ul>${stale.map(r => `<li>${esc(r.replace(/^-\s*/, ''))}</li>`).join('')}</ul>` : ''}
        <button type="submit" name="chatRulesAppend" value="1" class="btn btn-sm">${label}</button>
      </div>`;
}

/* Во что обходится один вопрос покупателя. Владелец правит инструкцию руками, и
 * длина этого поля — деньги на каждом ответе: без числа рядом об этом узнать
 * неоткуда. Токены считаются приблизительно (точный счёт даёт только
 * токенизатор OpenAI), поэтому и написано «примерно». */
function promptCostNote(settings, db) {
  const size = PROMPT.promptSize(db, settings);
  if (!size.total) return '';
  return `<p class="set-state">Запрос к модели — примерно ${size.total} токенов:
    инструкция ${size.instruction}, условия магазина ${size.store}, каталог с ценами ${size.catalog}.</p>`;
}

function chatSection(s, settings, checked, db) {
  return `${chatStateNote(settings)}
      ...
      <div class="field field-check"><label><input type="checkbox" name="aiPauseWhenAdminOnline" ${checked('aiPauseWhenAdminOnline', settings.aiPauseWhenAdminOnline !== false) ? 'checked' : ''}> Пока администратор онлайн, первым отвечает менеджер</label></div>
      <div class="a-form-grid">
      ...
      <div class="field"><label for="set-chat-prompt">Инструкция для ИИ</label>
        <input type="hidden" name="chatPromptComplete" value="1">
        <textarea id="set-chat-prompt" name="chatPrompt" rows="22" maxlength="${PROMPT.MAX_INSTRUCTION}" spellcheck="false">${esc(PROMPT.editableInstruction(s))}</textarea>
        ${newRulesBlock(s)}${promptCostNote(s, db)}</div>`;
}
...
const SET_ICONS = {
  ...
  // Домены: глобус с меридианом — общий образ адреса в сети.
  domains: '<circle cx="12" cy="12" r="8.4"/><path d="M3.6 12h16.8"/><path d="M12 3.6c2.2 2.3 3.4 5.3 3.4 8.4s-1.2 6.1-3.4 8.4c-2.2-2.3-3.4-5.3-3.4-8.4S9.8 5.9 12 3.6Z"/>',
  ...
  // Яндекс Метрика: столбики отчёта — счётчик, а не карта и не линия цен.
  ym: '<path d="M4 20h16"/><path d="M7.4 16.4v-4.2"/><path d="M12 16.4V7.6"/><path d="M16.6 16.4v-6.4"/>',
  ...
  // Кассы: платёжный терминал — экран и ряды клавиш.
  cashbox: '<rect x="4.2" y="3.4" width="15.6" height="17.2" rx="2.4"/><rect x="7.6" y="6.6" width="8.8" height="4.2" rx="1"/><path d="M8.2 14.8h.01M12 14.8h.01M15.8 14.8h.01M8.2 17.9h.01M12 17.9h.01M15.8 17.9h.01"/>',
  // Почта: конверт.
  mail: '<rect x="3" y="5.5" width="18" height="13" rx="2.4"/><path d="m3.9 7.2 7.2 5.3c.54.4 1.26.4 1.8 0l7.2-5.3"/>',
  // Личный кабинет: человек в кружке — тот же глиф, что у значка в шапке витрины.
  accounts: '<circle cx="12" cy="8.4" r="3.6"/><path d="M5.2 19.6c.6-3.4 3.3-5.5 6.8-5.5s6.2 2.1 6.8 5.5"/>',
  // Доступ: замок.
...
/* Плитки уже загруженных снимков магазина.
 *
 * Удаление — обычный чекбокс с подписью-крестиком, ни строчки скрипта: форма
 * работает и там, где скрипты панели не загрузились, а уходит всё одним
 * сохранением. Тот же приём, что у вложений в форме отзыва; отмеченная плитка
 * гаснет чистым CSS, без `:has()`.
 */
function storePhotoGrid(s) {
  const list = R.storePhotos(s);
  if (!list.length) return '';
  return `<div class="set-photos">${list.map(name => `
    <div class="set-photo">
      <input type="checkbox" id="drop-${esc(name)}" name="dropStorePhoto" value="${esc(name)}" class="set-photo-drop">
      <label class="set-photo-x" for="drop-${esc(name)}" title="Удалить снимок">✕</label>
      <img src="/uploads/${esc(name)}" alt="" loading="lazy">
    </div>`).join('')}</div>`;
}
...
function paySummary(settings, health) {
  ...
  if (PAYMENTS.mode(settings) === 'cashbox') {
    const max = PAYMENTS.cashboxMax(settings);
    const ceiling = max && max < PAYMENTS.boundsOf(settings).max
      ? ` до ${R.moneyText(max, settings)}, дороже — ${PAYMENTS.fallbackMode(settings) === 'own' ? 'свои реквизиты' : 'заявкой'}` : '';
    return ['ok', 'оплата на витрине · ' + on.map(p => p.name).join(' и ') + ceiling];
  }
  const own = PAYMENTS.ownRequisites(settings);
  if (PAYMENTS.ownEnabled(settings)) {
    const what = [own.phone ? 'СБП' : '', own.card ? 'карта' : ''].filter(Boolean).join(' и ');
  ...
  // заявок, и узнал бы об этом владелец от покупателя. Чинится здесь же, в этом
  // разделе, поэтому он и краснеет.
  if (own.on) return ['err', 'свои реквизиты включены, но не хватает получателя или номера'];
  /* Касса включена без ключей — витрина тоже без оплаты, но чинить это в
   * «Кассах»: туда и уедет красный тон. Здесь остаётся предупреждение, чтобы
   * владелец не читал «заявки без оплаты» как своё решение. */
  const nokeys = rows.filter(r => r.on && !r.ready);
  if (nokeys.length) return ['warn', 'заявки без оплаты — у ' + nokeys.map(r => r.name).join(' и ') + ' не заданы ключи'];
  return ['', 'заявки без оплаты — менеджер связывается с покупателем'];
}

/* Состояние касс — свой вопрос и свой раздел.
 *
 * «Оплата на витрине» отвечает на вопрос покупателя (чем платить), а здесь —
 * вопрос владельца: какие кассы включены и отвечают ли они. Красный тон только
 * у того, что чинится ровно тут: включённая касса без ключей.
 */
function cashboxSummary(settings, health) {
  const rows = health || [];
  const on = rows.filter(r => r.on);
  if (!on.length) return ['', 'ни одна не включена — оплату на витрине они не дают'];
  const names = list => list.map(r => r.name).join(' и ');
  const nokeys = on.filter(r => !r.ready);
  if (nokeys.length) return ['err', `${names(nokeys)}: ключи не заданы`];
  const mute = on.filter(r => r.state === 'auth' || r.state === 'down');
  if (mute.length) return ['warn', `${names(mute)} не отвечает`];
  const unverified = on.filter(r => r.state === 'unverified' || r.state === 'unknown');
  if (unverified.length) return ['warn', `${names(on)} · связь ещё не проверена`];
  return ['ok', `${names(on)} — на связи`];
}
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  // Отказ формы, который называет кассу по имени, раскрывает её свёртку.
  const flaggedProvider = name => flashType === 'err' && !!draft && String(flash || '').includes(name);
  ...
  // Яндекс Метрика: номер показываем как введён (черновик после ошибки — тоже),
  // а галочку ищем в черновике, как и остальные (см. `checked` выше).
  const ymId = YM.counterId(s) || '';
  const ymOnly = checked('ymOnlyYandex', YM.onlyYandex(settings));
  ...
  // `word` — как секрет называется в подписях: «ключ» у касс, «пароль» у почты.
  const secretInput = (name, saved, clearName, optional, word) => {
    const configured = !!String(saved || '').trim();
    const clearChecked = !!(draft && draft[clearName] !== undefined);
    const noun = word || 'ключ';
    const placeholder = configured
      ? 'задан — оставьте пустым, чтобы не менять'
      : (optional ? 'не обязателен' : 'введите ' + noun);
    return `<input name="${esc(name)}" type="password" value="" placeholder="${esc(placeholder)}" autocomplete="new-password" spellcheck="false">`
      + (configured ? `<label class="secret-clear"><input type="checkbox" name="${esc(clearName)}"${clearChecked ? ' checked' : ''}> Удалить сохранённый ${esc(noun)}</label>` : '');
  };
  ...
  const contactList = [String(s.contactTelegram || '').trim(),
    String(s.contactWhatsApp || '').trim()
      ? 'WhatsApp ' + (R.phoneText(s.contactWhatsApp) || String(s.contactWhatsApp).trim()) : '',
    String(s.contactPhone || '').trim() ? (R.phoneText(s.contactPhone) || String(s.contactPhone).trim()) : '',
    String(s.contactEmail || '').trim(),
    String(s.storeAddress || '').trim()].filter(Boolean);
  ...
  const [cashState, cashNote] = cashboxSummary(settings, health);
  /* Форма вернулась с ошибкой про конкретную кассу — раздел с ключами обязан
   * открыться сам: поле с ошибкой лежит в нём под свёрткой, до которой ещё
   * надо догадаться. */
  const cashTone = ['CrocoPAY', 'MeridianPay', 'Platega', 'Альфа', 'Solutiones'].some(flaggedProvider) ? 'err' : cashState;
  ...
  const legalInn = String(s.legalInn || '').trim();
  /* Прежняя строка «ИНН/ОГРН одним полем». Поля в форме больше нет, но пока
   * новые пустые, именно она едет в документы, — и владелец должен видеть,
   * откуда там взялись реквизиты, которых он в форме не находит. */
  const legalOld = (legalInn || String(s.legalOgrn || '').trim()) ? '' : String(s.legalDetails || '').trim();
  const bankReady = !!(String(s.bankAccount || '').trim() || String(s.bankName || '').trim());
  /* Домены. В поле возвращается ВВЕДЁННОЕ (форма пришла с ошибкой — строки
   * остаются как их набрали, включая негодную), а в свёрнутую строку идут
   * СОХРАНЁННЫЕ: она говорит про состояние магазина, а не про содержимое поля. */
  const domainList = DOMAINS.list(settings);
  const domainText = draft && draft.siteDomains !== undefined
    ? String(draft.siteDomains)
    : domainList.join('\n');
  // Кабинет: галочка из черновика, как все прочие; строка состояния — по
  // сохранённому, а поля формы — по введённому.
  const accountsOn = checked('accountsOn', CUSTOMERS.enabled(settings));
  const mailHost = String(settings.mailHost || '').trim();
  /* Callback URL — не подсказка, а значение, которое владелец копирует в
   * кабинет кассы. Адрес берётся тот же, что уходит кассе в `callback_url`
   * (`paymentOrigin()` в server.js); без него остаётся первый домен магазина,
   * а совсем без имени — один путь: выдумывать чужой адрес мы не будем. */
  const callbackBase = String(opts.origin || '').trim().replace(/\/+$/, '')
    || (domainList[0] ? 'https://' + domainList[0] : '');
  const callbackField = (id, path) => `<div class="field set-copy">
        <label for="${esc(id)}-callback">Callback URL для кабинета кассы</label>
        <div class="set-copy-row">
          <input id="${esc(id)}-callback" class="set-copy-url" value="${esc(callbackBase + path)}" readonly spellcheck="false">
          <button class="btn btn-sm" type="button" data-copy="${esc(callbackBase + path)}">Скопировать</button>
        </div>
      </div>`;

  /* Разделы страницы. Пять групп, и каждая отвечает на свой вопрос: что это за
   * магазин, как в нём покупают, как общаемся с покупателем, какие чужие
   * сервисы подключены и кто продаёт.
   *
   * ПОДСКАЗОК В РАЗДЕЛАХ НЕТ НИ ОДНОЙ. Абзацы под полями объясняли то, что поле
   * и так говорит подписью, плейсхолдером и своей строкой состояния: их читают
   * один раз, а видят каждый день, и из-за них страница разрасталась втрое. То
   * же правило уже работает в списке заказов, в каталоге и в форме отзыва.
   * Осталось то, чего иначе не узнать: строка состояния раздела, плашка режима
   * оплаты, цена запроса к модели и отказы формы — это не подсказки.
   */
  const sections = [
    ['Магазин', [
      {
        /* Магазин — только то, что говорит, ЧТО ЭТО ЗА МАГАЗИН: имя, слоган,
         * описание для выдачи и деньги. */
        ...
        ${/* Звёздочка осталась, а атрибута `required` нет: поле лежит внутри
             ...
             консоль. Пустое название ловит сервер. */''}
        <div class="field"><label for="set-store-name">Название магазина *</label><input id="set-store-name" name="storeName" value="${esc(s.storeName || '')}"></div>
        <div class="field"><label for="set-tagline">Слоган</label><input id="set-tagline" name="tagline" value="${esc(s.tagline || '')}"></div>
      ...
      ${/* Описание для поисковика — отдельное поле, а не слоган: слоган пишется
           под первый экран и в выдаче выглядит обрубком. Плейсхолдер показывает,
           что уедет в выдачу, если поле оставить пустым. */''}
      <div class="field"><label for="set-meta">Описание для поисковика</label>
        <textarea id="set-meta" class="a-area-sm" name="metaDescription" rows="2" maxlength="300" placeholder="${esc(String(s.tagline || '').trim() || 'Оригинальная техника Apple: доставка по России, гарантия 1 год')}">${esc(s.metaDescription || '')}</textarea></div>
      <div class="field"><label for="set-footer">Строка под копирайтом в подвале</label><input id="set-footer" name="footerNote" value="${esc(s.footerNote || '')}"></div>`
      },
      {
        ...
        note: `${R.cssColor(s.accentColor, '#0071e3').toUpperCase()} · шрифт «${R.logoFontName(s.logoFont)}» · ${s.logoImage ? 'логотип-картинка'
          ...
          + (R.cardNameStorageOn(settings) ? ' · память в названии карточки' : '')
          + (R.homeCatalogOn(settings) ? ' · весь каталог на главной' : ''),
        /* Кружок выбранного цвета: сказать «синий» словами нельзя — цвет
         * произвольный, — а показать его самим цветом можно. */
        mark: `<span class="set-dots" aria-hidden="true"><i style="background:${R.cssColor(s.accentColor, '#0071e3')}"></i></span>`,
        body: R.brandFields(Object.assign({}, s, {
          cardNameStorage: checked('cardNameStorage', R.cardNameStorageOn(settings)),
          homeCatalog: checked('homeCatalog', R.homeCatalogOn(settings))
        }))
      },
      {
        /* Связь и место: способы связи сверху, единственная офлайн-точка —
         * отдельной подсекцией. Покупатель ищет их в одном месте, когда решает,
         * написать, позвонить или приехать. */
        id: 'contacts', title: 'Контакты и адрес',
        tone: contactList.length ? '' : 'warn',
        /* Строка оглавления, а не список контактов целиком: с адресом точки она
         * занимала на телефоне три ряда. Всё, что не влезло, лежит в полях. */
        note: contactList.length
          ? setCut(contactList.join(' · '), 64)
          : 'контактов нет — связаться с магазином будет негде',
        body: `<div class="a-form-grid">
        <div class="field"><label for="set-tg">Telegram</label><input id="set-tg" name="contactTelegram" value="${esc(s.contactTelegram || '')}" placeholder="@manager"></div>
        <div class="field"><label for="set-wa">WhatsApp</label><input id="set-wa" name="contactWhatsApp" type="tel" value="${esc(s.contactWhatsApp ? R.phoneText(s.contactWhatsApp) || s.contactWhatsApp : '')}" placeholder="+7 999 123-45-67"></div>
        ${/* Телефон хранится в одном виде с телефоном заказа (E.164) и
             проверяется тем же модулем — сервер вернёт форму с ошибкой, если
             номера такого не бывает. */''}
        <div class="field"><label for="set-phone">Телефон</label><input id="set-phone" name="contactPhone" type="tel" value="${esc(s.contactPhone ? R.phoneText(s.contactPhone) || s.contactPhone : '')}" placeholder="+7 999 123-45-67"></div>
        <div class="field"><label for="set-email">Почта для покупателей</label><input id="set-email" name="contactEmail" type="email" inputmode="email" value="${esc(s.contactEmail || '')}" placeholder="shop@example.ru"></div>
        <div class="field"><label for="set-hours">Время работы</label><input id="set-hours" name="contactHours" value="${esc(s.contactHours || '')}" placeholder="09:00–22:00 МСК" maxlength="120"></div>
      </div>
      <div class="field"><label for="set-wa-message">Первая реплика в WhatsApp</label>
        <textarea id="set-wa-message" class="a-area-sm" name="contactWhatsAppMessage" rows="2" maxlength="500" placeholder="Здравствуйте! Хочу уточнить наличие товара.">${esc(s.contactWhatsAppMessage || '')}</textarea></div>
      <h3 class="a-subhead">Офлайн-точка</h3>
      <div class="a-form-grid">
        <div class="field"><label for="set-store-address">Адрес</label><input id="set-store-address" name="storeAddress" value="${esc(s.storeAddress || '')}" placeholder="г. Ноябрьск, проспект Мира, 88А, ТЦ «Ноябрьский»" maxlength="300"></div>
        <div class="field"><label for="set-store-since">Год открытия</label><input id="set-store-since" name="storeSinceYear" inputmode="numeric" value="${esc(s.storeSinceYear || '')}" placeholder="${esc(String(PROMPT.STORE_SINCE_YEAR))}" maxlength="4"></div>
        ${/* Координаты нужны одной карте — той, что стоит на «О компании». Карта
             там СВОЯ (тайлы OpenStreetMap), и нарисовать квартал по строке
             адреса ей нечем: геокодера у витрины нет. Пусто — раздела с картой
             на странице просто нет. */''}
        <div class="field"><label for="set-store-geo">Широта и долгота</label><input id="set-store-geo" name="storeGeo" value="${esc(s.storeGeo || '')}" placeholder="55.751244, 37.618423" maxlength="60"></div>
      </div>
      ${/* Снимки самой точки — для карточки магазина на «О компании». Обработка
           общая с фото товаров (WebP, метаданные снимаются), но БЕЗ подгонки в
           квадрат: у зала и вывески кадр свой. */''}
      <div class="field"><label for="set-store-photos">Фотографии, до ${R.STORE_PHOTOS_MAX}</label>
        ${storePhotoGrid(s)}
        <input id="set-store-photos" type="file" name="storePhotos" accept="image/*" multiple></div>`
      },
      {
        /* Домены. Мультидоменности это не возвращает: у всех имён одна витрина и
         * один каталог, приложение домен не разбирает вовсе (`lib/domains.js`). */
        id: 'domains', title: 'Домены',
        note: domainList.length
          ? setCut(domainList.join(' · '), 52)
          : 'запасных адресов нет — магазин открывается только по основному',
        body: `<div class="field">
        ${/* id поля не «set-domains»: он занят самим разделом (`set-` + его id),
             и два одинаковых id на странице ловит тест. */''}
        <label for="set-domain-list">Дополнительные домены, по одному в строке (до ${DOMAINS.MAX})</label>
        <textarea id="set-domain-list" name="siteDomains" rows="4" spellcheck="false" autocapitalize="off" placeholder="shop.example.com">${esc(domainText)}</textarea>
      </div>`
      }
    ]],
    ['Продажи', [
      {
        /* Оплата на витрине — чем платит ПОКУПАТЕЛЬ: режим, свои реквизиты,
         * способы, пределы суммы и валюта счёта. Ключи касс уехали в свой
         * раздел ниже: они про доступы, а не про покупку. */
      ...
      {
        /* Кассы — включатели, ключи и очередь опроса. Отдельным разделом,
         * потому что «Оплата на витрине» разрослась до пяти свёрток с
         * секретами: открыть её ради потолка суммы значило пролистать их все. */
        id: 'cashbox', title: 'Кассы', tone: cashTone, note: cashNote,
        body: `${payProviderSwitches(settings, checked)}
      ${payProviderFold(healthOf(CROCO.id), statOf(CROCO.id), `
      ${settings.crocopayEnabled && !CROCO.configured(settings)
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-crocopay" data-toast-message="CrocoPAY: Касса включена, но ключи не заданы — реквизиты она не выдаст.">Касса включена, но ключи не заданы — реквизиты она не выдаст.</p>' : ''}
      <div class="a-form-grid">
      ...
      </div>`, flaggedProvider('CrocoPAY'))}
      ${payProviderFold(healthOf(MERIDIAN.id), statOf(MERIDIAN.id), `
      ${settings.meridianpayEnabled && !MERIDIAN.configured(settings)
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-meridianpay" data-toast-message="MeridianPay: Касса включена, но ключ или Merchant ID не заданы — реквизиты она не выдаст.">Касса включена, но ключ или Merchant ID не заданы — реквизиты она не выдаст.</p>' : ''}
      <div class="a-form-grid">
      ...
      </div>`, flaggedProvider('MeridianPay'))}
      ${payProviderFold(healthOf(PLATEGA.id), statOf(PLATEGA.id), `
      ${settings.plategaEnabled && !PLATEGA.configured(settings)
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-platega" data-toast-message="Platega: Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.">Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.</p>' : ''}
      <div class="a-form-grid">
        <div class="field"><label for="platega-merchant">Merchant ID (UUID мерчанта)</label>
          <input id="platega-merchant" name="plategaMerchantId" value="${esc(s.plategaMerchantId || '')}" placeholder="00000000-0000-0000-0000-000000000000" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label>Secret</label>
          ${secretInput('plategaSecret', settings.plategaSecret, 'clearPlategaSecret')}</div>
        <div class="field"><label for="platega-fee">Комиссия Platega, включённая в цены, %</label>
          <input id="platega-fee" name="plategaFeePercent" type="number" min="0" max="100" step="0.01" value="${esc(s.plategaFeePercent == null ? 0 : s.plategaFeePercent)}"></div>
        <div class="field"><label for="platega-max">Максимальная сумма платежа, ₽</label>
          <input id="platega-max" name="plategaMaxTotal" inputmode="numeric" autocomplete="off"
            value="${esc(s.plategaMaxTotal == null || s.plategaMaxTotal === '' ? PLATEGA.DEFAULT_MAX_TOTAL : s.plategaMaxTotal)}" placeholder="${PLATEGA.DEFAULT_MAX_TOTAL}"></div>
      </div>
      ${callbackField('platega', '/api/pay/platega/callback')}`, flaggedProvider('Platega'))}
      ${payProviderFold(healthOf(ALFA.id), statOf(ALFA.id), `
      ${settings.alfabankEnabled && !ALFA.configured(settings)
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-alfabank" data-toast-message="Альфа-Банк: Касса включена, но доступ не задан — ссылку на оплату она не выдаст. Хватит любого из двух: платёжного токена или пары логин/пароль.">Касса включена, но доступ не задан — ссылку на оплату она не выдаст. Хватит любого из двух: платёжного токена или пары логин/пароль.</p>' : ''}
      ${/* Доступ подойдёт любой из двух: токен лежит в личном кабинете сразу,
           пару логин/пароль банк присылает письмом при подключении эквайринга.
           Об этом говорят сами подписи полей. */''}
      <div class="a-form-grid">
        <div class="field"><label for="alfa-token">Платёжный токен из ЛК</label>
          <input id="alfa-token" name="alfabankToken" value="${esc(s.alfabankToken || '')}" placeholder="или пара логин/пароль ниже" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label for="alfa-login">Логин API</label>
          <input id="alfa-login" name="alfabankLogin" value="${esc(s.alfabankLogin || '')}" placeholder="например, shop-api" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label>Пароль API</label>
          ${secretInput('alfabankPassword', settings.alfabankPassword, 'clearAlfabankPassword')}</div>
        <div class="field field-check"><label><input type="checkbox" name="alfabankTest"${
  checked('alfabankTest', !!s.alfabankTest) ? ' checked' : ''}> Тестовый контур банка, без настоящих денег</label></div>
      </div>`, flaggedProvider('Альфа'))}
      ${payProviderFold(healthOf(SOLUTIONES.id), statOf(SOLUTIONES.id), `
      ${settings.solutionesEnabled && !SOLUTIONES.configured(settings)
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-solutiones" data-toast-message="Solutiones: Касса включена, но Public key или Secret не заданы либо неверны — ссылку на оплату она не выдаст.">Касса включена, но Public key или Secret не заданы либо неверны — ссылку на оплату она не выдаст.</p>' : ''}
      <div class="a-form-grid">
        <div class="field"><label for="solutiones-key">Public key</label>
          <input id="solutiones-key" name="solutionesApiKey" value="${esc(s.solutionesApiKey || '')}" placeholder="pk_… из раздела «API-ключи»" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label>Secret</label>
          ${secretInput('solutionesApiSecret', settings.solutionesApiSecret, 'clearSolutionesApiSecret')}</div>
        <div class="field"><label>Webhook Secret (whsec_…)</label>
          ${secretInput('solutionesWebhookSecret', settings.solutionesWebhookSecret, 'clearSolutionesWebhookSecret', true)}</div>
        <div class="field"><label for="solutiones-max">Максимальная сумма платежа, ₽</label>
          <input id="solutiones-max" name="solutionesMaxTotal" inputmode="numeric" autocomplete="off"
            value="${esc(s.solutionesMaxTotal == null ? '' : s.solutionesMaxTotal)}" placeholder="пусто — предела нет"></div>
      </div>
      ${callbackField('solutiones', '/api/pay/solutiones/callback')}`, flaggedProvider('Solutiones'))}
      ${payPrimarySection(s)}`
      },
      {
        /* Здесь только значения по умолчанию: сам маршрут каждой посылки живёт
         * в её заказе и правится там же. Держать его в настройках было бы
         * неправдой — у каждой посылки он свой. */
        id: 'ship', title: 'Доставка и отслеживание',
        ...
        body: `<div class="a-form-grid">
        <div class="field"><label for="set-ship-city">Город отправки</label>
          <input id="set-ship-city" name="shipFromCity" value="${esc(s.shipFromCity || '')}" placeholder="${esc(TRACK.DEFAULT_FROM)}" maxlength="60"></div>
        ${/* Сборка входит в срок, который покупатель видит на оформлении и
             запоминает, — а была жёстким числом в коде. */''}
        <div class="field"><label for="set-ship-handling">Сборка заказа, рабочих дней</label>
          <input id="set-ship-handling" name="shipHandlingDays" inputmode="numeric" value="${esc(s.shipHandlingDays === 0 || s.shipHandlingDays ? String(s.shipHandlingDays) : '')}"
            placeholder="${SHIPDAYS.HANDLING} — 0 в день оплаты" autocomplete="off"></div>
        <div class="field"><label for="set-ship-hold">Сообщать о задержке через, дней</label>
          <input id="set-ship-hold" name="shipHoldDays" inputmode="numeric" value="${esc(s.shipHoldDays === 0 || s.shipHoldDays ? String(s.shipHoldDays) : '')}"
            placeholder="${TRACK.HOLD_NOTICE_DAYS}" autocomplete="off"></div>
      </div>`
      },
      {
    ...
    ['Покупатели', [
      {
        id: 'chat', title: 'Онлайн-чат', tone: chatWorst,
        note: (chatRows[0] && chatRows[0].short) || '',
        body: chatSection(s, settings, checked, db)
      },
      {
        /* Отзывы — два решения, которые правда принимают: пускать ли снимки
         * покупателей и будить ли себя уведомлением. Telegram остаётся
         * условием, и строка состояния говорит об этом прямо. */
      ...
      <div class="field field-check"><label><input type="checkbox" name="reviewPhotos" ${reviewPhotosOn ? 'checked' : ''}> Покупатель может приложить фото к отзыву, до ${R.REVIEW_PHOTOS_MAX}</label></div>
      ...
      {
        /* Кабинет — одна галочка, и строка состояния отвечает на единственный
         * вопрос: заводится ли он сам при заказе. Для этого нужна почта, и она
         * стоит следующим разделом. */
        id: 'accounts', title: 'Личный кабинет',
        note: !accountsOn
          ? 'выключен — значка в шапке нет, заказы без входа'
          : MAIL.configured(s)
            ? 'включён · кабинет заводится при заказе, пароль уходит письмом'
            : 'включён · без почты: покупатель регистрируется сам, пароль не восстановить',
        body: `<input type="hidden" name="accountsForm" value="1">
      <div class="field field-check"><label><input type="checkbox" name="accountsOn" ${accountsOn ? 'checked' : ''}> Личный кабинет покупателя на витрине</label></div>`
      },
      {
        /* Почта — свой раздел, а не хвост кабинета: полей у неё семь, и вместе
         * с ним они читались одной кучей. Письма магазин шлёт только два —
         * пароль от кабинета и ссылку восстановления. */
        id: 'mail', title: 'Почта для писем',
        tone: mailHost && !MAIL.configured(s) ? 'err' : '',
        note: MAIL.configured(s)
          ? `письма уходят через ${mailHost}`
          : mailHost
            ? 'настроена не до конца — нужен адрес отправителя'
            : 'не настроена — письма покупателям не уходят',
        body: `<div class="a-form-grid">
        <div class="field"><label for="set-mail-host">Сервер</label><input id="set-mail-host" name="mailHost" value="${esc(s.mailHost || '')}" placeholder="smtp.yandex.ru" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label for="set-mail-port">Порт</label><input id="set-mail-port" name="mailPort" inputmode="numeric" value="${esc(s.mailPort == null ? '' : String(s.mailPort))}" placeholder="пусто — ${MAIL.PORT_TLS}" autocomplete="off"></div>
        <div class="field"><label for="set-mail-user">Логин</label><input id="set-mail-user" name="mailUser" value="${esc(s.mailUser || '')}" placeholder="shop@example.ru" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label>Пароль приложения</label>${secretInput('mailPass', settings.mailPass, 'clearMailPass', false, 'пароль')}</div>
        <div class="field"><label for="set-mail-from">Адрес отправителя</label><input id="set-mail-from" name="mailFrom" type="email" inputmode="email" value="${esc(s.mailFrom || '')}" placeholder="пусто — логин" autocomplete="off"></div>
        <div class="field"><label for="set-mail-from-name">Имя отправителя</label><input id="set-mail-from-name" name="mailFromName" value="${esc(s.mailFromName || '')}" placeholder="пусто — ${esc(settings.storeName || 'название магазина')}" autocomplete="off"></div>
      </div>
      <div class="a-form-grid">
        <div class="field"><label for="set-mail-test">Проверить: отправить письмо на</label><input id="set-mail-test" name="mailTestTo" type="email" inputmode="email" placeholder="${esc(String(s.contactEmail || '').trim() || 'адрес для проверки')}" autocomplete="off"></div>
        ${/* Вторая кнопка отправки той же формы: настройки сохраняются как
             обычно, а потом уходит письмо — по ним же. Отдельной формой это не
             сделать: вложенных форм в HTML не бывает. */''}
        <div class="field set-mail-test"><button class="btn" type="submit" name="mailTest" value="1">Сохранить и отправить письмо</button></div>
      </div>`
      }
    ]],
    ['Подключения', [
      {
        ...
        body: `<div class="a-form-grid">
        <div class="field"><label for="set-tg-token">Токен бота от @BotFather</label><input id="set-tg-token" name="telegramBotToken" value="${esc(s.telegramBotToken || '')}" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label for="set-tg-chat">Chat ID получателя</label><input id="set-tg-chat" name="telegramChatId" value="${esc(s.telegramChatId || '')}" autocomplete="off"></div>
      </div>`
      },
      {
         ...
         * странице — это подпись, ведущая к свёртке вместо поля. */
        body: `<div class="field"><label for="set-dadata-key">Ключ dadata.ru (API-ключ «Подсказок»)</label>
          <input id="set-dadata-key" name="dadataToken" value="${esc(s.dadataToken || '')}" placeholder="пусто — подсказок не будет" autocomplete="off"></div>`
      },
      {
        /* Метрика — ради целей в Директе, и ради них же счётчик по умолчанию
         * получают только пришедшие из Яндекса (см. lib/yandex-metrika.js).
         * Номер — настройка ЭТОГО сайта: репозиторий открытый, а второй магазин
         * с тем же номером был бы публично связан с первым. */
        id: 'ym', title: 'Яндекс Метрика',
        note: ymId
          ? `счётчик ${ymId} · ${ymOnly ? 'только для пришедших из Яндекса' : 'все посетители'}`
          : 'счётчика нет — витрина Яндексу ничего не отдаёт',
        body: `<input type="hidden" name="ymForm" value="1">
      <div class="field"><label for="set-ym-id">Номер счётчика</label>
        <input id="set-ym-id" name="ymCounterId" value="${esc(s.ymCounterId || '')}" inputmode="numeric" placeholder="пусто — счётчика на витрине нет" autocomplete="off"></div>
      <div class="field field-check"><label><input type="checkbox" name="ymOnlyYandex" ${ymOnly ? 'checked' : ''}> Подключать только тем, кто пришёл из Яндекса</label></div>`
      }
    ]],
    ['Продавец и доступ', [
      {
        /* Кроме оператора данных здесь ИНН, ОГРНИП и банк — раздел отвечает на
         * вопрос «кто продаёт и куда идут деньги». Почта для обращений по
         * персональным данным остаётся здесь же: это тот же продавец. */
        id: 'legal', title: 'Реквизиты продавца',
        tone: legal ? '' : 'warn',
        /* Прежняя строка «ИНН/ОГРН одним полем» уехала в состояние раздела:
           поля в форме больше нет, а в документы пока едет именно она, и
           владелец должен видеть, откуда там реквизиты, которых он не находит. */
        note: legal
          ? [legal, legalInn ? 'ИНН ' + legalInn : '', bankReady ? 'банк указан' : 'без банковских реквизитов',
            legalOld ? 'в документах пока прежняя строка «' + setCut(legalOld, 40) + '»' : '']
            .filter(Boolean).join(' · ')
          : 'не заполнены — документы остаются без реквизитов продавца',
        body: `<div class="a-form-grid">
        <div class="field"><label for="set-legal-op">Продавец (ИП, ООО или ФИО)</label><input id="set-legal-op" name="legalOperator" value="${esc(s.legalOperator || '')}"></div>
        <div class="field"><label for="set-legal-inn">ИНН</label><input id="set-legal-inn" name="legalInn" inputmode="numeric" value="${esc(s.legalInn || '')}"></div>
        ${/* Подпись строки на витрине выводится из длины номера (15 цифр —
             ОГРНИП, 13 — ОГРН), поэтому поле одно и на то, и на другое. */''}
        <div class="field"><label for="set-legal-ogrn">ОГРНИП или ОГРН</label><input id="set-legal-ogrn" name="legalOgrn" inputmode="numeric" value="${esc(s.legalOgrn || '')}"></div>
        <div class="field"><label for="set-legal-addr">Адрес продавца</label><input id="set-legal-addr" name="legalAddress" value="${esc(s.legalAddress || '')}"></div>
      ...
      <h3 class="a-subhead">Банковские реквизиты</h3>
      <div class="a-form-grid">
        <div class="field"><label for="set-bank-acc">Расчётный счёт</label><input id="set-bank-acc" name="bankAccount" inputmode="numeric" value="${esc(s.bankAccount || '')}"></div>
        <div class="field"><label for="set-bank-name">Банк</label><input id="set-bank-name" name="bankName" value="${esc(s.bankName || '')}"></div>
        <div class="field"><label for="set-bank-bik">БИК банка</label><input id="set-bank-bik" name="bankBik" inputmode="numeric" value="${esc(s.bankBik || '')}"></div>
        <div class="field"><label for="set-bank-corr">Корреспондентский счёт</label><input id="set-bank-corr" name="bankCorr" inputmode="numeric" value="${esc(s.bankCorr || '')}"></div>
        <div class="field"><label for="set-bank-inn">ИНН банка</label><input id="set-bank-inn" name="bankInn" inputmode="numeric" value="${esc(s.bankInn || '')}"></div>
        <div class="field"><label for="set-bank-kpp">КПП банка</label><input id="set-bank-kpp" name="bankKpp" inputmode="numeric" value="${esc(s.bankKpp || '')}"></div>
      </div>`
      },
      {
        ...
        <div class="field"><label for="set-pass">Новый пароль, от 10 знаков</label><input id="set-pass" name="adminPassword" type="password" autocomplete="new-password" minlength="10" maxlength="500" placeholder="пусто — не менять"></div>
      </div>`
      }
    ]]
  ];
