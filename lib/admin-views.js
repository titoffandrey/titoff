...
<div class="a-shell">
  <main class="a-main">
    ${menu}
    <div class="a-topbar" data-live-part="topbar">${TA.render('header', { CONTENT: menuBtn + `${opts.active === 'orders' && Number.isFinite(opts.ordersPaidCount)
      ? `<div class="of-orders-heading"><h1>${esc(opts.title || '')}</h1><a class="of-paid-count" href="${esc(opts.ordersPaidHref || '/admin/orders?pay=ok')}" title="Оплаченные заказы по текущему поиску и периоду">Оплатили (${opts.ordersPaidCount})</a></div>`
      : `<h1>${esc(opts.title || '')}</h1>`}${liveMark}${
...
function dashboard(settings, db, pulse, opts) {
  ...
  // Пустое значение из адреса выпадает (`href` в lib/analytics-view.js), поэтому
  // «за всё время» — это просто `/admin`, без хвоста.
  const periodBar = `<div class="a-period"><div class="a-overview-heading"><h2>Продажи и заказы</h2><p>Результаты работы магазина за выбранный период</p></div>${periodSelect(period, d => R.esc('/admin' + (d ? '?period=' + d : '')))}</div>`;
  ...
  const body = `
    ...
    ${TA.render('recentOrders', { CONTENT: recent ? `<div class="o-recent">${recent}</div>`
      : `<p class="muted o-recent-empty">${period ? 'За выбранный период заказов нет' : 'Заказов пока нет'}</p>` })}
    <div class="ta-metrics">
      <a href="/admin/analytics">${TA.render('metric', { ICON: R.adminIcon('analytics'), VALUE: Number(live.online) || 0, LABEL: 'Сейчас на сайте', DETAIL: live.visitors ? 'сегодня ' + Number(live.visitors || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visitors, 'посетитель', 'посетителя', 'посетителей') + ' · ' + Number(live.visits || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visits, 'заход', 'захода', 'заходов') : 'сегодня заходов ещё не было' })}</a>
      <a href="/admin/reviews">${TA.render('metric', { ICON: R.adminIcon('reviews'), VALUE: pending, LABEL: 'Отзывов на модерации', DETAIL: pending ? 'ждут ответа' : 'очередь разобрана' })}</a>
      <a href="/admin/analytics/visitors">${TA.render('metric', { ICON: R.adminIcon('dash'), VALUE: 'Кто заходил', LABEL: 'История посещений за год', DETAIL: 'Открыть историю →' })}</a>
    </div>`;
...
function productsList(settings, db, flash) {
  ...
  return layout(settings, { active: 'products', title: 'Каталог', counts: navCounts(settings, db),
    ...
    flash, body: `<div class="a-panel a-panel-list"><table class="a-table a-table-sortable"><thead><tr><th><span class="sr-only">Порядок</span></th><th></th><th>Товар</th><th>Цена без скидки</th><th></th><th></th></tr></thead>
...
function ordersList(settings, db, flash, page, edit, filters, opts) {
  ...
  const rows = slice.items.map(o => {
    ...
    return `<tr id="order-${esc(o.id)}" class="${R.orderRowClass(o, now)}"><td class="of-order-cell">
      <article class="of-order" aria-labelledby="client-${esc(o.id)}">
  ...
  }).join('') || `<tr><td class="muted o-empty">${q || payFilter
  ...
  return layout(settings, { active: 'orders', title: 'Заказы', counts: navCounts(settings, db), flash, live: 'orders',
    ...
    body: '<div class="orders-page">' + periodBar
      ...
      })}<table class="a-table a-orders a-orders-focused"><caption class="sr-only">Список заказов: клиент, сумма и статус оплаты</caption><tbody>${rows}</tbody></table>${pager}</div>`
      + summaryDetails
