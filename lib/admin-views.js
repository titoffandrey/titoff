...
function priceFloatSection(s, settings, checked) {
  ...
  return `<div class="a-panel"><h2>Плавающие цены</h2>
    <p class="pay-mode-note">${note}</p>
...
function chatStateNote(settings) {
  ...
  else if (AI.configured(settings)) rows.push(['warn', 'Консультант выключен — на вопросы отвечает только менеджер в Telegram, покупатель ждёт живого ответа.']);
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const body = `<form class="a-form" method="post" enctype="multipart/form-data" action="/admin/settings">
    ...
    ${priceFloatSection(s, settings, checked)}
