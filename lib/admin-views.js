...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const sections = [
    ...
    ? '<p class="form-msg err">Касса включена, но платёжный токен не задан — ссылку на оплату она не выдаст.</p>' : ''}
      <div class="a-form-grid">
        <div class="field"><label for="alfa-token">Платёжный токен</label>
          <input id="alfa-token" name="alfabankToken" value="${esc(s.alfabankToken || '')}" placeholder="пусто — касса не работает" autocomplete="off" spellcheck="false">
          <p class="field-hint">Личный кабинет Альфа-Банка → Настройки → Платежный токен. Логин и пароль от API этой кассе не нужны: токеном можно только выставлять счета.</p></div>
  ...
  checked('alfabankTest', !!s.alfabankTest) ? ' checked' : ''}> Тестовый контур банка</label>
          <p class="field-hint">Настоящих денег не принимает. Тестовый контур банка отвечает не отовсюду — если касса «не отвечает», проверьте на боевом токене.</p></div>
      </div>`)}
      ${payPrimarySection(s)}
