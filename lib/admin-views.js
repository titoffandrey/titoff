...
function promoRow(entry, settings, used) {
  ...
  return `<div class="promo-row${entry.on ? '' : ' is-off'}">
    ...
    ${/* Правка под раскрытием, как ответ на отзыв: ни строчки скрипта, а
         свёрнутый список остаётся списком. Переименования тут нет намеренно —
         код это имя записи, на него ссылается и настройка «по умолчанию», и
         уже оформленные заказы. Нужен другой код — заводится новый. */''}
