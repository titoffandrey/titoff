...
function ordersList(settings, db, flash, page, edit, filters) {
  ...
  const visible = db.visibleOrders();
  const viewOrders = R.ordersInPeriod(visible, period);
  ...
  const rows = slice.items.map(o => {
    ...
    return `<tr id="order-${esc(o.id)}" class="${R.orderRowClass(o)}">
      ...
      <td class="o-payment"><div class="o-payment-main"><div class="o-payment-state">${R.orderStatus(o)}</div>${method}</div>${reconcile}${manual}${receipt}</td>
      <td class="o-sum"><b>${R.money(o.total, settings)}</b>${R.orderPromo(o, settings)}</td>
      <td class="o-act"><div class="o-acts">${remove}</div></td></tr>`;
  }).join('') || `<tr><td colspan="6" class="muted o-empty">${q || payFilter
  ...
  // Период уезжает с формой скрытым полем: без него «Найти» сбрасывало бы охват
  // страницы, хотя человек его не трогал. «Сбросить» снимает только отбор —
  // период он оставляет, потому что это не отбор, а срок, за который смотрят.
  const filterBar = `<form class="o-filters" method="get" action="/admin/orders">
    ...
    ${q || payFilter ? `<a class="link small" href="/admin/orders${period ? '?period=' + period : ''}">Сбросить</a>` : ''}
  </form>`;
   ...
   */
  const stats = R.orderStats(viewOrders, undefined, R.ordersInPeriod(
    typeof db.getOrders === 'function' ? db.getOrders() : visible, period));
  ...
  // Период — отдельной кнопкой ПЕРЕД формой отбора: он задаёт охват всей
  // страницы, а поиск и статус оплаты ищут внутри него.
  const periodBar = `<div class="a-period">${periodSelect(period, d => R.esc(listHref(1, { period: d })))}${filterBar}</div>`;
