...
function ordersList(settings, db, flash, page) {
  ...
  const rows = slice.items.map(o => {
    ...
    const remove = o.payment
      ? '<span class="muted" title="Заказ с платёжной историей нельзя удалить" aria-label="Удаление запрещено">🔒</span>'
      : `<form method="post" action="/admin/orders/${o.id}/delete" data-confirm="Удалить заказ?">${back}<button class="btn btn-sm btn-danger" aria-label="Удалить заказ">✕</button></form>`;
  ...
  return layout(settings, { active: 'orders', title: 'Заказы', pendingCount: db.pendingReviewCount(), flash,
    ...
    body: R.orderStatsBar(R.orderStats(orders), settings)
...
function payMethodRows(shown, live) {
  ...
  return ids.map(id => {
    ...
    return {
      ...
      // Касса ответила и этого способа у неё нет: строку оставляем (выбор
...
function payMethodsSection(shown, live) {
  ...
  const note = live
    ? `Список пришёл от кассы: способов — ${rows.filter(r => !r.off).length}, валют — ${live.currencies.length} (${live.currencies.map(esc).join(', ')}).`
    : 'Касса не ответила — показан встроенный список. Включите оплату и задайте ключи, чтобы видеть то, что у кассы есть на самом деле.';
  return `<h3 class="a-subhead">Способы оплаты на витрине</h3>
      ...
      <div class="pay-methods-grid">${rows.map(r => `<label class="pay-method-check${r.off ? ' is-off' : ''}">
        ...
        <span><b>${esc(r.name)}${r.fresh ? '<em class="pay-method-tag">новый у кассы</em>' : ''}${r.off ? '<em class="pay-method-tag pay-method-tag-off">нет у кассы</em>' : ''}</b>
        <i>${esc(r.hint)}${r.where.length ? ' · ' + esc(r.where.join(', ')) : ''}</i></span></label>`).join('')}</div>
...
function payModeNote(settings) {
  ...
  if (CROCO.enabled(settings)) {
    return `<p class="pay-mode is-on"><b>Сейчас: оплата на витрине.</b> После оформления покупатель попадает на страницу счёта с реквизитами, а состояние платежа видно в списке заказов.
      Один заказ — от ${CROCO.MIN_TOTAL.toLocaleString('ru-RU')} до ${money(CROCO.MAX_TOTAL)}: это пределы кассы, и товар дороже потолка на витрине не продаётся.</p>`;
  }
  return `<p class="pay-mode is-off"><b>Сейчас: заявки без оплаты.</b> Покупатель оформляет заказ, платить на сайте его не просят: он видит номер заявки и то, что менеджер свяжется с ним и подтвердит наличие.
    ...
    Пределов суммы в этом режиме нет: ${CROCO.MIN_TOTAL.toLocaleString('ru-RU')} – ${money(CROCO.MAX_TOTAL)} — ограничение кассы, а не магазина, поэтому дорогие товары продаются наравне с остальными.</p>`;
}
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const body = `<form class="a-form" method="post" enctype="multipart/form-data" action="/admin/settings">
    ...
    <div class="a-panel"><h2>Оплата на витрине</h2>
      <div class="field field-check"><label><input type="checkbox" name="crocopayEnabled"${checked('crocopayEnabled', !!settings.crocopayEnabled) ? ' checked' : ''}> Принимать оплату на витрине (CrocoPAY)</label></div>
      ${payModeNote(settings)}
      ${settings.crocopayEnabled && !CROCO.configured(settings)
        ? '<p class="form-msg err">Оплата включена, но ключи кассы не заданы — на витрине она не показывается, и заказы уходят заявками.</p>' : ''}
      <details class="pay-fold"${CROCO.enabled(settings) ? ' open' : ''}>
        <summary>Настройки кассы<span class="pay-fold-note">ключи, валюта, способы оплаты</span></summary>
      ...
      <p class="muted small">Состояние платежа видно в списке заказов: касса отдаёт настоящий статус счёта, поэтому «счёт истёк» и «оплата отменена» там тоже появляются, а не только «оплачено».
      Снятая галочка эти настройки не стирает — они лежат здесь и ждут; уже оформленные заказы и их счета выключение тоже не затрагивает.</p>
