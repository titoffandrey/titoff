...
function layout(settings, opts) {
  ...
  const liveMark = live
    ? `<span class="a-live" title="Страница обновляется сама, перезагружать не нужно"><i></i><b>живое</b></span>` : '';
...
function reviewsList(settings, db, flash, page) {
  ...
  const queue = slice.total
    ? `<div class="a-panel rv-panel">${reviewRows(slice, products, back, { showProduct: true })}${pager}</div>`
    : '';
  ...
  const body = `<div class="a-panel rv-prod-head">
    ...
    ${reviewToolbar(
      ...
      [(R.REVIEW_SORTS.find(([k]) => k === mode) || [])[1], (REVIEW_MEDIA.find(([k]) => k === kind) || [])[1]]
        .filter(Boolean).join(' · '))}
    <div class="a-panel rv-panel">${reviewRows(slice, products, back, { empty: 'У этого товара таких отзывов нет.' })}${pager}</div>`;
