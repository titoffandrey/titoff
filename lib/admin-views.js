...
function productsList(settings, db, flash) {
  ...
  const rows = list.map((p, i) => {
    ...
    return `<tr data-id="${esc(p.id)}" draggable="true"${hidden ? ' class="row-off"' : ''}>
      ...
      <td>${hidden ? '<span class="pill off">скрыт</span> ' : ''}${p.hotDeal ? '<span class="pill ok">🔥 акция</span>' : ''} ${r.count ? `★ ${r.avg} (${r.count})` : ''}</td>
      <td class="a-actions">
...
function reviewsList(settings, db, statusFilter, flash, page) {
  ...
  const tab = (k, l) => `<a class="a-tab${(statusFilter || 'all') === k ? ' active' : ''}" href="/admin/reviews?status=${k}">${l}</a>`;
  ...
  const rows = slice.items.map(rv => {
    ...
    // Куда вернуться после действия. Без этого админ, разбирающий очередь на
    ...
    // Раньше отзыв прятали в админке домена; прятать его теперь не от кого, а
    // убрать неудачный со страницы товара по-прежнему нужно — и удаление для
    // этого слишком грубо, отзыв покупателя восстановить неоткуда.
    const act = rv.status === 'pending'
      ? `<form method="post" action="/admin/reviews/${rv.id}/approve">${back}<button class="btn btn-sm btn-primary">Одобрить</button></form>`
      : `<form method="post" action="/admin/reviews/${rv.id}/hide">${back}<button class="btn btn-sm">Снять с витрины</button></form>`;
    return `<tr><td><div class="review-cell"><div class="rc-top"><b>${esc(rv.author)}</b> ${R.stars(rv.rating)} <span class="muted">${R.formatDate(rv.createdAt)}</span></div>
      ...
      <td class="a-actions">${act}<form method="post" action="/admin/reviews/${rv.id}/delete" data-confirm="Удалить отзыв?">${back}<button class="btn btn-sm btn-danger">Удалить</button></form></td></tr>`;
  ...
  return layout(settings, { active: 'reviews', title: 'Отзывы', pendingCount: db.pendingReviewCount(),
    ...
    flash, body: `<p class="muted small">На витрине видны только опубликованные. «На модерации» — то, что оставили посетители: пока отзыв там, его видит лишь сам автор.</p><div class="a-tabs">${tab('all', 'Все')}${tab('pending', 'На модерации')}${tab('approved', 'Опубликованные')}</div><div class="a-panel"><table class="a-table"><tbody>${rows}</tbody></table>${pager}</div>` });
}
...
function addReviewForm(settings, db, presetProductId, flash) {
  ...
  const body = `<form class="a-form" method="post" action="/admin/reviews/new" enctype="multipart/form-data">
    <div class="a-form-grid">
    ...
    <div class="a-form-actions"><button class="btn btn-primary" type="submit">Опубликовать</button><a class="btn" href="/admin/reviews">Отмена</a></div>
  </form>`;
  return layout(settings, { active: 'reviews', title: 'Добавить отзыв', pendingCount: db.pendingReviewCount(), flash, body });
}
...
module.exports = { loginPage, dashboard, productsList, productForm, reviewsList, addReviewForm, ordersList, analyticsPage, visitorPage, settingsPage };
