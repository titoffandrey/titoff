  ...
  const sections = [
    ['Витрина', [
      ...
      },
      {
        ...
        ${/* Координаты нужны одной карте — той, что стоит на «О компании». Поле
             необязательное: без него карта ищет саму строку адреса, а это ровно
             то, что покупатель сделал бы руками. Нужно оно там, где поиск по
             адресу промахивается — в ТЦ, в промзоне, в новом доме. */''}
        <div class="field"><label for="set-store-geo">Координаты на карте</label><input id="set-store-geo" name="storeGeo" value="${esc(s.storeGeo || '')}" placeholder="55.751244, 37.618423" maxlength="60">
          <p class="field-hint">Широта и долгота через запятую — скопируйте из Яндекс.Карт. Пусто — карта ищет по адресу выше.</p></div>
      </div>`
      },
      {
