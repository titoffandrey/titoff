...
<form class="login-card" method="post" action="/admin/login">
  ...
  ${/* Логин — e-mail или телефон в любой привычной записи, к хранимой форме
       его приводит сервер (lib/admin-login.js). Поле текстовое нарочно: под
       адрес и под номер клавиатуры разные, и угадывать здесь нечего. */''}
  <div class="field"><label for="admin-login">E-mail или телефон</label><input id="admin-login" name="username" autocomplete="username" autocapitalize="none" spellcheck="false" required autofocus></div>
...
function dashboard(settings, db, pulse, opts) {
  ...
  const periodBar = `<div class="a-period">${periodSelect(period, d => R.esc('/admin' + (d ? '?period=' + d : '')))}</div>`;
  ...
  checked('alfabankTest', !!s.alfabankTest) ? ' checked' : ''}> Тестовый контур банка</label>
    ...
    ['Магазин как продавец', [
      ...
      },
      {
        ...
        /* Логин — e-mail и телефон, войти можно по любому. Свёрнутая строка
         * называет их; пока не задан ни один, работает прежний логин, и строка
         * говорит об этом тёплым тоном — это временное состояние, а не выбор
         * владельца (lib/admin-login.js). Сохранённый телефон показывается с
         * разделителями, черновик после ошибки — как введён. */
        tone: LOGIN.configured(settings) ? '' : 'warn',
        note: LOGIN.configured(settings)
          ? 'вход: ' + setCut(LOGIN.describe(settings), 60)
          : `временный логин «${LOGIN.legacy(settings) || 'admin'}» — задайте e-mail или телефон`,
        body: `<div class="a-form-grid">
        <div class="field"><label for="set-login-mail">E-mail</label><input id="set-login-mail" name="adminEmail" type="email" inputmode="email" autocomplete="username" value="${esc(s.adminEmail || '')}" placeholder="owner@example.com"></div>
        <div class="field"><label for="set-login-phone">Телефон</label><input id="set-login-phone" name="adminPhone" type="tel" value="${esc(s.adminPhone ? R.phoneText(s.adminPhone) || s.adminPhone : '')}" placeholder="+7 999 123-45-67"></div>
      ...
      <p class="field-hint">Войти можно по e-mail или по номеру телефона — хватит и одного из них. Учётная запись одна и с полными правами, панель открывается по адресу <b>/admin</b>. Смена e-mail, телефона или пароля разлогинивает все открытые сессии — включая эту.</p>`
      }
    ]]
  ];
