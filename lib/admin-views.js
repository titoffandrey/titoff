...
/* Графики Bklit UI (vendor/bklit, собраны в public/bklit-charts.*) — только на
 * страницах, где есть острова `.bk-island`. Класс `bk-js` ставится ДО первой
 * отрисовки: по нему запасная разметка острова ждёт скрипт невидимой, а не
 * мелькает прежним графиком (см. vendor/bklit/integration.css). */
function chartsHead() {
  return `<link rel="stylesheet" href="/static/bklit-charts.css?v=${R.assetV('bklit-charts.css')}"><script>document.documentElement.classList.add('bk-js')</script>`;
}
...
<script src="/static/admin-presence.js?v=${R.assetV('admin-presence.js')}" defer></script>${opts.charts ? `<script src="/static/bklit-charts.js?v=${R.assetV('bklit-charts.js')}" defer></script>` : ''}
...
function analyticsPage(settings, db, snapshot, regionPage, opts) {
  ...
  return layout(settings, { active: 'analytics', title: 'Метрика', counts: navCounts(settings, db), wide: true, live: 'analytics orders', flash: opts.flash, charts: true, body });
}
  ...
  checked('alfabankTest', !!s.alfabankTest) ? ' checked' : ''}> Тестовый контур банка</label>
    ...
    ['Общение с покупателем', [
      ...
      <div class="a-form-grid">
        ...
        <div class="field"><label>Пароль</label>${secretInput('mailPass', settings.mailPass, 'clearMailPass', false, 'пароль')}</div>
