...
function layout(settings, opts) {
  ...
  const menu = `<input type="checkbox" id="a-menu" class="a-menu-check" aria-label="Разделы">
    <div class="a-menu-wrap">
      ...
      <nav class="a-menu-panel" data-live-part="menu">
        <div class="a-menu-brand"><a class="a-brand" href="/admin"><span class="a-brand-icon">${R.adminIcon('products')}</span><span class="a-brand-name">${esc(settings.storeName || 'Магазин')}<small>Панель управления</small></span></a><label class="a-menu-close" for="a-menu" role="button" tabindex="0" aria-label="Закрыть меню">×</label></div>
        <div class="a-menu-list" id="a-menu-navigation"><span class="a-menu-caption">Управление магазином</span>${nav}
...
<link rel="stylesheet" href="/static/admin-tailadmin.css?v=${R.assetV('admin-tailadmin.css')}">
...
<div class="a-shell">
  <main class="a-main">
    ${menu}
    <div class="a-topbar" data-live-part="topbar">${menuBtn}${opts.active === 'orders' && Number.isFinite(opts.ordersPaidCount)
...
function dashboard(settings, db, pulse, opts) {
  ...
  const body = `
    ...
    <div class="a-panel"><div class="a-panel-head"><h2>Последние заказы</h2><a class="link" href="/admin/orders">Все заказы →</a></div>
      ${recent ? `<div class="o-recent">${recent}</div>`
    : `<p class="muted o-recent-empty">${period ? 'За выбранный период заказов нет' : 'Заказов пока нет'}</p>`}</div>
    <div class="a-cards">
      <a class="a-stat a-stat-live${Number(live.online) ? '' : ' is-idle'}" href="/admin/analytics"><span class="a-stat-icon">${R.adminIcon('analytics')}</span><div class="a-stat-num"><i></i>${Number(live.online) || 0}</div><div class="a-stat-label">Сейчас на сайте</div><div class="a-stat-sub">${live.visitors ? 'сегодня ' + Number(live.visitors || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visitors, 'посетитель', 'посетителя', 'посетителей') + ' · ' + Number(live.visits || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visits, 'заход', 'захода', 'заходов') : 'сегодня заходов ещё не было'}</div></a>
      <a class="a-stat" href="/admin/reviews"><span class="a-stat-icon">${R.adminIcon('reviews')}</span><div class="a-stat-num">${pending}</div><div class="a-stat-label">Отзывов на модерации</div><div class="a-stat-sub">${pending ? 'ждут ответа' : 'очередь разобрана'}</div></a>
      <a class="a-stat a-stat-link" href="/admin/analytics/visitors"><span class="a-stat-icon">${R.adminIcon('dash')}</span><div class="a-stat-num">Кто заходил</div><div class="a-stat-label">История посещений за год</div></a>
    </div>`;
