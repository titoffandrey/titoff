...
function layout(settings, opts) {
  ...
  //
  // Счётчики продублированы одним числом на самой кнопке: со свёрнутым меню их
  // не видно вовсе, а «сколько всего ждёт меня» — то, что нужно знать, не
  // открывая ни одного раздела. Разбивка по разделам — в самом меню.
...
// единственного потока, то есть замершая витрина для всех посетителей.
function ordersList(settings, db, flash, page, view, edit, filters) {
  ...
  const viewOrders = archiveView ? archived : activeOrders;
  const f = filters || {};
  const q = String(f.q || '').trim().slice(0, 100);
  const payFilter = ['ok', 'wait', 'warn', 'off', 'none', 'draft'].includes(String(f.pay || ''))
    ? String(f.pay) : '';
  const allowedStatuses = Array.isArray(db.ORDER_STATUSES)
    ? db.ORDER_STATUSES : Object.keys(R.ORDER_WORKFLOW || {});
  const statusFilter = allowedStatuses.includes(String(f.status || '')) ? String(f.status) : '';
  ...
  const orders = viewOrders.filter(o => {
    if (statusFilter && String(o.status || 'new') !== statusFilter) return false;
  ...
  const rows = slice.items.map(o => {
    ...
    const back = `<input type="hidden" name="page" value="${slice.page}">`
      ...
      + (statusFilter ? `<input type="hidden" name="filterStatus" value="${esc(statusFilter)}">` : '');
    ...
    /* В «Удалённых» финансовая запись только восстанавливается: физически
     * стереть можно лишь чистый заказ без единой платёжной попытки. */
    ...
    const remove = archiveView
      ? `<form method="post" action="/admin/orders/${esc(o.id)}/restore">${back}<button class="btn btn-sm" aria-label="Восстановить заказ" title="Восстановить заказ">Вернуть</button></form>`
        + (o.payment
          ? '<span class="muted small o-fin-keep">Платёжная история хранится</span>'
    ...
    const workflow = archiveView
      ? R.orderWorkflowStatus(o)
      : `<form class="o-flow-form" method="post" action="/admin/orders/${esc(o.id)}/status">${back}`
        + `<select name="orderStatus" aria-label="Выполнение заказа ${esc(R.orderNo(o.number))}">`
        + allowedStatuses.map(status => `<option value="${esc(status)}"${String(o.status || 'new') === status ? ' selected' : ''}>${esc(R.orderWorkflowLabel(status))}</option>`).join('')
        + '</select><button class="btn btn-sm" type="submit">Сохранить</button></form>';
    ...
    const reconcile = !archiveView && checkable
      ? `<form class="o-reconcile" method="post" action="/admin/orders/${esc(o.id)}/reconcile">${back}`
        ...
        + '<button class="link-btn small" type="submit">Перепроверить в кассе</button></form>' : '';
    return `<tr id="order-${esc(o.id)}" class="${R.orderRowClass(o)}">
      ...
      <td class="o-flow">${workflow}</td>
      <td class="o-state">${R.orderStatus(o)}${reconcile}</td>
  ...
  }).join('') || `<tr><td colspan="8" class="muted">${q || payFilter || statusFilter ? 'По фильтру заказов нет.' : (archiveView ? 'Удалённых заказов нет.' : 'Заказов нет.')}</td></tr>`;
  ...
  const filterBar = `<form class="o-filters" method="get" action="/admin/orders">
    ...
    ${editing ? '<input type="hidden" name="edit" value="1">' : ''}
    <label><span class="sr-only">Поиск заказа</span><input type="search" name="q" value="${esc(q)}" placeholder="Номер, клиент, телефон, товар"></label>
    ...
    <label><span class="sr-only">Выполнение</span><select name="status"><option value="">Любое выполнение</option>${allowedStatuses.map(status => `<option value="${esc(status)}"${statusFilter === status ? ' selected' : ''}>${esc(R.orderWorkflowLabel(status))}</option>`).join('')}</select></label>
    <button class="btn btn-sm" type="submit">Найти</button>
    ${q || payFilter || statusFilter ? `<a class="link small" href="${archiveView ? '/admin/orders?view=archive' : '/admin/orders'}">Сбросить</a>` : ''}
  </form>`;
   ...
   *
   * Подтверждение называет число чистых заявок и отдельно — сколько финансовых
   * записей останутся для позднего callback и сверки.
   */
  const removable = archived.filter(o => !o.payment).length;
  const protectedCount = archived.length - removable;
  const purgeAllWarn = `Стереть черновики без платёжной истории (${removable})? Вернуть их будет неоткуда.`
    + (protectedCount ? ` Заказы с платёжной историей (${protectedCount}) останутся для сверки.` : '');
  const purgeAll = archiveView && removable
    ? `<form class="o-purge-all" method="post" action="/admin/orders/purge-all" data-confirm="${esc(purgeAllWarn)}">
        <input type="hidden" name="edit" value="1">
        <button class="btn btn-sm btn-danger" type="submit">Удалить чистые черновики</button></form>`
    : '';
  return layout(settings, { active: 'orders', title: 'Заказы', counts: navCounts(settings, db), flash, live: 'orders',
    ...
    body: switcher + filterBar + R.orderStatsBar(R.orderStats(viewOrders, undefined,
      typeof db.getOrders === 'function' ? db.getOrders() : activeOrders.concat(archived)), settings)
      + `<div class="a-panel">${R.editSwitch('orders-edit', { on: editing, extra: purgeAll })}<table class="a-table a-orders"><thead><tr><th>Заказ</th><th>Клиент</th><th>Состав</th><th>Выполнение</th><th>Статус оплаты</th><th>Способ</th><th class="o-sum">Сумма</th><th class="o-act"><span class="sr-only">Удалить</span></th></tr></thead><tbody>${rows}</tbody></table>${pager}</div>`
      ...
      + R.providerStatsBar(R.providerStats(viewOrders)) });
}
...
function chatAvatar(chat, opts) {
  ...
  const live = CHAT.presence(chat).online;
...
function chatTitle(chat) {
  return String((chat && chat.name) || (chat && chat.client && chat.client.city)
    || (chat && chat.city) || 'Посетитель').trim() || 'Посетитель';
}
    ...
    return `<a class="chat-row${unread ? ' is-unread' : ''}" href="/admin/chat/${esc(c.id)}" data-live-key="chat-${esc(c.id)}">
      ...
      <span class="chat-row-main">
        <span class="chat-row-top">
          <b class="chat-row-name">${esc(chatTitle(c))}</b>
          ${chatModeBadge(c.mode)}
          <span class="chat-row-when">${esc(chatWhen(c.lastAt))}</span>
        </span>
        <span class="chat-row-last">${mine ? '<i>Вы:</i> ' : ''}${esc(last ? last.text : 'Пустой диалог')}</span>
        ${chatMarks(c)}
      </span>
...
function chatEditBox(chat, m) {
  return `<details class="chat-edit">`
    + `<summary class="chat-edit-open" title="Изменить или удалить реплику">${CHAT_PENCIL}<span class="sr-only">Изменить реплику</span></summary>`
    ...
    + `<button class="btn btn-sm chat-edit-drop" type="submit" name="drop" value="1" formnovalidate`
    + ` data-confirm="Удалить эту реплику? Покупатель этого не увидит, но и вернуть её будет неоткуда.">Удалить</button>`
    + `</div></form></details>`;
}

function chatPage(settings, db, chat, flash, orders) {
  ...
  for (const m of chat.messages) {
     ...
     * это место обратно. */
    const bubble = `<div class="chat-line is-${esc(m.role)}">`
      + (who ? `<span class="chat-line-who">${esc(who)}</span>` : '')
      + `<span class="chat-line-text">${esc(m.text)}</span>`
      + `<span class="chat-line-at">${esc(chatTime(m.at))}${tick}</span></div>`;
  ...
  const body = `${R.CHAT_TICK_SPRITE}<div class="chat-view">
    <div class="chat-head-card">
      ${chatAvatar(chat, { big: true })}
      <div class="chat-head-who">
        <div class="chat-head-name">${esc(chatTitle(chat))}${chatModeBadge(chat.mode)}</div>
        <div class="chat-head-sub">${chatPresence(chat)}${chat.ip ? `<span class="chat-ip">${esc(chat.ip)}</span>` : ''}</div>
      </div>
    ...
    <form class="chat-answer" method="post" action="/admin/chat/${esc(chat.id)}/reply">
      <textarea name="text" rows="2" required placeholder="Написать покупателю…" maxlength="${CHAT.MAX_TEXT}"></textarea>
      <div class="chat-answer-acts">
        <button class="btn btn-primary chat-answer-send" type="submit">Отправить</button>
        <button class="btn btn-sm" type="submit" name="mode" value="ai" formnovalidate title="Дальше отвечает консультант">Вернуть ИИ</button>
        <button class="btn btn-sm" type="submit" name="mode" value="closed" formnovalidate title="Пометить разговор законченным">Завершить</button>
      </div>
    </form>
  </div>`;

  return layout(settings, {
    active: 'chat', title: 'Диалог', counts: navCounts(settings, db), flash,
    live: 'chat',
  ...
  const body = `<div class="chat-view">
    ...
    <div class="chat-head-card">
      ...
      <div class="chat-head-who">
        ...
        <div class="chat-head-sub">${esc(visitor.ip || '')}</div>
      </div>
...
module.exports = { loginPage, dashboard, chatList, chatPage, chatNewPage, paymentsPage, productsList, productForm, reviewsList, productReviews, reviewForm, ordersList, analyticsPage, visitorsPage, visitorPage, settingsPage, navCounts, noteOrder, noteReview, noteChat };
