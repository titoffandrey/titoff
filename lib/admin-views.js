...
function layout(settings, opts) {
  ...
  /* СООБЩЕНИЕ ОБ УДАЧНОМ ДЕЙСТВИИ — КАРТОЧКОЙ ПОВЕРХ СТРАНИЦЫ, А НЕ ПЛАШКОЙ
   * СВЕРХУ.
   *
   * Плашка стояла первой строкой содержимого, то есть увидеть её можно было,
   * только стоя наверху. А действие делают там, где строка: разобрав заявку в
   * конце списка из полусотни, менеджер получал ответ кассы за экран от себя —
   * и заодно оказывался в начале списка, потому что страница приходит заново.
   * Карточка падает в тот же угол, что и события канала (`noteCard()`): оттуда
   * её видно с любого места страницы, и раскладку она не двигает.
   *
   * ОШИБКА ОСТАЁТСЯ ПЛАШКОЙ В ПОТОКЕ, и это не непоследовательность: она
   * отвечает на другой вопрос — «почему не сохранилось», — нужна всё время,
   * пока человек правит форму, и место ей рядом с формой, а не в углу. По той
   * же причине она не гаснет сама (см. `.a-flash` в admin.css) и по ней же
   * страница с ошибкой не возвращается на прежнее место прокрутки: спрятать
   * объяснение значило бы отменить его вовсе.
   */
  const flashText = String(opts.flash == null ? '' : opts.flash);
  const flashErr = !!flashText && opts.flashType === 'err';
  const flashNoteHtml = flashText && !flashErr ? flashNote(flashText) : '';
...
<link rel="stylesheet" href="/static/admin.css?v=${R.assetV('admin.css')}">${shellStyle(settings)}${opts.active === 'orders' ? `<link rel="stylesheet" href="/static/admin-orders.css?v=${R.assetV('admin-orders.css')}">` : ''}${opts.active === 'analytics' ? `<link rel="stylesheet" href="/static/admin-metrics.css?v=${R.assetV('admin-metrics.css')}">` : ''}</head>
...
<div class="a-shell">
  <main class="a-main">
    ...
    ${flashErr ? `<div class="a-flash err" data-flash>${esc(flashText)}</div>` : ''}
    <div class="a-content${opts.wide ? ' a-content-wide' : ''}" data-live-part="content">${accessibleBody}</div>
  </main>
</div>
${live || flashNoteHtml ? /* Куда падают уведомления: события канала и ответ на
   собственное действие («Сохранено»). Стоит ВНЕ живых блоков намеренно: подмена
   разметки при обновлении страницы иначе сносила бы карточку прямо из-под
   курсора — она приходит по своему каналу и живёт своей жизнью.
   Контейнер нужен и странице без канала: «Сохранено» есть и у формы настроек, а
   живого обновления у неё нет и быть не должно.
   `aria-live="polite"` — чтобы о заказе узнал и тот, кто читает с экрана. */
  `<div class="a-notes" id="a-notes" aria-live="polite">${flashNoteHtml}</div>` : ''}
...
// Крестик один на все карточки: событие канала и «Сохранено» закрываются
// одинаково, и двух похожих крестиков в одном углу быть не должно.
const NOTE_X = `<button type="button" class="a-note-x" data-note-close aria-label="Скрыть">`
  + `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true">`
  + `<path d="M7 7l10 10M17 7 7 17"/></svg></button>`;

/* Ответ на собственное действие: «Сохранено», «Товар удалён», «Касса ещё ждёт
 * оплату».
 *
 * Карточка та же, что у событий, только без ссылки и миниатюры: вести отсюда
 * некуда — человек уже на той странице, где всё и случилось, и читает он одну
 * строку, а не выбирает, куда пойти.
 *
 * Уходит она САМА и чистым CSS (`.a-note-flash` в admin.css) — по тому же
 * правилу, по которому гасла плашка: карточку рисует сервер, и исчезать она
 * обязана и там, где скрипты панели не загрузились. Скрипту остаётся убрать
 * узел по концу анимации, поэтому число секунд лежит ровно в одном месте.
 */
function flashNote(text) {
  return `<div class="a-note a-note-flash" data-note role="status">`
    + `<span class="a-note-say"><span class="a-note-tick">`
    + `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12.5 10 17.5 19 7"/></svg>`
    + `</span><span class="a-note-msg">${esc(text)}</span></span>${NOTE_X}</div>`;
}
...
<link rel="icon" href="/static/favicon.ico?v=${R.assetV('favicon.ico')}">
<link rel="stylesheet" href="/static/styles.css?v=${R.assetV('styles.css')}">
<link rel="stylesheet" href="/static/admin.css?v=${R.assetV('admin.css')}">${shellStyle(settings)}</head>
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const sections = [
    ...
    ? '<p class="form-msg err">Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.</p>' : ''}
      <div class="a-form-grid">
    ...
    ? '<p class="form-msg err">Касса включена, но доступ не задан — ссылку на оплату она не выдаст. Хватит любого из двух: платёжного токена или пары логин/пароль.</p>' : ''}
  ...
  checked('alfabankTest', !!s.alfabankTest) ? ' checked' : ''}> Тестовый контур банка</label>
    ...
    ? '<p class="form-msg err">Касса включена, но Public key или Secret не заданы либо неверны — ссылку на оплату она не выдаст.</p>' : ''}
      <div class="a-form-grid">
