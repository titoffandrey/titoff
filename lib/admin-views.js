...
function newRulesBlock(settings) {
  ...
  return `<div class="a-newrules">
        ...
        <span class="muted small">Не нужно — просто не нажимайте: поле останется как есть.</span>
      </div>`;
}
...
function promptCostNote(settings, db) {
  ...
  return `<p class="field-hint">Запрос к модели — примерно ${size.total} токенов:
    инструкция ${size.instruction}, условия магазина ${size.store}, каталог с ценами ${size.catalog}.
    Первый вопрос в разговоре стоит полную цену, следующие идут из кэша впятеро дешевле.</p>`;
}
...
function chatSection(s, settings, checked, db) {
  return `${chatStateNote(settings)}
      ...
      <div class="field field-check"><label><input type="checkbox" name="aiPauseWhenAdminOnline" ${checked('aiPauseWhenAdminOnline', settings.aiPauseWhenAdminOnline !== false) ? 'checked' : ''}> Пока администратор онлайн, первым отвечает менеджер</label>
        <p class="field-hint">После закрытия последней вкладки панели консультант подхватит первый вопрос через ${CHAT.ADMIN_FIRST_WAIT_MIN} минуты, а начатый менеджером диалог — через срок возврата ниже.</p></div>
      <div class="a-form-grid">
      ...
      <div class="field"><label>Инструкция для ИИ</label>
        ...
        <p class="field-hint">Это полный набор статических правил Ксении — скрытых правил поведения вне этого поля нет. Каталог, актуальные цены, условия магазина и данные заказа подставляются автоматически.</p>
...
function paySummary(settings, health) {
  ...
  if (PAYMENTS.mode(settings) === 'cashbox') {
    ...
    const unverified = rows.filter(r => r.on && r.ready && r.state === 'unverified');
    if (unverified.length) return ['warn', `оплата на витрине · ${names(unverified)}: связь ещё не проверена`];
    const max = PAYMENTS.cashboxMax(settings);
  ...
  const sections = [
    ['Витрина', [
      ...
      },
      {
        ...
        /* Галочка «память в названии карточки» — из черновика, как все прочие
         * (см. `checked` выше): снятая приходит отсутствием поля, и из `s` она
         * подхватилась бы сохранённой. */
      ...
      },
      {
         ...
         *
         * Здесь собраны и способы связи, и адрес единственной офлайн-точки:
         * покупатель ищет их в одном месте, когда решает, написать, позвонить
         * или приехать. В общем списке они терялись между валютой и подвалом. */
        ...
        <div class="field"><label for="set-tg">Telegram для витрины</label><input id="set-tg" name="contactTelegram" value="${esc(s.contactTelegram || '')}" placeholder="@manager">
          <p class="field-hint">Строка в меню и кнопка в подвале.</p></div>
        <div class="field"><label for="set-wa">Номер WhatsApp</label><input id="set-wa" name="contactWhatsApp" type="tel" value="${esc(s.contactWhatsApp ? R.phoneText(s.contactWhatsApp) || s.contactWhatsApp : '')}" placeholder="+7 999 123-45-67">
          <p class="field-hint">Строка в меню и зелёная кнопка рядом с Telegram.</p></div>
        <div class="field"><label for="set-wa-message">Сообщение для WhatsApp</label>
          <textarea id="set-wa-message" class="a-area-sm" name="contactWhatsAppMessage" rows="2" maxlength="500" placeholder="Здравствуйте! Хочу уточнить наличие товара.">${esc(s.contactWhatsAppMessage || '')}</textarea>
          <p class="field-hint">Уже будет набрано в чате после перехода. Покупателю останется отправить его.</p></div>
        ...
        <div class="field"><label for="set-hours">Время работы</label><input id="set-hours" name="contactHours" value="${esc(s.contactHours || '')}" placeholder="09:00–22:00 МСК" maxlength="120">
          <p class="field-hint">Показывается в подвале и в ответах консультанта.</p></div>
        <div class="field"><label for="set-store-address">Адрес офлайн-магазина</label><input id="set-store-address" name="storeAddress" value="${esc(s.storeAddress || '')}" placeholder="г. Ноябрьск, проспект Мира, 88А, ТЦ «Ноябрьский»" maxlength="300">
          <p class="field-hint">Показывается в подвале и на странице «О компании». Консультант знает, что это единственная точка и в других городах магазинов нет.</p></div>
        ${/* Координаты нужны одной карте — той, что стоит на «О компании». Карта
             ...
             адреса ей нечем: геокодера у витрины нет, каждый такой запрос — это
             поход к чужому сервису на открытии страницы покупателем. Пусто —
             раздела с картой на странице просто нет. */''}
        <div class="field"><label for="set-store-since">Год открытия офлайн-точки</label><input id="set-store-since" name="storeSinceYear" inputmode="numeric" value="${esc(s.storeSinceYear || '')}" placeholder="${esc(String(PROMPT.STORE_SINCE_YEAR))}" maxlength="4">
          <p class="field-hint">Консультант отвечает «работает второй год» — считается от этого года по московскому календарю. Пусто — ${esc(String(PROMPT.STORE_SINCE_YEAR))}.</p></div>
        <div class="field"><label for="set-store-geo">Координаты на карте</label><input id="set-store-geo" name="storeGeo" value="${esc(s.storeGeo || '')}" placeholder="55.751244, 37.618423" maxlength="60">
          <p class="field-hint">Широта и долгота через запятую: в Яндекс.Картах правой кнопкой по точке → «Что здесь?». Пусто — карты на «О компании» не будет. Найти по адресу: <code>node scripts/geocode-store.js --apply</code>.</p></div>
        ${/* Снимки самой точки — для карточки магазина на «О компании». Витрина
             показывает технику, а покупатель перед предоплатой хочет увидеть,
             что за ней стоит живое место: витрина, вывеска, вход. Обработка
             общая с фото товаров (WebP, метаданные снимаются), но БЕЗ подгонки
             в квадрат: у зала и вывески кадр свой, и вписывать их в товарную
             рамку с серым полем незачем. */''}
        <div class="field"><label for="set-store-photos">Фотографии магазина</label>
          ${storePhotoGrid(s)}
          <input id="set-store-photos" type="file" name="storePhotos" accept="image/*" multiple>
          <p class="field-hint">До ${R.STORE_PHOTOS_MAX} снимков: витрина, вход, зал. Показываются карточкой рядом с картой на «О компании». Отметьте крестик на снимке, чтобы удалить его при сохранении.</p></div>
      </div>`
      },
      {
      ...
      {
        /* Домены. Раздел стоит последним в группе: смотрят его редко, а вот
         * когда смотрят — обычно потому, что основной адрес уже недоступен.
         *
         * Мультидоменности это не возвращает: у всех имён одна витрина и один
         * каталог, приложение домен не разбирает вовсе (`lib/domains.js`). */
        id: 'domains', title: 'Домены магазина',
        /* Свёрнутая строка отвечает на вопрос, ради которого раздел открывают:
         * есть ли запасной адрес. Список обрезается — доменов бывает несколько,
         * и в строку оглавления они не помещаются. */
        ...
        <label for="set-domain-list">Дополнительные домены</label>
        ...
        <p class="field-hint">По одному в строке, не больше ${DOMAINS.MAX}. Приставка <b>www</b> добавляется сама — вписывать её отдельной строкой не нужно.</p>
      </div>
      <p class="field-hint">Это тот же магазин под другим адресом: каталог, цены, заказы и панель у всех доменов общие. Домен заработает, когда его <b>A-запись</b> будет указывать на сервер этого магазина — сертификат выпускается сам при первом открытии, за несколько секунд. Основной адрес задаётся при установке и здесь не показывается: он работает, даже когда приложение остановлено.</p>`
      }
    ]],
    ['Деньги', [
      {
    ...
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-platega" data-toast-message="Platega: Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.">Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.</p>' : ''}
      <div class="a-form-grid">
        ...
        <div class="field"><label for="platega-fee">Комиссия Platega, включённая в цены, %</label>
          <input id="platega-fee" name="plategaFeePercent" type="number" min="0" max="100" step="0.01" value="${esc(s.plategaFeePercent == null ? 0 : s.plategaFeePercent)}">
          <p class="field-hint">Процент из тарифа Platega за СБП. Он сидит внутри цены: кассе уходит цена без него, покупатель платит ценник (округлённый вниз не больше чем на рубль). Выберет на странице кассы способ с другим тарифом — заплатит иначе, а магазин получит ту же сумму.</p></div>
        <div class="field"><label for="platega-max">Максимальная сумма платежа, ₽</label>
          <input id="platega-max" name="plategaMaxTotal" inputmode="numeric" autocomplete="off"
            value="${esc(s.plategaMaxTotal == null || s.plategaMaxTotal === '' ? PLATEGA.DEFAULT_MAX_TOTAL : s.plategaMaxTotal)}" placeholder="${PLATEGA.DEFAULT_MAX_TOTAL}">
          <p class="field-hint">Лимит Platega на один платёж по СБП вместе с комиссией. Заказ дороже уходит ${PAYMENTS.fallbackMode(settings) === 'own' ? 'переводом по своим реквизитам' : 'заявкой — менеджер связывается с покупателем сам'}. Поднимут лимит — впишите новое число.</p></div>
      </div>
      <p class="field-hint">В личном кабинете задайте Callback URL: <code>https://&lt;домен магазина&gt;/api/pay/platega/callback</code>. Способ «Онлайн-оплата» ниже должен быть отмечен: покупатель выбирает СБП уже на странице кассы. <a href="https://docs.platega.io/" target="_blank" rel="noopener noreferrer">Документация Platega</a>.</p>`, flaggedProvider('Platega'))}
    ...
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-alfabank" data-toast-message="Альфа-Банк: Касса включена, но доступ не задан — ссылку на оплату она не выдаст. Хватит любого из двух: платёжного токена или пары логин/пароль.">Касса включена, но доступ не задан — ссылку на оплату она не выдаст. Хватит любого из двух: платёжного токена или пары логин/пароль.</p>' : ''}
      <p class="field-hint">Доступ подойдёт <b>любой из двух</b> — что вам выдал банк. Токен лежит в личном кабинете сразу; пару логин/пароль (логин с суффиксом <code>-api</code>) банк присылает письмом при подключении эквайринга.</p>
      <div class="a-form-grid">
        <div class="field"><label for="alfa-token">Платёжный токен</label>
          <input id="alfa-token" name="alfabankToken" value="${esc(s.alfabankToken || '')}" placeholder="из ЛК: Настройки → Платежный токен" autocomplete="off" spellcheck="false"></div>
        <div class="field"><label class="field-check"><input type="checkbox" name="alfabankTest"${
  checked('alfabankTest', !!s.alfabankTest) ? ' checked' : ''}> Тестовый контур банка</label>
          <p class="field-hint">Настоящих денег не принимает. Отвечает не отовсюду — если касса «не отвечает», проверьте на боевом доступе.</p></div>
    ...
    ? '<p class="form-msg err" data-store-toast data-toast-type="warning" data-toast-id="admin-config-solutiones" data-toast-message="Solutiones: Касса включена, но Public key или Secret не заданы либо неверны — ссылку на оплату она не выдаст.">Касса включена, но Public key или Secret не заданы либо неверны — ссылку на оплату она не выдаст.</p>' : ''}
      <div class="a-form-grid">
        ...
        <div class="field"><label>Secret</label>
          ${secretInput('solutionesApiSecret', settings.solutionesApiSecret, 'clearSolutionesApiSecret')}
          <p class="field-hint">Показывается в кабинете один раз — при создании ключа.</p></div>
        <div class="field"><label>Webhook Secret</label>
          ${secretInput('solutionesWebhookSecret', settings.solutionesWebhookSecret, 'clearSolutionesWebhookSecret', true)}
          <p class="field-hint">Строка <code>whsec_…</code> из «Настроек» кабинета. Без неё уведомления кассы отклоняются, а оплата подтверждается фоновым опросом — реже, но так же надёжно.</p></div>
        <div class="field"><label for="solutiones-max">Максимальная сумма платежа, ₽</label>
          <input id="solutiones-max" name="solutionesMaxTotal" inputmode="numeric" autocomplete="off"
            value="${esc(s.solutionesMaxTotal == null ? '' : s.solutionesMaxTotal)}" placeholder="пусто — предела нет">
          <p class="field-hint">Пусто — любой заказ уходит в кассу: не выдала ссылку — покупатель видит отказ и может повторить. Впишете число — заказ дороже уйдёт ${PAYMENTS.fallbackMode(settings) === 'own' ? 'переводом по своим реквизитам' : 'заявкой — менеджер связывается с покупателем сам'}. Замер 17 сентября 2026: выше ~19 000 ₽ касса отвечает «FAILED» без ссылки при заявленном лимите 160 000.</p></div>
      </div>
      <p class="field-hint">В кабинете задайте Callback URL: <code>https://&lt;домен магазина&gt;/api/pay/solutiones/callback</code>. Касса выдаёт ссылку СБП на сумму заказа, поэтому ниже должен быть отмечен способ «СБП» со страницей оплаты (<code>SBP_ONLINE</code>). Комиссию касса удерживает из выплаты магазину; покупатель платит ценник.</p>`, flaggedProvider('Solutiones'))}
      ${payPrimarySection(s)}
    ...
    ['Общение с покупателем', [
      ...
      {
        /* Личный кабинет и почта — один раздел: почта в магазине нужна ровно
         * кабинету (пароль при заказе и ссылка восстановления), и владелец
         * настраивает их с одним вопросом — «будут ли покупатели получать
         * пароль». Свёрнутая строка отвечает именно на него. */
        id: 'accounts', title: 'Личный кабинет и почта',
        tone: accountsOn && mailHost && !MAIL.configured(s) ? 'err' : '',
        note: !accountsOn
          ? 'кабинет выключен — значка в шапке нет, заказы без входа'
          : MAIL.configured(s)
            ? `кабинет включён · письма через ${mailHost} — пароль при заказе уходит на почту`
            : mailHost
              ? 'кабинет включён · почта не настроена до конца — нужен адрес отправителя'
              : 'кабинет включён · почты нет — вход и регистрация работают, кабинет при заказе не создаётся',
      ...
      <p class="field-hint">Вход по e-mail и паролю не короче ${CUSTOMERS.PASSWORD_MIN} знаков: заказы, оплата, отправления и корзина. Заказ с указанной почтой заводит кабинет сам, а пароль уходит письмом — для этого нужен почтовый сервер ниже. Без него кабинет тоже работает: покупатель регистрируется сам, а восстановить забытый пароль сможет только через менеджера.</p>
      <h3 class="a-subhead">Почтовый сервер (SMTP)</h3>
      <div class="a-form-grid">
        ...
        <div class="field"><label for="set-mail-pass">Пароль</label>${secretInput('mailPass', settings.mailPass, 'clearMailPass', false, 'пароль')}</div>
      ...
      <p class="field-hint">Порт ${MAIL.PORT_TLS} — шифрование с первого байта, 587 — STARTTLS; без шифрования пароль не отправляется. У Яндекса и Mail.ru подходит только пароль приложения, а не от самой почты. Всё хранится на сервере, в браузер не уезжает.</p>
      <div class="a-form-grid">
    ...
    ['Магазин как продавец', [
      ...
      {
        /* Яндекс Метрика — ради целей в Директе, и ради них же счётчик по
         * умолчанию получают только пришедшие из Яндекса (см.
         * lib/yandex-metrika.js). Свёрнутая строка называет и номер, и режим:
         * «счётчик стоит, а кому он отдаётся» — единственный вопрос раздела.
         *
      ...
      <div class="field"><label for="set-ym-id">Номер счётчика</label>
        <input id="set-ym-id" name="ymCounterId" value="${esc(s.ymCounterId || '')}" inputmode="numeric" placeholder="пусто — счётчика на витрине нет" autocomplete="off">
        <p class="field-hint">Счётчик заводится с того же аккаунта, что и Директ. Он не подключается владельцу с открытой панелью, посетителям из правил «не считать» и отключившим метрику, а также на страницах панели, отслеживания и чека. Вебвизор и карта кликов выключены — поля оформления Яндексу не уходят.</p></div>
      ...
      <p class="field-hint">Счётчик получают визиты с yclid, utm_source=yandex или переходом с yandex.ru — и их возвраты ${YM.SOURCE_DAYS} дней. Остальных посетителей Яндекс не видит. Цели: <code>cart</code>, <code>order</code> (с суммой), <code>paid</code> (с суммой) и покупка электронной коммерции.</p>`
      },
      {
        /* Раздел называется «Реквизиты продавца», а не «Персональные данные»:
         * кроме оператора данных здесь теперь ИНН, ОГРНИП и банк, то есть он
         * отвечает на вопрос «кто продаёт и куда идут деньги». Почта для
         * обращений по персональным данным остаётся здесь же — это тот же
         * продавец, и заводить ради одной строки второй раздел незачем. */
        id: 'legal', title: 'Реквизиты продавца',
        tone: legal ? '' : 'warn',
        /* Свёрнутая строка отвечает ровно на вопрос раздела: кто продаёт и есть
         * ли банковские реквизиты. Прежняя строка «ИНН/ОГРН одной строкой»
         * приезжала из снятого поля и у нового магазина молчала бы всегда. */
        note: legal
      ...
      ${legalOld ? `<p class="field-hint">Раньше ИНН и ОГРНИП хранились одной строкой — «${esc(legalOld)}». Она и подставляется в документы, пока поля выше пустые.</p>` : ''}
      <p class="field-hint">Имя, ИНН и ОГРНИП стоят в подвале каждой страницы; целиком реквизиты подставляются в политику конфиденциальности, оба согласия, страницы гарантии, возврата и «О компании».</p>
      ...
      <p class="field-hint">Показываются только в разделе «Реквизиты продавца» на странице «О компании». В подвал и на страницу оплаты они не идут: счёт для перевода задаётся отдельно, в разделе «Оплата на витрине».</p>`
      },
      {
