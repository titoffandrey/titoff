      ...
      <div class="a-form-grid">
        ...
        <div class="field"><label for="solutiones-max">Максимальная сумма платежа, ₽</label>
          <input id="solutiones-max" name="solutionesMaxTotal" inputmode="numeric" autocomplete="off"
            value="${esc(s.solutionesMaxTotal == null || s.solutionesMaxTotal === '' ? SOLUTIONES.DEFAULT_MAX_TOTAL : s.solutionesMaxTotal)}" placeholder="${SOLUTIONES.DEFAULT_MAX_TOTAL}">
          <p class="field-hint">Выше этой суммы касса ссылку не выдаёт (замер: 19 000 — выдаёт, 19 050 и дороже — «FAILED» без ссылки, при заявленном лимите 160 000). Заказ дороже уходит ${PAYMENTS.fallbackMode(settings) === 'own' ? 'переводом по своим реквизитам' : 'заявкой — менеджер связывается с покупателем сам'}. Починят — впишите новое число; заявленный кассой лимит режет его сверху сам.</p></div>
      </div>
