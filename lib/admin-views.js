...
function layout(settings, opts) {
  ...
  const liveMark = live
    // Подпись «online», а не «живое»: рядом с зелёной пульсирующей точкой это
    // привычная всем отметка «на связи», а «живое» приходилось объяснять
    // всплывающей подсказкой — она осталась, но теперь как уточнение, а не как
    // единственный способ понять, что за плашка.
    ? `<span class="a-live" title="Страница обновляется сама, перезагружать не нужно"><i></i><b>online</b></span>` : '';
