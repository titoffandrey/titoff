...
const CHAT_ICONS = {
  ...
  /* Стрелка «назад» в шапке разговора. На телефоне экран занят разговором
     целиком, шапки панели там нет вовсе — и это единственный путь к списку. */
...
function chatPage(settings, db, chat, flash, orders, sent) {
   ...
   * лента, внизу поле ответа. На телефоне он занимает весь экран (см. `.a-app`
   * в `styles.css`), поэтому стрелка «назад» живёт ЗДЕСЬ, а не в шапке панели —
   * самой шапки панели там нет вовсе.
   *
   ...
   * а переписку читают всё время, и на 390 px эти три блока подряд съедали
   * полэкрана до первой реплики.
   *
    ...
    <div class="chat-head-card">
      ...
      <label class="chat-head-who" for="chat-profile" title="Подробности о собеседнике">
        ...
        <span class="chat-head-who-txt">
          ...
          <span class="chat-head-sub">${chatPresence(chat)}${chat.ip ? `<span class="chat-ip">${esc(chat.ip)}</span>` : ''}</span>
        </span>
        ${CHAT_ICONS.chev}
      </label>
...
module.exports = { loginPage, dashboard, chatList, chatPage, chatNewPage, paymentsPage, productsList, productForm, reviewsList, productReviews, reviewForm, ordersList, shipmentsPage, shipmentPage, analyticsPage, visitorsPage, visitorPage, settingsPage, navCounts, noteOrder, noteReview, noteChat };
