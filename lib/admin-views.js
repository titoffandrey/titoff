    ...
    ? '<p class="form-msg err">Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.</p>' : ''}
      <div class="a-form-grid">
        ...
        <div class="field"><label for="platega-fee">Комиссия Platega, включённая в цены, %</label>
          ...
          <p class="field-hint">Укажите процент из тарифа Platega. Он учитывается внутри стоимости заказа: цены и итог для покупателя не увеличиваются, отдельная комиссия не показывается. Это поле не меняет тариф у сервиса и действует только при оплате через Platega.</p></div>
      </div>
      <p class="field-hint">В личном кабинете задайте Callback URL: полный HTTPS-адрес этого магазина с путём <code>/api/pay/platega/callback</code>. Например, <code>https://&lt;домен магазина&gt;/api/pay/platega/callback</code>. Включите способ «СБП» с подсказкой «Оплата через СБП на защищённой странице» ниже: покупатель сразу перейдёт к оплате через СБП. <a href="https://docs.platega.io/" target="_blank" rel="noopener noreferrer">Документация Platega</a>.</p>`)}
