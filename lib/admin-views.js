...
function payCurrencySection(s, saved, draft, live, checked) {
  ...
  return `<h3 class="a-subhead">Валюта счёта</h3>
      ...
      ${live ? '' : 'Список валют приходит от касс; пока они не ответили, доступен только рубль.'}</p>`;
}
 ...
 * которая их дала. Ставят первой обычно ту, где ниже комиссия или полнее пул.
 *
 * Показывается, ТОЛЬКО когда включены обе: с одной кассой очередь состоит из
 * неё же, и выбор «спрашивать первой» отвечал бы на вопрос, которого нет. То же
 * правило, что у пустых плиток сводки заказов. Поля нет в теле формы — маршрут
 * оставляет сохранённое значение, поэтому выбор не теряется, пока вторая касса
 * выключена.
 */
function payPrimarySection(s, settings) {
  if (PAYMENTS.enabledProviders(settings).length < 2) return '';
