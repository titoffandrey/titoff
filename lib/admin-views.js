...
function chatSeenAgo(ms) {
  ...
  if (sameDay(d, new Date())) return 'в ' + p(d.getHours()) + ':' + p(d.getMinutes());
  const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
  if (sameDay(d, yesterday)) return 'вчера в ' + p(d.getHours()) + ':' + p(d.getMinutes());
...
function chatList(settings, db, flash, page) {
  ...
  const rows = slice.items.map(c => {
    ...
    return `<a class="chat-row" href="/admin/chat/${esc(c.id)}" data-live-key="chat-${esc(c.id)}">
      ${chatAvatar(c)}
...
function chatPage(settings, db, chat, flash) {
  ...
  for (const m of chat.messages) {
    ...
    if (stamp !== day) {
      day = stamp;
      lines.push(`<div class="chat-day"><span>${esc(stamp)}</span></div>`);
    }
     ...
     * это место обратно. */
    lines.push(`<div class="chat-line is-${esc(m.role)}">`
      + (who ? `<span class="chat-line-who">${esc(who)}</span>` : '')
      + `<span class="chat-line-text">${esc(m.text)}</span>`
      + `<span class="chat-line-at">${esc(chatTime(m.at))}</span></div>`);
  }
...
function payLogRow(a, settings) {
  ...
  return `<tr class="pay-log-row${a.rejected && a.rejected.requisite ? ' is-bad' : ''}">
    <td data-l="Когда"><span class="pay-log-at">${esc(payLogWhen(a.at))}</span></td>
    <td data-l="Заказ"><a href="/admin/orders#order-${esc(a.order.id)}">${esc(R.orderNo(a.order.number))}</a></td>
    <td data-l="Касса">${esc(PAYMENTS.nameOf(a.provider))}</td>
    <td data-l="Способ">${esc(PAY.describe(a.method).name)}${a.actualMethod && a.actualMethod !== a.method
      ? `<div class="muted small">касса дала ${esc(PAY.describe(a.actualMethod).name)}</div>` : ''}</td>
    <td data-l="Сумма">${esc(PAY.formatAmount(a.amount, a.currency))}</td>
    <td data-l="Реквизит">${req
      ? `<b class="pay-log-req">${esc(req.requisite)}</b>`
        + (owner || bank ? `<div class="muted small">${esc([owner, bank].filter(Boolean).join(' · '))}</div>` : '')
      : '<span class="muted">—</span>'}</td>
    <td data-l="Итог">${payLogResult(a)}</td>
  </tr>`;
}
...
function paymentsPage(settings, db, opts) {
  ...
  return layout(settings, {
    active: 'orders', title: 'Отчёт по кассам', pendingCount: db.pendingReviewCount(),
...
module.exports = { loginPage, dashboard, chatList, chatPage, paymentsPage, productsList, productForm, reviewsList, productReviews, reviewForm, ordersList, analyticsPage, visitorsPage, visitorPage, settingsPage };
