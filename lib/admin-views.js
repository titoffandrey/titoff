...
/* ---------------------- Касса: валюта счёта и способы ----------------------
 *
 * И то и другое приходит ЖИВЫМ ответом кассы (`payment-method/available`), а не
 * берётся из зашитого списка: у кассы способ могли включить или выключить, а
 * встроенный список правится только выкаткой. Касса не ответила (ключей нет,
 * оплата выключена, сеть) — показываем встроенный и прямо об этом говорим.
 */
...
function payMethodsSection(shown, live) {
  ...
  // Сколько касс ответило — важнее, чем кажется: список объединяет их
  // возможности, и «ответила одна из двух» означает, что часть способов сейчас
  // просто не видна.
  const answered = live && live.byProvider
    ? Object.keys(live.byProvider).filter(pid => live.byProvider[pid]).map(pid => PAYMENTS.nameOf(pid)) : [];
  const note = live
    ? `Список пришёл от касс (${esc(answered.join(', ')) || '—'}): способов — ${rows.filter(r => !r.off).length}, валют — ${live.currencies.length} (${live.currencies.map(esc).join(', ')}). Показывается объединение: способ доступен, если его умеет хотя бы одна касса.`
    : 'Кассы не ответили — показан встроенный список. Включите оплату и задайте ключи, чтобы видеть то, что у касс есть на самом деле.';
  return `<h3 class="a-subhead">Способы оплаты на витрине</h3>
      ...
      <p class="muted small">Покупатель увидит только отмеченные — и только те из них, что включены хотя бы у одной кассы. Названия касс в строках подсказывают, у кого есть подстраховка: способ, который умеет только одна, при её отказе не сработает. Трансграничные способы (для карт иностранных банков) по умолчанию сняты: в длинном списке обычный покупатель теряется, а нужны они редко. Понадобятся — отметьте здесь.</p>
...
function payCurrencySection(s, saved, draft, live, checked) {
  ...
  return `<h3 class="a-subhead">Валюта счёта</h3>
      ...
      <p class="muted small">Цены в каталоге и сумма заказа всегда в рублях — в валюту счёта пересчитывается только сумма перевода, по курсу отсюда. Курс пуст или ноль — валюта на витрине не предлагается вовсе: счёт вышел бы на выдуманную сумму.
      ${live ? '' : 'Список валют приходит от кассы; пока она не ответила, доступен только рубль.'}</p>`;
}
...
function payModeNote(settings) {
  ...
  if (on.length) {
    // Сколько касс работает — не мелочь: с одной включённой подстраховать её
    // некому, и отказ пула реквизитов покупатель увидит своими глазами.
    const crew = on.length > 1
      ? `Работают обе кассы (${esc(on.map(p => p.name).join(' и '))}): не выдаст реквизиты первая — вторую мы спросим сами, и покупатель этого не заметит.`
      : `Работает одна касса — ${esc(on[0].name)}. Вторая выключена: если у этой не окажется свободных реквизитов, подстраховать её будет некому.`;
    return `<p class="pay-mode is-on"><b>Сейчас: оплата на витрине.</b> После оформления покупатель попадает на страницу счёта с реквизитами, а состояние платежа видно в списке заказов.
      ${crew}
      Один заказ — от ${PAYMENTS.MIN_TOTAL.toLocaleString('ru-RU')} до ${money(PAYMENTS.MAX_TOTAL)}: это пределы касс, и товар дороже потолка на витрине не продаётся.</p>`;
  }
  return `<p class="pay-mode is-off"><b>Сейчас: заявки без оплаты.</b> Покупатель оформляет заказ, платить на сайте его не просят: он видит номер заявки и то, что менеджер свяжется с ним и подтвердит наличие.
    Заявки приходят в «Заказы» и в Telegram, как обычно, а знаки платёжных систем в подвале витрины не показываются.
    Пределов суммы в этом режиме нет: ${PAYMENTS.MIN_TOTAL.toLocaleString('ru-RU')} – ${money(PAYMENTS.MAX_TOTAL)} — ограничение касс, а не магазина, поэтому дорогие товары продаются наравне с остальными.</p>`;
}

/* Одна касса в настройках: свёртка с галочкой, ключами и справкой.
 *
 * Свёртка у каждой своя — иначе страница настроек превращается в стену полей, а
 * заглядывают в них раз в жизни. Свёрнутое содержимое браузер отправляет так же,
 * как открытое, поэтому галочки внутри не теряются (та же грабля, что со строкой
 * «нет у кассы» в списке способов).
 *
 * Подпись у свёртки говорит состояние, а не повторяет галочку: «включена, но не
 * настроена» — самый частый способ остаться без оплаты, и увидеть это надо не
 * открывая секцию.
 */
function payProviderFold(name, on, ready, body) {
  const state = on && ready ? 'работает' : on ? 'включена, но ключи не заданы' : 'выключена';
  return `<details class="pay-fold"${on ? ' open' : ''}>
 ...
 *
 * Порядок виден только нам: покупатель в любом случае получает реквизиты от
 * той, которая их дала. Ставят первой обычно ту, где ниже комиссия или полнее
 * пул реквизитов.
 */
function payPrimarySection(s) {
  ...
  return `<div class="a-form-grid">
    ...
    `<option value="${esc(p.id)}"${p.id === picked ? ' selected' : ''}>${esc(p.name)}</option>`).join('')}</select></div>
      ...
      <p class="muted small">Вторая касса подхватывает молча: покупатель нажимает «Получить реквизиты» один раз и не узнаёт, что первая отказала. Кто именно выдал реквизиты по каждому заказу — видно в «Заказах», там же и статистика отказов по кассам.</p>`;
}
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const body = `<form class="a-form" method="post" enctype="multipart/form-data" action="/admin/settings">
    ...
    <div class="a-panel"><h2>Оплата на витрине</h2>
      ...
      <div class="field field-check"><label><input type="checkbox" name="crocopayEnabled"${checked('crocopayEnabled', !!settings.crocopayEnabled) ? ' checked' : ''}> Принимать оплату через CrocoPAY</label></div>
      ...
      <p class="muted small">Ключи берутся в кабинете <a class="link" href="https://crocopay.tech/merchants" target="_blank" rel="noopener noreferrer">CrocoPAY</a>: «Кассы» → иконка шестерни у нужной кассы.</p>`)}
      ${payProviderFold('MeridianPay', !!settings.meridianpayEnabled, MERIDIAN.configured(settings), `
      <div class="field field-check"><label><input type="checkbox" name="meridianpayEnabled"${checked('meridianpayEnabled', !!settings.meridianpayEnabled) ? ' checked' : ''}> Принимать оплату через MeridianPay</label></div>
      ...
      <p class="muted small">Ключ и Merchant ID берутся в кабинете MeridianPay: страница «Интеграция» и раздел мерчантов. Оба обязательны — без Merchant ID касса не примет ни одной сделки.</p>
      <p class="muted small">Secret Key сейчас ни на что не влияет: алгоритм подписи уведомлений MeridianPay нигде не описан, поэтому оплату подтверждает не уведомление, а наш собственный запрос статуса по API-ключу. Поле оставлено, чтобы включить проверку сразу, как только алгоритм станет известен.</p>`)}
      <p class="muted small">Схема у обеих касс одна — H2H: счёт выставляем мы, покупатель платит переводом по реквизитам, не уходя с витрины. Номер его карты мы не спрашиваем и не получаем. Ключи хранятся только на сервере и на витрину не попадают.</p>
      <p class="muted small">Цены и сумма заказа всегда в <b>рублях</b>; в другую валюту пересчитывается только сумма счёта по заданному ниже курсу.</p>
      <p class="muted small">Состояние платежа видно в списке заказов: кассы отдают настоящий статус счёта, поэтому «счёт истёк» и «оплата отменена» там тоже появляются, а не только «оплачено».
      Снятая галочка настройки не стирает — они лежат здесь и ждут; уже оформленные заказы и их счета выключение тоже не затрагивает.</p>
  ...
  <div class="a-panel a-exit">
    <div>
      <h2>Выход</h2>
      <p class="muted small">Панель закроется, и войти в неё снова можно будет по логину и паролю. На витрину это никак не влияет.</p>
    </div>
