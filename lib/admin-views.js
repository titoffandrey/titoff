...
function ordersList(settings, db, flash, page, view, edit, filters) {
  /* Вкладки «Текущие / Удалённые» сняты вместе с самим архивом: удаление
   * заказа теперь окончательное. Прежде «удалить» означало убрать из рабочего
   * списка, а запись оставалась ради поздней оплаты по уже выданному счёту —
   * но кассы выключены, платят по своим реквизитам, и ждать денег по мёртвому
   * счёту больше не от кого. Заявки, заархивированные раньше, так и лежат в
   * файле невидимыми: стирать их молча мы не вправе. */
  const archiveView = false;
  ...
  const rows = slice.items.map(o => {
    ...
    const manual = !archiveView && !settled && (o.payMode === 'own' || !o.payment)
    ...
    /* Отмена оплаты — пара к отметке выше: покупатель нажал «Оплатить» и
     * передумал, ошибся суммой или оформил заказ дважды. Заказ перестаёт
     * числиться ждущим денег, а покупателю страница оплаты говорит, что он
     * закрыт. Счёт кассы это не трогает — там деньги бывают в пути. */
    const voidBtn = !archiveView && !settled && !o.manualPaid
      ? `<form class="o-reconcile" method="post" action="/admin/orders/${esc(o.id)}/void">${back}`
        + `<input type="hidden" name="voided" value="${o.manualVoid ? '0' : '1'}">`
        + `<button class="btn btn-sm${o.manualVoid ? '' : ' o-void'}" type="submit"`
        + (o.manualVoid ? '' : ` data-confirm="Отменить оплату этого заказа? Покупатель увидит, что заказ закрыт."`)
        + `>${o.manualVoid ? 'Вернуть в ожидание' : 'Отменить оплату'}</button></form>` : '';
    return `<tr id="order-${esc(o.id)}" class="${R.orderRowClass(o)}">
      ...
      <td class="o-num"><b>${esc(R.orderNo(o.number))}</b><span class="muted small">${R.formatDate(o.createdAt)}</span>${archivedAt}${shipMark(o)}</td>
      ...
      <td class="o-payment"><div class="o-payment-main"><div class="o-payment-state">${R.orderStatus(o)}</div>${method}</div>${reconcile}${manual}${voidBtn}</td>
      <td class="o-sum"><b>${R.money(o.total, settings)}</b></td>
      <td class="o-act"><div class="o-acts">${remove}</div></td></tr>`;
  }).join('') || `<tr><td colspan="6" class="muted o-empty">${q || payFilter ? 'По вашему запросу заказов нет.' : (archiveView ? 'Удалённых заказов нет.' : 'Заказов пока нет.')}</td></tr>`;
  const switcher = '';
  const payOptions = [
  ...
  const filterBar = `<form class="o-filters" method="get" action="/admin/orders">
    ...
    ${q || payFilter ? `<a class="link small" href="${archiveView ? '/admin/orders?view=archive' : '/admin/orders'}">Сбросить</a>` : ''}
  </form>`;
  ...
  const stats = R.orderStats(viewOrders, undefined,
    typeof db.getOrders === 'function' ? db.getOrders() : activeOrders.concat(archived));
...
function shipMark(order) {
  ...
  if (!ship || !Array.isArray(ship.steps) || !ship.steps.length) {
    return `<a class="o-ship is-none" href="${shipHref(order)}">Создать отправление</a>`;
  }
...
function shipStepRow(step, i, held) {
  const s = step || {};
  return `<tr>
      <td data-l="Когда"><input type="datetime-local" name="stepAt" value="${esc(s.at ? dtLocal(s.at) : '')}"></td>
      <td data-l="Событие"><input name="stepTitle" value="${esc(s.title || '')}" maxlength="120" placeholder="например, Принят на склад"></td>
      <td data-l="Город"><input name="stepPlace" value="${esc(s.place || '')}" maxlength="60"></td>
      <td data-l="Приписка"><input name="stepNote" value="${esc(s.note || '')}" maxlength="200" placeholder="видна покупателю"></td>
...
function shipmentPage(settings, db, order, opts) {
  ...
  const holdHours = pick('holdHours', ship && ship.holdHours, TRACK.holdHoursValue(settings.shipHoldHours));
  ...
  const params = `<div class="a-panel">
      ...
      <div class="a-form-grid">
        ...
        ${/* Сколько часов посылка должна простоять, прежде чем покупатель увидит
             просьбу подождать. Ноль — сказать сразу; пусто — сутки. */''}
        <div class="field"><label>Сообщать о задержке через, часов</label>
          <input name="holdHours" inputmode="numeric" value="${esc(holdHours)}" placeholder="${TRACK.holdHoursValue(settings.shipHoldHours)}"></div>
      </div>
    </div>`;
  ...
  const preview = ship && steps.length ? `<div class="a-panel ship-preview">
      <div class="a-panel-head"><h2>Что видит покупатель</h2></div>
  ...
  const body = `<form method="post" action="${shipHref(order)}" class="ship-form">
      ${params}
      ${table}
      ${actions}
    </form>
  ...
  return layout(settings, {
    ...
    actions: `<a class="btn btn-sm" href="/admin/orders#order-${esc(order.id)}">← К заказам</a>`
      + (ship && number ? `<a class="btn btn-sm" href="/track/${encodeURIComponent(number)}" target="_blank" rel="noopener">Страница покупателя ↗</a>` : ''),
...
function chatTitle(chat, named) {
  return String((chat && chat.name) || String(named || '').trim()
    || (chat && chat.client && chat.client.city) || (chat && chat.city) || 'Посетитель').trim() || 'Посетитель';
}
...
/* Что можно сделать с репликой — меню по нажатию на неё саму.
 *
 ...
 * Теперь нажимается сам пузырь — ровно так, как это делают в мессенджерах, — и
 * под ним раскрывается ряд из трёх действий.
 *
 * Всё на `<details>`: ни строчки скрипта, работает и там, где скрипты панели не
 * загрузились. Скрипту достаётся только копирование — буфер обмена из разметки
 * не открыть. «Изменить» — вложенный `<details>`: форма правки нужна редко, и
 * держать её открытой ради трёх кнопок незачем.
 */
function chatTools(chat, m) {
...
function chatPage(settings, db, chat, flash, orders, sent) {
  ...
  for (const m of chat.messages) {
    ...
    const photos = shots.length
      ? `<span class="chat-line-shots is-${shots.length}" data-media>`
        ...
        + `</span>`
      : '';
    const bubble = `<summary class="chat-line is-${esc(m.role)}" title="Что сделать с репликой">`
    ...
    lines.push(`<div class="chat-item is-${esc(m.role)}" data-live-key="msg-${m.at}">
      <details class="chat-bubble">${bubble}${chatTools(chat, m)}</details>
    </div>`);
  }
  ...
  // Оформил заказ — значит имя назвал: город в шапке был бы шагом назад.
  ...
  const body = `${R.CHAT_TICK_SPRITE}<div class="chat-view"${sent ? ' data-chat-sent' : ''}>
    <div class="chat-head-card">
      ${chatAvatar(chat, { big: true, named: buyerName })}
      <div class="chat-head-who">
        <div class="chat-head-name">${esc(chatTitle(chat, buyerName))}${chatModeBadge(chat.mode)}</div>
    ...
    <form class="chat-answer chat-answer-dock" method="post" action="/admin/chat/${esc(chat.id)}/reply">
      <div class="chat-answer-row">
        <textarea name="text" rows="1" required placeholder="Написать покупателю…" maxlength="${CHAT.MAX_TEXT}"></textarea>
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  /* Контакт витрины в свёрнутой строке — тот же, что увидит покупатель в
   * подвале. Ни одного — это не мелочь: связаться с магазином будет негде. */
  const shopContact = String(s.contactTelegram || '').trim()
    || (String(s.contactPhone || '').trim() ? R.phoneText(s.contactPhone) : '');
  ...
  const sections = [
    ['Витрина', [
      {
        ...
        tone: shopContact ? '' : 'warn',
        note: [String(s.storeName || '').trim() || 'без названия', String(s.currency || '₽').trim(),
          shopContact || 'контактов на витрине нет'].filter(Boolean).join(' · '),
      ...
      },
      {
        ...
        note: `шрифт «${R.logoFontName(s.logoFont)}» · ${s.logoImage ? 'логотип-картинка'
          : (String(s.logoText || '').trim() ? 'текст «' + String(s.logoText).trim() + '»' : 'название магазина текстом')}`,
        /* Кружки выбранных цветов: сказать «синий и голубой» словами нельзя —
         * цвет произвольный, — а показать его самим цветом можно. Значение идёт
         * через `cssColor()`, поэтому чужая строка в атрибут не уедет. */
        mark: `<span class="set-dots" aria-hidden="true"><i style="background:${R.cssColor(s.accentColor, '#0071e3')}"></i><i style="background:${R.cssColor(s.secondaryColor || s.accentColor, '#0071e3')}"></i></span>`,
      ...
      },
      {
        ...
        note: `отправляем из г. ${String(s.shipFromCity || '').trim() || TRACK.DEFAULT_FROM}`
          + ` · о задержке через ${TRACK.holdHoursValue(s.shipHoldHours)} ч`,
        body: `<div class="a-form-grid">
        <div class="field"><label>Город отправки по умолчанию</label>
          <input name="shipFromCity" value="${esc(s.shipFromCity || '')}" placeholder="${esc(TRACK.DEFAULT_FROM)}" maxlength="60"></div>
        <div class="field"><label>Сообщать о задержке через, часов</label>
          <input name="shipHoldHours" inputmode="numeric" value="${esc(s.shipHoldHours === 0 || s.shipHoldHours ? String(s.shipHoldHours) : '')}"
            placeholder="${TRACK.HOLD_NOTICE_HOURS}" autocomplete="off"></div>
      </div>`
      }
    ]],
    ['Деньги', [
    ...
    ['Общение с покупателем', [
      ...
      },
      {
        ...
        note: tgReady
          ? 'заявки уходят в Telegram' + (checked('notifyReviews', settings.notifyReviews !== false) ? ' · и новые отзывы тоже' : '')
...
module.exports = { loginPage, dashboard, chatList, chatPage, chatNewPage, paymentsPage, productsList, productForm, reviewsList, productReviews, reviewForm, ordersList, shipmentPage, analyticsPage, visitorsPage, visitorPage, settingsPage, navCounts, noteOrder, noteReview, noteChat };
