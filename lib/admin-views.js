...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const sections = [
    ...
    ? '<p class="form-msg err">Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.</p>' : ''}
      ...
      <p class="field-hint">В личном кабинете задайте Callback URL: полный HTTPS-адрес этого магазина с путём <code>/api/pay/platega/callback</code>. Например, <code>https://&lt;домен магазина&gt;/api/pay/platega/callback</code>. Включите способ «Онлайн-оплата» ниже: покупатель выберет способ на защищённой странице оплаты. <a href="https://docs.platega.io/" target="_blank" rel="noopener noreferrer">Документация Platega</a>.</p>`)}
