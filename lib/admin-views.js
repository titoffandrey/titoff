...
function shipmentPage(settings, db, order, opts) {
   ...
   */
  const trackUrl = number ? String(opts.origin || '').replace(/\/+$/, '') + '/track/' + number : '';
  const share = ship && number ? `<div class="a-panel ship-share${visible ? '' : ' is-hidden'}">
      <div class="ship-share-head">
        ...
        <div class="ship-share-cap"><b>Ссылка для покупателя</b>
          <span>${visible ? 'Отправьте её в чат или Telegram — страница откроется без входа.'
  ...
  const sections = [
    ['Витрина', [
      ...
      },
      {
        ...
        <div class="field"><label>Сообщать о задержке через, дней</label>
          <input name="shipHoldDays" inputmode="numeric" value="${esc(s.shipHoldDays === 0 || s.shipHoldDays ? String(s.shipHoldDays) : '')}"
