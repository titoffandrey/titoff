...
function newRulesBlock(settings) {
  ...
  return `<div class="a-newrules">
        <b>В проекте появились правила, которых нет в вашей инструкции — ${missing.length}</b>
        <ul>${missing.map(r => `<li>${esc(r)}</li>`).join('')}</ul>
        <button type="submit" name="chatRulesAppend" value="1" class="btn btn-sm">Дописать их в инструкцию</button>
        <span class="muted small">Не нужны — просто не нажимайте: поле останется как есть.</span>
      </div>`;
}
