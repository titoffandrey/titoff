...
function layout(settings, opts) {
  ...
  const liveMark = live
    /* Плашка состояния канала. Подпись «online», а не «живое»: рядом с зелёной
     ...
     * разъезжаются, а увидеть это можно только глазами.
     *
     * Сервер всегда рисует её подключённой — про связь конкретного браузера он
     * ничего не знает и знать не может. Отсюда же и правило в admin-live.js:
     * подмена блоков эту плашку не трогает вовсе. */
    ? `<span class="a-live" title="Страница обновляется сама, перезагружать не нужно"`
      + ` data-title-off="Связь с сервером потеряна — цифры на экране могли устареть">`
      + `<i></i><b class="a-live-on">online</b><b class="a-live-off">нет связи</b></span>` : '';
...
<body class="admin"${live ? ` data-live="${esc(live)}"` : ''}>
<div class="a-shell">
  <main class="a-main">
    <div class="a-topbar" data-live-part="topbar">${menu}<h1>${esc(opts.title || '')}</h1>${liveMark}${
...
function dashboard(settings, db, pulse) {
  ...
  const body = `
    ...
    <div class="a-cards">
      <a class="a-stat a-stat-live${Number(live.online) ? '' : ' is-idle'}" href="/admin/analytics"><div class="a-stat-num"><i></i>${Number(live.online) || 0}</div><div class="a-stat-label">Сейчас на сайте</div><div class="a-stat-sub">${live.pageViews ? 'сегодня ' + Number(live.visitors || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.visitors, 'посетитель', 'посетителя', 'посетителей') + ' · ' + Number(live.pageViews || 0).toLocaleString('ru-RU') + ' ' + R.pluralRu(live.pageViews, 'просмотр', 'просмотра', 'просмотров') : 'сегодня заходов ещё не было'}</div></a>
  ...
  return layout(settings, { active: 'dash', title: 'Обзор', pendingCount: pending, live: 'orders reviews products analytics', body });
}
...
function reviewsList(settings, db, flash, page) {
  ...
  const queue = slice.total
    ...
    : '';
  return layout(settings, { active: 'reviews', title: 'Отзывы', pendingCount: db.pendingReviewCount(), live: 'reviews',
...
function productReviews(settings, db, product, statusFilter, flash, page, sort, media) {
  ...
  return layout(settings, { active: 'reviews', title: 'Отзывы товара', pendingCount: db.pendingReviewCount(), live: 'reviews',
...
// единственного потока, то есть замершая витрина для всех посетителей.
function ordersList(settings, db, flash, page, view, edit) {
  ...
  const pager = R.adminPager(slice, n => listHref(archiveView ? 'archive' : 'active', n));
  const rows = slice.items.map(o => {
    ...
    /* В «Удалённых» кнопки две: вернуть в работу и стереть навсегда. Второе —
     * единственное место, где заказ пропадает из orders.json физически, поэтому
     * предупреждение называет то, что теряется, а не спрашивает «вы уверены?».
     * Про уже выданный счёт говорим только когда он был: у брошенного черновика
     * (а именно их и чистят) терять нечего, и лишняя строка в подтверждении
     * приучила бы жать «ОК», не читая. */
    const hadInvoice = ((o.payment && o.payment.attempts) || []).some(a => a && a.invoiceId)
      || !!(o.payment && o.payment.invoiceId);
    const purgeWarn = 'Удалить заказ ' + R.orderNo(o.number) + ' навсегда? Он исчезнет из хранилища, и вернуть его будет неоткуда.'
      + (hadInvoice ? ' По этому заказу уже выставлялся счёт: если деньги придут позже, привязать их будет не к чему.' : '');
    const remove = archiveView
      ? `<form method="post" action="/admin/orders/${esc(o.id)}/restore">${back}<button class="btn btn-sm" aria-label="Восстановить заказ" title="Восстановить заказ">Вернуть</button></form>`
        + `<form method="post" action="/admin/orders/${esc(o.id)}/purge" data-confirm="${esc(purgeWarn)}">${back}<button class="btn btn-sm btn-danger" aria-label="Удалить навсегда" title="Удалить навсегда">Удалить навсегда</button></form>`
  ...
  const switcher = `<nav class="o-list-switch" aria-label="Список заказов">
    <a href="${listHref('active', 1)}"${archiveView ? '' : ' aria-current="page"'}>Текущие <b>${activeOrders.length}</b></a>
    <a href="${listHref('archive', 1)}"${archiveView ? ' aria-current="page"' : ''}>Удалённые <b>${archived.length}</b></a>
  </nav>`;
  /* «Очистить» — стереть весь архив разом.
   *
   * Архив копится сам: брошенные черновики и тестовые заявки удаляют из
   * рабочего списка, и дальше они лежат там навсегда. По одной их чистить —
   * это подтверждение на каждую, и на полусотне записей за этим перестают
   * следить вовсе.
   *
   * Кнопка живёт В ШАПКЕ РЕЖИМА ПРАВКИ, рядом с «Изменить»: она показывается
   * тем же правилом `:checked ~ …`, что и крестики у строк, — стереть архив
   * одним нажатием из списка, открытого почитать, нельзя. И только на вкладке
   * «Удалённые»: в рабочем списке ей нечего делать.
   *
   * Подтверждение называет число заявок и отдельно — сколько из них со
   * счётом: деньги по такому счёту ещё могут прийти, и привязать их будет не к
   * чему. У брошенного черновика (а чистят именно их) терять нечего, поэтому
   * лишней строки там нет — иначе к «ОК» привыкают, не читая.
   */
  const withInvoice = archived.filter(o => ((o.payment && o.payment.attempts) || []).some(a => a && a.invoiceId)
    || !!(o.payment && o.payment.invoiceId)).length;
  const purgeAllWarn = `Стереть все удалённые заказы (${archived.length})? Они исчезнут из хранилища, и вернуть их будет неоткуда.`
    + (withInvoice ? ` По ${withInvoice} из них уже выставлялся счёт: если деньги придут позже, привязать их будет не к чему.` : '');
  const purgeAll = archiveView && archived.length
    ? `<form class="o-purge-all" method="post" action="/admin/orders/purge-all" data-confirm="${esc(purgeAllWarn)}">
        <input type="hidden" name="edit" value="1">
        <button class="btn btn-sm btn-danger" type="submit">Очистить корзину</button></form>`
    : '';
  return layout(settings, { active: 'orders', title: 'Заказы', pendingCount: db.pendingReviewCount(), flash, live: 'orders',
    ...
    body: switcher + R.orderStatsBar(R.orderStats(orders), settings)
      + `<div class="a-panel">${R.editSwitch('orders-edit', { on: editing, extra: purgeAll })}<table class="a-table a-orders"><thead><tr><th>Заказ</th><th>Клиент</th><th>Состав</th><th>Статус</th><th>Оплата</th><th class="o-sum">Сумма</th><th class="o-act"><span class="sr-only">Удалить</span></th></tr></thead><tbody>${rows}</tbody></table>${pager}</div>`
 ...
 * спрашивают, доходит ли чат до людей, что бот наговорил вчера ночью — это
 * читают списком, а не листая полсотни тем. И отсюда же можно ответить, не
 * открывая мессенджер, — режим у диалога общий, а не «панельный».
 */

// Когда это было. В списке важнее «как давно», чем точное время: диалог
// пятиминутной давности ещё живой, вчерашний — уже история.
function chatWhen(ms) {
  ...
  return R.formatDate(at);
}

/* Время реплики. В переписке важен ЧАС, а не день: разговор идёт минутами, и
 * «25.08.2026» у каждой строки подряд не отвечает ни на один вопрос. Дату
 * добавляем, только когда реплика не сегодняшняя. */
...
function chatList(settings, db, flash, page) {
  ...
  const rows = slice.items.map(c => {
    ...
    // нужно ли вообще его открывать.
    const last = c.messages.length ? c.messages[c.messages.length - 1] : null;
    // Имена собеседников берём из того же списка, что и витрина (`SPEAKERS` в
    // lib/chat.js): в панели разбирают ту же переписку, которую видит
    // покупатель, и разные подписи под одной репликой читались бы как разные
    // сообщения.
    const who = last ? (last.role === 'user' ? 'Покупатель' : CHAT.speakerOf(last.role, last.by)) : '';
    return `<a class="chat-row${c.mode === 'operator' ? ' is-live' : ''}" href="/admin/chat/${esc(c.id)}" data-live-key="chat-${esc(c.id)}">
      <span class="chat-row-head">
        <b>${esc(c.name || c.city || 'Посетитель')}</b>
        ${chatModeBadge(c.mode)}
        <span class="chat-row-when">${esc(chatWhen(c.lastAt))}</span>
      </span>
      <span class="chat-row-last">${who ? `<i>${esc(who)}:</i> ` : ''}${esc(last ? last.text : 'Пустой диалог')}</span>
      <span class="chat-row-meta">${esc([c.device, c.page].filter(Boolean).join(' · '))}</span>
    </a>`;
  }).join('');

  const body = all.length
    ? `<div class="a-panel a-panel-list"><div class="chat-rows">${rows}</div>${pager}</div>`
    // Пустой список — не повод рисовать панель ради слов «ничего нет»: то же
    // правило, что у разобранной очереди модерации.
    : `<div class="a-panel"><p class="muted">Диалогов пока нет. Кнопка чата ${CHAT.visible(settings) ? 'на витрине стоит' : '<b>на витрине не показывается</b> — включите чат в настройках'}.</p></div>`;

  return layout(settings, {
    active: 'chat', title: 'Чат', pendingCount: db.pendingReviewCount(), flash,
    live: 'chat', body
  });
}
 ...
 */
function chatPage(settings, db, chat, flash) {
  const rows = chat.messages.map(m => {
    const who = m.role === 'user' ? (chat.name || 'Покупатель') : CHAT.speakerOf(m.role, m.by);
    return `<div class="chat-line is-${esc(m.role)}">
      ${who ? `<span class="chat-line-who">${esc(who)}</span>` : ''}
      <span class="chat-line-text">${esc(m.text)}</span>
      <span class="chat-line-at">${esc(chatTime(m.at))}</span>
    </div>`;
  }).join('');

  // Карточка посетителя открывается тем же адресом, что из строки заказа: у
  // диалога и визита один и тот же человек, и связывать их руками незачем.
  ...
  const facts = [
    ...
    chat.ip && ['IP', chat.ip],
    ['Начат', R.formatDate(chat.at)],
    ['Реплик', String(chat.messages.length)]
  ].filter(Boolean).map(([k, v]) => `<div class="chat-fact"><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('');

  const body = `<div class="a-panel chat-view">
    <div class="chat-view-head">
      ${chatModeBadge(chat.mode)}
      <span class="muted small">${esc(chatWhen(chat.lastAt))}</span>
      ${visitor ? `<a class="link small" href="${esc(VISITOR_BASE + encodeURIComponent(visitor))}">Карточка посетителя →</a>` : ''}
    </div>
    <div class="chat-facts">${facts}</div>
    <div class="chat-thread" data-live-part="thread">${rows || '<p class="muted">Пока ни одной реплики.</p>'}</div>
    ${chat.mode === 'closed'
      ? '<p class="muted small">Диалог завершён. Покупатель может написать снова — тогда он продолжится.</p>'
      : `<form class="chat-answer" method="post" action="/admin/chat/${esc(chat.id)}/reply">
      <textarea name="text" rows="3" required placeholder="Ответить покупателю — он увидит сообщение в окне чата, и ИИ замолчит" maxlength="${CHAT.MAX_TEXT}"></textarea>
      <div class="chat-answer-acts">
        <button class="btn btn-primary" type="submit">Ответить</button>
        <button class="btn" type="submit" name="mode" value="ai" formnovalidate>Вернуть ИИ</button>
        <button class="btn" type="submit" name="mode" value="closed" formnovalidate>Завершить</button>
      </div>
    </form>`}
  </div>`;

  return layout(settings, {
    active: 'chat', title: 'Диалог', pendingCount: db.pendingReviewCount(), flash,
    live: 'chat',
...
function analyticsPage(settings, db, snapshot) {
  ...
  return layout(settings, { active: 'analytics', title: 'Метрика', pendingCount: db.pendingReviewCount(), wide: true, live: 'analytics orders', body });
}
...
function visitorsPage(settings, db, result, opts) {
  ...
  return layout(settings, { active: 'analytics', title: 'Кто заходил', pendingCount: db.pendingReviewCount(), wide: true, live: 'analytics orders', body });
}
...
function visitorPage(settings, db, visitor, opts) {
  ...
  return layout(settings, { active: 'analytics', title: 'Посетитель', pendingCount: db.pendingReviewCount(), wide: true, live: 'analytics orders', body });
}
...
function chatStateNote(settings) {
  ...
  const ai = AI.configured(settings);
  const tg = TGCHAT.configured(settings);
  const state = TGCHAT.state;
  const rows = [];

  if (!settings.chatEnabled) rows.push(['off', 'Чат выключен — кнопки на витрине нет.']);
  else if (!on) rows.push(['err', 'Чат включён, но отвечать некому: нет ни ключа OpenAI, ни Telegram-бота с группой.']);
  ...
  else if (ai && tg) rows.push(['ok', 'Отвечает ИИ, диалоги идут в Telegram — оператор может подключиться.']);
  else if (ai) rows.push(['warn', 'Отвечает ИИ, но Telegram не настроен: подключиться к разговору и ответить самому будет негде.']);
  else rows.push(['warn', 'ИИ не настроен — на вопросы отвечает только менеджер в Telegram, покупатель ждёт живого ответа.']);

  if (settings.chatEnabled && tg) {
    if (state.error) rows.push(['err', 'Telegram: ' + state.error]);
    else if (state.topics === false) rows.push(['err', 'Темы в группе не создаются — диалоги идут одной лентой. Проверьте, что это супергруппа с включёнными темами и бот в ней администратор.']);
    else if (state.running) {
      rows.push(['ok', 'Бот на связи' + (state.bot ? ' (' + esc(state.bot) + ')' : '')
        + (state.topics ? ', темы создаются' : '') + '.']);
    } else rows.push(['warn', 'Приём сообщений из Telegram ещё не запустился.']);
  }
  return `<div class="chat-state">${rows.map(([kind, text]) =>
    `<p class="chat-state-row is-${kind}">${esc(text)}</p>`).join('')}</div>`;
}

function chatSection(s, settings, checked) {
  return `<div class="a-panel"><h2>Онлайн-чат на витрине</h2>
      ${chatStateNote(settings)}
      ...
      <div class="field"><label>Инструкция для ИИ</label>
        <textarea name="chatPrompt" rows="10" maxlength="${PROMPT.MAX_INSTRUCTION}" spellcheck="false"
          placeholder="Как здороваться, что предлагать, чего не обещать, когда звать менеджера. Товары, цены и наличие подставляются сами.">${esc(s.chatPrompt || '')}</textarea></div>
    </div>`;
}
...
function settingsPage(settings, db, flash, flashType, opts) {
  ...
  const body = `<form class="a-form" method="post" enctype="multipart/form-data" action="/admin/settings">
    ...
    ${chatSection(s, settings, checked)}
...
module.exports = { loginPage, dashboard, chatList, chatPage, productsList, productForm, reviewsList, productReviews, reviewForm, ordersList, analyticsPage, visitorsPage, visitorPage, settingsPage };
