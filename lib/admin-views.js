...
function reviewsList(settings, db, statusFilter, flash, page, sort) {
  ...
  const slice = R.adminSlice(R.sortReviews(filterByStatus(db.getReviews(), status), mode), page);
  const back = { status, page: slice.page, sort: mode };
  // Смена вкладки или сортировки возвращает на первую страницу: на 12-й
  // странице прежний номер после пересортировки не значит ничего.
  const base = (k, m) => `/admin/reviews?status=${encodeURIComponent(k)}&amp;sort=${encodeURIComponent(m)}`;
