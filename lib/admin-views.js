...
function payModeNote(settings, health) {
  ...
  if (unverified.length) {
    ...
    return `<p class="pay-mode is-on"><b>Сейчас: оплата на витрине.</b> ${confirmed}${names(unverified)}: ключи заданы, связь ещё не проверена.</p>`;
  }
    ...
    ? '<p class="form-msg err">Касса включена, но Merchant ID или Secret не заданы либо неверны — ссылка на оплату недоступна.</p>' : ''}
      <div class="a-form-grid">
        ...
        <div class="field"><label for="platega-fee">Комиссия Platega, включённая в цены, %</label>
          ...
          <p class="field-hint">Укажите процент из тарифа Platega. Магазин учитывает его внутри стоимости заказа, отдельная комиссия на сайте не показывается. В общей платёжной форме итог зависит от выбранного способа: при другом тарифе сумма может отличаться. Это поле не меняет тариф у сервиса и действует только при оплате через Platega.</p></div>
      </div>
      <p class="field-hint">В личном кабинете задайте Callback URL: полный HTTPS-адрес этого магазина с путём <code>/api/pay/platega/callback</code>. Например, <code>https://&lt;домен магазина&gt;/api/pay/platega/callback</code>. Включите способ «Онлайн-оплата» ниже: покупатель выберет доступный способ на защищённой странице оплаты. <a href="https://docs.platega.io/" target="_blank" rel="noopener noreferrer">Документация Platega</a>.</p>`)}
