...
function dashboard(settings, db) {
  ...
  const body = `
    <div class="a-cards">
      ...
      <a class="a-stat" href="/admin/reviews"><div class="a-stat-num">${pending}</div><div>Отзывов на модерации</div></a>
...
function productForm(settings, db, product, opts) {
  ...
  const p = product || { name: '', category: '', price: '', oldPrice: '', inStock: true, visible: true, stockLevel: 'in', shortDesc: '', description: '', specs: '', images: [], hotDeal: false, hotDealPrice: '', hotDealUntil: null, colors: [], storages: [], bands: [], options: [] };
...
// Панель списка: вкладки слева, сортировка справа. Двумя рядами они занимали бы
// две строки на ровном месте, а на телефоне всё равно встают друг под другом.
function reviewToolbar(tabs, sorts) {
  return `<div class="a-toolbar">${tabs}${sorts}</div>`;
}
...
function productReviews(settings, db, product, statusFilter, flash, page, sort) {
  ...
  const slice = R.adminSlice(R.sortReviews(filterByStatus(all, status), mode), page);
  const back = { status, page: slice.page, product: product.id, sort: mode };
  const root = `/admin/reviews/product/${encodeURIComponent(product.id)}`;
  const base = (k, m) => `${root}?status=${encodeURIComponent(k)}&amp;sort=${encodeURIComponent(m)}`;
  const pager = R.adminPager(slice, n => `${base(status, mode)}&amp;page=${n}`);
  ...
  const body = `<div class="a-panel rv-prod-head">
    ...
    ${reviewToolbar(reviewTabs(status, k => base(k, mode)), reviewSorts(mode, m => base(status, m)))}
