'use strict';
/*
 * Platega: платёжная страница сервиса с выбором способа оплаты.
 * Документация: https://docs.platega.io (сверено 15 сентября 2026).
 *
 * POST /v2/transaction/process без paymentMethod возвращает transactionId и
 * url. Ранее созданные счета с фиксированным СБП продолжают сверяться GET.
...
const METHOD = 'ONLINE_PAYMENT';
const SBP_METHOD = 'SBP_ONLINE';
const CURRENCY = 'RUB';
const MINOR = 100;
...
/* Потолок ОДНОГО платежа, ₽ вместе с комиссией. Число Platega, а не наше:
 * на прямой запрос СБП свыше него касса отвечает HTTP 400
 * «SBPQR limit is 20000.00 RUB (including commission)», а на общей форме
 * покупатель упирается в ту же стену уже выбрав СБП (боевая проверка
 * 15 сентября 2026: все прямые счета на 27 700 – 35 500 ₽ упали, общие
 * ссылки создались, но заплатить по ним СБП нельзя). Публично лимит нигде
 * не описан, значит это свойство мерчанта, и завтра его могут поднять —
 * поэтому число живёт в настройках (`plategaMaxTotal`), а здесь только
 * значение по умолчанию. Пустое поле означает «как известно», а не «предела
 * нет»: снятый молча потолок вернул бы покупателя к стене на странице кассы. */
const DEFAULT_MAX_TOTAL = 20000;
...
function storedMethod(method) { return [METHOD, SBP_METHOD].includes(trimmed(method)); }
...
function maxTotal(settings) {
  const n = Number(settings && settings.plategaMaxTotal);
  return Number.isFinite(n) && n > 0 ? Math.round(n) : DEFAULT_MAX_TOTAL;
}
// Четвёртым аргументом идут настройки: потолок у Platega свой, и без него
// очередь касс (`chainFor`) отправляла бы заказ дороже 20 000 ₽ туда, где
// заплатить всё равно нельзя. Остальным кассам аргумент не нужен.
function acceptsAmount(amount, currency, own, settings) {
  const minor = toMinor(amount);
  if (minor === null || trimmed(currency).toUpperCase() !== CURRENCY) return false;
  return !settings || minor <= maxTotal(settings) * MINOR;
}
// Только база формы до выбора метода допускает четыре знака. Денежный факт
// из GET по-прежнему проходит через строгий toMinor без округления.
function baseUnits(value) {
  if (typeof value !== 'string' && typeof value !== 'number') return null;
  const parts = /^(\d+)(?:\.(\d{1,4}))?$/.exec(String(value));
  if (!parts) return null;
  const units = Number(parts[1]) * 10000 + Number((parts[2] || '').padEnd(4, '0'));
...
async function api(settings, path, body, timeoutMs) {
  ...
  const request = (async () => {
    ...
    if (!response.ok) {
      /* Из тела однозначного отказа 4xx читаем ОДНУ строку `message` — и
       * только чтобы прогнать её через общий словарь отказов: так HTTP 400
       * «SBPQR limit is 20000.00 RUB» становится кодом `amount` («не приняла
       * сумму»), а не «другой ошибкой», от которой владелец шёл бы чинить
       * ключи. Наружу уходит ТОЛЬКО узнанный код (`hint`): сама строка не
       * хранится и не логируется — тело отказа вправе повторять ключи запроса.
       * Тело 5xx не читаем вовсе. */
      let hint = '';
      if (creating && [400, 404, 405, 422].includes(http)) {
        try {
          const data = await readJson(response, controller);
          const message = object(data) && typeof data.message === 'string' ? data.message.slice(0, 300) : '';
      ...
      return {
        ...
        http, ...(hint ? { hint } : {}), ambiguous: creating && ![400, 401, 403, 404, 405, 422].includes(http)
      };
    }
...
function invoiceView(data) {
  ...
  return {
    ...
    // Общая форма не фиксирует способ. Старые счета SBP_ONLINE по-прежнему
    // требуют явное подтверждение СБП при финансовой сверке.
    method: isSbpMethod(data.paymentMethod) ? SBP_METHOD : METHOD,
    /* Чем платил покупатель — словами самой кассы («SBPQR», «Crypto»): у
     * общей формы способ выбирают у неё, и панели надо видеть, какой именно.
     * `fee` — комиссия кассы из того же ответа; по ней сверяется чистая сумма
     * магазина, когда покупатель выбрал способ с другим тарифом. */
    way: typeof data.paymentMethod === 'string' ? data.paymentMethod.trim().slice(0, 40) : '',
    fee: typeof data.comission === 'number' && Number.isFinite(data.comission) && data.comission >= 0 ? data.comission : null,
...
async function createInvoice(settings, params) {
  ...
  // Версия расчёта хранится в заказе: общая форма должна складывать базу
  // в копейках и комиссию без тысячных рубля. Старые снимки не пересчитываем.
  // Оплаченный итог GET по-прежнему проверяется строго до копейки.
  if (params.feeRounding !== undefined && (!['cents', 'rubles'].includes(params.feeRounding) || params.feePercent === undefined)) {
    return { ok: false, error: 'bad_base_amount' };
  }
  if (params.feeOriginalTotal !== undefined && params.feeRounding !== 'rubles') return { ok: false, error: 'bad_base_amount' };
  const included = params.feePercent === undefined ? null
    : params.feeRounding === 'rubles' ? FEE.roundedIncluded(params.feeOriginalTotal, params.feePercent)
      : (params.feeRounding === 'cents' ? FEE.includedCents : FEE.included)(totalMinor / MINOR, params.feePercent);
  let baseAmount;
  if (params.feePercent !== undefined) {
    if (!included || included.total !== totalMinor / MINOR || included.baseAmount <= 0
      || params.baseAmount !== included.baseAmount) return { ok: false, error: 'bad_base_amount' };
  ...
  const result = await api(settings, '/v2/transaction/process', body, CREATE_TIMEOUT);
  ...
  const view = {
    ...
    currency: CURRENCY, method: METHOD, requisite: safeUrl(data.url),
    bank: '', owner: '', expiresAt
  };
  // v2 не возвращает сумму: сохраняем ожидаемый итог заказа. Если сумма
  // всё-таки прислана, до выбора способа это база запроса без комиссии.
  // Доказательством оплаты остаётся GET с полной суммой, валютой и payload.
        ...
        : !(view.expiresAt > Date.now()) ? 'bad_expiry'
          : !matchesMerchant(settings, data) ? 'merchant_mismatch'
            : data.payload !== undefined && data.payload !== externalId ? 'payload_mismatch'
              : data.paymentDetails !== undefined && baseUnits(details.amount) !== baseUnits(baseAmount) ? 'amount_mismatch'
                : data.paymentDetails !== undefined && trimmed(details.currency).toUpperCase() !== CURRENCY ? 'currency_mismatch' : '';
  if (failure) return {
...
/* ОБЩАЯ ФОРМА ДАЁТ ПОКУПАТЕЛЮ ВЫБОР СПОСОБА, А У СПОСОБОВ РАЗНЫЕ ТАРИФЫ.
 *
 * Мы отправляем базу (цена ÷ 1,085) и ждём итог = база + 8,5 % — тариф СБП.
 * Но на своей странице Platega предлагает и другое: на боевом счёте #478234
 * покупатель выбрал Crypto с комиссией 5 %, и касса ждала 27 096,77 ₽ вместо
 * наших 28 000. Строгая сверка суммы отправила бы такую оплату в `mismatch` —
 * на разбор менеджеру, без отправления, — хотя магазин получил бы ровно то,
 * что просил: комиссию Platega берёт с покупателя СВЕРХ базы, и чистая сумма
 * мерчанта от способа не зависит.
 *
 * Поэтому второй законный исход: `amount − comission` кассы равен нашей базе
 * (в копейках, база бывает с четырьмя знаками — округляем её к копейке).
 * Итог покупателя при этом отличается от ценника — в обе стороны, если у
 * способа тариф выше, — и это честно записывается в заказ (`paidTotal`).
 * Меньше базы прийти не может: такое означало бы, что комиссия удержана из
 * денег магазина, и это остаётся `mismatch` — fail-closed. */
function netMatches(expected, actual) {
  const base = toMinor(roundKopecks(expected.baseAmount));
  const amount = toMinor(actual.amount);
  const fee = typeof actual.fee === 'number' ? toMinor(roundKopecks(actual.fee)) : null;
  if (base === null || amount === null || fee === null && actual.fee !== 0) return false;
  return amount >= base && amount - (fee || 0) === base;
}
function roundKopecks(value) {
  const n = Number(value);
  return Number.isFinite(n) && n >= 0 ? (Math.round(n * 100) / 100).toFixed(2) : null;
}
...
function matchesInvoice(expected, actual) {
  ...
  if (want === null || got === null || (got !== want && !netMatches(expected, actual))) return { ok: false, reason: 'amount' };
  ...
  // Общая форма разрешает выбор способа. Только ранее созданные счета
  // фиксированного СБП требуют соответствующего метода в ответе GET.
  if (!storedMethod(expected.method) || !storedMethod(actual.method)
    || (trimmed(expected.method) === SBP_METHOD && trimmed(actual.method) !== SBP_METHOD)) return { ok: false, reason: 'method' };
  return { ok: true };
}
...
function optionsShape(extra) {
  // Доступ API проверен; доступность выбранного способа определяет сама форма.
...
module.exports = {
  id: 'platega', name: 'Platega', METHOD, CURRENCY, MINOR, DEFAULT_MAX_TOTAL,
  unresolvedStartTtl: UNRESOLVED_START_TTL,
  // До выбора способа GET отдаёт базу без комиссии, а при другом способе —
  // другой итог: несовпадение суммы у висящего счёта здесь норма, а не событие.
  pendingAmountVaries: true,
  configured, enabled, supports, acceptsAmount, maxTotal, toMinor, stateOf,
