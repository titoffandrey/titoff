'use strict';
/*
 * Platega: покупатель выбирает способ на платёжной странице сервиса.
 * Документация: https://docs.platega.io (сверено 15 сентября 2026).
 *
 * POST /v2/transaction/process возвращает transactionId и url; поле
 * paymentMethod намеренно отсутствует — конкретный способ выбирается позже.
...
// expiresIn описан как HH:MM:SS, а не Unix-время или число секунд.
...
function invoiceView(data) {
  ...
  const details = object(data.paymentDetails) ? data.paymentDetails : {};
  ...
  return {
    ...
    currency: trimmed(details.currency).toUpperCase(), method: METHOD,
...
async function createInvoice(settings, params) {
  params = params || {};
  if (!configured(settings)) return { ok: false, error: 'not_configured' };
  if (toMinor(params.amount) === null) return { ok: false, error: 'bad_amount' };
  ...
  const body = {
    paymentDetails: { amount: toMinor(params.amount) / MINOR, currency: CURRENCY },
  ...
  const view = {
    ...
    state: stateOf(data.status), amount: body.paymentDetails.amount,
    currency: CURRENCY, method: METHOD, requisite: safeUrl(data.url),
    bank: '', owner: '', expiresAt: expiryMs(data.expiresIn, startedAt)
  };
  // POST по документации не возвращает сумму/валюту/payload. Здесь сохраняем
  // параметры запроса; доказательством оплаты будет только отдельный GET.
  // Если сервис прислал дополнительные поля, их расхождение не игнорируем.
      ...
      : view.state !== 'pending' ? 'bad_invoice_state'
        : !(view.expiresAt > Date.now()) ? 'bad_expiry'
          : data.payload !== undefined && data.payload !== externalId ? 'payload_mismatch'
            : data.paymentDetails !== undefined && (!object(data.paymentDetails)
              || toMinor(data.paymentDetails.amount) !== toMinor(params.amount)) ? 'amount_mismatch'
              : data.paymentDetails !== undefined && trimmed(data.paymentDetails.currency).toUpperCase() !== CURRENCY
                ? 'currency_mismatch' : '';
  if (failure) return {
...
async function invoice(settings, id) {
  ...
  if (!result.ok) return result;
  // В текущей схеме поле написано mechantId. Если сервис вернул владельца
  // транзакции, сверяем его с нашими ключами; отсутствующее поле не выдумываем.
  for (const field of ['mechantId', 'merchantId']) {
    if (result.data[field] !== undefined && (!validMerchantId(result.data[field])
      || trimmed(result.data[field]).toLowerCase() !== trimmed(settings.plategaMerchantId).toLowerCase())) {
...
function sameStartRequest(expected, actual) {
  ...
  return amount !== null && amount === toMinor(actual.amount)
    && supports(expected.method) && supports(actual.method)
...
function optionsShape(extra) {
  // Какие конкретно варианты включены у мерчанта, решает размещённая форма.
