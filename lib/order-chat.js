...
function create({ db, chat, settings, prepareShipment, busy = () => false }) {
  const enabled = (s = settings()) => chat.visible(s) && AI.enabled(s);
  ...
  function processOrder(order, now, s = settings()) {
    const job = order && order.chatFollowup;
    if (!job || job.paidAt || now - job.enrolledAt > MAX_AGE || !enabled(s)) return;
    ...
    const phone = PHONE.store(s.contactWhatsApp);
  ...
  function sweep(now = Date.now()) {
    const s = settings();
    if (!enabled(s)) return;
    for (const order of db.getOrders()) processOrder(order, now, s);
  }
