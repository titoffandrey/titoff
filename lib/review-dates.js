...
function arrange(group, perPage) {
  ...
  // Шаг раздачи считается по КРАЯМ, а не по долям: вложение достаётся и первому
  // свободному месту, и последнему. Иначе у товара, где на последнюю страницу
  // приходится один-два отзыва, она оказывалась текстовой — а её видит как раз
  // тот, кто долистал до конца (та же причина, по которой ролики стоят и там).
  ...
  if (media.length >= free.length) {
    ...
    for (let j = 0; j < media.length; j++) {
      mediaAt.add(Math.round(j * (free.length - 1) / (media.length - 1)));
    }
  }
