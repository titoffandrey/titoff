 ...
 *
 * ЧЕГО ЗДЕСЬ НЕТ — CALLBACK-УВЕДОМЛЕНИЙ, и не потому, что они не описаны.
 * Описаны подробно (`ecom.alfabank.ru/assets/instructions/callback-uvedomleniya.pdf`,
 * сверено 6 сентября 2026): GET на адрес мерчанта с параметрами `mdOrder`,
 * `orderNumber`, `operation`, `status`, `checksum`; контрольная сумма — HMAC-SHA256
 * по строке «имя;значение;…» из отсортированных ПО АЛФАВИТУ параметров без
 * самого `checksum`, в верхнем регистре; в ответ ждут ровно 200 OK, иначе шлюз
 * повторяет отправку. Есть и `dynamicCallbackUrl` — адрес на конкретный заказ,
 * что легло бы на наш поадресный token один в один.
 *
 * Не включено оно по другой причине: ключ для подписи выдаёт техподдержка
 * банка по отдельной заявке, а без ключа принимать такие уведомления нельзя —
 * это открытая дверь к чужому «заказ оплачен». Пока ключа нет, оплату
 * подтверждает опрос статуса: он и так идёт со страницы оплаты и из фоновой
 * сверки, и на боевых заказах доводит платёж до `paid` сам.
 *
 * Способ оплаты у этого пути ровно один — страница банка; валюта только
 * рублёвая. КАРТОЙ ИЛИ ЧЕРЕЗ СБП покупатель решает уже ТАМ, и по факту выбирают
 * СБП: у всех трёх оплаченных заказов банк вернул `paymentWay: SBP_C2B`
 * (замер на боевых данных 6 сентября 2026).
 */
 ...
 *
 * Мануал говорит об этом дважды и одинаково: у `sessionTimeoutSecs` —
 * «если параметр не задан, будет использовано значение из настроек мерчанта или
 * время по умолчанию (1200 секунд = 20 минут)», у кода отказа −2007 — «истёк
 * срок, отведённый на ввод данных карты с момента регистрации платежа (таймаут
 * по умолчанию 20 минут; продолжительность сессии может быть указана при
 * регистрации заказа)». Боевые данные это подтверждают: заказ, зарегистрирован-
 * ный в 09:54:31, банк закрыл кодом −2007 в 10:14:31 — ровно 1200 секунд.
 *
 * `expirationDate` рядом НЕ ПЕРЕДАЁМ намеренно: по мануалу он перебивает
 * `sessionTimeoutSecs`, то есть два поля об одном сроке спорили бы между собой.
 *
...
/* Описание заказа. В запрос мануал пускает 512 знаков, но В ПРОЦЕССИНГ банка
 * уходит не больше 99 и «запрещены к использованию %, +, конец строки \r и
 * перенос строки \n». Режем и чистим здесь, а не надеемся на вызывающего:
 * строка собирается из данных заказа, а плюс в ней — обычное дело («eSIM +
 * физическая SIM»).
 */
const DESCRIPTION_MAX = 99;
function safeDescription(value) {
  return trimmed(value).replace(/[%+\r\n]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, DESCRIPTION_MAX);
}
...
const HOSTS = {
  live: {
    host: 'pay.alfabank.ru', base: '/payment/rest',
    /* Ключ сертификата CN=pay.alfabank.ru, O=Alfa-Bank (выпущен «Russian Trusted
     * Sub CA» Минцифры, действует до 3 декабря 2026 — сверено 6 сентября 2026).
     * После этой даты пин придётся обновить, и панель скажет об этом сама:
     * проверка связи ходит тем же путём и покажет «касса не отвечает». */
    pins: ['hgwdhtFKwhAzbdy5PbIum3BpRsOeg6RVr3DXRbSX7MQ=']
  },
  /* Тестовый контур — тот, что назван КООРДИНАТАМИ ПОДКЛЮЧЕНИЯ в мануале банка:
   * `https://tws.egopay.ru/api/ab/rest/…`. Путь у него свой, поэтому он и живёт
   * рядом с хостом, а не общей константой.
   *
   * Раньше здесь стоял `alfa.rbsuat.com` с боевым путём, и это не работало бы
   * дважды: адрес не тот, что выдают тестовому мерчанту Альфы, а его сертификат
   * подписан тем же российским центром — цепочка снаружи РФ не собирается, и
   * запрос упал бы на проверке ещё до отправки ключей. У `tws.egopay.ru`
   * сертификат обычный, публично доверенный (проверено запросом без `-k`),
   * поэтому пин ему не нужен: пустой список означает обычную проверку цепочки.
   */
  test: { host: 'tws.egopay.ru', base: '/api/ab/rest', pins: [] }
};

const PATH_REGISTER = '/register.do';
const PATH_STATUS = '/getOrderStatusExtended.do';
...
function post(env, path, payload, timeoutMs) {
  return new Promise(resolve => {
    ...
    const req = https.request({
      // Путь склеивается из базы среды: у тестового контура она своя
      // (`/api/ab/rest`), у боевого — `/payment/rest`.
      host: env.host, path: env.base + path, method: 'POST', agent: agentFor(env),
      headers: {
      ...
      // Ограничиваем весь ответ, а не отдельный chunk. Иначе мелкие чанки
      // бесконечно накапливались, а большой молча вырезался из JSON.
      res.on('data', chunk => {
        if (done) return;
      ...
      // Оборванный ответ может не породить ни end, ни request.error.
      // Ожидание такого счёта должно завершиться безопасным отказом.
      res.on('aborted', () => finish({ ok: false, error: 'response_aborted' }));
      res.on('error', () => finish({ ok: false, error: 'response_error' }));
      res.on('end', () => {
        if (done) return;
        if (res.statusCode < 200 || res.statusCode >= 300) {
          return finish({ ok: false, error: 'http_' + res.statusCode, http: res.statusCode });
        }
        const raw = text.trim();
    ...
    // Общий срок включает DNS, очередь keep-alive агента и медленный ответ.
    // Только socket timeout перезапускался каждым байтом и не ограничивал их.
    const timeout = () => {
      finish({ ok: false, error: 'timeout' });
      req.destroy();
    };
    timer = setTimeout(timeout, Number(timeoutMs) || CREATE_TIMEOUT);
    if (timer.unref) timer.unref();
    req.setTimeout(Number(timeoutMs) || CREATE_TIMEOUT, timeout);
    req.on('error', e => {
...
async function createInvoice(settings, params) {
  ...
  const description = safeDescription(params && params.description);
...
/* Чем платил покупатель НА СТРАНИЦЕ БАНКА. Способ у нас один — «страница
 * банка», а карту или СБП человек выбирает уже там, и банк говорит это полем
 * `paymentWay`. Знать это стоит: на боевых данных ВСЕ оплаченные заказы прошли
 * через СБП, и без этой строки владелец считал бы, что у него платят картой.
 * Незнакомое значение показываем как есть — выдумывать перевод хуже. */
// Подписи в именительном падеже: они встают и после «Оплата:», и через точку
// рядом с причиной отказа. «Оплата через картой» — ровно то, что получается,
// если положить сюда падежную форму.
const WAYS = { SBP_C2B: 'СБП', CARD: 'карта', APPLE_PAY: 'Apple Pay', GOOGLE_PAY: 'Google Pay', SAMSUNG_PAY: 'Samsung Pay', MIR_PAY: 'Mir Pay' };
function wayOf(raw) {
  const key = trimmed(raw).toUpperCase();
  if (!key || key === 'UNKNOWN') return '';
  return WAYS[key] || key.toLowerCase();
}

/* Почему не прошло — словами самого банка.
 *
 * `getOrderStatusExtended` возвращает `actionCode` и `actionCodeDescription`
 * («Истек срок ожидания ввода данных.», «Операция отклонена. Проверьте
 * введённые данные, достаточность средств…»), и это ровно тот ответ, которого
 * менеджеру не хватало: без него все незакрытые счета выглядели одинаково —
 * «оплата не прошла», хотя половина из них это просто брошенная страница, а
 * половина — отказ банка покупателя.
 *
 * Замер на боевых данных (6 сентября 2026, 15 счетов): 8 × −2007 «истёк срок
 * ввода данных», 4 × −2014 «операция отклонена», 3 оплачены.
 *
 * Код держим рядом с описанием: описание банк однажды переформулирует, а по
 * коду отказ узнаётся всегда.
 */
function reasonOf(data, state) {
  const way = wayOf(data && data.paymentWay);
  const code = Number(data && data.actionCode);
  if (state === 'paid') return way ? 'Оплата: ' + way : '';
  const text = trimmed(data && data.actionCodeDescription).replace(/\s+/g, ' ').replace(/\.$/, '');
  if (!text || !Number.isFinite(code) || code === 0) return '';
  return ('Банк: ' + text + ' (' + code + ')' + (way ? ' · ' + way : '')).slice(0, 120);
}
 ...
 *
 * ВАЖНО ПРО ДОСТУП: мануал разрешает `token` только в регистрации заказа, а у
 * статуса перечисляет ровно `userName`/`password`. То есть магазин, у которого
 * задан один платёжный токен, счёт выставит, а подтвердить оплату не сможет —
 * и панель честно скажет «ключи не приняты кассой» (`availableOptions` ходит
 * этим же запросом). Полный доступ выдаётся письмом при подключении эквайринга.
 */
async function invoice(settings, id) {
  ...
  return {
    ...
    invoice: {
      ...
      // Причина отказа (или чем оплатили) — словами банка. Маршрут сверки
      // положит её припиской к попытке, и панель покажет под плашкой.
      reason: reasonOf(r.data, state)
    }
  };
}
...
async function availableOptions(settings) {
  ...
  const key = env.host + '|' + crypto.createHash('sha256').update(JSON.stringify(auth)).digest('hex');
  ...
  // Одновременные открытия оплаты и панели ждут один запрос проверки.
  if (_probe && _probe.key === key) return _probe.job;
  ...
  entry.job = (async () => {
    try {
      ...
      // «Не знаю такого заказа» означает, что ключи банк принял.
      ...
      // Сброс из настроек не позволяет старому ответу вернуть прежний кэш.
      if (_probe === entry) {
        _live.key = key;
        _live.at = Date.now();
        _live.ok = !error;
    ...
    } finally { if (_probe === entry) _probe = null; }
  })();
  return entry.job;
}

function forgetMethods() { _probe = null; _live.key = ''; _live.at = 0; _live.ok = false; _live.error = ''; }
...
module.exports = {
  ...
  // Наружу — только тестам: они сверяют пределы описания и разбор ответа банка.
  safeDescription, reasonOf,
