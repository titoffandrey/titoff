 ...
 *
 * ЛИМИТ — ДВА РАЗНЫХ ЧИСЛА, И ВЕРИТЬ НАДО МЕНЬШЕМУ. Формальный лимит мерчанта
 * касса отдаёт в `/me` полем `maxPaymentAmount` (копейки строкой) и выше него
 * отвечает HTTP 400 «Превышен лимит на сумму платежа: максимум N RUB» — первый
 * контрольный заказ на 55 500 ₽ (17 сентября 2026) получил ровно это при
 * лимите 10 000. Менеджер поднял лимит до 160 000 — и оказалось, что ВЫШЕ
 * ~19 000 ₽ ССЫЛКА НЕ ВЫДАЁТСЯ: POST отвечает 200 со `status: "FAILED"` и
 * `payUrl: null` (замер с сервера: 19 000 — PENDING со ссылкой, 19 050,
 * 19 999, 30 000, 160 000 — FAILED).
 *
 * РЕШЕНИЕ ВЛАДЕЛЬЦА (17 сентября 2026): СВОЕГО ПОТОЛКА У МАГАЗИНА НЕТ. Любой
 * заказ уходит в кассу, не выдала ссылку — покупатель видит отказ и может
 * повторить; переводом по своим реквизитам или заявкой заказ НЕ уводится.
 * Поэтому `solutionesMaxTotal` по умолчанию пуст (ноль — «предела нет»,
 * действует только общий `payMaxTotal`), а заявленный кассой
 * `maxPaymentAmount` из `/me` в очередь касс не вмешивается — он только
 * показывается. Впишет владелец число — заказ дороже пойдёт другим путём
 * (`PAYMENTS.modeFor`), как у Platega с её 20 000. Счёт, пришедший FAILED
 * уже при создании, — однозначный отказ: платить по нему нечем.
 */
...
// Ноль — своего потолка нет: решение владельца, см. шапку файла.
...
// Ноль — потолка нет (только общий `payMaxTotal`).
function maxTotal(settings) {
  const n = Number(settings && settings.solutionesMaxTotal);
  return Number.isFinite(n) && n > 0 ? Math.round(n) : DEFAULT_MAX_TOTAL;
}
// Четвёртым аргументом идут настройки: впишет владелец потолок — очередь касс
// (`chainFor`) не отправит заказ дороже туда, где ссылки не будет. Без него
// заказ уходит в кассу на любую сумму: не выдала — отказ (решение владельца).
// Живой `maxPaymentAmount` из `/me` (третий аргумент) в отбор не вмешивается.
function acceptsAmount(amount, currency, own, settings) {
  const minor = toMinor(amount);
  if (minor === null || trimmed(currency).toUpperCase() !== CURRENCY) return false;
  const ceiling = maxTotal(settings);
  return !ceiling || minor <= ceiling * MINOR;
}
...
async function createInvoice(settings, params) {
  ...
  if (failure) {
    /* Счёт, закрытый уже в ответе на создание (`FAILED` без ссылки — так касса
     * отвечает на суммы выше своего рабочего потолка), — не двусмысленность, а
     * отказ: платить по нему нечем, и запирать покупателя на защитный интервал
     * незачем. Сам счёт сохраняется для истории и сверки. */
    const closed = ['failed', 'cancelled', 'expired'].includes(view.state);
    return {
      ok: false, error: failure, ambiguous: !closed,
      ...(closed ? { hint: 'amount' } : {}),
...
async function availableOptions(settings) {
  ...
  if (optionsCache && optionsCache.key === key && Date.now() - optionsCache.at < OPTIONS_TTL) {
    return optionsCache.error ? { ok: false, error: optionsCache.error, cached: true } : optionsShape({ cached: true, ...(optionsCache.extra || {}) });
  }
  ...
  entry.job = (async () => {
    try {
      ...
      // Заявленный кассой потолок на платёж — в рублях; мусор или его отсутствие
      // означают «не знает», а не «нет предела».
      const liveMax = valid ? minorOf(data.merchant.maxPaymentAmount) : null;
      const extra = liveMax ? { maxTotal: liveMax / MINOR } : {};
      if (optionsJob === entry) optionsCache = { key, at: Date.now(), error, extra };
      return error ? { ok: false, error } : optionsShape(extra);
...
module.exports = {
  id: 'solutiones', name: 'Solutiones', METHOD, CURRENCY, MINOR, API, DEFAULT_MAX_TOTAL,
  configured, enabled, supports, acceptsAmount, maxTotal, toMinor, minorOf, stateOf,
