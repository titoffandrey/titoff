 ...
 * 19 999, 30 000, 160 000 — FAILED). Поэтому потолок — настройка
 * (`solutionesMaxTotal`, по умолчанию 19 000), а живой `maxPaymentAmount`
 * только режет её сверху. Заказ дороже в кассу не идёт вовсе, а уходит другим
 * путём (`PAYMENTS.modeFor`) — как у Platega с её 20 000. Счёт, пришедший
 * FAILED уже при создании, — однозначный отказ: платить по нему нечем.
 */
...
// Четвёртым аргументом идут настройки: без потолка очередь касс (`chainFor`)
// отправляла бы заказ дороже 19 000 ₽ туда, где ссылки не будет, а витрина
// обещала бы оплату на сайте, которой нет. Третий (`own`) — живой ответ
// `/me`: заявленный кассой `maxPaymentAmount` режет потолок сверху.
function acceptsAmount(amount, currency, own, settings) {
  const minor = toMinor(amount);
  if (minor === null || trimmed(currency).toUpperCase() !== CURRENCY) return false;
  if (own && Number.isFinite(own.maxTotal) && own.maxTotal > 0 && minor > own.maxTotal * MINOR) return false;
