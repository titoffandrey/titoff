...
// 7.6 — раскрытый экран складного: у Apple он и нарисован лежащим прямоугольником,
// а не телефоном. Внешнего экрана (5,4″) в наборе нет — такая строка уйдёт в общий
// `display`, и это честнее подставленной чужой диагонали.
const DISPLAY_SIZES = { '6.1': 'display-61', '6.3': 'display-63', '6.5': 'display-65', '6.7': 'display-67', '6.9': 'display-69', '7.6': 'display-76' };
...
const CHIPS = [
  ...
  [/m3|m2|m1/i, 'chip-m4'],
  // У часов бейдж тоже свой, и подставлять вместо него общий `chip` нельзя:
  // в НЁМ нарисовано «S10». То есть до появления этого правила у Series 11,
  // Series 12 и обеих Ultra рядом со строкой «Чип: S11 SiP» стоял бейдж
  // ЧУЖОГО поколения — ошибка, которую видно только глазами.
  [/s11/i, 'chip-s11'],
  [/s10/i, 'chip-s10'],
  // A20 бывает пока только в версии Pro — свой бейдж у Apple есть именно у неё;
  // обычный A20 попадёт в общий контур `chip`, как и всякий чип без бейджа.
  [/a20\s*pro/i, 'chip-a20-pro'],
...
const RULES = [
  ...
  [/диафрагм|апертур|ƒ\/|\bf\/\d/i, 'aperture'],
  ...
  // «гб» и «тб» — только с числом перед ними: без него «тб» находится внутри
  // «ноутбука», и «режим ноутбука» у складного уехал бы в накопитель.
  [/озу|память|накопит|\bssd\b|хранил|\d\s*(?:гб|тб)|\bram\b|storage/i, 'storage'],
  ...
  // Рама корпуса — глиф слоёв, тот самый, что Apple ставит у строки «Case materials».
  // Ловится он именно по раме, петле и unibody: голый «алюминий» рядом с Ceramic
  // Shield остаётся щитом, как эта строка и читается сейчас у всего каталога.
  [/титановая рама|алюминиевая рама|рама корпуса|петл|шарнир|unibody|унибоди/i, 'frame'],
  [/титан|алюмин|стекл|керамик|ceramic shield|сапфир|материал|прочн|стойкос/i, 'shield'],
  // «длина» — про кабели: у них это главная характеристика после разъёмов, а
  // глиф линейки Apple ставит рядом с размерами любого рода.
  [/вес|толщин|габарит|размер|компакт|длина/i, 'size'],
...
function pickCamera(v) {
  ...
  // CENTER STAGE — ОДИН ГЛИФ НА ЛЮБОЕ ЧИСЛО МЕГАПИКСЕЛЕЙ. У Apple строки «12MP
  // Center Stage» и «18MP Center Stage» подписаны ОДНИМ рисунком (человек в
  // рамке со стрелками поворота, `3f2f6bc3` — один и тот же на обе). Прежняя
  // развилка по «18 Мп» ставила складному iPhone обычный TrueDepth без стрелок,
  // то есть глиф чужой строки.
  if (/center stage/i.test(v)) return 'camera-center-stage';
  if (/теле|зум\s*8|8×|8x/i.test(v)) return 'camera-system';                        // тройная Pro, зум 8×
  // Двойная камера СКЛАДНОГО нарисована у Apple на сложенном корпусе — блок
  // лежит поперёк, а не вдоль, и от обычной «двойной Fusion» отличается именно
  // силуэтом. Проверяем раньше общего «+»: текст строки у них одинаковый, и
  // различить их можно только по тому, что рядом названы Duo Preview и Smart
  // Take — функции, которые есть у Apple ровно в этой строке.
  if (/duo preview|складн/i.test(v)) return 'camera-dual-fold';
...
function choose(key, value) {
  ...
  if (/экран|дисплей|диагонал/i.test(k)) {
    // Кавычку дюйма пишут по-разному: обычную набирают руками, а «″» и «”» приезжают
    // копированием с apple.com — не узнать их значит показать общий глиф там, где
    // диагональ названа.
    const m = v.match(/(\d+[.,]\d+)\s*(?:"|”|″|''|inch|дюйм)/i);
    const named = m && DISPLAY_SIZES[m[1].replace(',', '.')];
    if (named && ICONS[named]) return named;
    /* ДИАГОНАЛЬ НАЗВАНА, А ГЛИФА НА НЕЁ НЕТ — берём общий `display`, и дальше не
     * идём. Ниже стоит `display-always-on`, а это ЦИФЕРБЛАТ ЧАСОВ: у Apple он и
     * нарисован корпусом с колёсиком. Строка «Внешний экран: 5.4" … Always-On» у
     * складного iPhone уходила туда и приносила на карточку телефона часы.
     * Диагональ — надёжный признак: у экрана часов её не пишут. */
    if (m) return 'display';
  ...
  // «Автономность: до 44 ч видео на внешнем экране» — про батарею, а не про экран:
  // у складного время работы и считают по экранам, и общий разбор отдал бы такую
  // строку глифу дисплея, потому что правило экрана стоит выше правила питания.
  if (/автоном|аккум|батаре|время работы/i.test(k) && ICONS['battery']) return 'battery';
  ...
  // «Корпус: … 42 мм» — часы (у Ultra свой глиф на 49 мм); «Корпус: титановая рама
  // и крышка петли» — слои, как у Apple в строке «Case materials»; «Материал: титан,
  // Ceramic Shield» — щит, потому что строка называет ещё и стекло.
  if (/корпус|материал/i.test(k)) {
    if (/\d+\s*мм/.test(v)) return /49\s*мм/.test(v) && ICONS['case-ultra'] ? 'case-ultra' : 'case';
    if (/рама|петл|шарнир|unibody|унибоди/i.test(v) && ICONS['frame']) return 'frame';
    return 'shield';
  }
