...
function forFile(file, buffer) {
  ...
  // Уже собранный сторонний компонент: сохраняем байты и ссылку на лицензии.
  if (/(?:^|[\\/])smoothui-toast\.(?:js|css)$/.test(ext)) return buffer;
  if (/(?:^|[\\/])bklit-charts\.(?:js|css)$/.test(ext)) return buffer;
