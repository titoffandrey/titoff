...
/* Номер дома: 5, 5а, 12/3, 7к2, а также слитные строения и корпуса — 13с1,
 * 23Ак2, 2с3В, 40соор1. Слитную запись пишут и покупатели, и справочники
 * перевозчиков: на официальном списке пунктов СДЭК прежняя регулярка отвергала
 * 85 адресов из 9888, и все до единого — из-за строения.
 *
 * Шесть цифр подряд — это индекс, а не дом, поэтому больше четырёх не берём, и
 * продолжать номер цифрами можно только через разделитель.
 *
 * Буква после дефиса («3-Ж») намеренно НЕ считается домом: от порядкового
 * названия улицы («1-я Тверская») её не отличить, а принять улицу за дом хуже,
 * чем не узнать редкую запись номера.
 */
const HOUSE = /^\d{1,4}(?:[а-я]{1,5}\d{0,3}){0,2}(?:[-/]\d{1,3}[а-я]{0,2})?$/;
...
// Предел адреса — общий у заказа (`textValue(data.address, 400)` в lib/db.js),
// кабинета покупателя и поля на витрине (`maxlength`).
const MAX = 400;

/* Адрес хранится ОДНОЙ строкой: переводы строк и ряды пробелов схлопываются в
 * пробел, края обрезаются. Поле на витрине растущее и переносит длинный адрес
 * само, а перенос, вставленный в строку вместе с адресом, уехал бы в
 * накладную и в Telegram как есть. Правило одно на заказ и на кабинет. */
...
module.exports = { checkAddress, normalize, MAX, EXAMPLE, SETTLEMENT_MARKERS, HOUSE };
