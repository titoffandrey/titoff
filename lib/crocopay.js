...
/*
 * Пределы одной покупки: 1 000 ₽ снизу и 250 000 ₽ сверху. Числа кассы — она
 * не проводит платежи за этими границами.
 *
 * Заказ вне пределов НЕ ОФОРМЛЯЕТСЯ вовсе: кнопка на оформлении гаснет, а
 * `/api/order` отвечает отказом. Правило действует на любой заказ, а не только
 * на оплачиваемый онлайн: «один заказ — не больше 250 000 ₽» должно работать
 * одинаково, иначе снятая на день галочка оплаты меняла бы условия продажи.
 *
...
function limitError(total) {
  const sum = Number(total);
  if (!Number.isFinite(sum)) return 'Сумма заказа некорректна';
  if (sum > MAX_TOTAL) return `Один заказ — не более ${MAX_TOTAL.toLocaleString('ru-RU')} ₽. Разделите покупку на несколько заказов.`;
  if (sum < MIN_TOTAL) return `Минимальная сумма заказа — ${MIN_TOTAL.toLocaleString('ru-RU')} ₽.`;
  return '';
}

/*
 * Пределы — ограничение КАССЫ, а не правило магазина, поэтому действуют они
 * ровно тогда, когда покупатель платит на витрине. Оплата выключена — заказ
 * уходит заявкой, её разбирает менеджер, и запрещать заявку на Vision Pro
 * незачем: касса в этом пути не участвует вовсе.
 *
 * Ноль означает «предела нет» — так его читает и витрина (`public/app.js`
 * везде проверяет `if (!ORDER_MAX)`), и разметка товара.
 */
function limits(settings) {
  return enabled(settings) ? { min: MIN_TOTAL, max: MAX_TOTAL } : { min: 0, max: 0 };
}

// Отказ по сумме с оглядкой на режим витрины. Сам `limitError()` остаётся
// безусловным: он отвечает про кассу, а её пределы от наших настроек не зависят
// (`/api/pay/crocopay/start` спрашивает именно его).
function limitFor(settings, total) { return enabled(settings) ? limitError(total) : ''; }
 ...
 */
const START_ERRORS = [
  ['route_mismatch', /method_mismatch|currency_mismatch/i, 'Касса не подтвердила выбранный российский способ оплаты. Иностранные реквизиты не показаны — выберите другой способ.'],
  ['invoice_invalid', /bad_invoice_state|bad_expiry/i, 'Касса вернула счёт без подтверждённого безопасного срока. Реквизиты не показаны — выберите другой способ.'],
  ['no_requisite', /requisite|реквизит/i, 'Сейчас нет свободных реквизитов для этого способа. Выберите другой способ оплаты или попробуйте через пару минут.'],
  ['method_unavailable', /payment_option|not enabled|not supported|not available/i, 'Этот способ оплаты сейчас недоступен — выберите другой.'],
  ['timeout', /timeout|fetch failed|network/i, 'Платёжный сервис не отвечает. Заказ сохранён; попробуйте ещё раз через минуту.'],
  ['amount', /amount|limit/i, 'Платёжный сервис не принял эту сумму — менеджер свяжется с вами и поможет с оплатой.']
];
function startErrorCode(error) {
  const text = String(error == null ? '' : error);
  for (const [code, re] of START_ERRORS) if (text === code || re.test(text)) return code;
  return 'provider_error';
}
function startError(error) {
  const text = String(error == null ? '' : error);
  for (const [code, re, message] of START_ERRORS) if (text === code || re.test(text)) return message;
  return 'Не удалось выставить счёт. Выберите другой способ оплаты или попробуйте позже — заказ уже сохранён.';
}
...
module.exports = {
  CURRENCY, MINOR, MIN_TOTAL, MAX_TOTAL, payable, limitError, limits, limitFor, configured, enabled, toMinor, paidEnough, stateOf, validInvoiceId,
