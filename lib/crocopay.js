...
// Текст отказа для витрины. Валюта всегда рублёвая: касса магазина рублёвая, и
// пределы заданы в тех же рублях, что и сумма заказа.
 ...
 */
const START_ERRORS = [
  [/requisite|реквизит/i, 'Сейчас нет свободных реквизитов для этого способа. Выберите другой способ оплаты или попробуйте через пару минут.'],
  [/payment_option|not enabled|not supported|not available/i, 'Этот способ оплаты сейчас недоступен — выберите другой.'],
  [/timeout|fetch failed|network/i, 'Платёжный сервис не отвечает. Попробуйте ещё раз через минуту.'],
  [/amount|limit/i, 'Платёжный сервис не принял эту сумму — менеджер свяжется с вами и поможет с оплатой.']
];
function startError(error) {
  const text = String(error == null ? '' : error);
  for (const [re, message] of START_ERRORS) if (re.test(text)) return message;
...
module.exports = {
  ...
  createInvoice, startError, invoice, availableOptions, parseOptions, forgetMethods, verify
};
