 ...
 *
 * Исключение одно — способ с оплатой НА СТРАНИЦЕ БАНКА (`hosted`, у нас это
 * `CARD_ONLINE` Альфы). CrocoPAY — P2P-процессинг: она выдаёт реквизиты для
 * перевода, а платёжной страницы у неё нет, и такого `payment_option` в её
 * живом списке не бывает. Пока «любой код» включал и его, касса попадала в
 * очередь на способ, которого не умеет: с выключенной Альфой и отмеченной
 * «Картой» покупателя вели на несуществующую страницу банка, а в таблице «Кассы»
 * копился отказ CrocoPAY на запрос, который она обслужить не могла в принципе.
 * Консультанту в чате тот же список говорил, что «карта на странице банка»
 * есть, — при одной P2P-кассе это неправда.
 */
function supports(methodId) { return trimmed(methodId) !== '' && !PAY.isHosted(methodId); }
