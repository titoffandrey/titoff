...
let dir = '';
let timer = null;
// Серверу нужен только переход «панель появилась / панель ушла»: по нему
// консультант уступает первый ответ живому менеджеру. Сами сессии и чаты этот
// независимый транспорт не знает, поэтому наружу уходит один callback.
...
function presence(fn) { presenceListener = typeof fn === 'function' ? fn : null; }
function hasClients() { return clients.size > 0; }
function tellPresence(value) {
  if (!presenceListener) return;
  try { presenceListener(!!value); }
  // Ошибка логики чата не должна оборвать живое обновление всей панели.
  catch (e) { console.error('Живое обновление: ошибка обработчика присутствия — ' + e); }
}
...
function drop(client) {
  ...
  if (!clients.size) { stop(); tellPresence(false); }
}
...
function subscribe(req, res, wanted) {
  ...
  const wasOnline = hasClients();
  clients.add(client);
  if (!wasOnline) tellPresence(true);
...
module.exports = {
  TOPICS, POLL_MS, NOTE_MAX,
  watch, bump, note, online, presence, subscribe, revisions,
  hasClients, clientCount: () => clients.size
};
