...
const MAX_STEPS = 40;
// Меньше получаса между соседними событиями — это уже не склад, а машина.
...
const HUBS = {
  ...
  pfo: ['Нижний Новгород', 'Казань'],
  yug: ['Воронеж', 'Ростов-на-Дону'],
  ural: ['Нижний Новгород', 'Екатеринбург'],
  sfo: ['Екатеринбург', 'Омск', 'Новосибирск'],
  dfo: ['Новосибирск', 'Иркутск', 'Хабаровск'],
  ru: ['Нижний Новгород', 'Екатеринбург']
};

/* Сортировочный центр в городе — то, что в настоящем трекинге стоит вместо
 * голого названия города: «Москва, СЦ Внуково».
 *
 * Названия здесь — РЕАЛЬНЫЕ логистические районы этих городов (Внуково, Шушары,
 * Кольцово, Толмачёво, Аксай — аэропорты и промзоны, где склады и стоят), а не
 * подтверждённые площадки СДЭК и OZON: их состав перевозчики не публикуют, и
 * выдавать догадку за факт мы не будем. Отсюда правило: ГОРОДА, В КОТОРОМ МЫ НЕ
 * УВЕРЕНЫ, В ТАБЛИЦЕ НЕТ ВОВСЕ — у такого шага остаётся один город, как раньше.
 * Придумать «СЦ Придонской» проще, чем потом объяснять покупателю, почему такого
 * центра не существует.
 *
 * Ставится название только у СКЛАДСКИХ шагов: «Принят на склад · Москва, СЦ
 * Внуково» читается как строка трекинга, а «Отправлен в г. Казань · Казань, СЦ
 * Столбище» — как повтор одного и того же дважды.
 */
const SORT_CENTERS = {
  'москва': 'СЦ Внуково',
  'санкт петербург': 'СЦ Шушары',
  'нижний новгород': 'СЦ Дзержинск',
  'казань': 'СЦ Столбище',
  'екатеринбург': 'СЦ Кольцово',
  'новосибирск': 'СЦ Толмачёво',
  'ростов на дону': 'СЦ Аксай'
};
// Город плюс его сортировочный центр, если он нам известен. Ключ нормализуется
// тем же `Z.words`, что и хабы: «Санкт-Петербург» и «санкт петербург» — одно.
function warehouseAt(city) {
  const name = String(city || '').trim();
  if (!name) return '';
  const sc = SORT_CENTERS[Z.words(name).join(' ')];
...
function cdekSteps(from, hubs, to, mode) {
  const steps = [
    ...
    { kind: 'warehouse', title: 'Принят на склад', place: warehouseAt(from), span: 0.5 },
    { kind: 'warehouse', title: 'Выдан на отправку', place: warehouseAt(from), span: 0.35 },
    { kind: 'transit', title: 'Сдан перевозчику', place: from, span: 0.3 }
  ];
  for (const hub of hubs) {
    steps.push({ kind: 'transit', title: 'Отправлен в г. ' + hub, place: hub, span: 1.4 });
    steps.push({ kind: 'arrive', title: 'Встречен в г. ' + hub, place: hub, span: 0.7 });
    steps.push({ kind: 'warehouse', title: 'Принят на склад', place: warehouseAt(hub), span: 0.4 });
    steps.push({ kind: 'warehouse', title: 'Выдан на отправку', place: warehouseAt(hub), span: 0.4 });
  }
  ...
  steps.push({ kind: 'warehouse', title: 'Принят на склад доставки', place: warehouseAt(to), span: 0.5 });
  if (mode === 'courier') {
...
function ozonSteps(from, hubs, to, mode) {
  const steps = [
    ...
    { kind: 'warehouse', title: 'Заказ собран на складе', place: warehouseAt(from), span: 0.5 },
    { kind: 'transit', title: 'Передан в доставку', place: from, span: 0.35 }
  ];
  for (const hub of hubs) {
    steps.push({ kind: 'transit', title: 'В пути в г. ' + hub, place: hub, span: 1.4 });
    steps.push({ kind: 'arrive', title: 'Прибыл в сортировочный центр', place: warehouseAt(hub), span: 0.7 });
    steps.push({ kind: 'warehouse', title: 'Обработан', place: warehouseAt(hub), span: 0.4 });
    steps.push({ kind: 'warehouse', title: 'Отправлен дальше', place: warehouseAt(hub), span: 0.4 });
  }
...
/* Сборка маршрута.
 *
 * `days` — сколько всего идёт посылка ОТ ПЕРЕДАЧИ ПЕРЕВОЗЧИКУ. Число задаёт
 * админ; по умолчанию берётся верхняя граница транзита из нашей же сетки
 * сроков (`lib/delivery-days.js`) — см. `defaultDays` ниже. Обещать в
 * отслеживании быстрее, чем обещали при покупке, нельзя.
 */
function build(opts) {
  opts = opts || {};
  ...
  const steps = template.map((step, i) => {
     ...
     *
     * Зазор между соседними событиями — не меньше `MIN_GAP`, и это не придирка
     * к красоте. Чем подробнее маршрут, тем плотнее события ложатся в дневное
     * окно (ночные `workHour` переносит в утро), и на длинном пути соседние
     * строки сходились до ПЯТИ МИНУТ: «Принят на склад 10:02 · Выдан на отправку
     * 10:07» — так склады не работают, и лента сразу выдаёт генератор. Дрожание
     * добавляется поверх, чтобы зазор не был одинаковым у всех пар. */
    const gap = MIN_GAP + Math.round(rnd() * MIN_GAP);
    const fixed = at < prev + gap ? prev + gap : at;
    prev = fixed;
...
/* Сколько дней идёт маршрут по умолчанию — ВЕРХНЯЯ граница транзита из той же
 * сетки, что показывает оформление (`lib/delivery-days.js`).
 *
 * Именно транзит, а не полный срок покупателя: лента отслеживания начинается с
 * «Заказ создан» У ПЕРЕВОЗЧИКА, то есть в тот момент, когда посылку ему уже
 * отдали. Сборка магазина случилась до первой строки маршрута, и прибавлять её
 * к длине пути значило бы растянуть дорогу на дни, которые посылка простояла у
 * нас на столе.
 */
function defaultDays(carrier, mode, zone) {
  const range = SHIPDAYS.transitFor(carrier, mode, zone);
...
function normalize(data) {
  ...
  return {
    ...
    /* Новое отслеживание видно покупателю по умолчанию. Явное скрытие
     * менеджером сохраняем; старые отправления без поля тоже видны. */
...
function stateText(shipment, now) {
  ...
  /* Ни одного пройденного шага — маршрут ещё не начался: так выглядит
   * отправление, собранное заранее (например, автоматически в момент оплаты) на
   * день передачи перевозчику. «В пути» здесь было бы неправдой ровно в том
   * месте, где менеджер решает, пора ли отдавать посылку. */
  if (!v.current) return 'Ожидает отправки';
...
function shortState(shipment, now) {
  ...
  // Маршрут собран заранее и ещё не начался — см. `stateText` выше.
  if (!v.current) return 'ожидает отправки';
  return 'в пути';
}
