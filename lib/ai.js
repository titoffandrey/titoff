 ...
 * получаса — ровно на паузы в разговоре, когда покупатель отвлёкся.
 *
...
const EXTRAS = {
  ...
  prompt_cache_options: { ttl: CACHE_TTL },
  stream_options: { include_usage: true }
};
const EXTRA_NAMES = Object.keys(EXTRAS);
...
function bodyOf(model, messages, shape) {
  ...
  for (const name of EXTRA_NAMES) if (shape.off.indexOf(name) < 0) body[name] = EXTRAS[name];
  return body;
}
...
function logUsage(usage) {
  ...
  const answer = Math.max(0, (usage.completion_tokens || 0) - reasoning);
  console.log('ИИ: вход ' + (usage.prompt_tokens || 0) + ' (из кэша ' + cached + '), ответ ' + answer
