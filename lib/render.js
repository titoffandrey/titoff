...
function layout(settings, opts) {
  ...
  /* Значки у строк меню есть ТОЛЬКО здесь, и это не исключение из правила, а
   * само правило: у категорий каталога («iPhone», «Vision», «Аксессуары») глиф
   * пришлось бы выдумывать, а у мессенджера он существует — знак самого
   * сервиса. Строки стоят под волосяной линией отдельной группой, поэтому
   * значок у них читается не как поломка ряда, а как признак этой группы.
   * Ведут себя знак и подпись так же, как в подвале: цвет строки в покое,
   * фирменный на наведении, — и приезжают теми же `msg-*` (`nav-msg` даёт им
   * зазор и перебивает наведение самой строки). */
  const navMessengers = [
    settings.contactTelegram
      ? `<a class="nav-item nav-msg msg-tg" href="https://t.me/${esc(tgUser(settings))}" target="_blank" rel="noopener">${tgMark()}<span>Telegram</span></a>` : '',
    waUrl
      ? `<a class="nav-item nav-msg msg-wa" href="${esc(waUrl)}" target="_blank" rel="noopener">${waMark()}<span>WhatsApp</span></a>` : ''
  ].filter(Boolean).join('');
  const navContacts = navMessengers
    ? `<span class="nav-sep" aria-hidden="true"></span>${navMessengers}` : '';
