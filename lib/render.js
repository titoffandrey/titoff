...
function orderDelivery(order, opts) {
  ...
  // Код пункта выдачи стоит рядом с адресом, а не вместо него: накладную
  // менеджер оформляет по коду, а найти пункт глазами проще по улице.
  const code = o.pickupCode ? `<b class="o-pvz">${esc(o.pickupCode)}</b>` : '';
  const line = [esc(way), cost, [code, esc(o.address)].filter(Boolean).join(' ')].filter(Boolean).join(' · ');
