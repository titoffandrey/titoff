...
function orderPayMethod(order) {
  ...
  return esc(method + extra) + (via ? `<div class="muted small o-via">${esc(via)}</div>` : '');
}
...
function providerStats(list) {
  ...
  const row = id => {
    ...
    if (!rows.has(key)) {
      rows.set(key, { id: key, name: PAYMENTS.nameOf(key), tries: 0, issued: 0, paid: 0, failed: 0, errors: {} });
    }
    return rows.get(key);
  };
  ...
  for (const o of list || []) {
    ...
    for (const a of attempts) {
      ...
      r.tries++;
      if (a.invoiceId && a.requisite) r.issued++;
...
function providerStatsBar(rows) {
  ...
  const body = list.map(r => {
    ...
    return `<tr><td><b>${esc(r.name)}</b></td>`
      + `<td>${statNumber(r.tries)}</td>`
      + `<td>${statNumber(r.issued)}</td>`
      + `<td>${statNumber(r.paid)}</td>`
      + `<td>${r.failed ? `${statNumber(r.failed)} <span class="muted">(${share}%)</span>` : '<span class="muted">—</span>'}</td>`
      + `<td class="muted small">${why || '—'}</td></tr>`;
  }).join('');
...
<header class="site-header">
  <div class="container header-row">
    ...
    <input type="checkbox" id="search-open" class="search-switch"${opts.q ? ' checked' : ''}>
    <label class="icon-btn search-toggle" for="search-open" aria-label="Поиск">
  ...
  <div class="container footer-grid">
    ...
    ${(settings.contactTelegram || settings.contactPhone) ? `<div class="footer-contacts">
      ${settings.contactTelegram ? `<div class="foot-tg">
        ...
        <span class="foot-tg-user">${esc(settings.contactTelegram)}</span>
      </div>` : ''}
