...
function providerStatsBar(rows) {
  ...
  return `<div class="a-panel"><h2>Кассы</h2>
    <p class="muted small">Считается по запросам реквизитов, а не по заказам: у одного заказа их бывает несколько. Покупатель отказа не видит — если первая касса не дала реквизиты, вторую мы спрашиваем сами.</p>
