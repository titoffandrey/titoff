...
function payLive(pay, now) {
  ...
  const until = Number(pay.expiresAt) || 0;
 ...
 * дальше двигает его только ответ кассы — то есть опрос со страницы оплаты.
 * Ушедший покупатель этот опрос не делает, и в панели заказ висел «ждём оплату»
 * хоть месяц. Отсюда три разных случая под одним статусом:
 *
...
function payView(order, now) {
  ...
  if (state === 'pending') {
    ...
    if (pay.invoiceId && pay.requisite) {
      return { tone: 'off', label: PAY_STATES.expired[1], when: payMoment(pay.expiresAt), whenLabel: 'срок вышел' };
    }
