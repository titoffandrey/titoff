...
// Выполнение заказа и состояние оплаты показываются раздельно. «Оплачен» —
// ответ кассы, «Передан в доставку» — действие магазина; смешивать их в одном
// статусе означало бы потерять один из двух фактов.
const ORDER_WORKFLOW = {
  new: ['Новый', 'new'],
  confirmed: ['Подтверждён', 'confirmed'],
  processing: ['Собирается', 'processing'],
  shipped: ['Передан в доставку', 'shipped'],
  done: ['Выполнен', 'done'],
  cancelled: ['Отменён', 'cancelled']
};
function orderWorkflowLabel(status) {
  return (ORDER_WORKFLOW[String(status || '')] || ORDER_WORKFLOW.new)[0];
}
function orderWorkflowStatus(order) {
  const state = String(order && order.status || 'new');
  const item = ORDER_WORKFLOW[state] || ORDER_WORKFLOW.new;
  return `<span class="order-flow order-flow-${item[1]}">${esc(item[0])}</span>`;
}
...
module.exports = {
  esc, money, formatDate, mskDate, mskNow, mskTime, mskDateTime, mskSameDay, mskInputValue, parseMskInput, MSK_ZONE, orderStatus, orderPayMethod, orderTone, orderRowClass, orderWorkflowLabel, orderWorkflowStatus, ORDER_WORKFLOW, orderStats, orderStatsBar, ORDER_TONES,
