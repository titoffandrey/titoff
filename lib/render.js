...
function productCard(p, settings, db, opts) {
  ...
  const compareHtml = cmp
    ? `<span class="price-was"><span class="old-price">${money(cmp, settings)}</span>${hot && inStock ? `<span class="save">−${pct}%</span>` : ''}</span>`
    : '';
    ...
    <section class="store-hero">
      <div class="container store-hero-inner">
        <div class="store-hero-copy">
          <h1>${APPLE_MARK}${esc(bindShortWords(settings.tagline || 'Выбирайте нужную модель — мы поможем с остальным.'))}</h1>
        </div>
      </div>
      ${BENEFIT_SPRITE}
