...
function payPage(settings, order, opts) {
  ...
  if (ownPaid && state !== 'paid') {
    ...
    card = `<div class="co-block pay-invoice">
      ...
      <div class="pay-req">
        ${phoneText ? payRow('Телефон для СБП', phoneText, true, own.phone.replace(/\D+/g, '')) : ''}
        ${cardText ? payRow('Номер карты', cardText, true, own.card) : ''}
        ${payRow('Банк получателя', own.bank, false)}
        ${payRow('Получатель', own.owner, false)}
      </div>
      <p class="pay-hint"><b class="pay-exact">Переведите точную сумму</b>Так менеджер найдёт ваш перевод среди других и быстрее подтвердит заказ. В сообщении к переводу ничего писать не нужно.</p>
