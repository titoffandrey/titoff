...
// Разрешение заменить реквизиты связано с конкретным живым invoice. Значение
// отдаётся странице и возвращается POST-ом; старая вкладка после появления
// нового счёта перестаёт совпадать.
function payReplaceInvoice(pay, choose, now) {
  return choose && payLive(pay, now) ? String(pay.invoiceId || '') : '';
}
...
function payCurrencyChoice(order, codes, current, amounts, settings) {
  ...
  const href = code => `/pay/${encodeURIComponent(order.id)}?choose=1&amp;currency=${encodeURIComponent(code)}`;
...
function payPage(settings, order, opts) {
  ...
  const live = !opts.choose && payLive(pay);
  const replacingLive = !!opts.choose && payLive(pay);
  ...
  if (state === 'paid') {
    ...
    card = `<div class="co-block pay-invoice">
      ...
      <p class="pay-timer" id="pay-timer"${liveUntil ? '' : ' hidden'}>Реквизиты действительны ещё <b id="pay-left">—</b></p>
    ...
    card = `<div class="co-block pay-choice">
      ...
      ${replacingLive ? '<p class="pay-why">Прежний счёт ещё действует и не отменяется у кассы. Не оплачивайте оба: после выпуска нового используйте только последние показанные реквизиты.</p>' : ''}
  ...
  const body = `<div class="container checkout-page pay-page" id="pay-page"
    ...
    data-replace-invoice="${esc(payReplaceInvoice(pay, opts.choose))}"
    ...
    <div class="legal-card">
      ...
      <section class="legal-section" id="purposes">
        ...
        <div class="legal-table-wrap"><table class="legal-table purposes-table"><thead><tr><th>Цель</th><th>Данные</th><th>Основание</th></tr></thead><tbody>
          ...
          <tr><td>Корзина и восстановление оформления</td><td>Локальные идентификаторы, выбранные товары, имя, телефон, контакт и адрес</td><td>Запрос посетителя на использование функций сайта</td></tr>
      ...
      <section class="legal-section" id="cookies">
        ...
        <div class="legal-table-wrap"><table class="legal-table cookies-table"><thead><tr><th>Название</th><th>Тип</th><th>Назначение</th><th>Срок</th></tr></thead><tbody>
          ...
          <tr><td><code>checkout_v1</code></td><td>localStorage</td><td>Восстанавливает имя, фамилию, телефон, дополнительный контакт и адрес после возврата к оформлению.</td><td>До 7 дней после последнего сохранения; просроченная запись удаляется при следующем открытии оформления либо при очистке данных сайта</td></tr>
...
module.exports = {
  ...
  payLive, payUntil, payReplaceInvoice, payView, payLeftText,
