...
function orderStatus(order) {
  ...
  // выглядели по-своему.
  return `<span class="pay-tag pay-${view[0]}"><i></i>${view[1]}</span>`
...
function payPage(settings, order, opts) {
  ...
  const payAmount = (live && pay && pay.amount) ? Number(pay.amount) : Number(opts.amount || order.total);
  const payCur = live ? (PAY.currencyCode(pay && pay.currency) || PAY.BASE) : currency;
...
module.exports = {
  ...
  editSwitch, adminIcon, orderNo, orderDelivery, orderClient, orderItems, clientMarks, visitorHref, stars, startPrice, sellable, layout, homePage, notFoundPage, productPage, checkoutPage, payPage, privacyPage,
