...
// первую порцию, а остальные догружаются через GET /api/reviews той же разметкой.
const REVIEWS_PER_PAGE = 8;
const REVIEW_SORTS = ['helpful', 'new'];

function reviewSortMode(sort) { return REVIEW_SORTS.includes(String(sort)) ? String(sort) : 'helpful'; }

function sortReviews(list, sort) {
  const arr = list.slice();
  if (reviewSortMode(sort) === 'new') return arr.sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
  // «Сначала полезные»: выше оценка, при равной — более развёрнутый текст, потом свежесть.
  return arr.sort((a, b) =>
...
function sellable(p) {
  ...
  return choicesOk && (!storages.length || storages.some(variantInStock));
}
...
function productCard(p, settings, db, opts) {
  ...
  <article class="card${hot ? ' card-hot' : ''}">
    ...
    <div class="card-add">${!inStock
      ...
      : ((p.colors || []).length || (p.storages || []).length || (p.bands || []).length)
...
function productPage(settings, db, p, site, opts) {
  ...
  const pageHref = n => `${reviewsBase}?rsort=${slice.sort}&amp;rpage=${n}#reviews`;
  const rest = slice.total - slice.to;
  // Ссылки настоящие: без JS это обычная постраничная навигация, с JS — догрузка.
  const pager = slice.pages > 1 ? `
      <div class="reviews-pager" id="reviews-pager" data-product="${esc(p.id)}" data-sort="${slice.sort}" data-page="${slice.page}" data-pages="${slice.pages}" data-total="${slice.total}">
        ${slice.page > 1 ? `<a class="btn btn-sm reviews-prev" href="${pageHref(slice.page - 1)}" rel="prev">Предыдущие</a>` : ''}
        ${slice.page < slice.pages ? `<a class="btn reviews-more" href="${pageHref(slice.page + 1)}" rel="next">Показать ещё ${Math.min(REVIEWS_PER_PAGE, rest)}</a>` : ''}
  ...
  <div class="container">
    ...
    <section class="section" id="reviews">
      ...
      ${breakdown}
      ${r.count ? `<div class="reviews-toolbar"><span class="muted small">Сортировка:</span>${
        [['helpful', 'Сначала полезные'], ['new', 'Сначала новые']].map(([mode, label]) =>
...
module.exports = {
  ...
  assetV, brandFields, cssColor, sellable, colorAvailable, accessibleFields, PRIVACY_VERSION,
  reviewsSlice, reviewCard, reviewsRangeText, REVIEWS_PER_PAGE
};
