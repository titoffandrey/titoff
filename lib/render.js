...
// Отзывы с вложениями уходят в КОНЕЦ ленты, а обычные идут первыми — и те и
// другие строго по дате внутри своей группы.
//
// Главное здесь — дата: сортировка называется «Новые», и на первой странице
// обязаны стоять самые свежие отзывы подряд, без исключений. Любая перестановка
// поверх даты это ломает, и её сразу видно: отзыв за 8 августа над отзывом за
// 18-е читается как сбой витрины. Ровно так ломала ленту прежняя раскладка
// «видео на каждое третье место» — она тасовала весь список.
//
// Поэтому вложения не «поднимаются», а опускаются: группа с фото и видео стоит
// целиком после текстовых, и в ней свой порядок по дате. Внутри группы видео от
// фото не отделяется — иначе дата поехала бы уже там.
function mediaLast(list) {
  const plain = list.filter(rv => !hasMedia(rv));
  if (!plain.length || plain.length === list.length) return list;
  return plain.concat(list.filter(hasMedia));
}
...
function sortReviews(list, sort) {
  ...
  // Вложения опускаются только в порядке по умолчанию: выбрав «с высокой
  // оценкой», покупатель просил именно оценки, и переставлять их нельзя.
  return mediaLast(arr.sort(fresh));
}
      ...
      <div class="review-form-wrap">
        ...
        <form id="review-form" enctype="multipart/form-data">
          ...
          ${REVIEW_PHOTOS ? `<div class="field">
            ...
            <span class="field-note">До ${REVIEW_PHOTOS_MAX} снимков.</span>
          </div>` : ''}
