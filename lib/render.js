...
function tabBar(settings, opts) {
  ...
  const item = (key, label, attrs, badge, extra) => {
    const on = active === key;
    const tag = /href=/.test(attrs) ? 'a' : 'button';
    return `<${tag} class="tabbar-item${on ? ' is-active' : ''}${extra || ''}" ${attrs}${on ? ' aria-current="page"' : ''}>`
  ...
  ${/* Точка «вошёл» у кабинета — та же, что у значка в шапке (`is-in`): на
        телефоне значка в шапке нет, вкладка его заменяет целиком. */''}
  ${CUSTOMERS.enabled(settings) ? item('account', 'Кабинет', 'href="/account"', '', opts.customer ? ' is-in' : '') : ''}
...
<header class="site-header">
  <div class="container header-row">
    ...
    <div class="header-actions">
      ...
      ${CUSTOMERS.enabled(settings) ? `<a class="icon-btn account-btn${opts.customer ? ' is-in' : ''}" href="/account" aria-label="Личный кабинет" title="Личный кабинет">${ACCOUNT_ICON}</a>` : ''}
