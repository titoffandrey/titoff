...
// Оригинальные линейные иконки (в стиле Apple), подбираются по ключу характеристики.
const SPEC_ICON_MAP = [
  [/чип|процесс|нейро|gpu|graphics/i, '<rect x="5" y="5" width="14" height="14" rx="2"/><rect x="9" y="9" width="6" height="6" rx="1"/><path d="M9 2v3M15 2v3M9 19v3M15 19v3M2 9h3M2 15h3M19 9h3M19 15h3"/>'],
  [/экран|дисплей|яркост|разрешен|promotion|retina/i, '<rect x="3" y="4" width="18" height="13" rx="2"/><path d="M8 21h8M12 17v4"/>'],
  [/камер|фронтал|зум|селфи|фото|видео/i, '<path d="M4 8h3l1.4-2h7.2L17 8h3a1 1 0 0 1 1 1v9a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1z"/><circle cx="12" cy="13" r="3.2"/>'],
  [/аккум|автоном|батаре|заряд|питани/i, '<rect x="2" y="7" width="16" height="10" rx="2"/><path d="M22 11v2M6 12h6"/>'],
  [/разъ|порт|usb|thunderbolt|подключ|hdmi/i, '<rect x="6" y="9" width="12" height="6" rx="3"/><path d="M9 9V7h6v2"/>'],
  [/озу|память|накопит|ssd|хранил|гб|тб/i, '<rect x="3" y="6" width="18" height="12" rx="2"/><path d="M7 6v12M11 6v12M15 6v12"/>'],
  [/материал|корпус|титан|алюмин|стекл|керамик/i, '<path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/>'],
  [/защит|водо|пыл|ip6|ip5|прочн/i, '<path d="M12 3l7 3v5c0 4.5-3 7.6-7 9-4-1.4-7-4.5-7-9V6l7-3z"/><path d="M9 12l2 2 4-4"/>'],
  [/вес|толщин|габарит|размер|диагонал/i, '<path d="M3 8h18M6 5L3 8l3 3M18 5l3 3-3 3"/><rect x="6" y="13" width="12" height="7" rx="1"/>'],
  [/аудио|звук|шумоподавл|динамик|наушник|акустик/i, '<path d="M4 9v6h4l5 4V5L8 9H4z"/><path d="M17 8.5a5 5 0 0 1 0 7"/>'],
  [/датчик|сенсор|gps|компас|кнопк/i, '<circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/>']
];
const SPEC_ICON_FALLBACK = '<circle cx="12" cy="12" r="9"/><path d="M8.4 12.4l2.6 2.6 4.8-5.4"/>';
function specIcon(key) {
  const inner = (SPEC_ICON_MAP.find(([re]) => re.test(key)) || [null, SPEC_ICON_FALLBACK])[1];
...
function specHighlights(p) {
  ...
  const items = String(p.specs).split('\n').map(l => l.trim()).filter(Boolean).map(line => {
    ...
    return `<div class="spec-item"><span class="spec-ico">${specIcon(key || val)}</span><div class="spec-meta">${key ? `<div class="spec-key">${esc(key)}</div>` : ''}<div class="spec-val">${esc(val)}</div></div></div>`;
  }).join('');
  return `<section class="section"><h2>Характеристики</h2><div class="spec-grid">${items}</div></section>`;
}
...
function tgUser(settings) { return String(settings.contactTelegram || '').replace(/^@/, ''); }
