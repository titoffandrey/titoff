...
// Товар вообще можно купить? Нужен хотя бы один доступный цвет и один вариант памяти.
function sellable(p) {
  if (!p || !p.inStock) return false;
  const colors = p.colors || [], storages = p.storages || [];
  return (!colors.length || colors.some(variantInStock)) && (!storages.length || storages.some(variantInStock));
}
...
function checkoutPage(settings, opts) {
  ...
  const body = `<div class="container checkout-page" id="checkout-page">
    <nav class="breadcrumb"><a href="/">Главная</a> / <span>Оформление заказа</span></nav>
