...
/* Значки мессенджеров в контактах подвала — ЗАЛИТЫЕ фирменные кружки, а не
 * контуры и не одноцветные глифы. Три причины, и все видны глазами:
 * в 20 px контурная фигура не читается вовсе; пара «залитый самолётик +
 * контурный пузырь» выглядит значками из двух разных наборов; а перекрашенный
 * в один цвет логотип перестаёт быть логотипом — сервис узнают именно по
 * цвету. Белые детали нарисованы поверх заливки, поэтому значок самодостаточен
 * и не зависит от фона кнопки (он у неё меняется на наведении).
 * Цвета фирменные, а не притемнённые: правило контраста 4.5:1 в проекте про
 * ТЕКСТ, а подпись рядом остаётся тёмной (см. «Цены и подписи витрины»). */
function tgMark() {
  return '<svg class="msg-ico" viewBox="0 0 24 24" aria-hidden="true">'
    + '<circle cx="12" cy="12" r="11" fill="#229ED9"/>'
    + '<path fill="#fff" transform="translate(12 12.35) scale(.63) translate(-12 -12)" d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71l-4.15-3.06-1.99 1.94c-.23.23-.42.42-.83.42z"/>'
    + '</svg>';
}
function waMark() {
  return '<svg class="msg-ico" viewBox="0 0 24 24" aria-hidden="true">'
    + '<path fill="#25D366" d="M12 1.6a10.4 10.4 0 0 0-8.86 15.84L1.72 22.1a.62.62 0 0 0 .78.78l4.74-1.45A10.4 10.4 0 1 0 12 1.6Z"/>'
    + '<path fill="#fff" d="M9.06 6.93c-.2-.45-.42-.46-.61-.47l-.53-.01c-.18 0-.48.07-.73.34-.25.27-.96.94-.96 2.28s.98 2.64 1.12 2.82c.14.19 1.9 3.04 4.7 4.14 2.33.92 2.8.74 3.31.69.5-.05 1.62-.66 1.85-1.3.23-.65.23-1.2.16-1.32-.07-.11-.25-.18-.53-.32-.28-.14-1.63-.81-1.88-.9-.25-.09-.44-.14-.62.14-.18.27-.71.9-.87 1.08-.16.18-.32.21-.6.07-.28-.14-1.17-.43-2.23-1.38-.82-.73-1.38-1.63-1.54-1.9-.16-.28-.02-.43.12-.56.13-.13.28-.33.42-.49.14-.16.18-.28.28-.46.09-.19.05-.35-.02-.49-.07-.14-.61-1.49-.84-2.04Z"/>'
    + '</svg>';
}
