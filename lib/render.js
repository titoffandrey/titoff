...
function orderClient(order, opts) {
  ...
  return `<div class="o-who">${head}</div>${orderDelivery(o)}${marks}${orderTech(o, opts)}`;
}
...
function checkoutPage(settings, opts) {
  ...
  // Пределы одной покупки приходят от сервера, а не зашиты в app.js: по ним
  // витрина гасит кнопку и объясняет причину, а отказ всё равно за сервером —
  // свои числа в скрипте разъехались бы с его.
  const limits = ` data-order-min="${CROCO.MIN_TOTAL}" data-order-max="${CROCO.MAX_TOTAL}"`;
  ...
  const body = `<div class="container checkout-page" id="checkout-page"${pay}${limits}${delivery}>
    ${DLOGO.sprite()}
...
module.exports = {
  esc, money, formatDate, paymentBadge, orderNo, orderDelivery, orderClient, orderItems, clientMarks, visitorHref, stars, layout, homePage, notFoundPage, productPage, checkoutPage, payPage, privacyPage,
