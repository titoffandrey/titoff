...
const PAY_MARKS = {
  // «Мир» и СБП — официальные знаки с Викисклада (public domain), приведены к
  // currentColor. viewBox обрезан по фигуре, поэтому высота в CSS = высота знака.
  ...
  // У СБП взят только сам знак, без слова и подписи: в ряду он стоит рядом с
  // подписью-невидимкой, а слово мельче 7 px всё равно не прочитать. Сегменты
  // знака стыкуются вплотную и в один цвет слились бы в пятно, поэтому в CSS у
  // них обводка цветом подвала — она возвращает фирменные зазоры.
  ...
  // Знак Mastercard — фирменная геометрия: круги радиуса 8 на расстоянии 10
  // и линза их пересечения (крайние точки x=13, y=8±6.245). В одноцветной версии
  // круги приглушены, а линза остаётся плотной — знак читается без цветов.
  mastercard: '<svg viewBox="0 0 26 16" fill="currentColor" aria-hidden="true"><circle cx="8" cy="8" r="8" opacity=".55"/><circle cx="18" cy="8" r="8" opacity=".55"/><path d="M13 1.755A8 8 0 0 1 13 14.245A8 8 0 0 1 13 1.755z"/></svg>',
...
const PAY_METHODS = `<div class="footer-pay" role="list" aria-label="Способы оплаты">
        ...
        <span class="pay pay-sbp" role="listitem">${PAY_MARKS.sbp}<span class="sr-only">СБП</span></span>
      </div>`;
