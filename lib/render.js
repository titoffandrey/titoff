...
function tabBar(settings, opts) {
  ...
  const chatBadge = `<span class="tabbar-badge" data-chat-badge aria-hidden="true"${wait ? '' : ' hidden'}>${wait ? esc(wait > 9 ? '9+' : String(wait)) : '0'}</span>`;
  ...
  ${/* Чат — кнопка, а не ссылка: окно открывает тот же скрипт, что и круглая
        кнопка в углу (data-chat-open ловит public/chat.js). Чат выключен —
        вкладки нет, как нет и самой кнопки. */''}
