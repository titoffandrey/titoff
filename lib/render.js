...
function productPage(settings, db, p, site, opts) {
  ...
  const reviewItems = reviews.length ? reviews.map(rv => `
    ...
    </article>`).join('') : `<p class="muted">Отзывов пока нет. Оставьте первый!</p>`;
  ...
  <div class="container">
    ...
    <div class="product">
      ...
      <div class="product-info">
        ...
        ${ratingSummary}
        ${dealBanner}
        <div class="product-price${hot ? ' price-hot' : ''}">${money(eff, settings)} ${discount}</div>
        ...
        <div class="buy-row">
          ...
          <button class="btn btn-primary btn-lg add-to-cart" data-id="${p.id}" data-name="${esc(p.name)}"
            data-price="${eff}" data-qty-source ${p.inStock ? '' : 'disabled'}>
    ...
    <section class="section" id="reviews">
      <div class="section-head"><h2>Отзывы ${r.count ? `(${r.count})` : ''}</h2>${r.count ? `<div>${stars(r.avg)} <b>${r.avg}</b></div>` : ''}</div>
...
module.exports = { esc, money, formatDate, stars, layout, homePage, productPage, imageMarkup, placeholderSvg, assetV };
