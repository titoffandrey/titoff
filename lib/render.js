...
function checkoutPage(settings, opts) {
  ...
  const account = !acc ? '' : ` data-account="1"${acc.auto ? ' data-account-auto="1"' : ''}`
    + (acc.email ? ` data-account-email="${esc(acc.email)}" data-account-name="${esc(acc.name || '')}" data-account-phone="${esc(acc.phone || '')}"`
...
function accountNoteText(note) {
  if (!note || !note.email) return '';
  if (note.created) return `Мы создали для вас личный кабинет: пароль отправили на ${note.email}. Там будут все ваши заказы и отправления.`;
  if (note.exists) return `На ${note.email} уже есть личный кабинет — войдите, и этот заказ появится там.`;
  return '';
}
...
function accountAuthPage(settings, opts) {
  ...
  const emailField = (auto) => `<div class="field"><label for="acc-email">E-mail</label>
      <input type="email" id="acc-email" name="email" value="${esc(email)}" maxlength="120" inputmode="email" autocomplete="${auto}" required autofocus></div>`;
  ...
  if (mode === 'login') {
    ...
    body = mailOn ? `<p class="acc-auth-copy">Пришлём на почту ссылку — по ней зададите новый пароль. Ссылка действует час.</p>
    ...
    body = `<p class="acc-auth-copy">Если на ${esc(email)} есть кабинет, ссылка для нового пароля уже в пути. Проверьте и папку «Спам».</p>`;
    foot = `<p class="acc-links"><a href="/account/login">Вернуться ко входу</a></p>`;
  } else {
    title = 'Новый пароль';
...
function accountPage(settings, opts) {
  ...
  const html = `<div class="container acc-page">
    <header class="acc-head">
      <h1>Личный кабинет</h1><p class="acc-sub">${esc(c.email)}</p>
    </header>
    ${accountFlash(opts)}
    <div class="acc-grid">
      ...
      <section class="acc-card" id="profile" aria-labelledby="acc-profile-title">
        ...
        <form method="post" action="/account/profile" class="acc-form">
          ...
          <p class="field-hint">Подставляются в оформление заказа, чтобы не набирать заново.</p>
