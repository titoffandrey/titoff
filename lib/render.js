...
function payView(order, now) {
  ...
  if (order && order.payMode === 'own' && !order.payment) {
    return { tone: 'wait', label: 'ждём перевод' };
  }
 ...
 *
 * У кассы срок — это жизнь чужой карты: истёк, и платить по ней нельзя. Свои
 * реквизиты не сгорают, но окно всё равно нужно, и по делу: по его окончании
 * страница перечитывает настройки, и если владелец сменил карту или телефон,
 * покупатель увидит новые — а не переведёт деньги на снятый реквизит.
 *
 * Считается окно ОТ ЗАКАЗА и катится само: ничего не храним, а покупатель,
 * обновивший страницу, видит тот же остаток, а не новые тридцать минут.
 * Покупателю про перечитывание настроек знать незачем — для него это просто
 * время на оплату.
 */
const OWN_PAY_WINDOW = 30 * 60 * 1000;
function ownPayUntil(order, now) {
  const at = now || Date.now();
  const start = Number(order && order.createdAt) || 0;
  if (!start || start > at) return at + OWN_PAY_WINDOW;
  return start + (Math.floor((at - start) / OWN_PAY_WINDOW) + 1) * OWN_PAY_WINDOW;
}
...
function chatWidget(settings, waiting) {
  ...
  return `${CHAT_TICK_SPRITE}<div class="chat-widget" id="chat-widget" data-mode="ai" data-ai-name="${esc(CHAT.SPEAKERS.ai)}"${wait ? ` data-chat-waiting="${wait}"` : ''}>
...
function payPage(settings, order, opts) {
  ...
  const ownUntil = own && !ownPaid && state !== 'paid' ? ownPayUntil(order) : 0;
  ...
  } else if (own && state !== 'paid' && !orderArchived) {
