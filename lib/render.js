...
// Сетка сроков доставки: «О компании» рассказывает про отправку теми же числами,
// какими считает оформление. Цикла нет — `lib/delivery-days.js` рендер не
// подключает, и через `lib/tracking.js` он и так уже здесь.
...
function infoMark() {
  return '<svg class="nav-glyph" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="8.6"/><path d="M12 11.4v4.9"/><path d="M12 7.9h.01"/></svg>';
}
  ...
  <div class="container footer-grid">
    <div class="footer-cols">
      ...
      <div class="footer-col footer-contacts">
        ...
        ${/* Адрес — с булавкой вместо подписи «Офлайн-магазин»: слово повторяло
             то, что и так говорит сам адрес. Значок озвучить нечем, поэтому имя
             строке даёт спрятанное слово рядом. */''}
        ...
        ${/* Часы работы стоят рядом с телефоном: они отвечают на вопрос «а
             сейчас ответят?», который задают ровно перед звонком. */''}
        ...
        ${footMessengers ? `<div class="msg-links">${footMessengers}</div>` : ''}
      </div>
    </div>
...
/* ------------------------------ О компании ----------------------------------
 *
 * Страница отвечает на три вопроса разом: кто продаёт, как здесь всё устроено и
 * куда прийти. До неё эти ответы лежали по углам витрины — условия в правовых
 * документах, контакты мелким шрифтом в подвале, адрес одной строкой без карты,
 * — и покупателю, который решает, доверять ли магазину, собирать их приходилось
 * самому.
 *
 * ГЛАВНОЕ ПРАВИЛО: ни одного факта руками. Перевозчики берутся из
 * `lib/delivery.js`, сроки — из той же сетки, что показывает оформление, режим
 * оплаты — у router'а касс, контакты и реквизиты — из настроек. Переписанные
 * словами, они разошлись бы с витриной ровно так же, как разъехался бы каталог
 * в промпте консультанта, а увидеть это можно только глазами.
 */

/* Карта — ЯНДЕКС, и грузится она ТОЛЬКО ПО НАЖАТИЮ.
 *
 * Встроенный чужой фрейм на странице — это запрос к чужому серверу у каждого,
 * кто её открыл: Яндекс получал бы адрес и время визита всякого посетителя, а
 * магазин, который прячет домен за Tor и не пускает с витрины ни одного лишнего
 * запроса, так делать не вправе. Поэтому сервер рисует только адрес и обычную
 * ссылку «Открыть в Яндекс.Картах» (она работает и без скриптов), а сам фрейм
 * вставляет `initStoreMap()` в public/app.js — после того, как покупатель сам
 * попросил карту. До нажатия наружу не уходит ничего.
 *
 * `frame-src` под это в CSP открыт узко — только яндексовый хост и только для
 * фрейма: ни скриптов, ни картинок, ни соединений оттуда мы не разрешаем.
 */
const MAP_HOST = 'https://yandex.ru';
/* Координаты офлайн-точки, если владелец их задал. Формат тот, в котором их
 * копируют из Яндекс.Карт: «55.751244, 37.618423» — сначала широта. Дробная
 * часть только через точку: с запятой пара «55,75, 37,61» неотличима от четырёх
 * чисел, и разбирать её значило бы гадать. Мусор — это «координат нет», и карта
 * тогда строится по самому адресу. */
...
/* Два адреса на одну точку: `frame` — виджет, `link` — обычные Яндекс.Карты.
 * Собираются из одного набора параметров, иначе кнопка и карта показывали бы
 * разные места. Нет адреса — нет и карты: точка на карте без адреса под ней
 * ничего не говорит. */
function storeMap(settings) {
  const address = String((settings || {}).storeAddress || '').trim();
  if (!address) return null;
  const pt = storePoint(settings);
  const q = pt
    ? `ll=${pt.lon}%2C${pt.lat}&z=17&pt=${pt.lon}%2C${pt.lat}%2Cpm2rdm`
    : `text=${encodeURIComponent(address)}&z=16`;
  return { frame: `${MAP_HOST}/map-widget/v1/?${q}`, link: `${MAP_HOST}/maps/?${q}`, point: pt, address };
}

// Точка в конце фразы, если её там нет. Строки из настроек (слоган) пишутся без
// знака в конце, а на этой странице за ними идёт продолжение текста.
...
function aboutPage(settings, opts) {
  ...
  // Город отправки и сборка — из настроек и из той же сетки, что считает срок на
  // оформлении. Зашитая здесь Москва пережила бы переезд склада и продолжала бы
  // звучать покупателям.
  const shipFrom = TRACK.cleanCity(settings.shipFromCity) || TRACK.DEFAULT_FROM;
  const handling = SHIPDAYS.handlingDays(settings);
  const handlingLine = handling === 0
    ? 'Заказ собираем и передаём перевозчику в день оплаты'
    : `Заказ собираем и передаём перевозчику за ${handling} ${pluralRu(handling, 'рабочий день', 'рабочих дня', 'рабочих дней')} после оплаты`;
  /* Оплата описывается по ТОМУ режиму, в котором витрина работает прямо сейчас
   * (`PAYMENTS.mode()` — одно место на весь проект): обещать страницу с
   * реквизитами магазину, где заявку разбирает менеджер, — значит соврать
   * покупателю на самом важном для него шаге. */
  const mode = PAYMENTS.mode(settings);
  const payLine = mode === 'request'
    ? 'После оформления заказа менеджер связывается с покупателем и подтверждает заказ — условия оплаты он согласует лично.'
    : `Сразу после оформления открывается страница оплаты с реквизитами: перевод по СБП на номер телефона или на карту. Оплата идёт предоплатой, наложенного платежа и оплаты при получении нет. Данные банковской карты магазин не спрашивает и не получает — перевод покупатель делает из своего банковского приложения. На оплату даётся ${PAY_WINDOW / 60000} минут, дальше заказ закрывается и оформляется заново.`;
  const contacts = [
    phone ? `<div><dt>Телефон</dt><dd><a href="tel:${esc(phoneHref(phone))}">${esc(phoneText(phone) || phone)}</a></dd></div>` : '',
    email ? `<div><dt>Почта</dt><dd><a href="mailto:${esc(email)}">${esc(email)}</a></dd></div>` : '',
    tg ? `<div><dt>Telegram</dt><dd><a href="https://t.me/${esc(tg)}" target="_blank" rel="noopener">@${esc(tg)}</a></dd></div>` : '',
    waUrl ? `<div><dt>WhatsApp</dt><dd><a href="${esc(waUrl)}" target="_blank" rel="noopener">${esc(phoneText(waPhone) || waPhone)}</a></dd></div>` : '',
    hours ? `<div><dt>Время работы</dt><dd>${esc(hours)}</dd></div>` : ''
  ].filter(Boolean).join('');
  const zones = SHIPDAYS.summaryRows();
  ...
  const body = `<div class="container legal-page about-page">
    ...
    <header class="legal-hero">
      ...
      ${/* Слоган владельца точкой обычно не заканчивается — он пишется под
           первый экран («Оригинальная техника Apple»). Без неё он слипается со
           следующей фразой в одно предложение, поэтому знак дописывается, если
           его нет. */''}
      <p>${esc(sentence(settings.tagline || 'Оригинальная техника Apple с гарантией и доставкой по России'))} Здесь собрано всё, что стоит знать до покупки: что мы продаём, как считаются доставка и оплата, куда писать и куда приехать.</p>
      ${TRUST_BLOCK}
    </header>

    <div class="legal-card">
      <section class="legal-section" id="shop">
        <h2>Чем мы занимаемся</h2>
        <p>Мы продаём новую оригинальную технику Apple: смартфоны, компьютеры и ноутбуки, планшеты, часы, наушники и аксессуары. Каждая позиция в каталоге — конкретная сборка с ценой, наличием и характеристиками производителя, а не «уточняйте у менеджера».</p>
        <p>Цены у нас закупочные, поэтому заметно ниже розничных: мы работаем напрямую с поставщиками и не держим сеть салонов. На технику действует гарантия, а вернуть или обменять товар можно на условиях, описанных ниже.</p>
        ${cats.length ? `<p>Разделы каталога:</p>
        <ul class="about-cats">${cats.map(c => `<li><a class="link" href="/?category=${encodeURIComponent(c)}">${esc(c)}</a></li>`).join('')}</ul>` : ''}
      </section>

      <section class="legal-section" id="how">
        <h2>Как проходит покупка</h2>
        <ol class="about-steps">
          <li><b>Выбор.</b> На странице товара собирается нужная сборка — цвет, память, дополнительные характеристики. Цена и наличие меняются вместе с выбором.</li>
          <li><b>Оформление.</b> Нужны имя, телефон и адрес: по адресу считаются стоимость и срок доставки, и оба видны до оплаты.</li>
          <li><b>Оплата.</b> ${mode === 'request' ? 'Заявка уходит менеджеру, он связывается и подтверждает заказ.' : 'Перевод по реквизитам, которые открываются сразу после оформления.'}</li>
          <li><b>Отправка.</b> ${esc(handlingLine)}, а посылку можно <a class="link" href="/track">отследить по ссылке</a>, которую пришлёт менеджер.</li>
        </ol>
      </section>

      <section class="legal-section" id="delivery">
        <h2>Доставка</h2>
        <p>Отправляем ${esc(carrierNames())} — в пункт выдачи или курьером до двери, по всей России. Город отправки — ${esc(shipFrom)}. Стоимость и срок считаются по адресу прямо на оформлении и показываются там же, до оплаты.</p>
        ${/* Подпись мобильной карточки приезжает из `data-l` и слово в слово
             повторяет заголовок столбца: вторая формулировка в CSS разошлась бы
             с первой молча — то же правило, что у таблицы касс в панели. */''}
        ${zones.length ? `<div class="legal-table-wrap"><table class="legal-table"><thead><tr><th>Куда</th><th>Сколько идёт</th></tr></thead><tbody>
          ${zones.map(z => `<tr><td data-l="Куда">${esc(z.name)}</td><td data-l="Сколько идёт">${esc(z.days)}</td></tr>`).join('')}
        </tbody></table></div>
        <p>В таблице — время перевозки до пункта выдачи обычной (не экспресс) доставкой. Курьером до двери на день-два дольше. ${esc(handlingLine)}; оплаченные вечером и в выходные уезжают следующим рабочим днём, поэтому срок от дня заказа выходит длиннее.</p>` : ''}
      </section>

      <section class="legal-section" id="pay">
        <h2>Оплата</h2>
        <p>${esc(payLine)}</p>
      </section>

      <section class="legal-section" id="rules">
        <h2>Гарантия, возврат и документы</h2>
        <p>На технически сложные товары действует гарантия 1 год, на аксессуары — 1 месяц. Заводские недостатки устраняются бесплатно, а при существенном недостатке товар меняется либо деньги возвращаются.</p>
        <p>Отказаться от товара при покупке через сайт можно в течение семи дней после получения, а если письменная памятка о порядке возврата не выдавалась — в течение трёх месяцев. Полные условия и порядок отправки — на отдельных страницах.</p>
        <div class="legal-actions">
          <a class="btn" href="/warranty">Гарантия</a>
          <a class="btn" href="/returns">Возврат и обмен</a>
          <a class="btn" href="/privacy">Политика конфиденциальности</a>
          <a class="btn" href="/personal-data-consent">Согласие на обработку данных</a>
          <a class="btn" href="/track">Отследить заказ</a>
        </div>
      </section>

      ${contacts ? `<section class="legal-section" id="contacts">
        <h2>Контакты</h2>
        <p>Пишите и звоните по любому вопросу: подобрать сборку, уточнить наличие, узнать, где посылка. ${CHAT.visible(settings) ? 'Можно и не уходя с сайта — в чате в правом нижнем углу.' : ''}</p>
        <dl class="legal-details">${contacts}</dl>
      </section>` : ''}

      <section class="legal-section" id="address">
        <h2>${map ? 'Как нас найти' : 'Где мы находимся'}</h2>
        ${map ? `<p>Магазин работает и офлайн. Приехать, посмотреть технику и купить её можно по адресу — точка одна, филиалов в других городах нет.</p>
        ${/* Часы стоят здесь, только когда раздела «Контакты» на странице нет
             вовсе: в двух соседних разделах одно и то же значение читается как
             две разные строки, которые зачем-то сверяют. */''}
        <dl class="legal-details">
          <div><dt>Адрес</dt><dd>${esc(map.address)}</dd></div>
          ${hours && !contacts ? `<div><dt>Время работы</dt><dd>${esc(hours)}</dd></div>` : ''}
        </dl>
        ${/* Карта не грузится сама: до нажатия здесь одна ссылка, а фрейм
             вставляет public/app.js. Адрес виджета уезжает атрибутом, чтобы
             скрипт не собирал его сам — второй сборщик того же адреса
             разошёлся бы с этим на первой правке. */''}
        <div class="about-map" data-map="${esc(map.frame)}" data-map-title="${esc(map.address)}">
          <a class="about-map-link" href="${esc(map.link)}" target="_blank" rel="noopener">Открыть в Яндекс.Картах</a>
        </div>`
      : `<p>Магазин работает через интернет: розничных точек, шоурума и самовывоза нет — товар едет перевозчиком (${esc(carrierNames())}) в пункт выдачи или курьером до двери.</p>`}
      </section>
