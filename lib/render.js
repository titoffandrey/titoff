...
function payClosed(order, now, archived) {
  ...
  if (state === 'paid' || state === 'mismatch') return true;
...
function payPage(settings, order, opts) {
  ...
  if (ownPaid && state !== 'paid' && !orderArchived) {
    ...
    card = `<section class="order-success" role="status" aria-live="polite">
      ...
      <p class="order-success-copy">Касса подтвердила перевод на ${esc(payTotal)}. ${esc(trackPromise(order))}</p>
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
    ...
    card = `<div class="co-block pay-invoice">
      ...
      <p class="pay-hint">${hosted
        ? `Оплата ${pay.provider === 'alfabank' ? 'через СБП' : 'картой'} пройдёт на защищённой странице банка — она откроется в новой вкладке, сумма там уже указана. Эту страницу не закрывайте: как только банк подтвердит оплату, она обновится сама.`
  ...
  const paid = state === 'paid' || !!order.manualPaid;
...
function receiptPage(settings, order, opts) {
  ...
  const paid = paidByProvider || !!manual;
