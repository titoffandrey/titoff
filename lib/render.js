...
// Плашка над формой: «получилось» гаснет сама, ошибка остаётся.
function accountFlash(opts) {
  if (opts.error) return `<p class="form-msg err acc-flash" role="alert" data-store-toast data-toast-type="error" data-toast-duration="0" data-toast-id="account-result">${esc(opts.error)}</p>`;
