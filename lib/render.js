...
function orderStatus(order) {
  ...
  const view = pay && pay.status ? PAY_STATES[pay.status] : null;
  if (!view) return '<span class="muted">—</span>';
  return `<span class="pay-tag ${view[0]}">${view[1]}</span>`
    + (pay.note ? `<div class="muted small o-note">${esc(pay.note)}</div>` : '');
}
...
function orderPayMethod(order) {
  const pay = order && order.payment;
  const method = pay ? PAY.nameOf(pay.method) : '';
  return method ? esc(method) : '<span class="muted">—</span>';
}

/* ===== Оболочка панелей: глифы разделов и кнопка «спрятать меню» =====
 *
 ...
 * всём проекте один. Раздел опознаётся по значку быстрее, чем по слову, а на
 * телефоне лента разделов из-за них перестала быть сплошной стеной текста.
 *
...
/* Кнопка «спрятать меню». Панель занимает четверть ширины, а нужна раз в
 * несколько минут — на каталоге и метрике это место дороже отдать таблице.
 *
 * Само состояние держит `public/admin-ui.js` (класс `nav-off` на <html> и
 * localStorage), поэтому выбор переживает переходы между страницами: панель,
 * раскрывающаяся заново на каждом клике по разделу, бесила бы сильнее, чем
 * узкий экран. Без скрипта меню просто остаётся открытым — как было.
 */
function adminNavToggle() {
  return `<button type="button" class="a-nav-toggle" id="a-nav-toggle" aria-controls="a-sidebar" aria-expanded="true" aria-label="Скрыть меню" title="Скрыть меню">
...
// Состояние панели применяется ДО первой отрисовки: иначе на каждой странице
// меню моргало бы открытым у того, кто его спрятал.
const ADMIN_NAV_BOOT = `<script>try{if(localStorage.getItem('admin_nav_off')==='1')document.documentElement.classList.add('nav-off')}catch(e){}</script>`;
...
// Через сколько позиций ставить следующий отзыв с видео. Ровно поэтому «не
// подряд»: между двумя видео всегда есть отзыв с фото или без медиа.
const VIDEO_EVERY = 3;

function hasVideo(rv) { return !!(rv && rv.videos && rv.videos.length); }
function hasPhoto(rv) { return !!(rv && rv.photos && rv.photos.length); }

// Видео — самое убедительное, что есть в ленте, и его должно быть видно с первых
// страниц. Но поставить их подряд нельзя: страница из восьми роликов грузится
// тяжело и читается как реклама. Поэтому каждое третье место отдаём видео, а
// между ними идут отзывы с фото и обычные — в своём же порядке по дате.
function arrangeVideos(list) {
  const videos = list.filter(hasVideo);
  if (!videos.length || videos.length === list.length) return list;
  const others = list.filter(rv => !hasVideo(rv));
  const photos = others.filter(hasPhoto);
  const plain = others.filter(rv => !hasPhoto(rv));
  const out = [];
  let vi = 0, pi = 0, ti = 0;
  const nextFiller = () => (pi < photos.length ? photos[pi++] : (ti < plain.length ? plain[ti++] : null));
  while (out.length < list.length) {
    const wantVideo = out.length % VIDEO_EVERY === 0 && vi < videos.length;
    const pick = wantVideo ? videos[vi++] : nextFiller();
    // Заполнители кончились — дальше идут оставшиеся видео, деться им некуда.
    out.push(pick || (vi < videos.length ? videos[vi++] : null));
    if (out[out.length - 1] == null) { out.pop(); break; }
  }
  return out;
}
...
function sortReviews(list, sort) {
  ...
  // Раскладка видео только в порядке по умолчанию: выбрав «с высокой оценкой»,
  // покупатель просил именно оценки, и переставлять их под видео нельзя.
  return arrangeVideos(arr.sort(fresh));
}
...
function reviewDelivery(id) {
  ...
  return `<span class="rv-ship" title="Доставка: ${esc(label)}"><span class="rv-ship-cap">Доставка:</span>${mark}</span>`;
}
...
function reviewMedia(videos, photos, poster) {
  const items = []
    .concat(videos.map(src => ({ src, kind: 'video' })))
    .concat(photos.map(src => ({ src, kind: 'photo' })));
  if (!items.length) return '';
  return `<div class="review-media">${items.map((it, i) => {
    const href = `/uploads/${esc(it.src)}`;
    const inner = it.kind === 'video'
      ? `<video src="${href}#t=0.1" preload="metadata" muted playsinline${poster ? ` poster="/uploads/${esc(poster)}"` : ''}></video><span class="rv-play" aria-hidden="true"></span>`
      : `<img src="${href}" alt="Фото к отзыву" loading="lazy">`;
    const label = it.kind === 'video' ? 'Видео к отзыву' : 'Фото к отзыву';
    return `<a class="rv-item rv-${it.kind}" href="${href}" data-kind="${it.kind}" data-i="${i}" target="_blank" rel="noopener" aria-label="${label}">${inner}</a>`;
  }).join('')}</div>`;
}
...
function reviewCard(rv) {
  ...
  // Какую сборку купил этот покупатель: цвет, память и прочее. Серым и мелким —
  // это уточнение к отзыву, а не его содержание.
  const config = rv.config
    ? `<div class="review-config">${esc(rv.config)}</div>` : '';
  return `
    <article class="review" data-rating="${rating}" data-ts="${ts}" data-len="${String(rv.text || '').length}"${videos.length ? ' data-video="1"' : ''}>
      ...
      ${reviewMedia(videos, photos, rv.poster)}
      ${reviewDelivery(rv.delivery)}
    </article>`;
}
...
module.exports = {
  esc, money, formatDate, orderStatus, orderPayMethod, editSwitch, adminIcon, adminNavToggle, ADMIN_NAV_BOOT, orderNo, orderDelivery, orderClient, orderItems, clientMarks, visitorHref, stars, startPrice, sellable, layout, homePage, notFoundPage, productPage, checkoutPage, payPage, privacyPage,
