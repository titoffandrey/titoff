    ...
    <div class="footer-cols">
      ...
      <div class="footer-col footer-contacts">
        ...
        ${/* Телефон и почта — ссылки, а не строки текста: с телефона по ним
             ...
             СО ЗНАЧКОМ, как строки мессенджеров под ними и как адрес с булавкой
             над ними: без него в колонке оставались бы две безымянные строки
             посреди подписанных значками соседей. Глифы те же, что в контактах
             «О компании», и того же размера — `.msg-ico`. */''}
        ${settings.contactPhone ? `<div class="foot-contact"><a href="tel:${esc(phoneHref(settings.contactPhone))}">${phoneIcon()}<span>${esc(phoneText(settings.contactPhone) || settings.contactPhone)}</span></a></div>` : ''}
        ${settings.contactEmail ? `<div class="foot-contact"><a href="mailto:${esc(settings.contactEmail)}">${mailIcon()}<span>${esc(settings.contactEmail)}</span></a></div>` : ''}
