...
function productCard(p, settings, db, opts) {
  ...
  // Процент спрашиваем у промокодов, а не у товара напрямую: выключенная
  // промоакция снимает с витрины и плашку, и зачёркнутую цену (`PROMO.pctFor`).
...
function productPage(settings, db, p, opts) {
  ...
  // Как и в карточке каталога: выключенная промоакция обнуляет процент, и
  // страница показывает одну цену без зачёркнутой (`PROMO.pctFor`).
  const pct = PROMO.pctFor(p, settings);
