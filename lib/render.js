...
/* Промокоды. Витрина спрашивает у них ровно одно — процент, который видит
 * покупатель (`PROMO.pctFor`): скидка витрины и есть скидка кода по умолчанию,
 * поэтому выключенная промоакция убирает плашку и зачёркнутую цену. Цикла нет:
 * `lib/promo.js` подключает только `./discount`. */
...
/* Яндекс Метрика: отдавать ли счётчик, решает сервер (`decide` в server.js →
 * `opts.ym`), а разметка только подключает `public/ym.js` и подписывает <html>
 * номером. Здесь модуль нужен политике конфиденциальности: собираем — значит
 * называем, и абзацы про Яндекс появляются только у магазина со счётчиком. */
const YM = require('./yandex-metrika');
...
const PAYMENT_FEE = require('../public/payment-fee');
...
// Карта магазина: тайловая математика и кэш тайлов там, здесь только разметка.
// Цикла нет — `lib/map-tiles.js` не подключает из проекта ничего.
const MAP = require('./map-tiles');
...
// Личный кабинет: порог пароля для форм и признак «кабинет включён» для шапки.
// Модуль ходит в lib/db.js, а тот рендер не подключает — кольца нет.
const CUSTOMERS = require('./customers');
const CUSTOMER_PASSWORD_MIN = CUSTOMERS.PASSWORD_MIN;
// Предел адреса — общий у поля на витрине, заказа и кабинета.
...
// 6 сентября 2026: в сроки хранения добавлены признаки посетителя, которому
// владелец закрыл доступ, — они живут, пока действует само ограничение.
...
/* Критические стили — правила первого экрана, которые уезжают в `<head>`
 * ИНЛАЙНОМ, чтобы отрисовка не ждала запроса за `styles.css`. PageSpeed
 * 19 сентября 2026 показал этот запрос единственным блокирующим на витрине:
 * 26 КиБ и 210 мс до первого кадра на телефоне — а первому экрану из них
 * нужна седьмая часть. Что именно нужно, решает не список селекторов руками,
 * а живой браузер: `scripts/build-critical-css.js` открывает страницы в
 * headless Chrome и оставляет правила, чьи элементы стоят в первом экране
 * (см. там же). Наборов четыре — у первого экрана товара (галерея, ряды выбора,
 * кнопка) правил вдвое больше, чем у главной, и одним общим набором главная
 * возила бы чужие; у главной с «весь каталог на главной» (`homeCatalog`) свой,
 * `home-catalog`: под слоганом там сразу сетка, а не плитки категорий. Ключ
 * набора — вид страницы; страница без набора грузит
 * стили как раньше, блокирующей ссылкой: мигнуть голой разметкой ей нельзя.
 *
 * Читаются один раз при старте и чистятся той же `MIN.css`, что идёт на
 * отдаче статики (дважды — второй проход схлопывает пробелы на месте
 * комментариев). Файла нет — набора нет, и страница остаётся на блокирующей
 * ссылке: витрина не должна ломаться из-за несобранного файла. */
const MINIFY = require('./minify');
const CRITICAL_KINDS = ['home', 'home-catalog', 'catalog', 'product'];
const CRITICAL_CSS = {};
for (const kind of CRITICAL_KINDS) {
  try {
    const raw = fs.readFileSync(path.join(__dirname, '..', 'public', 'critical', kind + '.css'), 'utf8');
    const text = MINIFY.css(MINIFY.css(raw)).trim();
    // `</style>` внутри инлайна закрыл бы тег; в CSS такой строки нет и быть не
    // может, но файл генерируется, и проверять обязаны.
    if (text && !/<\/style/i.test(text)) CRITICAL_CSS[kind] = text;
  } catch (e) { /* файла нет — страница грузит стили блокирующей ссылкой */ }
}
// Вид страницы для набора критических стилей: товар, главная (обычная или
// «весь каталог сразу», см. `homeCatalogOn` — у той первый экран другой:
// сетка карточек под слоганом вместо плиток категорий), каталог.
function criticalKind(opts) {
  if (opts.productPage) return 'product';
  if (opts.tab === 'home') return opts.homeCatalog ? 'home-catalog' : 'home';
  if (opts.tab === 'catalog') return 'catalog';
  return '';
}
/* Тег стилей витрины. На ПОСАДОЧНОМ заходе страницы с набором — инлайн
 * критических правил, а полный файл едет `preload` с низким приоритетом и
 * подключается по загрузке (`<noscript>` держит обычную ссылку для браузера
 * без скриптов; CSP витрины разрешает inline-обработчики). На заходе со своей
 * же страницы (`Sec-Fetch-Site: same-origin` — его браузер шлёт и при нашем
 * `Referrer-Policy: no-referrer`) полный файл уже в кэше на год, и инлайн был
 * бы 4–6 КБ лишних на каждый переход — остаётся обычная ссылка, из кэша она
 * не стоит ничего. Признак посадочного захода ставит `pageOpts()` в server.js;
 * без него (старый вызов, тест) — обычная ссылка. Низкий приоритет у preload — чтобы полный файл не отбирал
 * канал у снимков первого экрана: рисовать страницу он больше не гейтит. */
function stylesTag(opts) {
  const href = `/static/styles.css?v=${assetV('styles.css')}`;
  const kind = criticalKind(opts);
  // Быстрый путь — ЯВНЫЙ: его включает `pageOpts()` сервера по заголовку
  // запроса. Без признака (старый вызов, тест) — прежняя ссылка.
  const critical = kind && opts.landing === true ? CRITICAL_CSS[kind] : '';
  if (!critical) return `<link rel="stylesheet" href="${href}">`;
...
// Слоган в заголовке первого экрана: текст брендовым шрифтом
...
function heroFit(settings, text) {
  ...
  return fitValue(textUnits(text, scale, -0.038));
}
...
/* Сумма ПРОСТЫМ ТЕКСТОМ, без экранирования: она уезжает не только в разметку,
 * но и в готовое сообщение покупателю в WhatsApp (`orderMessage` ниже). Там
 * `esc()` был бы виден буквами — у магазина с «&» в названии валюты в реплике
 * стояло бы «&amp;». Считать деньги двумя способами при этом нельзя, поэтому
 * арифметика здесь одна, а `money()` — тот же результат под `esc()`: цифры и
 * пробелы экранирование не трогает, поэтому разметка не изменилась ни в одном
 * месте. */
function moneyText(n, settings) {
...
function money(n, settings) { return esc(moneyText(n, settings)); }
...
/* Начало московских суток — НАСТОЯЩЕЙ меткой времени, а не сдвинутой.
 *
 * `mskDate()` отдаёт дату, у которой московское время показывают ЛОКАЛЬНЫЕ поля,
 * то есть её `getTime()` временем уже не является — вычитать такие даты друг из
 * друга нельзя. Поэтому сколько прошло с полуночи считается по самим полям, а
 * миллисекунды берутся у исходной метки: `mskDate` их не сохраняет.
 *
 * Отсчёт по часам ПРОЦЕССА (`setHours(0,0,0,0)`) здесь не годится: сервер живёт
 * в UTC, и «сегодня» у него начиналось бы в три часа ночи по Москве — то есть
 * заказы первых трёх часов суток считались бы вчерашними.
 */
function mskDayStart(ms) {
  const at = Number(ms) || Date.now();
  const d = mskDate(at);
  if (Number.isNaN(d.getTime())) return at;
  const passed = ((d.getHours() * 60 + d.getMinutes()) * 60 + d.getSeconds()) * 1000 + at % 1000;
  return at - passed;
}
 ...
 *
 * Кнопка в строке заказа открывает диалог с ЕГО номером и готовым сообщением про
 * ЕГО заказ. Иначе менеджер переписывает состав руками — из списка в мессенджер,
 * по позиции за раз, — и делает это по каждой заявке.
 *
 * ЭТО НЕ КОПИЯ УВЕДОМЛЕНИЯ МЕНЕДЖЕРУ, хотя состав в них один и тот же. В Telegram
 * уходит служебная сводка: город по IP, устройство, браузер и сам адрес
 * посетителя. Слать это человеку незачем — он и так знает, с какого телефона
 * заказывал, а «Устройство: iPhone · Safari» в переписке с продавцом читается как
 * слежка. Здесь остаётся ровно то, ради чего пишут: номер заказа, что куплено,
 * куда едет посылка и сколько платить.
 *
 * Текст ПЛОСКИЙ, без единого тега: WhatsApp разметки не понимает, и «<b>» уехало
 * бы в реплику буквами. Поэтому и деньги считает `moneyText()`, а не `money()`.
 */
/* Позиций в сообщении. Текст уезжает в query-строку ссылки, а кириллица занимает
 * там по шесть знаков на символ: заказ на десяток позиций — обычное дело, на
 * полсотни — нет. Остаток называется числом, иначе выглядело бы, будто в заказе
 * только показанное. */
const WA_ITEMS_MAX = 15;

function orderTrackingUrl(order, origin) {
  const shipment = order && order.shipment;
  if (!TRACK.shownToBuyer(shipment)) return '';
  const path = TRACK.trackPath(shipment);
  if (!path) return '';
  try {
...
function orderMessage(order, settings, opts) {
  const o = order || {};
  const shop = String((settings && settings.storeName) || '').trim();
  /* Обращение по ИМЕНИ, а не по «имя фамилия»: в переписке зовут по имени. У
   * прежних заявок отдельного поля нет вовсе, и первое слово собранной строки —
   * ровно оно же. */
  const rawName = String(o.firstName || '').trim()
    || String(o.customerName || '').trim().split(/\s+/)[0];
  const name = rawName.replace(/^./u, letter => letter.toUpperCase());
  const items = Array.isArray(o.items) ? o.items : [];
  const list = items.slice(0, WA_ITEMS_MAX).map(i => {
    const qty = Number(i && i.qty) || 1;
    // «× 1» не пишем — то же правило, что у состава заказа в панели.
    return `• ${String((i && i.name) || 'Товар')} - ${qty > 1 ? qty + ' × ' : ''}${moneyText(i && i.price, settings)}`;
  });
  const rest = items.length - list.length;
  if (rest > 0) list.push(`• …и ещё ${rest} ${pluralRu(rest, 'позиция', 'позиции', 'позиций')}`);

  const no = orderNo(o.number);
  const head = `Ваш заказ${no === '—' ? '' : ' ' + no}${o.createdAt ? ' от ' + formatDate(o.createdAt) : ''}:`;
  const way = [DELIVERY.nameOf(o.delivery), DELIVERY.shortModeOf(o.delivery, o.deliveryMode)]
    .filter(Boolean).join(', ');
  const tail = [];
  if (way) {
    tail.push(`Доставка: ${way}`
      + (Number(o.deliveryPrice) > 0 ? ' - ' + moneyText(o.deliveryPrice, settings) : ''));
  }
  /* Куда едет посылка — одна строка, а не две: у заказа в пункт выдачи адрес
   * покупателя тоже есть, но посылка едет не туда, и обе строки разом читались
   * бы как два разных адреса. Покупателю он и так известен. */
  if (o.pickupAddress) {
    // Код пункта отбит тире, а не запятой: запятых в самом адресе хватает, и в
    // одном ряду с ними код читался бы его частью.
    tail.push(`Пункт выдачи: ${o.pickupCode ? o.pickupCode + ' - ' : ''}${o.pickupAddress}`);
  }
  else if (o.address) tail.push(`Адрес доставки: ${o.address}`);
  // Промокод объясняет итог, поэтому стоит рядом с ним, а без выгоды в рублях
  // строка бессмысленна — как и в уведомлении менеджеру.
  if (o.promoCode && Number(o.promoDiscount) > 0) {
    tail.push(`Промокод ${o.promoCode} - выгода ${moneyText(o.promoDiscount, settings)}`);
  }
  tail.push(`Итого: ${moneyText(o.total, settings)}`);
  // В мессенджере нужен полный адрес именно этого отправления. Скрытый
  // маршрут не откроется покупателю, поэтому ссылку на него не подставляем.
  const trackingUrl = orderTrackingUrl(o, opts && opts.origin);

  return [`Здравствуйте${name ? ', ' + name : ''}!${shop ? ` Это магазин «${shop}».` : ''}`, head]
    .concat(list.length ? ['', list.join('\n')] : [])
    .concat(['', tail.join('\n')])
    .concat(trackingUrl ? ['', `🔗 Ссылка отслеживания: ${trackingUrl}`] : []).join('\n');
}

/* Ссылка `wa.me` с номером покупателя и готовым текстом.
 *
 * Номер в заказе хранится в E.164, но повторный `PHONE.store()` — та же
 * страховка, что у контакта магазина: у прежних заявок телефона нет вовсе, а
 * испорченную руками строку нельзя превращать в чужой номер. Не разобрался —
 * ссылки нет: диалог «ни с кем» хуже её отсутствия.
 *
 * Зарегистрирован ли номер в WhatsApp, отсюда узнать нечем и незачем: это скажет
 * сам WhatsApp — либо откроет диалог, либо сообщит, что такого номера у него нет.
 */
function orderWaHref(order, settings, opts) {
  const stored = PHONE.store(order && order.phone);
  if (!stored) return '';
  // Текст кодируется целиком: кириллица, переносы строк и «&» в названии товара
  // остаются одной репликой, а не становятся частью query-строки.
  return `https://wa.me/${stored.slice(1)}?text=${encodeURIComponent(orderMessage(order, settings, opts))}`;
}

function orderTelHref(order) {
  const stored = PHONE.store(order && order.phone);
  return stored ? `tel:${stored}` : '';
}

/* Второй контакт заказа — Telegram ИЛИ почта, а не гарантированный username.
 * Обычный username может быть без @, но должен целиком подходить под маску
 * из 5–32 символов. Не вырезаем «@…» из e-mail или произвольного текста.
 * Короткие коллекционные имена принимаем только с @ или в ссылке на профиль.
 * Существование и тип аккаунта проверит сам Telegram.
 * Служебные маршруты и ссылки на сообщения/бот-команды не являются профилем.
 * https://core.telegram.org/api/links#public-username-links */
const TELEGRAM_LINK_ROUTES = new Set(('addemoji addlist addstickers addstyle addtheme auction auth boost call '
  + 'confirmphone contact giftcode invoice joinchat login m nft proxy setlanguage share socks web a k z c s msg').split(' '));
...
function telegramUsername(contact) {
  ...
  else if (/^[a-z][a-z0-9_]{4,31}$/i.test(value)) username = value;
  else {
    const link = value.match(/^(?:https?:\/\/)?(?:t\.me|telegram\.me|telegram\.dog)\/([a-z][a-z0-9_]{0,31})\/?(?:\?([^#]*))?(?:#[^#]*)?$/i);
    if (!link) return '';
    // Скопированный текст или флаг профиля безопасно заменяются нашим
    // черновиком; start, post, direct и прочие режимы не ведут к обычному чату.
    for (const key of new URLSearchParams(link[2] || '').keys()) {
      if (key !== 'text' && key !== 'profile') return '';
    }
    username = link[1];
  }
  if (!/^[a-z][a-z0-9_]{0,31}$/i.test(username) || TELEGRAM_LINK_ROUTES.has(username.toLowerCase())) return '';
  return username;
}

function orderTelegramHref(order, settings, opts) {
  const username = telegramUsername(order && order.contact);
  if (!username) return '';
  // Только открытие диалога с текстом в поле ввода, без отправки. Telegram
  // применяет свой лимит message_length_max; здесь состав тот же, что в WhatsApp.
  return `https://t.me/${username}?text=${encodeURIComponent(orderMessage(order, settings, opts))}`;
}
...
function orderClient(order, opts) {
  ...
  const head = [
    ...
    // Почта из заказа — рядом с телефоном: по ней покупатель входит в кабинет,
    // и менеджеру при разборе заявки видно, есть ли у неё кабинет вообще.
    o.email ? `<span class="o-contact">${esc(o.email)}</span>` : '',
  ...
  /* «Написать в WhatsApp» — с номером покупателя и составом его заказа
   * (`orderMessage` выше).
   *
   * Кнопка стоит СНАРУЖИ свёртки и видна всегда: телефон есть почти у каждой
   * заявки, а написать по нему — первое, что с ней делают. Внутри `summary`
   * ссылке не место: клик по ней браузер засчитывает заодно нажатием на саму
   * свёртку, и заявка раскрывалась бы на ровном месте.
   *
   * Настройки берутся из того же `opts.money`, что и цена доставки в строке:
   * сообщение считает те же деньги, что показаны в списке, — двух источников
   * цены у одного заказа быть не должно. Номер не разобрался (прежняя заявка без
   * телефона) — кнопки нет вовсе. */
  const waHref = orderWaHref(o, (opts && opts.money) || null, opts);
  const wa = waHref
    ? `<a class="o-wa" href="${esc(waHref)}" target="_blank" rel="noopener"`
      + ` title="Написать в WhatsApp: ${esc(phoneText(o.phone))}">${waMark()}<span>Написать в WhatsApp</span></a>`
    : '';
  ...
  if (!body) return `<div class="o-who o-who-flat">${wrap}</div>${wa}`;
  return `<details class="o-who"><summary>${wrap}</summary><div class="o-who-body">${body}</div></details>${wa}`;
}
 ...
 */
const ITEMS_SHOWN = 3;
function orderItems(order, opts) {
  const list = (order && order.items) || [];
  if (!list.length) return '<span class="muted">—</span>';
  const line = i => `<div>${esc(i.name)}${Number(i.qty) > 1 ? ` <b>× ${esc(String(i.qty))}</b>` : ''}</div>`;
  const limit = opts && opts.all === true ? list.length : ITEMS_SHOWN;
  const shown = list.slice(0, limit).map(line).join('');
...
function payDisplay(pay, now) {
  if (!pay || ['paid', 'mismatch', 'refunded'].includes(pay.status)) return pay || null;
...
function payExpired(order, now) {
  ...
  if (state === 'paid' || state === 'mismatch' || state === 'refunded') return false;
  return !payLive(pay, at);
}

/* ЗАПЛАТИТ ЛИ ПОКУПАТЕЛЬ ПО ЭТОМУ ЗАКАЗУ ЕЩЁ ХОТЬ КОГДА-НИБУДЬ.
 *
 * Один вопрос — одно место. Страница оплаты отвечает на него ветками (у каждой
 * свои слова: «время вышло», «вы отменили», «спасибо за заказ»), а оформлению
 * нужен только ответ «да/нет»: оно решает, вернуть ли покупателю прежний заказ
 * вместо нового.
 *
 * Зачем это понадобилось: `/api/order` умеет отдать уже записанный заказ вместо
 * второго — так теряется меньше денег, когда ответ на нажатие не дошёл и браузер
 * повторил его. Но проверялось при этом только «не оплачен и не удалён», а
 * ЗАКРЫТЫЙ ПО СРОКУ заказ под это подходит. Получался замкнутый круг: полчаса
 * вышли, покупатель собирает ту же корзину заново, а оформление раз за разом
 * возвращает его на мёртвую страницу «Оформите заказ заново» — и так целые
 * сутки, пока живёт окно повтора. Заказ, по которому уже нельзя заплатить,
 * защищать не от чего: второго списания по нему не случится.
 *
 * Про архив спрашиваем не сами, а принимаем ответ (`archived`): признак живёт в
 * `lib/db.js`, и вторая копия его устройства разъехалась бы с первой — по той же
 * причине его отдельным полем принимает и `payPage`.
 */
function payClosed(order, now, archived) {
  if (!order) return true;
  // Решение человека: заказ убрали из работы или покупатель сам отменил оплату.
  if (archived || order.manualVoid) return true;
  // Деньги пришли — и по кассе, и рукой менеджера по своим реквизитам. Новый
  // заказ на то же самое покупатель вправе оформить, а этот уже закрыт.
  if (order.manualPaid) return true;
  const state = (order.payment && order.payment.status) || '';
  if (state === 'paid' || state === 'mismatch' || state === 'refunded') return true;
  return payExpired(order, now);
}
...
function payView(order, now) {
  // Возвращённый платёж не считается выручкой даже при старой ручной отметке.
  if (order && order.payment && order.payment.status === 'refunded') {
    return { tone: 'off', label: PAY_STATES.refunded[1], when: payMoment(order.payment.closedAt), whenLabel: 'возврат подтверждён' };
  }
 ...
 */
/* Приписка кассы в списке — ОДНОЙ строкой, а не абзацем.
 *
 * Банк присылает вместе с кодом отказа инструкцию ПОКУПАТЕЛЮ: «Операция
 * отклонена. Проверьте введённые данные, достаточность средств на карте и
 * повторите операцию». Менеджеру из неё нужно первое предложение и код —
 * остальное он читает у каждой заявки заново, а стоит это пять строк в столбце
 * и растягивает всю строку заказа втрое.
 *
 * Режется ПРИ ПОКАЗЕ, а не при записи: в заказе лежит то, что правда ответила
 * касса (по нему потом и разговаривают с её поддержкой), а сокращение чинит
 * заодно все уже сохранённые приписки. Хвост со скобкой сохраняется целиком —
 * в нём код отказа и способ («(-2014) · СБП»), то есть ровно то, ради чего
 * строку и читают.
 */
function payNoteShort(note) {
  const text = String(note == null ? '' : note).trim();
  if (!text) return '';
  const cut = text.search(/\.\s+\S/);
  if (cut < 0) return text;
  const tail = text.slice(cut).match(/\((-?\d+)\)[\s\S]*$/);
  return text.slice(0, cut) + (tail ? ' ' + tail[0].trim() : '');
}

function orderStatus(order, now, opts) {
  const view = payView(order, now);
  if (!view) return '<span class="pay-tag pay-none"><i></i>без онлайн-оплаты</span>';
  // `short` — одна плашка без приписок: для сводки на «Обзоре», где столбец
  // узкий, а подробности лежат в самом списке заказов.
  if (opts && opts.short) return `<span class="pay-tag pay-${view.tone}"><i></i>${esc(view.label)}</span>`;
  ...
  // Полный ответ кассы остаётся под курсором: сокращение — про место в списке,
  // а не про то, чтобы что-то от менеджера спрятать.
  const short = payNoteShort(pay.note);
  const note = short
    ? `<div class="muted small o-note"${short !== pay.note ? ` title="${esc(pay.note)}"` : ''}>${esc(short)}</div>`
    : '';
  return tag + left + when + note;
}

// Чем платят — своим столбцом. У заказа без онлайн-оплаты прочерк: менеджер
// договаривается сам, и «—» здесь честнее пустой ячейки.
function orderPayMethod(order, now) {
  const pay = payDisplay(order && order.payment, now);
  ...
  const method = pay && pay.method ? PAY.describe(pay.method, pay.provider).name : '';
...
// Банк относится к показанной попытке, а не к последнему отказу в истории.
// Старые собственные переводы не содержат банк: текущие настройки не могут
// подменить этот исторический факт. Используем только сохранённый снимок.
function orderBankMark(order, now) {
  const pay = payDisplay(order && order.payment, now);
  const own = !pay && order && order.payMode === 'own' && order.ownSnapshot;
  const stored = pay ? pay.bank : own && own.bank;
  const name = (typeof stored === 'string' ? stored.trim() : '')
    || (pay && pay.provider === 'alfabank' ? 'Альфа-Банк' : '');
  if (!name) return '';
  return `<span class="bank-mark">${bankLogo(name)}<span class="bank-name">${esc(name)}</span></span>`;
}
...
function orderRequisites(pay, order) {
  ...
  if (pay && pay.requisite) {
    ...
    rows.push(
      [PAY.requisiteLabel(pay.method), req.text, PAY.isPayLink(pay.requisite) ? pay.requisite : ''],
      ['Банк', pay.bank],
  ...
  /* РЕКВИЗИТ-ССЫЛКА ПОКАЗЫВАЕТСЯ ССЫЛКОЙ, А НЕ СВОИМ АДРЕСОМ. У эквайринга
   * реквизит — это `https://pay…/payment.html?mdOrder=…&language=ru`: полторы
   * сотни знаков без единого пробела. В узком столбце браузер ломает такую
   * строку по буквам, и одна заявка занимала восемь строк адресом, который
   * читать всё равно никто не станет — по нему переходят. Ссылка «Открыть
   * страницу оплаты» отвечает на тот же вопрос одной строкой, а сам заказ у
   * кассы ищут по номеру счёта строкой ниже. */
  return '<details class="o-req"><summary>Реквизиты</summary><div class="o-req-list">'
    + rows.filter(([, value]) => value)
      .map(([label, value, href]) => `<div><span>${esc(label)}</span>`
        + (href
          ? `<a class="o-req-link" href="${esc(href)}" target="_blank" rel="noopener">Открыть страницу оплаты</a>`
          : `<b>${esc(value)}</b>`)
        + '</div>').join('')
    + bad + '</div></details>';
}
...
/* ===== Период, за который смотрят заказы =====
 *
 * Тот же выбор, что над отчётом метрики, и по той же причине: «выручка 1 480 000»
 * без ответа «за какой срок» не с чем сравнить — стало лучше или хуже, по одному
 * числу за всё время не увидеть.
 *
 * Список закрытый, и по умолчанию период НЕ ограничен: «Обзор» показывал выручку
 * за всё время, и молча превратить её в сегодняшнюю значило бы поменять числа
 * под руками у владельца, ничего ему не сказав. Мусор в адресе (`?period=99`)
 * читается как «за всё время» — адрес правят руками, и падать из-за этого
 * нельзя, то же правило, что у номера страницы и вкладок.
 */
const ORDER_PERIODS = [
  [0, 'За всё время'],
  // «Сегодня» — это ТЕКУЩИЕ московские сутки, а не последние 24 часа: магазин
  // работает по Москве, и день у него начинается там же, где у владельца.
  [1, 'Сегодня'],
  [-1, 'Вчера'],
  [7, 'Последние 7 дней'],
  [30, 'Последние 30 дней'],
  [365, 'Последние 12 месяцев']
];
function orderPeriod(raw) {
  const value = Number(raw), days = Math.floor(value);
  // Дробный мусор вроде -0.5 раньше означал «за всё время»: добавление -1
  // не должно молча превратить такой адрес во вчерашний период.
  return (days !== -1 || value === -1) && ORDER_PERIODS.some(([d]) => d === days) ? days : 0;
}
function orderPeriodLabel(raw) {
  const days = orderPeriod(raw);
  return (ORDER_PERIODS.find(([d]) => d === days) || ORDER_PERIODS[0])[1];
}
// Обе границы берутся от одних часов: переход через полночь во время расчёта
// не должен добавить к «Вчера» ещё одни сутки. Ноль означает отсутствие границы.
function orderPeriodRange(raw, now) {
  const days = orderPeriod(raw);
  if (!days) return { since: 0, until: 0 };
  ...
  if (!Number.isNaN(new Date(at).getTime())) {
    ...
    if (mskFormat) {
      parts = {};
      for (const part of mskFormat.formatToParts(new Date(at))) parts[part.type] = part.value;
    } else {
      const shifted = new Date(at + MSK_FALLBACK_OFFSET * 60000);
      parts = { hour: shifted.getUTCHours(), minute: shifted.getUTCMinutes(), second: shifted.getUTCSeconds() };
    }
    // Не создаём Date с московскими часами в ЛОКАЛЬНОЙ зоне сервера: в день
    // её перехода на летнее время локальных 02:30 может вообще не существовать.
    const passed = (((Number(parts.hour) % 24) * 60 + Number(parts.minute)) * 60 + Number(parts.second)) * 1000;
    start -= passed + ((at % 1000 + 1000) % 1000);
  }
  return days === -1 ? { since: start - 86400000, until: start }
    : { since: start - (days - 1) * 86400000, until: 0 };
}
// Нижняя граница нужна также ссылкам и сводкам, которые не фильтруют список.
function orderPeriodSince(raw, now) {
  return orderPeriodRange(raw, now).since;
}
function ordersInPeriod(list, raw, now) {
  const all = Array.isArray(list) ? list : [];
  const { since, until } = orderPeriodRange(raw, now);
  return since ? all.filter(o => {
    const at = Number(o && o.createdAt);
    return at >= since && (!until || at < until);
  }) : all;
}
// Общая сумма списка заказов. Спрашивают её и сводка, и шапка списка (там она
// считается по НАЙДЕННЫМ заявкам), поэтому арифметика одна на оба места.
function ordersTotal(list) {
  let sum = 0;
  for (const o of list || []) sum += Number(o && o.total) || 0;
  return sum;
}
...
function orderStats(list, now, financialList) {
  ...
  const at = now || Date.now();
  // «Сегодня» — с начала текущих МОСКОВСКИХ суток. Это единственное число
  ...
  //
  // Отсечка та же, что у периода «Сегодня» в меню над сводкой: два разных
  // «сегодня» на одной странице читались бы как ошибка в счёте.
  const since = mskDayStart(at);
  ...
  let revenue = 0, paid = 0, financialSum = 0;
  ...
  /* `sum` — по показанному списку, `financialSum` — по тому же журналу, что и
   * выручка. Числа разные, и путать их нельзя: рядом с выручкой обязана стоять
   * сумма ТЕХ ЖЕ заявок, иначе «оформили на 300 000, получили 150 000» окажется
   * про два разных набора заказов. */
  return { count, financialCount: financial.length, sum, financialSum, revenue, paid, avg, today, tones };
}
...
function orderStatsBar(stats, settings) {
  ...
  /* Общая сумма заказов — сколько оформили, вместе с неоплаченным. Рядом с
   * выручкой она отвечает на второй вопрос владельца: сколько денег ещё не
   * дошло. Совпала с выручкой (всё оплачено) — строки нет вовсе: она повторяла
   * бы число, которое стоит над ней крупно. */
  const total = Number.isFinite(s.financialSum) ? s.financialSum : s.sum;
  if (total && total !== s.revenue) facts.push(['Сумма заказов', money(total, settings)]);
  if (s.paid) facts.push(['Средний чек', money(s.avg, settings)]);
  /* «Сегодня» показывается, только когда это ЧАСТЬ показанного. За выбранный
   * период «Сегодня» такой же строкой повторяло бы «Оплачено N из N» и выручку
   * над ней — то есть отвечало бы на вопрос, который уже задан периодом. */
  if (s.today && s.today.n && s.today.n !== financialCount) {
...
const A_ICONS = {
  ...
  /* Промокоды: ярлык с дырочкой. Скидка в проекте уже подписана процентом и
     плашкой «Промоакция», а раздел — про сам КОД, поэтому и значок про ярлык, а
 ...
 *
 * `opts.sum` — ГОТОВАЯ разметка суммы рядом с подписью списка. Именно разметка,
 * а не число: собирает её `money()`, который валюту уже экранирует, и второй
 * `esc()` поверх дал бы `&amp;amp;` у магазина с амперсандом в её названии — та
 * же грабля, что была у адресов рейтинга метрики.
 */
function editSwitch(id, opts) {
  ...
  return `<input type="checkbox" id="${esc(id)}" class="edit-switch sr-only"${o.on ? ' checked' : ''}>
    <div class="a-panel-head a-panel-edit">
      ${o.title ? `<div class="o-list-title"><h2>${esc(o.title)}</h2>${o.note ? `<span>${esc(o.note)}</span>` : ''}${o.sum ? `<b class="o-list-sum">${o.sum}</b>` : ''}</div>` : ''}
...
/* Ширина СНИМКА в плитке категории на главной (`.cat-grid`), а не самой
 * плитки — `sizes` отвечает за то, что рисуется. На телефоне три колонки с
 * зазором 8 при полях страницы 20 и полях плитки 10: снимок — треть ширины
 * экрана минус 39 px (около 99 CSS-пикселей на 412). От 801 px четыре колонки
 * с зазором 16 при полях 24 и полях плитки 18: четверть экрана минус 60; в
 * контейнере 1180 это 240. Прежние «30vw» и «22vw» брали ширину ПЛИТКИ, без
 * полей — на телефоне это 124 вместо 99, и браузер просил 216 точек вместо
 * 173, то есть копию 320 там, где хватило бы 200 (см. CARD_SIZES в
 * lib/images.js). Числа обязаны идти рядом с `.cat-grid`, `.cat-tile-media`
 * и `.container` в styles.css. */
const TILE_SIZES_ATTR = '(min-width:1248px) 240px, (min-width:801px) calc(25vw - 60px), calc(33.3vw - 39px)';
...
function imageMarkup(product, index, opts) {
  ...
  if (imgs.length) {
    ...
    // `fetchpriority="high"` — у того, что может оказаться LCP. Плитки категорий
    // на главной одинаковые, и какая из первого ряда станет LCP, решает Chrome:
    // 19 сентября 2026 приоритет стоял у первой, а PageSpeed назвал LCP вторую
    // (AirPods) и потребовал у неё `fetchpriority=high`. Поэтому высокий
    // приоритет получает весь первый ряд; девять разом по-прежнему не ставим —
    // девять «высоких» означали бы, что высокого нет ни у одной.
    const priority = eager && !(opts && opts.priority === false);
    const card = ((opts && (opts.card || opts.small || opts.tile)) && opts.dir) ? IMG.cardSources(opts.dir, src) : null;
    ...
    const sizes = opts && opts.tile ? TILE_SIZES_ATTR : CARD_SIZES_ATTR;
    const srcset = (card && !small)
      ? ` srcset="${card.sizes.map(s => `/uploads/${esc(s.name)} ${s.size}w`).join(', ')}" sizes="${sizes}"`
      : '';
    // Запасной `src` у плитки — самая мелкая копия: на телефоне (три колонки)
    // её и выберет браузер, а без srcset так и было.
    const pick = card ? ((small || (opts && opts.tile)) ? card.sizes[0] : card.sizes[card.sizes.length - 1]) : null;
...
/* ------------------------- Вордмарк adc КОНТУРАМИ -------------------------
 *
 * Здесь не «похожий шрифт», а сам рисунок из референсного SVG. Контуры `a`
 * и `d` перенесены без единой изменённой координаты. Буква `c` построена из того
 * же овала, что у `d`: его кривые перенесены вправо, разомкнуты и подрезаны по
 * кривым Безье. Так у всех трёх букв совпадают высота, контраст штриха и пластика
 * овала; прежняя генерация из кругов давала другой, намного более жирный знак.
 * Инлайн нужен для `currentColor` и одинакового рисунка в витрине и предпросмотре панели.
 */
const ADC_MARK = {
  viewBox: '0 0 101.469 57',
  paths: [
    'M5.63721 45.6229C5.63721 47.4264 6.32504 48.8808 7.70395 49.9819C9.08137 51.0847 10.8982 51.6358 13.1533 51.6358C15.7084 51.6358 18.0124 51.0343 20.0681 49.8321C22.1216 48.6294 23.7249 47.0392 24.8781 45.0593C26.0305 43.0804 26.6071 40.9627 26.6071 38.7081V34.1985L26.9828 34.349C26.7818 35.5022 26.2309 36.3909 25.3291 37.0171C24.427 37.644 23.1494 38.0573 21.4961 38.2571L13.3034 39.1589C10.9981 39.4595 9.14351 40.1492 7.74134 41.2258C6.33809 42.3038 5.63721 43.7697 5.63721 45.6229ZM2.21292e-08 45.9232C2.21292e-08 42.5163 1.2401 39.8862 3.72043 38.0315C6.20063 36.1782 9.41978 35.0513 13.3785 34.6494L21.8717 33.6722C23.4747 33.522 24.6647 33.1087 25.4417 32.4323C26.2183 31.7554 26.6071 30.7163 26.6071 29.313C26.6071 26.9585 25.7799 25.1038 24.1268 23.7512C22.4731 22.3983 20.2934 21.7219 17.5878 21.7218C14.8818 21.7218 12.5518 22.3983 10.5977 23.7512C8.64371 25.1038 7.31524 27.1838 6.61422 29.9893L1.12752 28.486C2.02935 24.7282 3.97033 21.8474 6.95209 19.8428C9.93276 17.8392 13.5032 16.8365 17.6625 16.8365C21.9717 16.8365 25.4795 17.9766 28.1851 20.2559C30.8909 22.5367 32.2438 25.7311 32.2438 29.8392V55.6191H26.6071V43.7118H27.2079C26.0056 48.172 24.1015 51.4473 21.4777 53.4815C18.7866 55.5678 15.5322 56.3706 11.7249 56.3706C8.26751 56.3706 5.4489 55.4198 3.26945 53.5146C1.08964 51.6109 -0.000179313 49.0804 2.21292e-08 45.9232Z',
    'M63.0896 44.007C61.7058 46.1582 60.1074 47.9788 57.9032 49.2306C55.6979 50.4833 53.1926 51.1096 50.3873 51.1096C47.8814 51.1097 45.5638 50.4708 43.4347 49.1928C41.3043 47.9153 39.6264 46.1746 38.3994 43.9691C37.1708 41.765 36.5578 39.3094 36.5578 36.6034C36.5578 33.8979 37.1708 31.4434 38.3994 29.2377C39.6264 27.0332 41.3043 25.2917 43.4347 24.0141C45.5638 22.7363 47.8814 22.0973 50.3873 22.0973C53.1927 22.0973 55.6978 22.7245 57.9032 23.9763C60.1074 25.2296 61.7481 27.0232 63.0896 29.1999C63.9493 30.5947 64.5496 33.8979 64.5496 36.6034C64.5496 39.3094 64.1065 42.4262 63.0896 44.007ZM64.9685 2.28882e-05V28.635C64.7049 27.9168 64.4078 27.214 64.0667 26.5318C62.5635 23.5253 60.4704 21.1582 57.7908 19.4292C55.1095 17.7004 52.1162 16.8363 48.8088 16.8363C45.4514 16.8363 42.3944 17.7004 39.6395 19.4292C37.102 21.0208 35.0706 23.1603 33.5315 25.8344C33.7435 26.8899 33.8677 27.9832 33.8677 29.102V47.944C35.3596 50.3536 37.2678 52.3009 39.6017 53.7777C42.3323 55.5066 45.4009 56.3706 48.8088 56.3706C52.1162 56.3706 55.1095 55.5066 57.7908 53.7777C60.4704 52.0492 62.5635 49.6815 64.0667 46.6751C64.4078 45.993 64.7049 45.2903 64.9685 44.572V55.6191H70.6058V2.28882e-05H64.9685Z',
    'M101.4682 24.8046C100.077 22.604 98.3087 20.8124 96.165 19.4292C93.4837 17.7004 90.4904 16.8363 87.183 16.8363C83.8256 16.8363 80.7686 17.7004 78.0137 19.4292C75.4762 21.0208 73.4448 23.1603 71.9057 25.8344C72.1177 26.8899 72.2419 27.9832 72.2419 29.102V47.944C73.7338 50.3536 75.642 52.3009 77.9759 53.7777C80.7065 55.5066 83.7751 56.3706 87.183 56.3706C90.4904 56.3706 93.4837 55.5066 96.165 53.7777C98.3087 52.3949 100.077 50.603 101.4682 48.4023L99.1783 46.9561C98.3302 47.8366 97.3795 48.6047 96.2774 49.2306C94.0721 50.4833 91.5668 51.1096 88.7615 51.1096C86.2556 51.1097 83.938 50.4708 81.8089 49.1928C79.6785 47.9153 78.0006 46.1746 76.7736 43.9691C75.545 41.765 74.932 39.3094 74.932 36.6034C74.932 33.8979 75.545 31.4434 76.7736 29.2377C78.0006 27.0332 79.6785 25.2917 81.8089 24.0141C83.938 22.7363 86.2556 22.0973 88.7615 22.0973C91.5669 22.0973 94.072 22.7245 96.2774 23.9763C97.3795 24.603 98.3407 25.3647 99.1941 26.2418Z'
  ]
};
/* Конкретный знак рисуется только для `adc`. Любая другая надпись и
 * {\u0444игурные скобки} остаются текстом, как и до появления знака.
 */
function wordmark(text, label) {
  if (String(text || '').trim().toLowerCase() !== 'adc') return '';
  return `<svg class="logo-mark" viewBox="${ADC_MARK.viewBox}" role="img"`
    + ` aria-label="${esc(label || text)}">${ADC_MARK.paths.map(d => `<path d="${d}" fill="currentColor"/>`).join('')}</svg>`;
}
...
function logoMarkup(settings) {
  ...
  const text = (settings.logoText && settings.logoText.trim()) || settings.storeName;
  const mark = wordmark(text, settings.storeName);
  if (mark) return mark;
...
/* Знак витрины — локальная копия точного SVG из глобальной навигации
 * apple.com. Он намеренно не зависит от загруженного логотипа магазина:
 * покупатель видит один и тот же знак Apple в шапке и подвале, а внешний
 * ресурс не узнаёт IP посетителя и не может пропасть вместе с чужим CDN. */
function storefrontMark() {
  // width/height — пропорции холста SVG (viewBox 14×17): по ним браузер знает
  // ширину знака до загрузки файла, и шапка не перекладывается, когда он
  // приедет. Сам размер задаёт CSS (`.apple-logo-mark{height:…;width:auto}`),
  // атрибуты — только отношение сторон. PageSpeed без них отмечал CLS.
  return `<img class="apple-logo-mark" src="/static/apple-logo.svg?v=${assetV('apple-logo.svg')}" width="14" height="17" alt="">`;
}
...
/* Значки мессенджеров в контактах подвала — НАСТОЯЩИЕ логотипы сервисов, а не
 * нарисованные похожие фигуры: у знака бренда узнаётся ровно оригинал, и своя
 * версия круга с самолётиком выдавала себя сразу. Контуры взяты с Викисклада
 * (Telegram_logo.svg, WhatsApp.svg) и приведены к одному виду:
 *  - у WhatsApp оставлены только пузырь и трубка. В исходнике сверху лежат ещё
 *    серая тень с фильтром размытия и белая подложка под неё: в 22 px тень не
 *    видна вовсе, а `feGaussianBlur` — это отдельный проход отрисовки на каждой
 *    странице витрины;
 *  - `viewBox` обрезан по рамке пузыря (замер `getBBox` в браузере: 26.01 25.23
 *    122.31×122.32). В исходном холсте вокруг него оставлено поле под ту самую
 *    тень, и без обрезки WhatsApp выглядел бы заметно мельче Telegram при
 *    одинаковых 22 px;
 *  - **знак ОДНОЦВЕТНЫЙ и берёт цвет текста** (`fill:currentColor` у `.msg-ico`
 *    в styles.css). Мессенджеры стали обычными строками контактов, а в строке
 *    два фирменных пятна рядом с серой почтой и телефоном читались как реклама
 *    двух сервисов, а не как способы связи. Градиентов и их id с приставкой
 *    `msg-` поэтому больше нет вовсе — вместе с ними ушла и грабля «два логотипа
 *    на странице красятся градиентом друг друга».
 * Деталь внутри знака (самолётик, трубка) ВЫРЕЗАЕТСЯ цветом подвала — класс
 * `.msg-cut`, тот же приём, что у обводки знаков оплаты (`.pay-mc`): силуэт
 * остаётся плотным и узнаваемым, а на наведении, когда фигура берёт фирменный
 * цвет, получается ровно логотип сервиса. Шейдинг крыла у Telegram отброшен —
 * в одноцветном знаке ему не на чем читаться, и у самой Telegram монохромная
 * версия рисуется так же. */
function tgMark() {
  return "<svg class=\"msg-ico\" viewBox=\"0 0 240 240\" aria-hidden=\"true\"><circle cx=\"120\" cy=\"120\" r=\"120\"/><path class=\"msg-cut\" d=\"M81.486,130.178,52.2,120.636s-3.5-1.42-2.373-4.64c.232-.664.7-1.229,2.1-2.2,6.489-4.523,120.106-45.36,120.106-45.36s3.208-1.081,5.1-.362a2.766,2.766,0,0,1,1.885,2.055,9.357,9.357,0,0,1,.254,2.585c-.009.752-.1,1.449-.169,2.542-.692,11.165-21.4,94.493-21.4,94.493s-1.239,4.876-5.678,5.043A8.13,8.13,0,0,1,146.1,172.5c-8.711-7.493-38.819-27.727-45.472-32.177a1.27,1.27,0,0,1-.546-.9c-.093-.469.417-1.05.417-1.05s52.426-46.6,53.821-51.492c.108-.379-.3-.566-.848-.4-3.482,1.281-63.844,39.4-70.506,43.607A3.21,3.21,0,0,1,81.486,130.178Z\"/></svg>";
}
function waMark() {
  return "<svg class=\"msg-ico\" viewBox=\"26.01 25.23 122.31 122.32\" aria-hidden=\"true\"><path d=\"M87.184 25.227c-33.733 0-61.166 27.423-61.178 61.13a60.98 60.98 0 0 0 9.349 32.535l1.455 2.313-6.179 22.558 23.146-6.069 2.235 1.324c9.387 5.571 20.15 8.517 31.126 8.523h.023c33.707 0 61.14-27.426 61.153-61.135a60.75 60.75 0 0 0-17.895-43.251 60.75 60.75 0 0 0-43.235-17.928z\"/><path class=\"msg-cut\" fill-rule=\"evenodd\" d=\"M68.772 55.603c-1.378-3.061-2.828-3.123-4.137-3.176l-3.524-.043c-1.226 0-3.218.46-4.902 2.3s-6.435 6.287-6.435 15.332 6.588 17.785 7.506 19.013 12.718 20.381 31.405 27.75c15.529 6.124 18.689 4.906 22.061 4.6s10.877-4.447 12.408-8.74 1.532-7.971 1.073-8.74-1.685-1.226-3.525-2.146-10.877-5.367-12.562-5.981-2.91-.919-4.137.921-4.746 5.979-5.819 7.206-2.144 1.381-3.984.462-7.76-2.861-14.784-9.124c-5.465-4.873-9.154-10.891-10.228-12.73s-.114-2.835.808-3.751c.825-.824 1.838-2.147 2.759-3.22s1.224-1.84 1.836-3.065.307-2.301-.153-3.22-4.032-10.011-5.666-13.647\"/></svg>";
}
/* «О компании» в той же группе меню, что и мессенджеры, поэтому со значком: у
 * двух строк из трёх он выглядел бы потерянным. Глиф свой — общепринятый
 * кружок «информация», нарисованный тем же волосяным штрихом 1.7, что бургер,
 * лупа и корзина в шапке: это язык значков витрины, а не второй набор. Фигура
 * контурная, а не залитая, — залитый кружок в 20 px читался бы пятном рядом со
 * знаками сервисов, у которых заливка своя по природе. */
function infoMark() {
  /* Холст обрезан по кружку ВМЕСТЕ С ЕГО ОБВОДКОЙ (12 ± (8.6 + 1.7/2)), а не по
   * общим 24×24: у Telegram и WhatsApp рядом фигура заполняет холст целиком, и
   * на одинаковых 20 px этот кружок выходил на пятую часть мельче соседей по
   * группе. Правишь радиус или толщину штриха — пересчитай рамку. */
  return '<svg class="nav-glyph" viewBox="2.55 2.55 18.9 18.9" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="8.6"/><path d="M12 11.4v4.9"/><path d="M12 7.9h.01"/></svg>';
}
/* wa.me принимает номер только цифрами, без `+`, пробелов и скобок. Номер в
 * настройках уже проверен и хранится в E.164, но повторный `PHONE.store()`
 * защищает витрину от старого или вручную испорченного settings.json. Текст
 * кодируется целиком: кириллица, переносы и `&` остаются одной репликой, а не
 * становятся частью query-строки. */
function whatsAppHref(settings) {
  const stored = PHONE.store(settings && settings.contactWhatsApp);
  if (!stored) return '';
  const message = String(settings.contactWhatsAppMessage || '').trim();
  return `https://wa.me/${stored.slice(1)}${message ? '?text=' + encodeURIComponent(message) : ''}`;
}
/* Булавка у адреса офлайн-точки в подвале. Подпись «Офлайн-магазин» над строкой
 * ушла: слово повторяло то, что и так говорит сам адрес, и занимало отдельную
 * строку в колонке контактов. Глиф — та же ЗАЛИТАЯ булавка, что стоит у подписи
 * «Адрес» на оформлении (NOTE_ICONS.pin в public/app.js): в 16 px контурная не
 * читается, и второй рисунок для одного и того же понятия заводить незачем. */
function pinIcon() { return '<svg class="foot-pin" viewBox="0 0 24 24" aria-hidden="true"><path fill-rule="evenodd" d="M12 2.2a7.4 7.4 0 0 0-7.4 7.4c0 5.2 6.6 11.6 6.88 11.87a.75.75 0 0 0 1.04 0c.28-.27 6.88-6.67 6.88-11.87A7.4 7.4 0 0 0 12 2.2Zm0 10.1a2.7 2.7 0 1 1 0-5.4 2.7 2.7 0 0 1 0 5.4Z"/></svg>'; }
/* Значки контактов на странице «О компании». Знаки мессенджеров там те же, что в
 * подвале (`tgMark`/`waMark`), поэтому телефону, почте и часам нужны глифы того
 * же вида — иначе половина столбика читалась бы как значки из другого набора.
 * Взяты ЗАЛИТЫЕ контуры из `NOTE_ICONS` в public/app.js, которыми подписаны поля
 * на оформлении: в 18 px контурный глиф не читается вовсе. Класс тот же
 * (`.msg-ico`), поэтому цвет берётся у строки — под курсором значок
 * перекрашивается вместе с подписью. */
/* ХОЛСТ ОБРЕЗАН ПО САМОЙ ФИГУРЕ, и это не мелочь. Знаки мессенджеров заполняют
 * свой `viewBox` целиком (у Telegram круг во весь холст, у WhatsApp рамка так и
 * обрезана — см. `waMark`), а нарисованные на общем холсте 24×24 конверт и часы
 * занимают его на две трети: в столбике контактов они выглядели заметно мельче
 * соседей, хотя `width`/`height` у всех одинаковые. Поэтому у каждого глифа свой
 * `viewBox` по его рамке — числа ниже это границы контуров, и правка пути
 * требует их пересчёта, иначе фигура уедет с середины. Вписывается она по
 * ДЛИННОЙ стороне (`preserveAspectRatio` по умолчанию), то же правило, что у
 * глифов преимуществ в `glyphBody()`: конверт шире, чем выше, и растянуть его до
 * высоты круга значило бы его исказить. */
function contactIcon(box, body) { return `<svg class="msg-ico" viewBox="${box}" aria-hidden="true">${body}</svg>`; }
function phoneIcon() { return contactIcon('6 1.6 12 20.8', '<path fill-rule="evenodd" d="M8.6 1.6h6.8A2.6 2.6 0 0 1 18 4.2v15.6a2.6 2.6 0 0 1-2.6 2.6H8.6A2.6 2.6 0 0 1 6 19.8V4.2a2.6 2.6 0 0 1 2.6-2.6Zm1.15 2.6a.55.55 0 0 0 0 1.1h4.5a.55.55 0 0 0 0-1.1h-4.5Z"/>'); }
function mailIcon() { return contactIcon('2.25 4.5 19.5 14.5', '<path d="M2.4 6.2A2 2 0 0 1 4.4 4.5h15.2a2 2 0 0 1 2 1.7L12 12.6 2.4 6.2Z"/><path d="M2.25 8.2 11.6 14.4a.7.7 0 0 0 .8 0l9.35-6.2V17a2 2 0 0 1-2 2H4.25a2 2 0 0 1-2-2V8.2Z"/>'); }
function clockIcon() { return contactIcon('2.4 2.4 19.2 19.2', '<path fill-rule="evenodd" d="M12 2.4a9.6 9.6 0 1 0 0 19.2 9.6 9.6 0 0 0 0-19.2Zm-.85 4.6a.85.85 0 0 1 1.7 0v4.92l3.29 1.9a.85.85 0 1 1-.85 1.47l-3.71-2.14a.85.85 0 0 1-.43-.74V7Z"/>'); }
...
function brandPreview(s) {
  ...
  /* При правке надписи скрипту нужен тот же готовый знак, что рисует сервер.
   * Если `adc` уже сохранено, исходником станет SVG в самом предпросмотре; если
   * сейчас там другая надпись, кладём знак в inert-<template>. Второй копии
   * контуров в admin-ui.js поэтому нет и быть не должно. Логотип-картинку на
   * лету не подменяем вовсе — пока файл не сохранён, показывается текущий. */
  const currentText = (s.logoText && s.logoText.trim()) || s.storeName;
  const currentMark = !s.logoImage && wordmark(currentText, s.storeName);
  const markTemplate = !s.logoImage && !currentMark
    ? `<template data-brand-mark>${wordmark('adc', s.storeName)}</template>` : '';
...
function brandFields(s) {
  ...
  return `${brandPreview(s)}
    ...
    ${/* Снятая галочка приходит отсутствием поля, и скрытый `brandForm` говорит,
         что секция вообще пришла, — то же правило, что у отзывов и Метрики.
         Что именно дописывается, объясняет `cardName()` выше. */''}
    <input type="hidden" name="brandForm" value="1">
    <div class="field field-check"><label><input type="checkbox" name="cardNameStorage"${s.cardNameStorage ? ' checked' : ''}> В названии карточки каталога — объём памяти сборки, чья цена показана</label></div>
    <p class="field-hint">«iPhone 15 Pro Max» → «iPhone 15 Pro Max 256 ГБ». Только у товаров с выбором памяти; часов, наушников и товаров без цены не касается.</p>
    ${/* Весь каталог на главной — см. `homeCatalogOn()` и `homePage()`. */''}
    <div class="field field-check"><label><input type="checkbox" name="homeCatalog"${s.homeCatalog ? ' checked' : ''}> На главной — сразу весь каталог</label></div>
    <p class="field-hint">Под слоганом идёт сетка всех товаров: без плиток категорий, «Популярного» и заголовка. Снята — главная с плитками и подборкой, каталог целиком на своей странице.</p>`;
}
...
/* КАРТОЧКА БЕЗ ЦЕНЫ.
 *
 * Галочка «Не показывать цену» у товара (`hidePrice`). Нужна там, где цены ещё
 * нет: модель объявлена, прайс поставщика не пришёл, а карточку показать уже
 * надо — с фотографиями, характеристиками и вариантами.
 *
 * Спрашивают её ВСЕ одним способом — этой функцией: карточка каталога, страница
 * товара, разметка для поисковика и факты консультанта. Второе место, решающее
 * «показывать ли цену», разъехалось бы с первым молча, и товар без ценника на
 * витрине называл бы цену в чате.
 *
 * Поля нет — цена показывается: так читаются и catalog.js, и все карточки,
 * созданные до появления галочки.
 */
function priceHidden(p) { return !!(p && p.hidePrice); }

/* ЦЕНУ НЕ ПОКАЗЫВАЕМ — ЗНАЧИТ И НЕ ПРОДАЁМ, и это не перестраховка.
 * Корзина считает цену сама (`lib/pricing.js`) и показала бы ту самую сумму,
 * которую карточка скрыла: покупатель узнавал бы её уже после «в корзину».
 * Прятать ценник и продавать по нему же — это не «без цены», а цена мелким
 * шрифтом на шаг позже. */
function sellable(p, settings) {
  if (priceHidden(p)) return false;
   ...
   * в отказ на оформлении. Сравниваем ЦЕНУ ПОКУПАТЕЛЯ: касса проводит её, а не
   * полную цену каталога, и при работающей промоакции товар вполне может
   * помещаться в предел со скидкой. */
  return fitsOrderLimit(PROMO.shopPrice(startPrice(p, settings), p, settings).price, orderMax(settings));
}
...
/* ОБЪЁМ ПАМЯТИ СТАРТОВОЙ СБОРКИ В НАЗВАНИИ КАРТОЧКИ — настройка сайта, а не код.
 *
 * Владелец второго магазина попросил (20 сентября 2026): в каталоге у названия
 * стоит объём той сборки, цену которой карточка и показывает — «iPhone 15 Pro
 * Max 256 ГБ». Первому сайту это не нужно, поэтому это галочка `cardNameStorage`
 * в «Оформлении», по умолчанию снятая: правка для одного сайта — настройка с
 * умолчанием «как было», а не ветка по домену (см. «Два сайта из одного
 * репозитория» в CLAUDE.md).
 *
 * Объём берётся у ТОЙ ЖЕ сборки, что и цена (`defaultChoices()`, им же живёт
 * `startPrice()`): иначе при распроданных 256 ГБ карточка называла бы «256 ГБ»
 * ценой за 512. Из метки конфигурации вынимается только память — у MacBook Neo
 * она записана как «256 ГБ · Magic Keyboard», и в названии от неё нужны одни
 * «256 ГБ»; у часов в конфигурациях размер корпуса, у наушников конфигураций
 * нет вовсе — им дописывать нечего. Без ценника («Цена уточняется») нет и
 * сборки, «цена которой показывается», — дописки тоже нет. Название, в котором
 * объём уже стоит (вписали руками в панели), второй раз его не получает.
 */
function cardNameStorageOn(settings) { return !!(settings && settings.cardNameStorage); }

/* ВЕСЬ КАТАЛОГ НА ГЛАВНОЙ — тоже настройка сайта (`homeCatalog`, галочка в
 * «Оформлении», по умолчанию снята). Владелец второго магазина попросил
 * (20 сентября 2026): на главной — просто все товары, без плиток категорий,
 * «Популярного» и без заголовка «Каталог» или «Главная». Это ровно та главная,
 * какой она была до 18 сентября 2026: первый экран (слоган и бегущая строка) и
 * сразу сетка всего каталога — `homePage()` ниже. Первый сайт остаётся с
 * плитками и подборкой; `/catalog` с заголовком и рядом категорий у обоих
 * сайтов на месте — туда ведут вкладка «Каталог», меню и поиск. */
function homeCatalogOn(settings) { return !!(settings && settings.homeCatalog); }
// \b с кириллицей не работает: конец единицы проверяем явно — за «ГБ» не должна
// идти буква. Число с запятой («1,5 ТБ») тоже объём.
const STORAGE_LABEL_RE = /(\d+(?:[.,]\d+)?)\s*(ГБ|ТБ|GB|TB)(?![A-Za-zА-Яа-яЁё])/i;
// Между числом и единицей — неразрывный пробел: в узкой карточке телефона
// «256 ГБ» иначе рвалось надвое, и «ГБ» висело одно на второй строке.
function startStorageLabel(p) {
  if (!p || !((p.storages || []).length)) return '';
  const m = STORAGE_LABEL_RE.exec(String(defaultChoices(p).storage || ''));
  return m ? `${m[1]}\u00a0${m[2].toUpperCase()}` : '';
}
const plainSpaces = text => String(text).replace(/\u00a0/g, ' ').toLowerCase();
function cardName(p, settings) {
  const name = String((p && p.name) || '').trim();
  if (!cardNameStorageOn(settings) || priceHidden(p)) return name;
  const storage = startStorageLabel(p);
  if (!storage || plainSpaces(name).includes(plainSpaces(storage))) return name;
  return `${name} ${storage}`;
}
...
// Значки строки отзывов и плашки промоакции — по образцу карточки Ozon: одна
...
/* Что стоит на месте ценника у карточки без цены. Строка ОДНА на каталог и
 * страницу товара: два разных ответа на один вопрос читались бы как разные
 * состояния товара. «Цена уточняется», а не «по запросу» — запрашивать её нам
 * негде: прайс поставщика ещё не пришёл, и это ровно то, что здесь сказано. */
const PRICE_SOON = '<span class="price-soon">Цена уточняется</span>';
...
function productCard(p, settings, db, opts) {
   ...
   */
  /* `startPrice()` отдаёт ПОЛНУЮ цену сборки, а сколько платит покупатель —
   * решает промокод, применённый по умолчанию (`PROMO.shopPrice`): выключенная
   * промоакция оставляет полную цену и убирает зачёркнутую вместе с плашкой.
   * Считать скидку здесь самим нельзя — цена карточки разошлась бы с корзиной,
   * которая идёт через ту же функцию. */
  const pct = PROMO.pctFor(p, settings);
  const shown = PROMO.shopPrice(startPrice(p, settings), p, settings);
  const eff = shown.price;
  // Зачёркнутая — полная цена той же сборки: процент у товара один на все
  // сборки, а вот рубли выгоды у более дорогой сборки свои.
  const cmp = shown.compare;
  ...
  /* Плашка «Промоакция» лежит на снимке слева внизу — там, где её ставит Ozon.
   * Только у того, что можно купить: обещать скидку на распроданном незачем,
   * ровно как и процент выгоды рядом с ценой. Выключенная промоакция обнуляет
   * `pct` выше, поэтому плашка исчезает вместе с зачёркнутой ценой — одним
   * условием на всё, без второго выключателя. */
  const saleBadge = (inStock && cmp && pct > 0)
    ? `<span class="card-sale">${SALE_FLAME}Промоакция</span>` : '';
  ...
  <article class="card${inStock ? '' : ' card-out'}">
    ...
    <div class="card-body">
      <a class="card-name" href="${href}">${esc(cardName(p, settings))}</a>
      ${ratingLine}
      ${priceHidden(p)
        ? `<div class="card-price card-price-none">${PRICE_SOON}</div>`
...
const BANK_ALIASES = {
  юmoney: 'yoomoney', юмани: 'yoomoney', юманей: 'yoomoney', yumoney: 'yoomoney', yoomoney: 'yoomoney',
  альфа: 'alfabank', альфабанк: 'alfabank', аоальфабанк: 'alfabank', alfa: 'alfabank', alfabank: 'alfabank', alfabankjsc: 'alfabank'
};
const BANK_LOGOS = { alfabank: 'banks/alfa.svg' };
function bankKey(name) {
  return String(name == null ? '' : name).toLowerCase().replace(/[^0-9a-zа-яё]+/g, '');
}
function bankLogo(name) {
  const key = BANK_ALIASES[bankKey(name)] || bankKey(name);
  const file = BANK_LOGOS[key];
  if (file) return `<img class="bank-logo" src="/static/${file}?v=${assetV(file)}" width="24" height="24" alt="" aria-hidden="true" decoding="async">`;
  return BANK_MARKS[key] ? BANK_MARKS[key].replace('<svg ', '<svg class="bank-logo" ') : '';
}
...
const FOOTER_SBP_MARK = `<span class="pay pay-sbp" role="listitem">${PAY_MARKS.sbp}${PAY_MARKS.sbpWord}<span class="sr-only">СБП</span></span>`;
...
// Проверка только локальных настроек, без запросов к кассам. Для выбора
// способа учитываем его маршрут; для подвала — все активные способы оплаты.
function onlyAlfaPayments(settings, method, currency) {
  const providers = method ? PAYMENTS.chainFor(settings, null, method, currency)
    : PAYMENTS.enabledProviders(settings);
  return providers.length > 0 && providers.every(provider => provider.id === 'alfabank');
}
function footerPayMethods(settings) {
  if (PAYMENTS.mode(settings) === 'request') return '';
  // Свои реквизиты активны только при отключённых кассах. Сохранённый номер
  // карты не означает приём карт, пока единственная включённая касса — Альфа.
  return onlyAlfaPayments(settings)
    ? `<div class="footer-pay" role="list" aria-label="Способы оплаты">${FOOTER_SBP_MARK}</div>`
    : PAY_METHODS;
}
...
/* Значок кнопки чата — залитый пузырь с тремя точками, снятый с виджета
 * amoCRM на mobilworld.by (18 сентября 2026): контур перенесён координата в
 * координату, холст 30×24. Тот же глиф стоит в аватаре шапки окна — у кнопки и
 * у окна один знак, иначе покупатель, нажав на одно, открывает что-то другое.
 * Прежний контурный пузырь снят: рядом с волнами вокруг кнопки волосяной глиф
 * читался бледной мелочью, залитый — знаком. */
const CHAT_FAB_ICON = '<svg class="chat-fab-ico" viewBox="0 0 30 24" aria-hidden="true"><path fill="currentColor" d="M20.612 0H9.505C4.565 0 .56 4.433.56 9.895c0 4.368 2.558 8.064 6.105 9.38L10.916 24l3.803-4.206h5.893c4.942 0 8.947-4.43 8.947-9.899C29.559 4.433 25.553 0 20.612 0ZM8.154 11.998c-1.096 0-1.985-.98-1.985-2.195 0-1.211.889-2.196 1.985-2.196s1.985.984 1.985 2.196c0 1.214-.889 2.195-1.985 2.195Zm6.905 0c-1.096 0-1.985-.98-1.985-2.195 0-1.211.888-2.196 1.985-2.196 1.096 0 1.985.984 1.985 2.196 0 1.214-.89 2.195-1.985 2.195Zm6.905 0c-1.097 0-1.986-.98-1.986-2.195 0-1.211.889-2.196 1.986-2.196 1.096 0 1.985.984 1.985 2.196 0 1.214-.89 2.195-1.985 2.195Z"/></svg>';
// Крестик открытого состояния — оттуда же (холст 19×19). Кнопка при открытом
// окне не прячется, а становится кнопкой «закрыть»: белый круг с крестиком.
const CHAT_FAB_CLOSE = '<svg class="chat-fab-x" viewBox="0 0 19 19" aria-hidden="true"><path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M9.33274 11.3695L2.46267 18.2395C1.89896 18.8032 0.98461 18.8057 0.42219 18.2433C-0.144151 17.677 -0.138549 16.7673 0.42597 16.2028L7.29603 9.33276L0.42597 2.46267C-0.137747 1.89896 -0.14023 0.984599 0.42219 0.422179C0.988531 -0.144162 1.89816 -0.138538 2.46267 0.425981L9.33274 7.29605L16.2028 0.425981C16.7666 -0.137736 17.6809 -0.140241 18.2433 0.422179C18.8097 0.98852 18.8041 1.89816 18.2395 2.46267L11.3694 9.33276L18.2395 16.2028C18.8033 16.7665 18.8057 17.6809 18.2433 18.2433C17.677 18.8097 16.7674 18.804 16.2028 18.2395L9.33274 11.3695Z"/></svg>';
...
function chatWidget(settings, waiting) {
  ...
  ${/* Кнопка — точь-в-точь виджет amoCRM (mobilworld.by): плоский круг 60 px
        акцентного цвета без тени, залитый пузырь, две расходящиеся волны того
        же цвета (::before/::after в styles.css). Открытое окно кнопку не
        прячет — она становится белым кругом с крестиком и закрывает чат:
        оба глифа лежат в разметке, какой показать, решает класс .is-open. */''}
  <button class="chat-fab" id="chat-open" type="button" aria-label="Чат с магазином" aria-expanded="false" aria-controls="chat-panel">
    ${CHAT_FAB_ICON}${CHAT_FAB_CLOSE}
  ...
  <section class="chat-panel" id="chat-panel" role="dialog" aria-label="Чат с магазином" aria-hidden="true" inert>
    <header class="chat-head">
      <span class="chat-avatar" aria-hidden="true">${CHAT_FAB_ICON}</span>
...
/* ===== Нижняя панель разделов на телефоне — снята с i-store.by =====
 *
 * Панель точь-в-точь как у их мобильной версии (18 сентября 2026, числа с их
 * живой страницы, а не по памяти): белая полоса 61 px внизу экрана, верхние
 * углы 12 px, тень `0 -3px 10px rgba(0,0,0,.1)` вверх, пять вкладок «значок над
 * подписью 10 px», выбранная — цветом темы (у них #0071e3, у нас акцент
 * магазина). У них это «Главная · Каталог · Корзина · Избранные · Сравнение»;
 * избранного и сравнения у нас нет, а есть кабинет и чат — они и стоят на их
 * месте. Значков берём не их, а свои, тем же волосяным штрихом, что лупа,
 * корзина и кабинет в шапке: один язык значков на всю витрину.
 *
 * ГЛАВНАЯ И КАТАЛОГ — РАЗНЫЕ СТРАНИЦЫ, и панель этого требует: две первые
 * вкладки обязаны вести в разные места. Отсюда `/catalog` (см. catalogPage).
 *
 * Панель есть только на телефоне (≤800 px, тот же порог, что у остального
 * мобильного блока); на компьютере разделы и так в шапке и в меню.
 */
const TAB_ICONS = {
  home: '<path d="M3.6 10.3 12 3.2l8.4 7.1v9.2a1.5 1.5 0 0 1-1.5 1.5h-4.7v-5.3a2.2 2.2 0 0 0-4.4 0V21H5.1a1.5 1.5 0 0 1-1.5-1.5z"/>',
  catalog: '<path d="M4 6.3h.7M9.2 6.3h10.8M4 12h.7M9.2 12h10.8M4 17.7h.7M9.2 17.7h10.8"/>',
  // Тот же контур тележки, что у кнопки корзины в шапке.
  cart: '<path d="M6 6h15l-1.5 9h-12z M6 6l-1-3H2 M9 20a1 1 0 100 2 1 1 0 000-2 M18 20a1 1 0 100 2 1 1 0 000-2"/>',
  // Тот же силуэт, что у значка кабинета в шапке (ACCOUNT_ICON).
  account: '<circle cx="12" cy="8.2" r="4.1"/><path d="M4.6 20.4c.7-3.9 3.8-6.3 7.4-6.3s6.7 2.4 7.4 6.3"/>',
  chat: '<path d="M12 3.6c-4.8 0-8.6 3.2-8.6 7.1 0 2.2 1.1 4.2 3 5.5l-.8 4.2 4.2-2c.7.2 1.4.3 2.2.3 4.8 0 8.6-3.2 8.6-7.1S16.8 3.6 12 3.6z"/><path d="M8.4 10.9h.01M12 10.9h.01M15.6 10.9h.01"/>'
};
function tabBar(settings, opts) {
  const active = opts.tab || '';
  const item = (key, label, attrs, badge) => {
    const on = active === key;
    const tag = /href=/.test(attrs) ? 'a' : 'button';
    return `<${tag} class="tabbar-item${on ? ' is-active' : ''}" ${attrs}${on ? ' aria-current="page"' : ''}>`
      + `<span class="tabbar-ico"><svg viewBox="0 0 24 24" aria-hidden="true">${TAB_ICONS[key]}</svg>${badge || ''}</span>`
      + `<span class="tabbar-label">${label}</span></${tag}>`;
  };
  /* Счётчики — те же числа, что на кнопке корзины в шапке и на кнопке чата:
   * скрипты витрины (updateBadge в app.js, paintBadge в chat.js) находят их по
   * data-атрибуту и пишут туда то же значение. Скринридеру число не нужно —
   * оно и так уходит в имя кнопки в шапке.
   *
   * Значок у чата — КРАСНЫЙ (`tabbar-badge-alert`), как у круглой кнопки на
   * компьютере: это не количество, а «вам написали», и на вкладке, значок
   * которой и так пульсирует акцентом, синий кружок сливался бы с волной.
   * Счётчик корзины остаётся цветом темы — это число товаров, а не сигнал. */
  const cartBadge = '<span class="tabbar-badge" data-cart-count aria-hidden="true" hidden>0</span>';
  const wait = Math.max(0, Math.floor(Number(opts.chatWaiting) || 0));
  const chatBadge = `<span class="tabbar-badge tabbar-badge-alert" data-chat-badge aria-hidden="true"${wait ? '' : ' hidden'}>${wait ? esc(wait > 9 ? '9+' : String(wait)) : '0'}</span>`;
  return `<nav class="tabbar" aria-label="Разделы">
  ${item('home', 'Главная', 'href="/"')}
  ${item('catalog', 'Каталог', 'href="/catalog"')}
  ${item('cart', 'Корзина', 'href="/checkout"', cartBadge)}
  ${/* Отметки «вошёл» у кабинета НЕТ (снята 20 сентября 2026): синяя точка в
        углу силуэта читалась уведомлением — «там что-то новое», — а нового там
        не было; владелец так её и прочитал. Вошёл покупатель или нет, скажет
        сама страница кабинета. То же у значка в шапке. */''}
  ${CUSTOMERS.enabled(settings) ? item('account', 'Кабинет', 'href="/account"') : ''}
  ${/* Чат — кнопка, а не ссылка: окно открывает тот же скрипт, что и круглая
        кнопка в углу (data-chat-open ловит public/chat.js). На телефоне это
        ЕДИНСТВЕННАЯ дверь в чат — круглая кнопка там спрятана (см. блок
        панели в styles.css), а внимание к вкладке держит пульс значка. Чат
        выключен — вкладки нет, как нет и самой кнопки. */''}
  ${CHAT.visible(settings) ? item('chat', 'Чат', 'type="button" data-chat-open aria-controls="chat-panel"', chatBadge) : ''}
</nav>`;
}
...
function layout(settings, opts) {
  ...
  /* Разделы — строками выезжающей панели, как в нашей же панели управления и в
   * боковом меню Google Trends, откуда она снята. Ряда вкладок под шапкой
   * больше нет: он занимал 48 px на каждой странице ради навигации, к которой
   * возвращаются пару раз за визит, и на телефоне превращался в ленту,
   * прокручиваемую вбок, — то есть в третий способ навигации рядом с логотипом
   * и корзиной. Цена решения честная: разделы перестали быть видны с первого
   * взгляда, и найти их можно только нажав кнопку. */
  const navItem = (href, label, active) =>
    `<a href="${href}" class="nav-item${active ? ' active' : ''}"${active ? ' aria-current="page"' : ''}>${label}</a>`;
  /* «Каталог» выделен только на самой странице каталога без фильтра: раньше
   * первый пункт («Все») горел на любой странице без категории, включая «О
   * компании», — а теперь у главной своя страница, и на ней каталог не открыт. */
  const navItems = navItem('/catalog', 'Каталог', opts.tab === 'catalog' && !opts.activeCategory)
    + cats.map(c => navItem('/catalog?category=' + encodeURIComponent(c), esc(c), opts.activeCategory === c)).join('');
  /* Форма поиска одна. Она лежит в раскрывающейся части шапки и до нажатия на
   * лупу вынута из обхода клавиатурой через inert. Вторая копия в меню больше
   * не нужна: два поля с одним значением усложняли доступность и расходились по
   * виду на разных ширинах. */
  const searchForm = (cls, id) => `<form class="${cls}" action="/catalog" method="get" role="search">
      <input id="${id}" type="search" name="q" placeholder="Поиск товаров" value="${esc(opts.q || '')}" aria-label="Поиск товаров">
    </form>`;
  /* Мессенджеры — обычные строки меню под одной волосяной линией. WhatsApp
   * получает ту же ссылку с готовой репликой, что и кнопка в подвале: два
   * разных URL для одного действия разошлись бы после первой правки настроек. */
  const waUrl = whatsAppHref(settings);
  /* Значки у строк меню есть ТОЛЬКО в этой группе, и это не исключение из
   * правила, а само правило: у категорий каталога («iPhone», «Vision»,
   * «Аксессуары») глиф пришлось бы выдумывать, а здесь он существует — знак
   * сервиса и общепринятая «информация». Группа стоит под волосяной линией,
   * поэтому свой вид читается признаком группы, а не поломкой ряда; значок при
   * этом обязан быть у КАЖДОЙ её строки — у двух из трёх он выглядел бы
   * потерянным. Геометрию строки со значком даёт `nav-ico`, фирменный цвет
   * наведения — `nav-msg` вместе с `msg-tg`/`msg-wa`, как в подвале.
   *
   * «О компании» стоит первой: это страница магазина, а мессенджеры уводят
   * наружу, и внешним ссылкам место в конце. Группа есть всегда — страница
   * никуда не девается, даже когда контакты в настройках не заполнены. */
  const navMessengers = [
    settings.contactTelegram
      ? `<a class="nav-item nav-ico nav-msg msg-tg" href="https://t.me/${esc(tgUser(settings))}" target="_blank" rel="noopener">${tgMark()}<span>Telegram</span></a>` : '',
    waUrl
      ? `<a class="nav-item nav-ico nav-msg msg-wa" href="${esc(waUrl)}" target="_blank" rel="noopener">${waMark()}<span>WhatsApp</span></a>` : ''
  ].filter(Boolean).join('');
  const navContacts = `<span class="nav-sep" aria-hidden="true"></span>`
    + `<a class="nav-item nav-ico" href="/about">${infoMark()}<span>О компании</span></a>${navMessengers}`;
  /* В подвале те же два адреса — двумя обычными строками контактов, каждая на
   * своей. Вид у них ОДИН и живёт в `msg-link`: цвет текста подвала, значок тем
   * же цветом, ни пилюли, ни рамки — рядом с почтой и телефоном две белые
   * кнопки читались как реклама сервисов, а не как способы связи. Различаются
   * они ровно одним — фирменным цветом наведения, и он приезжает единственной
   * переменной `--msg-brand` из класса-приставки (`msg-tg` / `msg-wa`). Полная
   * пара классов на сервис означала бы, что Telegram и WhatsApp разъедутся по
   * виду на первой правке, а увидеть это можно только глазами.
   *
   * Своей обёртки у пары больше НЕТ: строки лежат прямо в колонке контактов,
   * рядом с телефоном и почтой. Прежний `.msg-links` держал собственный зазор
   * (7 px против 10 у колонки), и в столбике из четырёх одинаковых строк между
   * почтой и Telegram щель выходила заметно шире, чем между самими
   * мессенджерами. Одним зазором на всю колонку разъезжаться нечему. */
  const footMessengers = [
    settings.contactTelegram
      ? `<a class="msg-link msg-tg" href="https://t.me/${esc(tgUser(settings))}" target="_blank" rel="noopener">${tgMark()}<span>Telegram</span></a>` : '',
    waUrl
      ? `<a class="msg-link msg-wa" href="${esc(waUrl)}" target="_blank" rel="noopener">${waMark()}<span>WhatsApp</span></a>` : ''
  ].filter(Boolean).join('');
  // Те же категории списком в подвале: меню разделов уезжает вместе с шапкой, а
  // до подвала покупатель доходит, дочитав страницу до конца, — и там ему проще
  // перейти в соседний раздел, чем возвращаться наверх.
  const footCats = cats.map(c =>
    `<li><a href="/catalog?category=${encodeURIComponent(c)}">${esc(c)}</a></li>`
  ).join('');
  /* Номер счётчика Яндекс Метрики — в атрибуте <html>, а сам загрузчик в
   * `public/ym.js`, и оба появляются ТОЛЬКО когда сервер решил отдать счётчик
   * этому посетителю (`opts.ym`, см. lib/yandex-metrika.js). Скрипт стоит
   * раньше app.js: цели витрина отмечает через `window.ymGoal`, и он обязан
   * существовать к моменту, когда app.js начнёт работать. Где счётчика нет, в
   * разметке нет ни номера, ни скрипта, ни слова про Яндекс. */
  const ym = opts.ym && opts.ym.id ? String(opts.ym.id) : '';
  return `<!DOCTYPE html>
<html lang="ru"${ym ? ` data-ym="${esc(ym)}"` : ''}>
<head>
<meta charset="utf-8">
...
<link rel="icon" href="/static/favicon.ico?v=${assetV('favicon.ico')}">
<meta name="theme-color" content="#ffffff">
<meta name="color-scheme" content="light">
${stylesTag(opts)}
<link rel="stylesheet" href="/static/smoothui-toast.css?v=${assetV('smoothui-toast.css')}">
...
<body class="storefront${opts.productPage ? ' product-page' : ''}">
${PH_SPRITE}${CARD_SPRITE}
${/* Состояние меню держит скрытый чекбокс, а не <details>: у шапки есть
      backdrop-filter, а он делает элемент опорным для position:fixed — панель
      внутри неё считала бы координаты от шапки, а не от окна. Поэтому чекбокс
      лежит РЯДОМ с шапкой и ПЕРЕД ней: и панель, и кнопка находятся селектором
      соседа (~), и перестановка молча оставила бы кнопку без действия. Имя даём
      самому чекбоксу — внутри подписи один глиф и ни буквы текста. */''}
<input type="checkbox" id="nav-menu" class="nav-check" aria-label="Меню">
<header class="site-header">
  <div class="container header-row">
    <label class="nav-btn" for="nav-menu" title="Меню">
    ...
    <a class="apple-logo" href="/" aria-label="На главную">${storefrontMark()}</a>
    <div class="header-actions">
      <button class="icon-btn search-toggle" type="button" aria-label="Открыть поиск" aria-expanded="false" aria-controls="header-search">
        <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="10.8" cy="10.8" r="6.4"/><path d="m15.6 15.6 4.1 4.1"/></svg>
      </button>
      ${/* Личный кабинет — между лупой и корзиной, тем же волосяным штрихом.
            Ссылка, а не кнопка: страница обычная, скрипта ей не нужно. Кабинет
            выключен в настройках — значка нет вовсе. */''}
      ${CUSTOMERS.enabled(settings) ? `<a class="icon-btn account-btn" href="/account" aria-label="Личный кабинет" title="Личный кабинет">${ACCOUNT_ICON}</a>` : ''}
  ...
  <div class="header-search" id="header-search" aria-hidden="true" inert>
    <div class="container header-search-inner">${searchForm('search', 'search-top')}</div>
  </div>
</header>
${/* Панель РАСКРЫВАЕТСЯ ШИРИНОЙ, а не съезжает transform, — так это сделано и у
      Google: содержимое лежит в блоке фиксированной ширины (.nav-list), поэтому
      при движении текст не пересобирается, а по экрану идёт край панели.
      Затемнение — подпись к тому же чекбоксу, поэтому закрытие нажатием мимо
      работает без единой строчки скрипта. */''}
<div class="nav-wrap">
  <label class="nav-scrim" for="nav-menu" aria-hidden="true"></label>
  <nav class="nav-panel" aria-label="Разделы"><div class="nav-list">
    ${navItems}
    ${navContacts}
  </div></nav>
</div>
...
${/* Подвал — четыре раздела, как в больших магазинах (раскладка снята с
     ads-store.ru): магазин, каталог, справочные страницы и контакты, а знаки
     оплаты с копирайтом ушли в нижнюю полосу. Одной строкой контактов подвал
     не отвечал ни на один вопрос: гарантию, возврат и отслеживание покупатель
     находил только в мелком ряду ссылок под копирайтом, а категории из подвала
  ...
  <div class="container footer-grid">
    <div class="footer-cols">
      <div class="footer-about">
        ...
        ${/* Правовая приписка владельца стоит здесь, а не под копирайтом: это
             рассказ о магазине, и в колонке магазина ему и место. */''}
        ${settings.footerNote ? `<div class="foot-legal">${esc(settings.footerNote)}</div>` : ''}
        ${/* Адрес офлайн-точки стоит здесь, в колонке магазина, а не среди
             контактов: это не способ связи, а место, и рядом с ИП, ИНН и
             ОГРНИП он отвечает на тот же вопрос — кто и откуда продаёт.
             Заодно колонка контактов стала ровным столбиком одинаковых строк.
             Булавка вместо подписи «Офлайн-магазин»: слово повторяло то, что и
             так говорит сам адрес. Значок озвучить нечем, поэтому имя строке
             даёт спрятанное слово рядом.

             СТРОКА — ССЫЛКА НА «О КОМПАНИИ»: там та же точка стоит на карте, с
             маршрутом и контактами. Адрес и есть тот вопрос, ради которого эту
             страницу открывают, а нажать на него покупатель пробует первым
             делом — и до этой правки нажатие не делало ничего. */''}
        ${settings.storeAddress ? `<a class="foot-address" href="/about">${pinIcon()}<span><span class="sr-only">Адрес магазина: </span>${esc(settings.storeAddress)}</span></a>` : ''}
        ${/* Кто продаёт: имя, ИНН и ОГРНИП. Отдельными строками с подписями, а
             не одной строчкой мелким шрифтом, — по ним покупатель проверяет
             продавца перед предоплатой, и искать в сплошном тексте номер из
             пятнадцати цифр он не должен. */''}
        ${footerRequisites(settings)}
      </div>
      ${/* Обёртка нужна ТОЛЬКО телефону, и на десктопе она растворяется
           (`display:contents`): там колонки обязаны остаться прямыми
           участниками ряда из четырёх. На телефоне же магазин и контакты идут
           во всю ширину, а эти два раздела стоят парой по центру — и без своей
           строки пара разъезжается: перенос во flex решается ДО сжатия, так что
           на 360 px, где паре не хватает шести пикселей, «Информация» уезжала
           под «Каталог» вместо того, чтобы ужаться. */''}
      <div class="footer-navs">
        ${/* Свой класс у каталога нужен телефону: там колонка спрятана целиком
             (см. `.footer-cats` в мобильном блоке styles.css) — девять строк
             категорий занимали треть подвала, а те же разделы стоят во вкладке
             «Каталог» нижней панели и в меню шапки. */''}
        ${cats.length ? `<nav class="footer-col footer-cats" aria-label="Каталог">
          <div class="footer-col-head">Каталог</div>
          <ul class="footer-col-list">${footCats}</ul>
        </nav>` : ''}
        ${/* «Информация» — рассказ о магазине и всё, что нужно уже после нажатия
             «купить». «О компании» стоит первой: с неё начинают, решая, доверять
             ли продавцу, а отслеживание и документы открывают, когда заказ уже
             сделан. */''}
        <nav class="footer-col" aria-label="Информация">
          <div class="footer-col-head">Информация</div>
          <ul class="footer-col-list">
            <li><a href="/about">О компании</a></li>
            ${/* Отслеживание — единственный путь к своей посылке, если страница
                 оплаты закрыта, а ссылку из переписки покупатель не нашёл. */''}
      ...
      <div class="footer-col footer-contacts">
        ...
        ${/* Часы работы стоят рядом с телефоном: они отвечают на вопрос «а
             сейчас ответят?», который задают ровно перед звонком. Со значком и
             тем же кеглем, что соседи, — иначе единственная строка без значка
             посреди столбика читается как обрывок. Ссылкой она при этом не
             становится: нажимать тут нечего, и наведение её не красит. */''}
        ${settings.contactHours ? `<div class="foot-hours">${clockIcon()}<span>${esc(settings.contactHours)}</span></div>` : ''}
        ${/* Телефон и почта — ссылки, а не строки текста: с телефона по ним
             звонят и пишут одним нажатием, а выделять номер пальцем, чтобы
             скопировать, — то же самое, что не дать его вовсе.

             РАЗМЕТКА У НИХ ТА ЖЕ, что у мессенджеров ниже (`msg-link` со
             значком), и обёртки над ними нет: все четыре контакта — прямые
             строки колонки, одного роста и на одном шаге. Пока телефон с почтой
             лежали в своём `div`, их строка выходила на пять пикселей выше
             соседних, и столбик стоял неровно при одинаковом зазоре. */''}
        ${settings.contactPhone ? `<a class="msg-link" href="tel:${esc(phoneHref(settings.contactPhone))}">${phoneIcon()}<span>${esc(phoneText(settings.contactPhone) || settings.contactPhone)}</span></a>` : ''}
        ${settings.contactEmail ? `<a class="msg-link" href="mailto:${esc(settings.contactEmail)}">${mailIcon()}<span>${esc(settings.contactEmail)}</span></a>` : ''}
        ${footMessengers}
      </div>
    </div>
    <div class="footer-bottom">
      ${footerPayMethods(settings)}
...
${tabBar(settings, opts)}

${/* Корзина. Закрытая панель не просто спрятана от скринридера: inert убирает
...
<p class="form-msg" id="toast" data-store-toast data-toast-id="store-notice" hidden></p>
...
${ym ? `<script src="/static/ym.js?v=${assetV('ym.js')}" defer></script>` : ''}
...
${opts.paymentFee ? `<script src="/static/payment-fee.js?v=${assetV('payment-fee.js')}" defer></script>` : ''}
<script src="/static/smoothui-toast.js?v=${assetV('smoothui-toast.js')}" defer></script>
...
/* ===== Главная и каталог — две разные страницы =====
 *
 * До 18 сентября 2026 главная и была каталогом: слоган, бегущая строка и сразу
 * сетка всех товаров. Нижняя панель разделов на телефоне (tabBar) начинается с
 * двух вкладок «Главная» и «Каталог», и вести в одно место они не могут, —
 * поэтому каталог уехал на `/catalog` (catalogPage), а главная стала
 * страницей входа: первый экран, плитки категорий и подборка популярного.
 *
 * Прежние адреса `/?category=` и `/?q=` живут в закладках и в выдаче, поэтому
 * сервер уводит их на `/catalog` постоянным редиректом (см. server.js).
 *
 * Прежняя главная — слоган и сразу вся сетка — осталась НАСТРОЙКОЙ сайта
 * (`homeCatalog`, см. `homeCatalogOn`): владельцу второго магазина нужна
 * именно она, а код у сайтов один.
 */

/* Первый экран — слоган и бегущая строка преимуществ. Один на главную; на
 * каталоге и категориях его нет: там заголовок стоит сразу под шапкой. */
function heroBlock(settings) {
    ...
    <section class="store-hero">
      <div class="container store-hero-inner">
        <div class="store-hero-copy">
          <h1 style="--fit:${heroFit(settings, heroText)}">${esc(heroText)}</h1>
        </div>
      </div>
      ${BENEFIT_SPRITE}
...
/* Категории со снимком и числом товаров — по образцу плиток каталога
 * i-store.by: снимок, название, «14 товаров». Снимок — первый в порядке
 * витрины товар категории, у которого есть фото; его плита (#f5f5f7) того же
 * цвета, что подложка плитки, поэтому кадр сливается с ней без рамки. Копия
 * берётся самая мелкая (320): на плитке снимок занимает полторы сотни
 * CSS-пикселей, и srcset карточки каталога заставил бы качать 640.
 *
 * Считается один раз на страницу и раздаётся двум местам: сетке на главной
 * (`cat-grid`) и ряду на каталоге (`cat-row`, тот же список, но горизонтальной
 * лентой с выделенной текущей категорией). */
function categoryTiles(db, settings, opts) {
  opts = opts || {};
  const all = db.visibleProducts();
  const cats = db.visibleCategories();
  if (!cats.length) return '';
  const tiles = cats.map((c, i) => {
    const items = all.filter(p => p.category === c);
    const withPhoto = items.find(p => (p.images || []).length);
    /* Сетка на главной — первый экран телефона целиком (три ряда по три), и
     * LCP там — снимок одной из плиток ПЕРВОГО РЯДА: PageSpeed 19 сентября
     * 2026 сперва показал его с `loading="lazy"` и без приоритета (браузер
     * откладывал ровно ту картинку, по которой считается скорость), а после
     * правки назвал LCP не первую плитку, а вторую — при одинаковом размере
     * какая из трёх нарисуется первой, наперёд не знает никто. Поэтому первый
     * ряд грузится сразу и с высоким приоритетом целиком (три снимка по 4–5 КБ
     * — не девять); ряд каталога (`row`) остаётся мелкими копиями. */
    const photo = withPhoto
      ? imageMarkup(withPhoto, 0, opts.row
        ? { small: true, card: true, dir: db && db.UPLOAD_DIR }
        : { tile: true, dir: db && db.UPLOAD_DIR, eager: i < 3 })
      : (items[0] ? placeholderSvg(items[0]) : '');
    const on = opts.active === c;
    return `<a class="cat-tile${on ? ' is-active' : ''}" href="/catalog?category=${encodeURIComponent(c)}"${on ? ' aria-current="page"' : ''}>`
      + `<span class="cat-tile-media">${photo}</span>`
      + `<span class="cat-tile-body"><span class="cat-tile-name">${esc(c)}</span>`
      + `<span class="cat-tile-count">${items.length} ${pluralRu(items.length, 'товар', 'товара', 'товаров')}</span></span></a>`;
  }).join('');
  return `<div class="${opts.row ? 'cat-row' : 'cat-grid'}">${tiles}</div>`;
}

/* Главная: первый экран, категории и популярное. Товаров здесь не весь
 * каталог, а восемь, и «популярное» здесь не слово, а мерка: берутся те, что
 * можно купить и у которых есть цена, по числу отзывов — оно и есть спрос
 * (у демо-набора объём отзывов задан по популярности модели, у привезённых —
 * настоящий). Порядок витрины тут не годится: сверху владелец держит новинки,
 * а у них ни цены, ни наличия — восемь карточек «цена уточняется» подряд на
 * первом экране главной звали бы в магазин, где нечего купить. При равном
 * числе отзывов остаётся порядок витрины. Дальше — кнопка в каталог: главная
 * зовёт, каталог продаёт. */
const HOME_PICKS = 8;
function popularPicks(db, settings) {
  const all = db.visibleProducts();
  const buyable = all.filter(p => sellable(p, settings) && !priceHidden(p));
  const pool = buyable.length >= HOME_PICKS ? buyable : buyable.concat(all.filter(p => !buyable.includes(p)));
  const count = p => (db.ratingFor ? db.ratingFor(p.id) : { count: 0 }).count || 0;
  return pool.map((p, i) => ({ p, i, n: count(p) }))
    .sort((a, b) => (b.n - a.n) || (a.i - b.i))
    .slice(0, HOME_PICKS).map(x => x.p);
}
function homePage(settings, db, opts) {
  opts = opts || {};
  const catalogHome = homeCatalogOn(settings);
  /* «Весь каталог на главной» (`homeCatalogOn`): под первым экраном сразу
   * сетка всех товаров в порядке витрины — без плиток, без «Популярного», без
   * заголовка. Отступ до сетки даёт `.store-hero + .section`, как у прежней
   * главной; `eager` у первого ряда — как на каталоге. */
  if (catalogHome) {
    const all = db.visibleProducts();
    const cards = all.map((p, i) => productCard(p, settings, db, { eager: i < 4 })).join('');
    return homeLayout(settings, db, opts, `
    ${heroBlock(settings)}
    <section class="container section home-catalog">
      ${all.length ? `<div class="grid">${cards}</div>` : `<p class="empty">Каталог пока пуст.</p>`}
    </section>`, true);
  }
  const picks = popularPicks(db, settings);
  const cards = picks.map((p, i) => productCard(p, settings, db, { eager: i < 2 })).join('');
  const tiles = categoryTiles(db, settings);
  const body = `
    ${heroBlock(settings)}
    ${tiles ? `<section class="container section section-cats">
      <div class="section-head"><h2>Категории</h2><a class="section-more" href="/catalog">Весь каталог${MORE_ARROW}</a></div>
      ${tiles}
    </section>` : ''}
    ${picks.length ? `<section class="container section section-picks">
      <div class="section-head"><h2>Популярное</h2><a class="section-more" href="/catalog">Все товары${MORE_ARROW}</a></div>
      <div class="grid">${cards}</div>
      <div class="section-cta"><a class="btn btn-primary btn-lg" href="/catalog">Смотреть весь каталог</a></div>
    </section>` : `<section class="container section"><p class="empty">Каталог пока пуст.</p></section>`}`;
  return homeLayout(settings, db, opts, body, false);
}
// Обвязка главной — одна на оба вида (`homeCatalog` решает только тело и
// набор критических стилей): разметка для поисковика, canonical, вкладка.
function homeLayout(settings, db, opts, body, catalogHome) {
  const origin = opts.origin || '';
  const ld = origin ? jsonLd({
    '@context': 'https://schema.org', '@graph': [
      { '@type': 'WebSite', url: origin, name: settings.storeName, inLanguage: 'ru', potentialAction: { '@type': 'SearchAction', target: origin + '/catalog?q={search_term_string}', 'query-input': 'required name=search_term_string' } },
  ...
  return layout(settings, {
    ...
    payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, customer: opts.customer, landing: opts.landing,
    tab: 'home',
    homeCatalog: catalogHome,
    title: '',
...
// Стрелка у ссылки «Весь каталог» в заголовке раздела.
const MORE_ARROW = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m9.5 6 6 6-6 6"/></svg>';

/* Каталог: все товары, категория или результаты поиска. Это прежняя главная
 * без первого экрана: заголовок, ряд категорий и сетка. */
function catalogPage(settings, db, opts) {
  opts = opts || {};
  const all = db.visibleProducts();
  ...
  /* Заголовок — ЕДИНСТВЕННЫЙ на странице, поэтому `<h1>`: слогана здесь нет, и
   * без него страница категории начиналась бы сразу со второго уровня. Для
   * читающего с экрана это сломанный план документа, а категории ИНДЕКСИРУЮТСЯ
   * и стоят в карте сайта: `<h1>` — самый сильный признак того, о чём страница.
   *
   * Рядом с названием — число товаров в пилюле, как у заголовка категории на
   * i-store.by: оно отвечает на первый вопрос к списку раньше, чем список
   * прокрутят. У поиска число тоже своё — сколько нашлось. */
  const heading = opts.q ? `Результаты: «${esc(opts.q)}»` : esc(opts.category || 'Каталог');
  const count = `<span class="cat-count">${list.length}</span>`;
  const crumbs = `<nav class="breadcrumb"><a href="/">Главная</a> / ${opts.category || opts.q
    ? `<a href="/catalog">Каталог</a> / <span>${opts.q ? 'Поиск' : esc(opts.category)}</span>`
    : '<span>Каталог</span>'}</nav>`;
  /* Ряд категорий стоит на каждой странице каталога — и на общей, и внутри
   * категории (с выделенной текущей): с него переходят в соседний раздел, не
   * возвращаясь наверх и не открывая меню. У поиска ряда нет: он отвечает на
   * другой вопрос, и категории под строкой «Результаты» читались бы как
   * фильтры, которых нет. */
  const tiles = opts.q ? '' : categoryTiles(db, settings, { row: true, active: opts.category });
  const body = `
    <div class="container">${crumbs}</div>
    <section class="container section section-top catalog-page">
      <div class="section-head section-head-center"><h1>${heading}${count}</h1></div>
      ${tiles}
  ...
  return layout(settings, {
    ...
    tab: 'catalog',
    q: opts.q,
    title: opts.category || (opts.q ? 'Поиск' : 'Каталог'),
    ...
    origin,
    canonicalPath: opts.category && categoryValid ? '/catalog?category=' + encodeURIComponent(opts.category) : '/catalog',
    description: shopDescription(settings), ogType: 'website'
  });
}
...
function notFoundPage(settings, opts) {
  ...
  const body = `<section class="container section not-found">
    ...
    <a class="btn btn-primary" href="/catalog">Вернуться в каталог</a></div>
  </section>`;
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Страница не найдена',
    origin: opts.origin || '', noindex: true, noCanonical: true,
    description: 'Запрошенная страница не найдена'
  });
}

/* Что видит заблокированный посетитель (см. lib/visitor-rules.js).
 *
 * СТРАНИЦА САМОДОСТАТОЧНА: ни стилей, ни скриптов, ни логотипа файлом. Блок
 * тотальный и стоит РАНЬШЕ статики — значит `/static/styles.css` такому
 * посетителю тоже не отдаётся, и обычная разметка витрины пришла бы к нему
 * голым текстом. Поэтому здесь свой маленький документ с встроенным стилем.
 *
 * КРОМЕ НАЗВАНИЯ МАГАЗИНА И ЗАГОЛОВКА ЗДЕСЬ НЕТ НИЧЕГО. Причину не называем:
 * подробность вроде «вас узнали по устройству» — это подсказка, как
 * возвращаться, а строка-пояснение под заголовком повторяла бы его же.
 * Название при этом остаётся: человек должен понимать, куда попал, а не думать,
 * что сайт сломался.
 */
function blockedPage(settings) {
  const s = settings || {};
  const name = esc(s.storeName || 'Магазин');
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
<meta name="robots" content="noindex, nofollow">
<title>Доступ закрыт — ${name}</title>
<style>
html,body{height:100%}
body{margin:0;display:flex;align-items:center;justify-content:center;padding:24px;background:#f5f5f7;color:#1d1d1f;font:400 15px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;text-align:center}
main{max-width:420px}
h1{margin:0;font-size:23px;letter-spacing:-.02em}
b{display:block;margin-bottom:18px;color:#8e8e93;font-size:12px;font-weight:600;letter-spacing:.08em;text-transform:uppercase}
</style></head><body><main>
<b>${name}</b>
<h1>Доступ к магазину закрыт</h1>
</main></body></html>`;
}
...
function productPage(settings, db, p, opts) {
  ...
  /* Как и в карточке каталога: полная цена сборки идёт через промокод,
   * применённый по умолчанию. Выключенная промоакция оставляет полную цену и
   * убирает зачёркнутую вместе с процентом (`PROMO.shopPrice`). */
  const pct = PROMO.pctFor(p, settings);
  /* Состояние промокода берём ОДИН раз на страницу: `stateOf()` перебирает коды,
   * а цену через него считает и стартовая сборка, и каждый вариант при проверке
   * потолка — вариантов на странице бывает не один десяток. */
  const promoState = PROMO.stateOf(settings, null);
  const shopSum = sum => PROMO.priceFor(sum, pct, promoState).price;
  const startShown = PROMO.priceFor(startSum, pct, promoState);
  const cmp = startShown.compare;
  // id нужны скрипту: при выборе памяти и доп. характеристик цена и зачёркнутая
  // сумма пересчитываются вместе (`applyVariant()` в public/app.js). Процент при
  // этом НЕ меняется — он и есть скидка товара, — а меняются обе суммы.
  const discount = cmp
  ...
  // Кандидаты приходят ПОЛНЫМИ суммами сборок, а касса проводит цену, которую
  // платит покупатель (`shopSum` выше), — её и сравниваем с потолком.
  const overLimit = candidate => !fitsOrderLimit(shopSum(candidate), orderCap);
  ...
  const body = `
  <div class="container">
    <nav class="breadcrumb"><a href="/">Главная</a> / <a href="/catalog?category=${encodeURIComponent(p.category)}">${esc(p.category)}</a> / <span>${esc(p.name)}</span></nav>
    <div class="product">
      ...
      <div class="product-info">
        ...
        ${ratingSummary}
        ${priceHidden(p)
          ? `<div class="product-price product-price-none">${PRICE_SOON}</div>`
          : `<div class="product-price${cmp ? ' price-sale' : ''}"><span class="price-now" id="product-price">${money(startShown.price, settings)}</span> ${discount}</div>`}
          ...
          ${p.options.map(g => {
            ...
            <div class="option-opts">
              ${(g.values || []).map((v, i) => {
                ...
                return `<button type="button" class="option-opt${i === sel ? ' active' : ''}${ok ? '' : ' out'}" data-label="${esc(v.label)}" data-add="${add}" data-for-storage="${esc((v.forStorage || []).join('|'))}" data-for-choice="${esc(JSON.stringify(v.forChoice || null))}" aria-pressed="${i === sel ? 'true' : 'false'}"${ok ? '' : ' disabled title="Нет в наличии"'}${V.optionFits(v, storage, defChoice.picked) ? '' : ' hidden'}><span class="opt-name">${esc(v.label)}</span>${priceHidden(p) ? '' : `<span class="opt-add">${add > 0 ? '+ ' + money(add, settings) : 'без доплаты'}</span>`}${ok ? '' : '<span class="opt-note">нет в наличии</span>'}</button>`;
              }).join('')}
            </div>
          </div>`;
          }).join('')}
    ...
    <section class="section" id="reviews">
      ...
      <div class="review-form-wrap">
        ...
        <form id="review-form" class="rf-grid" enctype="multipart/form-data">
          ...
          <p class="form-msg" id="review-msg" data-store-toast data-toast-id="review-result" hidden></p>
        </form>
      </div>
    </section>
  </div>`;
  ...
  const ld = origin ? jsonLd({
    ...
    /* У карточки без цены предложения НЕТ ВОВСЕ — ни цены, ни наличия.
     * `Offer` без `price` схема не описывает, а подставить туда скрытую сумму
     * значило бы отдать поисковику ровно то число, которое витрина не
     * показывает: в выдаче встал бы ценник, которого на странице нет. Это то же
     * правило, по которому сюда не кладут доставку и возврат, — соврать в
     * структурированных данных хуже, чем промолчать. */
    offers: priceHidden(p) ? undefined : {
      // Цена стартовой сборки, а не базовая: поисковику обещаем ровно ту сумму,
      // которую покупатель увидит, открыв страницу.
      '@type': 'Offer', price: startShown.price, priceCurrency: currencyCode(settings.currency),
  ...
  const crumbs = origin ? jsonLd({
    ...
    itemListElement: [
      ...
      p.category ? { '@type': 'ListItem', position: 2, name: p.category, item: origin + '/catalog?category=' + encodeURIComponent(p.category) } : null,
  ...
  return layout(settings, {
    body,
    productPage: true,
    // На вкладках нижней панели товар — часть каталога, как у i-store.by.
    tab: 'catalog',
    categories: db.visibleCategories(),
    payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, customer: opts.customer, landing: opts.landing,
    title: p.name,
...
/* Цифры реквизита. Номера вписывают с пробелами («4080 2810 0400…»), а
 * показывать и сверять надо один и тот же номер — тот же приём, что у телефона
 * магазина: хранится одна форма, разделители остаются оформлением. */
function reqDigits(value) { return String(value || '').replace(/\D+/g, ''); }

/* Подпись номера регистрации выводится ИЗ ЕГО ДЛИНЫ: 15 цифр — ОГРНИП у
 * предпринимателя, 13 — ОГРН у организации. Отдельной настройкой это спрашивать
 * нельзя: владелец выбрал бы подпись, не совпадающую с собственным номером, и
 * ошибку в документах никто бы не заметил. */
function ogrnLabel(value) { return reqDigits(value).length === 13 ? 'ОГРН' : 'ОГРНИП'; }

// «ИП» перед именем: владелец вписывает имя и так, и этак, а в подвале подпись
// строки уже говорит «ИП» — «ИП: ИП Иванов» читалось бы как опечатка.
const IP_PREFIX = /^(индивидуальный предприниматель|инд\.?\s*предприниматель|ип)[\s.,]+/i;

function operatorDetails(settings) {
  const operator = String(settings.legalOperator || settings.storeName || 'Владелец сайта').trim();
  const inn = String(settings.legalInn || '').trim();
  const ogrn = String(settings.legalOgrn || '').trim();
  /* Прежняя строка «ИНН/ОГРН/ОГРНИП» одним полем. Форма её больше не пишет, но
   * магазин, который ещё не пересохраняли, обязан остаться с реквизитами в
   * документах — поэтому она показывается, пока не заполнено ни одно из новых
   * полей. Заполнили — старое значение молчит, как снятый `secondaryColor`. */
  const registration = (inn || ogrn) ? '' : String(settings.legalDetails || '').trim();
  ...
  const contact = email
    ...
    // Номер показывается разделителями, как везде на витрине: хранится он в
    // E.164 (+79991234567), и в таком виде его читают по цифре.
    : (phone ? esc(phoneText(phone) || phone) : (telegram ? esc(telegram) : 'через контакты, указанные на сайте'));
  // Предприниматель узнаётся по самим реквизитам: у ИП ИНН из 12 цифр и ОГРНИП
  // из 15, у организации 10 и 13. Имя с приставкой «ИП» — третий признак.
  const isIp = IP_PREFIX.test(operator) || reqDigits(inn).length === 12 || reqDigits(ogrn).length === 15;
  const bank = {
    name: String(settings.bankName || '').trim(),
    account: String(settings.bankAccount || '').trim(),
    bik: String(settings.bankBik || '').trim(),
    corr: String(settings.bankCorr || '').trim(),
    inn: String(settings.bankInn || '').trim(),
    kpp: String(settings.bankKpp || '').trim()
  };
  bank.has = !!(bank.name || bank.account || bank.bik || bank.corr);
  return {
    operator, inn, ogrn, ogrnLabel: ogrnLabel(ogrn), registration, address, email, contact, bank,
    kind: isIp ? 'ИП' : 'Продавец',
    // Имя без приставки — для подвала, где её место занимает подпись строки.
    name: isIp ? (operator.replace(IP_PREFIX, '').trim() || operator) : operator
  };
}
...
// `opts.bank` дописывает банковские реквизиты: они нужны там, где блок отвечает
// на вопрос «кому и куда платят» (страница «О компании»), а в политике
// конфиденциальности расчётный счёт рядом с обработкой данных ни к чему.
function operatorCard(settings, role, contactLabel, opts) {
  ...
  return `<dl class="legal-details">
    ...
    ${row('ИНН', info.inn)}
    ${row(info.ogrnLabel, info.ogrn)}
    ${row('Реквизиты', info.registration)}
    ${row('Адрес', info.address)}
    ${opts && opts.bank && bank.has ? `${row('Расчётный счёт', bank.account)}
    ${row('Банк', bank.name)}
    ${row('БИК банка', bank.bik)}
    ${row('Корреспондентский счёт', bank.corr)}
    ${row('ИНН банка', bank.inn)}
    ${row('КПП банка', bank.kpp)}` : ''}
    <div><dt>${esc(contactLabel || 'Обращения по персональным данным')}</dt><dd>${info.contact}</dd></div>
  </dl>`;
}

/* Реквизиты продавца в подвале — имя, ИНН и ОГРНИП, по строке на каждый.
 * Это то, по чему покупатель проверяет, кому платит, и требование закона о
 * дистанционной торговле; банковские реквизиты сюда не идут (см. lib/db.js).
 * Двоеточие после подписи рисует CSS: оно оформление, а не часть текста. */
function footerRequisites(settings) {
  const info = operatorDetails(settings);
  const rows = [[info.kind, info.name]];
  if (info.inn) rows.push(['ИНН', info.inn]);
  if (info.ogrn) rows.push([info.ogrnLabel, info.ogrn]);
  if (info.registration) rows.push(['Реквизиты', info.registration]);
  // Одно только имя — это не реквизиты, а повтор названия магазина из шапки.
  if (rows.length < 2) return '';
  return `<dl class="foot-req">${rows
...
function checkoutPage(settings, opts) {
  opts = opts || {};
  /* Что витрина знает о платёжке — только то, что нужно подписать кнопку и
   * посчитать сумму кассы; ключи остаются на сервере, как и ключ подсказок
   * адреса. Режим считается ПО СУММЕ (см. `PAYMENTS.modeFor` и `checkoutMode`
   * в public/app.js), поэтому кроме «оплата включена» уезжают пределы кассы и
   * путь заказа, который в неё не помещается: свои реквизиты или заявка.
   * Своих чисел в скрипте нет — разошлись бы с серверными. */
  const cashbox = !!opts.payCashbox;
  const fallback = PAYMENTS.fallbackMode(settings);
  const payOnline = cashbox || fallback === 'own';
  const bounds = PAYMENTS.boundsOf(settings);
  // Платить на сайте нечем — витрина об оплате не знает ни одним атрибутом.
  const pay = !payOnline ? '' : ` data-pay="1" data-pay-fallback="${fallback}"`
    + (cashbox ? ` data-pay-cashbox="1" data-pay-min="${esc(bounds.min)}" data-pay-max="${esc(PAYMENTS.cashboxMax(settings))}"` : '');
  const fee = cashbox ? PAYMENTS.checkoutFee(settings, 0) : null;
  const paymentMethods = cashbox ? PAYMENTS.offeredMethods(settings) : [];
  const roundedOnly = paymentMethods.length === 1 && paymentMethods[0].provider === 'platega';
  const feeMax = fee ? PAYMENTS.providerMax(PAYMENTS.provider('platega'), settings) : 0;
  const feeAttr = fee ? ` data-payment-fee-percent="${esc(fee.percent)}" data-payment-fee-max="${esc(feeMax)}"${roundedOnly ? ' data-payment-rounded-only="1"' : ''}` : '';
  const directSbp = paymentMethods.length === 1 && paymentMethods[0].id === 'SBP_ONLINE';
  const flowAttr = directSbp ? ' data-pay-flow="sbp"' : fee ? ' data-pay-flow="choice"' : '';
  ...
  /* Личный кабинет. Вошедшему подставляются почта, имя, телефон и адрес; тому,
   * кто не вошёл, поле почты обещает кабинет — но только когда есть чем
   * прислать пароль (`auto`). Ключей почты здесь нет: витрина знает лишь
   * «пришлём». У адреса рядом стоит отметка времени его правки: по ней
   * скрипт решает, что подставлять — адрес из кабинета или набранный на
   * оформлении (см. `initCheckoutMemory` в public/app.js). */
  const acc = opts.account || null;
  // Вошёл — признак `data-account-in`, а не наличие почты: кабинет бывает и
  // по одному телефону, и подставлять имя с адресом ему надо так же.
  const accIn = !!acc && (acc.in || !!acc.email);
  const account = !acc ? '' : ` data-account="1"${acc.auto ? ' data-account-auto="1"' : ''}`
    + (accIn ? ` data-account-in="1" data-account-email="${esc(acc.email || '')}" data-account-name="${esc(acc.name || '')}" data-account-phone="${esc(acc.phone || '')}"`
      + ` data-account-address="${esc(acc.address || '')}" data-account-address-at="${esc(Number(acc.addressAt) || 0)}"` : '');
  const body = `<div class="container checkout-page" id="checkout-page"${pay}${feeAttr}${flowAttr}${delivery}${promo}${restore}${account}>
    ${DLOGO.sprite()}
    ...
    ${opts.notice ? `<p class="form-msg ok checkout-return-note" role="status" data-store-toast data-toast-type="info" data-toast-id="checkout-return">${esc(opts.notice)}</p>` : ''}
  ...
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, customer: opts.customer, title: 'Оформление заказа',
    tab: 'cart',
    paymentFee: !!fee,
...
function payMethodChoice(methods, current, amounts, currency, settings, originalTotal) {
  ...
  return `<div class="co-choice pay-choices" role="radiogroup" aria-label="Способ оплаты">${methods.map((m, i) => {
    ...
    const amount = amounts && amounts[m.id];
    const discount = currency === 'RUB' && Number.isFinite(originalTotal) && Number.isFinite(amount)
      ? Math.round((originalTotal - amount) * 100) / 100 : 0;
    const sum = Number.isFinite(amount) ? (currency === 'RUB' ? money(amount, settings) : PAY.formatAmount(amount, currency)) : '';
    return `<label class="co-choice-opt pay-opt"><input type="radio" name="pay-method" value="${esc(m.id)}"${on ? ' checked' : ''}${PAY.isHosted(m.id) ? ' data-hosted="1"' : ''}>
      <span class="pay-opt-mark" aria-hidden="true">${payMark(m.mark)}</span>
      <span class="co-choice-text"><b>${esc(m.name)}</b>${sum ? `<i>К оплате ${esc(sum)}${discount > 0 ? ` · скидка ${money(discount, settings)}` : ''}</i>` : ''}${m.hint ? `<i>${esc(m.hint)}</i>` : ''}</span>
 ...
 *
 * Заказ кассы тоже можно закрыть. История счетов и сверка сохраняются:
 * поздний перевод должен найтись, даже если покупатель уже отменил заказ.
 *
...
function payCancelRow(order) {
  const own = order.payMode === 'own';
  return `<form class="pay-cancel" method="post" action="/pay/${encodeURIComponent(order.id)}/cancel"
    data-confirm="${own ? 'Отменить оплату этого заказа? Товары вернутся в корзину — оформить его заново можно будет сразу.' : 'Отменить этот заказ? После отмены можно сразу оформить новый. Если вы уже отправили деньги, дождитесь подтверждения оплаты.'}">
    <button class="pay-cancel-btn" type="submit">${own ? 'Отменить оплату' : 'Отменить заказ'}</button>
  </form>`;
}
...
/* Что будет дальше — одной фразой под «спасибо за заказ».
 *
 * Отправление готовится автоматически в момент оплаты, но покупателю оно
 * показывается не раньше, чем посылку правда отдали перевозчику (`visible` у
 * отправления). До этого момента «Принят на склад» у коробки, которая лежит на
 * столе, — обещание, которого никто не давал, поэтому и фраза здесь другая: не
 * «следите за посылкой», а «ссылка появится, когда она уедет».
 */
function trackPromise(order) {
  return TRACK.shownToBuyer(order && order.shipment)
    ? 'Отслеживание уже доступно — ссылка ниже.'
    : 'Отслеживание появится здесь, как только посылка уедет со склада; менеджер свяжется с вами и подтвердит отправку.';
}

/* Ссылка на товарный чек — только у оплаченного заказа.
 *
 * У неоплаченного документ о покупке был бы обещанием того, чего ещё не
 * случилось, а покупателю на той странице нужны реквизиты, а не бумага. */
function receiptLink(order) {
  if (order && order.payment && order.payment.status === 'refunded') return '';
  const paid = !!(order && ((order.payment && order.payment.status === 'paid') || order.manualPaid));
  if (!paid || !order.id) return '';
  return `<a class="btn btn-lg" href="/receipt/${esc(order.id)}">Товарный чек</a>`;
}
...
function payPage(settings, order, opts) {
  ...
  const state = (pay && pay.status) || '';
  const methods = (Array.isArray(opts.methods) ? opts.methods : []).map(method => {
    if (method.id !== 'CARD_ONLINE') return method;
    return onlyAlfaPayments(settings, method.id, opts.currency)
      ? { ...method, ...PAY.describe(method.id, 'alfabank') } : method;
  });
  ...
  const method = pay && pay.method ? PAY.describe(pay.method, pay.provider) : null;
  ...
  const live = payLive(pay) && !order.manualVoid;
  ...
  const fee = orderPaymentFee(order);
  ...
  const useStoredInvoice = live || (storedInvoice && ['paid', 'mismatch', 'refunded'].includes(state));
  ...
  const ownUntil = own && !ownPaid && state !== 'paid' && state !== 'refunded' && !expired ? ownPayUntil(order) : 0;

  let card;
  if (state === 'refunded') {
    card = `<section class="order-success order-success-neutral" role="status" aria-live="polite">
      <div class="order-success-check" aria-hidden="true">↩</div>
      <p class="order-success-eyebrow">Возврат платежа</p>
      <h3>Платёж возвращён</h3>
      <p class="order-success-copy">Подтверждён возврат платежа на ${esc(payTotal)}. Повторно платить по этому заказу не нужно. Если нужна помощь с заказом, свяжитесь с менеджером.</p>
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      <a class="btn btn-lg" href="/catalog">Вернуться в каталог</a>
    </section>`;
  } else if (ownPaid && state !== 'paid' && !orderArchived) {
    ...
    card = `<section class="order-success" role="status" aria-live="polite">
      ...
      <p class="order-success-copy">Мы получили перевод на ${esc(total)}. ${esc(trackPromise(order))}</p>
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      ${receiptLink(order)}${trackLink(order)}<a class="btn btn-lg" href="/catalog">Продолжить покупки</a>
    </section>`;
    ...
    card = `<section class="order-success" role="status" aria-live="polite">
      ...
      <p class="order-success-copy">Касса подтвердила ${PAY.isHosted(pay.method) ? 'оплату' : 'перевод'} на ${esc(payTotal)}. ${esc(trackPromise(order))}</p>
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      ${receiptLink(order)}${trackLink(order)}<a class="btn btn-primary btn-lg" href="/catalog">Продолжить покупки</a>
    </section>`;
    ...
    card = `<section class="order-success order-success-neutral" role="status">
      ...
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      <a class="btn btn-lg" href="/catalog">Продолжить покупки</a>
    </section>`;
  } else if (live) {
    const label = PAY.requisiteLabel(pay.method);
    const qr = method && method.kind === 'qr' && !PAY.isHosted(pay.method);
    ...
    /* Платит ли покупатель НА СТРАНИЦЕ БАНКА (эквайринг) или переводит сам
     * (P2P). От этого зависят почти все подписи ниже, и разница настоящая: при
     * переводе сумму набирает человек и ошибиться в ней можно, при оплате картой
     * она зашита в счёт. Требовать «точную сумму» там, где её не вводят, —
     * неправда и лишняя тревога на последнем шаге покупки. */
    const hosted = PAY.isHosted(pay.method);
    // Телефон показываем разделителями, а копируем голыми цифрами.
    const req = requisiteView(pay.method, pay.requisite);
    card = `<div class="co-block pay-invoice">
      <h2 class="co-block-title"><span class="co-step" aria-hidden="true">1</span>${hosted ? 'Оплатите заказ' : 'Переведите сумму'}</h2>
      <div class="pay-amount"><span>${hosted ? 'Сумма к оплате' : 'Сумма перевода'}</span><b>${esc(payTotal)}</b>
        ${hosted ? '' : `<button type="button" class="pay-copy" data-copy="${esc(String(payAmount))}" aria-label="Скопировать сумму">Копировать</button>`}</div>
      ...
      <div class="pay-timer" id="pay-timer" role="timer" aria-live="off"${liveUntil ? '' : ' hidden'}>
        <span class="pay-timer-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="8.5"/><path d="M12 7.5v5l3.2 2"/></svg></span>
        <span class="pay-timer-copy"><span class="pay-timer-label">${hosted ? 'На оплату осталось' : 'Реквизиты действительны ещё'}</span><b id="pay-left">—</b></span>
      </div>
      ${hosted && pay.method === 'SBP_ONLINE' ? '' : `<p class="pay-hint">${hosted
        ? (pay.provider === 'alfabank'
          ? 'Оплата через СБП пройдёт на защищённой странице банка — она откроется в новой вкладке, сумма там уже указана. Эту страницу не закрывайте: как только банк подтвердит оплату, она обновится сама.'
          : 'Защищённая страница оплаты откроется в новой вкладке. Выберите доступный способ и следуйте инструкциям на ней. Эту страницу не закрывайте: когда оплата подтвердится, она обновится сама.')
        : `<b class="pay-exact">Переведите точную сумму</b>Касса сводит перевод с заказом по сумме, поэтому лишний или недостающий рубль придётся разбирать вручную. ${payLink
      ...
      <p class="pay-state" id="pay-state" role="status" aria-live="polite">${hosted ? 'Ждём оплату…' : 'Ждём перевод…'}</p>
      <div class="pay-actions">
        <button type="button" class="btn" id="pay-recheck">${hosted ? 'Проверить оплату' : 'Проверить перевод'}</button>
      </div>
    </div>`;
  } else if (expired) {
    ...
    card = `<section class="order-success order-success-neutral" role="status">
      <div class="order-success-check" aria-hidden="true">×</div>
      <p class="order-success-eyebrow">${byBuyer ? (order.payMode === 'own' ? 'Оплата отменена' : 'Заказ отменён') : 'Заказ закрыт'}</p>
      <h3>${byBuyer ? 'Заказ можно оформить заново' : 'Оформите новый заказ'}</h3>
      <p class="order-success-copy">${byBuyer
      ? 'Вы отменили этот заказ. Можно сразу оформить новый. Если деньги уже отправлены, дождитесь подтверждения — поступление проверится автоматически.'
      ...
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      <a class="btn btn-primary btn-lg" href="${byBuyer ? '/checkout' : '/catalog'}">${byBuyer ? 'Вернуться к оформлению' : 'Вернуться в каталог'}</a>
    </section>`;
  } else {
    ...
    const picked = methods.find(m => pay && m.id === pay.method) || methods[0];
    card = `<div class="co-block pay-choice">
      ...
      ${methods.length
        ? payMethodChoice(methods, pay && pay.method, opts.methodAmounts, currency, settings,
          order.paymentFee && order.paymentFee.originalTotal)
        + `<button type="button" class="btn btn-primary btn-lg" id="pay-create">${picked && PAY.isHosted(picked.id) ? 'Перейти к оплате' : 'Получить реквизиты'}</button>`
  ...
  /* Цели Яндекс Метрики (`public/pay.js` → `window.ymGoal`): «заказ» уходит
   * при выставлении счёта, «оплачено» и покупка для электронной коммерции —
   * когда страница пришла оплаченной. Сумма — рублёвый `total` заказа вместе с
   * доставкой, то есть то, что покупатель правда заплатил; состав едет только
   * у оплаченного — незачем класть его в разметку на каждое открытие. */
  const paid = state !== 'refunded' && (state === 'paid' || !!order.manualPaid);
  const ymItems = paid ? (Array.isArray(order.items) ? order.items : []).map(it => ({
    id: String(it.id || ''), name: String(it.name || ''), price: Number(it.price) || 0, quantity: Number(it.qty) || 1
  })) : null;
  const body = `<div class="container checkout-page pay-page" id="pay-page"
    data-order="${esc(order.id)}" data-state="${esc(order.manualVoid && !['paid', 'mismatch', 'refunded'].includes(state) ? 'order_cancelled' : state || 'none')}"
    data-attempt="${esc(String((pay && (pay.id || pay.attemptId)) || ''))}"
    data-currency="${esc(currency)}"
    data-hosted="${pay && PAY.isHosted(pay.method) ? '1' : '0'}"${
  pay && PAY.isHosted(pay.method) && liveUntil && PAY.isPayLink(pay.requisite) ? ` data-hosted-url="${esc(pay.requisite)}"` : ''}
    data-number="${esc(String(order.number || ''))}" data-total="${esc(String(Number(order.total) || 0))}"${paid ? ` data-paid="1" data-items="${esc(JSON.stringify(ymItems))}"` : ''}
    ...
    <header class="checkout-head">
      ...
      ${orderPaymentDiscount(order) > 0 ? `<p class="checkout-sub">Скидка при оплате: ${money(orderPaymentDiscount(order), settings)}</p>`
    : orderPaymentDiscount(order) < 0 ? `<p class="checkout-sub">В том числе комиссия выбранного способа оплаты: ${money(-orderPaymentDiscount(order), settings)}</p>` : ''}
      ${fee ? `<p class="checkout-sub">В том числе ${esc(paymentFeeLabel(fee).toLocaleLowerCase('ru-RU'))}: ${money(fee.amount, settings)}</p>` : ''}
    </header>
    ${accountNote(opts.accountNote)}
    <div class="pay-wrap">${card}${!own && !ownPaid && !orderArchived && !canDiscardDraft
      && !['paid', 'mismatch', 'refunded'].includes(state) ? payCancelRow(order) : ''}</div>
  </div>`;
  return layout(settings, {
    ...
    chatWaiting: opts.chatWaiting, ym: opts.ym,
...
/* ===================== Товарный чек =====================
 *
 * Документ покупателю о том, что и почём он купил: продавец с реквизитами,
 * состав заказа, доставка, итог и отметка об оплате.
 *
 * ЭТО НЕ КАССОВЫЙ ЧЕК И НЕ ЗАМЕНЯЕТ ЕГО. Фискального чека магазин не пробивает
 * вовсе — ни ККТ, ни ОФД в проекте нет, — и появление кассы этот документ не
 * отменяет: тогда рядом с ним встанет настоящий чек. Здесь товарный чек в том
 * смысле, в каком он назван на странице гарантии: «основанием для обращения
 * служит кассовый ИЛИ товарный чек» — то есть подтверждение покупки.
 *
 * Собирается он ЦЕЛИКОМ ИЗ ЗАКАЗА, а не считается заново: цены плавают, скидка
 * меняется, промокод переименовывают — а чек обязан показывать те деньги,
 * которые покупатель заплатил. Заказ хранит цену каждой позиции и итог сам, и
 * это единственный источник правды для документа.
 */
function orderPaymentFee(order) {
  const stored = order && order.paymentFee;
  if (!stored || stored.provider !== 'platega' || !['SBP_ONLINE', 'ONLINE_PAYMENT'].includes(stored.method)) return null;
  // Новые цены уже включают расходы магазина на оплату. Отдельную строку
  // сохраняем только у прежних заказов, где комиссия добавлялась сверху.
  if (stored.mode === 'included') return null;
  const quote = PAYMENT_FEE.quote(stored.baseAmount, stored.percent);
  return quote && quote.amount > 0 && quote.amount === stored.amount && quote.total === Number(order.total) ? quote : null;
}
/* Насколько заплаченное отличается от ценника. У заказа через Platega цена
 * округляется вниз не больше чем на рубль (`rounding: 'rubles'`), а на самой
 * странице кассы покупатель волен выбрать способ с другим тарифом — тогда
 * итог плывёт дальше рубля в любую сторону (боевой счёт #478234: Crypto, 5 %
 * вместо 8,5 %, 27 096,77 вместо 28 000 ₽), а чистая сумма магазина остаётся
 * прежней. `order.total` уже держит то, что покупатель правда заплатил
 * (см. `syncPayment` в lib/db.js); здесь только разница для строки чека.
 * Положительное число — скидка, отрицательное — переплата по тарифу способа. */
function orderPaymentDiscount(order) {
  const fee = order && order.paymentFee;
  if (!fee || fee.rounding !== 'rubles' || !(fee.originalTotal > 0) || !(Number(order.total) > 0)) return 0;
  return Math.round((fee.originalTotal - Number(order.total)) * 100) / 100;
}
function paymentFeeLabel(fee) {
  return `Комиссия платёжного сервиса (${fee.percent.toLocaleString('ru-RU')}%)`;
}

function receiptRows(order, settings) {
  const list = (order && order.items) || [];
  // Название позиции уже несёт вариант целиком («… 512 ГБ, Космический чёрный,
  // Только eSIM»): собирать его заново из товара нельзя — вариант могли снять с
  // продажи, а в чеке обязано остаться купленное.
  return list.map((item, i) => {
    const qty = Math.max(1, Number(item.qty) || 1);
    const price = Number(item.price) || 0;
    return `<tr>
      <td class="rc-num">${i + 1}</td>
      <td class="rc-name">${esc(item.name || '—')}</td>
      <td class="rc-qty">${qty}</td>
      <td class="rc-price">${money(price, settings)}</td>
      <td class="rc-sum">${money(price * qty, settings)}</td>
    </tr>`;
  }).join('');
}

function receiptPage(settings, order, opts) {
  opts = opts || {};
  const info = operatorDetails(settings);
  const pay = (order && order.payment) || null;
  const manual = (order && order.manualPaid) || null;
  const paidByProvider = !!(pay && pay.status === 'paid');
  const paid = !(pay && pay.status === 'refunded') && (paidByProvider || !!manual);
  /* Когда пришли деньги. У кассы это `paidAt` попытки, у перевода по своим
   * реквизитам — момент, когда менеджер отметил приход. Не нашлось ни того, ни
   * другого — не выдумываем дату: строка про оплату останется без неё. */
  const paidAt = paidByProvider ? Number(pay.paidAt) || 0 : Number(manual && manual.at) || 0;
  const method = paidByProvider && pay.method ? PAY.describe(pay.method, pay.provider) : null;
  const goods = Number(order.itemsTotal) || 0;
  const ship = Number(order.deliveryPrice) || 0;
  const fee = orderPaymentFee(order);
  const saved = Number(order.promoDiscount) || 0;
  const code = String((order && order.promoCode) || '').trim();
  const way = [DELIVERY.nameOf(order.delivery), DELIVERY.shortModeOf(order.delivery, order.deliveryMode)]
    .filter(Boolean).join(', ');

  const line = (label, value) => value
    ? `<div class="rc-line"><dt>${esc(label)}</dt><dd>${value}</dd></div>` : '';
  // Реквизиты продавца — те же, что в подвале и на «О компании»: у документа и
  // у сайта продавец обязан быть одним и тем же, до цифры.
  const seller = [
    line(info.kind, esc(info.operator)),
    line('ИНН', esc(info.inn)),
    line(info.ogrnLabel, esc(info.ogrn)),
    line('Реквизиты', esc(info.registration)),
    line('Адрес', esc(info.address))
  ].join('');
  const buyer = [
    line('Покупатель', esc(order.customerName || '—')),
    line('Телефон', esc(phoneText(order.phone) || order.phone || '')),
    line('Доставка', way ? esc(way) : ''),
    line('Адрес доставки', esc(order.pickupAddress || order.address || ''))
  ].join('');

  const body = `<div class="container legal-page receipt">
    <div class="rc-actions no-print">
      <a class="btn" href="/pay/${esc(order.id)}">← К заказу</a>
      <button type="button" class="btn btn-primary" data-print>Распечатать</button>
    </div>

    <div class="legal-card rc-card">
      <header class="rc-head">
        <div>
          <h1>Товарный чек ${esc(orderNo(order.number))}</h1>
          <p class="rc-date">от ${esc(mskDateTime(order.createdAt))}</p>
        </div>
        <div class="rc-stamp${paid ? ' is-paid' : ''}">${paid ? 'Оплачено' : 'Не оплачен'}</div>
      </header>

      <div class="rc-parties">
        <section><h2>Продавец</h2><dl class="rc-list">${seller}</dl></section>
        <section><h2>Покупатель</h2><dl class="rc-list">${buyer}</dl></section>
      </div>

      <div class="legal-table-wrap"><table class="legal-table rc-table">
        <thead><tr><th>№</th><th>Наименование</th><th>Кол-во</th><th>Цена</th><th>Сумма</th></tr></thead>
        <tbody>${receiptRows(order, settings)}</tbody>
      </table></div>

      <dl class="rc-totals">
        <div class="rc-line"><dt>Товары</dt><dd>${money(goods, settings)}</dd></div>
        ${/* Скидка идёт строкой «в том числе», а не вычитанием.
            *
            * В заказе цена позиции хранится УЖЕ со скидкой, и «Товары» — это их
            * сумма. Пока рядом стояло голое «Скидка −12 000 ₽», столбик не
            * сходился ни в какую сторону: 67 990 − 12 000 + 1 010 давало 57 000
            * при итоге 69 000, и документ, который читают по строкам ради
            * проверки, сам себе противоречил. Показать «Товары» ДО скидки тоже
            * нельзя — тогда с ними разойдётся таблица выше, а там цены той же
            * позиции. */
    saved > 0 ? `<div class="rc-line rc-line-note"><dt>в том числе скидка${code ? ` по промокоду ${esc(code)}` : ''}</dt><dd>${money(saved, settings)}</dd></div>` : ''}
        ${ship > 0 ? `<div class="rc-line"><dt>Доставка</dt><dd>${money(ship, settings)}</dd></div>` : ''}
        ${fee ? `<div class="rc-line"><dt>${esc(paymentFeeLabel(fee))}</dt><dd>${money(fee.amount, settings)}</dd></div>` : ''}
        ${orderPaymentDiscount(order) > 0 ? `<div class="rc-line"><dt>Скидка при оплате</dt><dd>−${money(orderPaymentDiscount(order), settings)}</dd></div>`
    : orderPaymentDiscount(order) < 0 ? `<div class="rc-line"><dt>Комиссия выбранного способа оплаты</dt><dd>${money(-orderPaymentDiscount(order), settings)}</dd></div>` : ''}
        <div class="rc-line rc-total"><dt>Итого</dt><dd>${money(order.total, settings)}</dd></div>
      </dl>

      <p class="rc-paid">${paid
        ? `Оплачено${method ? `, ${esc(method.name)}` : ''}${paidAt ? ` · ${esc(mskDateTime(paidAt))}` : ''}`
        : 'Оплата по заказу не подтверждена.'}</p>

      <div class="rc-sign">
        <div><span class="rc-sign-label">Продавец</span><span class="rc-sign-line"></span></div>
        <div><span class="rc-sign-label">Покупатель</span><span class="rc-sign-line"></span></div>
      </div>
    </div>
  </div>`;

  return layout(settings, {
    body, categories: opts.categories || [], title: `Товарный чек ${orderNo(order.number)}`,
    chatWaiting: opts.chatWaiting,
    origin: opts.origin || '', canonicalPath: '/receipt/' + order.id, noindex: true,
    scripts: ['receipt.js'],
    description: `Товарный чек по заказу в ${settings.storeName}`
  });
}
...
function trackingPage(settings, opts) {
  ...
  return layout(settings, Object.assign({}, opts, {
    ...
    // Счётчика Яндекса здесь не бывает: в адресе ленты стоит секретный ключ
    // отслеживания, и уехать в чужую статистику он не должен. `opts` сюда
    // приходят целиком, поэтому признак гасится явно, а не «не передаётся».
    ym: null,
...
function privacyPage(settings, opts) {
  opts = opts || {};
  const info = operatorDetails(settings);
  /* Яндекс Метрика. Абзацы про неё появляются только у магазина со счётчиком:
   * собираем — значит называем, а не собираем — не обещаем. Режим «только из
   * Яндекса» тоже назван прямо: покупателю из Telegram важно знать, что его
   * счётчик не касается вовсе. */
  const ymOn = !!YM.counterId(settings);
  const accountsOn = CUSTOMERS.enabled(settings);
  const ymOnly = ymOn && YM.onlyYandex(settings);
  const ymWho = ymOnly
    ? 'только у посетителей, пришедших из Яндекса (по метке рекламного перехода, UTM-источнику или переходу с сайтов Яндекса), и при их возвращении на сайт в течение ' + YM.SOURCE_DAYS + ' дней'
    : 'у всех посетителей';
  const body = `<div class="container legal-page">
    ...
    <div class="legal-card">
      ...
      <section class="legal-section" id="data">
        ...
        <ul>
          <li><b>Заказы и обращения:</b> имя и фамилия получателя, телефон, e-mail, адрес доставки, состав и параметры заказа.</li>
          ${accountsOn ? `<li><b>Личный кабинет:</b> e-mail, пароль в виде необратимого хеша, имя, телефон и адрес доставки из профиля, дата регистрации и последнего входа, привязка заказов к кабинету. Кабинет создаётся по запросу посетителя либо автоматически при заказе с указанным e-mail — тогда пароль отправляется на этот адрес письмом. Письма (пароль от кабинета, ссылка для восстановления) отправляются через почтовый сервер, указанный Оператором; иных сообщений и рассылок сайт не ведёт.</li>` : ''}
          ...
          <li><b>Собственная метрика:</b> случайный идентификатор посетителя, посещённые страницы, число, продолжительность и время визитов, глубина прокрутки страницы, нажатия на ссылки, ведущие на другие сайты (без их адреса — только название сервиса), показатели скорости загрузки страницы, источник и UTM-метки, IP-адрес, приблизительные город, регион и страна, факт оформления заявки.</li>
          ${ymOn ? `<li><b>Яндекс Метрика:</b> ${ymWho} подключается счётчик сервиса Яндекс Метрика (ООО «Яндекс»). Ему передаются посещённые страницы, источник перехода, данные об устройстве и браузере, IP-адрес, факты добавления товара в корзину, оформления и оплаты заказа с его суммой и составом. Содержимое полей форм, имя, телефон, адрес и платёжные реквизиты этому сервису не передаются: запись действий на странице (Вебвизор) выключена.</li>` : ''}
      ...
      <section class="legal-section" id="purposes">
        ...
        <div class="legal-table-wrap"><table class="legal-table purposes-table"><thead><tr><th>Цель</th><th>Данные</th><th>Основание</th></tr></thead><tbody>
          ...
          ${accountsOn ? `<tr><td>Личный кабинет покупателя</td><td>E-mail, хеш пароля, имя, телефон, адрес доставки, заказы кабинета</td><td>Запрос посетителя на использование функций сайта; исполнение договора (п. 5 ч. 1 ст. 6 Закона № 152-ФЗ)</td></tr>` : ''}
          ...
          ${ymOn ? `<tr><td>Оценка эффективности рекламы в Яндексе</td><td>Идентификаторы cookie Яндекс Метрики, посещения, устройство, источник, IP, факты и суммы заказов</td><td>Осуществление прав и законных интересов Оператора при условии соблюдения прав и свобод посетителя (п. 7 ч. 1 ст. 6 Закона № 152-ФЗ); статистика обезличивается</td></tr>` : ''}
        </tbody></table></div>
      </section>
      ...
      <section class="legal-section" id="processing">
        ...
        <p>Если для сайта настроены уведомления в Telegram, сведения из заказа или отзыва, включая IP, приблизительный город и данные устройства, могут быть направлены менеджеру через этот сервис. Переход посетителя по внешним ссылкам Telegram и WhatsApp регулируется также документами соответствующего сервиса.</p>
        ...
        ${ymOn ? `<p>Для оценки эффективности рекламы ${ymWho} на страницах сайта работает счётчик <a class="link" href="https://yandex.ru/legal/confidential/" target="_blank" rel="noopener noreferrer">Яндекс Метрики</a>: скрипт сервиса загружается в браузере посетителя и передаёт ООО «Яндекс» сведения, перечисленные в разделе 3. Сервер сайта в Яндекс ничего не отправляет. Счётчик не подключается на страницах отслеживания заказа и товарного чека, а также посетителям, отключившим метрику на этом устройстве (раздел 8).</p>` : ''}
        <p>Оператор не продаёт персональные данные и не передаёт их рекламным сетям${ymOn ? ', кроме описанной выше обезличенной статистики Яндекс Метрики' : ', и не использует сторонние рекламные cookie'}. Передача государственным органам допускается только в случаях, предусмотренных законом.</p>
      </section>

      <section class="legal-section" id="storage">
        <h2>6. Сроки хранения и защита</h2>
        <p>Данные заказа хранятся до завершения работы с заявкой и далее в течение срока, необходимого для исполнения договора, разрешения споров и выполнения требований закона. Данные отзыва хранятся до удаления отзыва, отзыва согласия либо прекращения работы сайта. Идентифицируемая карточка посетителя в собственной метрике автоматически ограничивается 365 днями. Признаки посетителя, доступ которому Оператор ограничил (идентификатор метрики, IP-адрес и производный признак устройства), хранятся, пока действует само ограничение.</p>
      ...
      <section class="legal-section" id="cookies">
        ...
        <p>При открытии сайта автоматически запускается собственная first-party метрика. Она работает без стороннего аналитического скрипта и не используется для показа рекламы. Короткий технический сигнал отправляется не чаще одного раза в минуту и только пока страница видима.${ymOn ? ` Отдельно ${ymWho} загружается счётчик Яндекс Метрики (раздел 5).` : ''}</p>
        <div class="legal-table-wrap"><table class="legal-table cookies-table"><thead><tr><th>Название</th><th>Тип</th><th>Назначение</th><th>Срок</th></tr></thead><tbody>
          <tr><td><code>sess</code></td><td>Cookie, HttpOnly, SameSite=Lax, Secure при HTTPS</td><td>Защищённая сессия: авторизация панели${accountsOn ? ' и личного кабинета покупателя' : ''}, принадлежность заказа покупателю и доступ к его странице оплаты. Для обычного просмотра без оформления не создаётся.</td><td>До 7 дней или до выхода</td></tr>
          ...
          ${ymOnly ? `<tr><td><code>${YM.COOKIE}</code></td><td>Cookie, HttpOnly, SameSite=Lax, Secure при HTTPS</td><td>Признак, что посетитель пришёл из Яндекса: только таким посетителям подключается счётчик Яндекс Метрики.</td><td>До ${YM.SOURCE_DAYS} дней</td></tr>` : ''}
          ${ymOn ? `<tr><td><code>_ym_uid</code>, <code>_ym_d</code>, <code>_ym_isad</code> и другие</td><td>Cookie и localStorage сервиса Яндекс Метрика</td><td>Устанавливаются скриптом Яндекс Метрики для подсчёта уникальных посетителей и связи визита с рекламным переходом. Перечень и сроки — в <a class="link" href="https://yandex.ru/support/metrica/general/cookie.html" target="_blank" rel="noopener noreferrer">справке сервиса</a>.</td><td>До 1 года</td></tr>
          <tr><td><code>ym_goal_…</code></td><td>localStorage</td><td>Не даёт отправить в Яндекс Метрику одну и ту же цель по заказу дважды; хранит только момент отправки.</td><td>До 30 дней</td></tr>` : ''}
        </tbody></table></div>
        ...
        <p>Метрику можно отключить для этого устройства: идентификатор и связанная с ним карточка посетителя будут удалены${ymOn ? ', а счётчик Яндекс Метрики на этом устройстве больше подключаться не будет' : ''}. Обезличенные итоговые счётчики посещений могут сохраниться, а данные уже оформленных заказов хранятся по правилам раздела 6.</p>
  ...
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Политика конфиденциальности',
...
function personalDataConsentPage(settings, opts) {
  ...
  return layout(settings, { body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Согласие на обработку персональных данных', origin: opts.origin || '', canonicalPath: '/personal-data-consent', description: `Согласие на обработку персональных данных ${settings.storeName}` });
}
...
function publicationConsentPage(settings, opts) {
  ...
  return layout(settings, { body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Согласие на публикацию отзыва', origin: opts.origin || '', canonicalPath: '/personal-data-publication-consent', description: `Согласие на публикацию отзыва ${settings.storeName}` });
}
...
/* «Куда придут деньги» — самый частый вопрос обеих правовых страниц, и отвечать
 * на него абзацем посреди списка нельзя: покупатель читает документ по
 * диагонали и находит взглядом выделенное. Поэтому плашка, а не строка перечня.
 *
 * ТЕКСТ ОДИН НА ОБЕ СТРАНИЦЫ. Гарантия ведёт к возврату денег (ст. 18), возврат
 * — тем более; двумя копиями формулировка разъехалась бы молча, и один документ
 * обещал бы не то, что второй. По этой же причине из перечня «Возврат денежных
 * средств» убрана прежняя строка про ту же карту: она говорила ровно это, а
 * повторять одно и то же дважды на одном экране незачем.
 *
 * Глиф — стрелка возврата из BENEFIT_GLYPHS, та же, что в бегущей строке
 * главной: второго рисунка для того же понятия в проекте быть не должно. */
const REFUND_NOTE = '<aside class="legal-note">'
  + `<svg class="legal-note-ico" viewBox="0 0 35 35" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true">${glyphBody('refund')}</svg>`
  + '<p><b>Деньги возвращаются на те же реквизиты, с которых была оплата, — в том числе при отказе от товара в момент получения.</b>'
  + ' Нужен возврат на другую карту или другой счёт — свяжитесь с менеджером и уточните порядок заранее: такие реквизиты согласуются отдельно.</p>'
  + '</aside>';
...
function warrantyPage(settings, opts) {
  ...
  const body = `<div class="container legal-page">
    ...
    <header class="legal-hero">
      ...
      <p>Мы продаём новую оригинальную технику. Ремонтом мы не занимаемся: если в течение гарантийного срока проявится заводской недостаток, мы возвращаем деньги за товар в полном объёме.</p>
    </header>
    ...
    <div class="legal-card">
      ...
      ${/* РЕМОНТА НЕТ — ПРИ ЗАВОДСКОМ НЕДОСТАТКЕ ДЕНЬГИ ВОЗВРАЩАЮТСЯ. Прежний раздел
          обещал «гарантийное обслуживание до 45 дней в сервисных центрах» и
          продление гарантии на время ремонта — ничего из этого магазин не
          делает, и консультант в чате повторял это обещание покупателям. Условие
          задано владельцем (11 сентября 2026): один исход, возврат полной суммы. */''}
      <section class="legal-section" id="service">
        <h2>3. Что происходит при заводском недостатке</h2>
        <ul>
          <li>ремонт и замена не производятся: при подтверждённом заводском недостатке покупатель получает обратно всю уплаченную за товар сумму;</li>
          <li>деньги возвращаются на те же реквизиты, с которых была оплата, в сроки, указанные в условиях возврата;</li>
          <li>гарантия действует весь установленный срок и распространяется только на заводские недостатки — случаи, когда она не действует, перечислены ниже.</li>
        </ul>
        <p>Рекомендуем снимать распаковку на видео — одним непрерывным кадром от целой упаковки до включения устройства. Такая запись подтверждает состояние товара при получении и ускоряет решение о возврате.</p>
      </section>

      <section class="legal-section" id="exclusions">
        <h2>4. Когда гарантия не действует</h2>
        <p>Гарантия не действует, если товар:</p>
        <ul>
      ...
      <section class="legal-section" id="how">
        ...
        <p>Сообщите о недостатке продавцу: ${info.contact}. Укажите номер заказа, модель, опишите, как проявляется недостаток, и приложите видео распаковки, если оно есть.</p>
        <p>Для возврата понадобятся лист с описанием недостатка, упаковка, полная комплектация, товарный чек или номер заказа и гарантийный талон, если он выдавался.</p>
        <p>Товар отправляется перевозчиком (${esc(carrierNames())}) с полным страхованием отправления и вручением лично в руки. Транспортировка товара весом менее 5 кг для проверки и возврата производится покупателем самостоятельно. После получения товара мы подтверждаем недостаток и возвращаем деньги.</p>
      </section>
  ...
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Гарантия',
    origin: opts.origin || '', canonicalPath: '/warranty',
    description: `Гарантия на технику в ${settings.storeName}: сроки, возврат денег при заводском недостатке и порядок обращения`
  });
}
...
function returnsPage(settings, opts) {
  ...
  const body = `<div class="container legal-page">
    ...
    <div class="legal-card">
      ...
      <section class="legal-section" id="defect">
        ...
        ${/* Ремонта и замены у магазина нет: единственный исход при заводском
            недостатке — возврат всей суммы, и в течение ВСЕГО гарантийного срока.
            Это шире, чем требует закон от продавца технически сложного товара
            (после 15 дней там нужен существенный недостаток), поэтому прежний
            перечень «предъявить требования можно, если…» снят — он ограничивал
            то, что магазин и так обещает без условий. */''}
        <p>При обнаружении заводского недостатка в течение гарантийного срока продавец возвращает покупателю всю уплаченную за товар сумму. Ремонт и замену товара продавец не производит — вместо них возвращаются деньги, и это не зависит от того, является ли недостаток существенным. Требование о замене на такой же или другой товар (ст. 18 Закона РФ от 07.02.1992 № 2300-1 «О защите прав потребителей») удовлетворяется возвратом уплаченной суммы, после чего покупатель может оформить новый заказ.</p>
        <p>Порядок возврата денег при заводском недостатке и случаи, в которых гарантия не действует, описаны на странице <a class="link" href="/warranty">Гарантия</a>. Рекомендуем снимать распаковку на видео — одним кадром от целой упаковки до включения устройства: запись подтверждает состояние товара при получении.</p>
      </section>
      ...
      <section class="legal-section" id="how">
        ...
        <p>Товар отправляется перевозчиком (${esc(carrierNames())}) с полным страхованием отправления и доставкой лично в руки. Транспортировка товара весом менее 5 кг для возврата производится покупателем самостоятельно.</p>
      </section>
  ...
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Возврат и обмен',
...
/* ------------------------------ О компании ----------------------------------
 *
 * Страница отвечает на два вопроса: КТО ПРОДАЁТ и КАК С НИМ СВЯЗАТЬСЯ. Разделов
 * ровно четыре — реквизиты продавца, гарантия с возвратом, контакты, карта, — и
 * порядок именно такой: первым идёт то, ради чего страницу открывают перед
 * предоплатой. Условия покупки стоят вторыми, потому что «кто продаёт» и «на
 * каких условиях» — половины одного решения; сами документы при этом лежат на
 * своих страницах, здесь только путь к ним.
 *
 * Рассказ про доставку, оплату, гарантию и порядок покупки отсюда СНЯТ. Каждый
 * из этих разделов пересказывал своими словами то, что витрина и так считает
 * сама — сроки на оформлении, режим касс, страницы `/warranty` и `/returns`, —
 * и покупателю приходилось листать четыре экрана пересказа, чтобы дойти до
 * телефона и адреса.
 *
 * ГЛАВНОЕ ПРАВИЛО прежнее: ни одного факта руками. Реквизиты и контакты берутся
 * из настроек, знаки мессенджеров — те же, что в подвале.
 */

/* Карта — СВОЯ, на тайлах OpenStreetMap (`lib/map-tiles.js`).
 *
 * Прежде здесь стоял виджет Яндекса. Он рисовал внутри чужого фрейма свои
 * кнопки — «Открыть в Яндекс Картах», «Пробки», зум, — и убрать их было нельзя
 * ни нам, ни владельцу: фрейм с другого домена, туда не дотягивается ни CSS, ни
 * скрипт, а их условия и вовсе требуют оставлять эту ссылку. Заодно он показывал
 * Яндексу каждого, кто долистал до карты.
 *
 * Теперь карта наша целиком: своя метка, своя кнопка маршрута, свой вид, а тайлы
 * отдаёт НАШ сервер — с витрины не уходит ни одного запроса на сторону. Цена
 * решения тоже названа честно: тайлы OSM проще яндексовских, пробок и панорам
 * на них нет, а внизу обязана стоять строка «© OpenStreetMap» — её требует
 * лицензия данных, и снимать её нельзя.
 *
 * Скрипта у карты нет вовсе: разметку рисует сервер, поэтому она работает и там,
 * где скрипты витрины не загрузились. Двигать и приближать её нельзя — это
 * снимок квартала, а не редактор; за маршрутом ведёт своя кнопка.
 */
/* Координаты офлайн-точки. Формат тот, в котором их копируют из карт:
 * «55.751244, 37.618423» — сначала широта. Дробная часть только через точку: с
 * запятой пара «55,75, 37,61» неотличима от четырёх чисел, и разбирать её
 * значило бы гадать. Мусор — это «координат нет», а без координат карты не
 * бывает: нарисовать квартал по строке адреса мы не умеем (своего геокодера у
 * витрины нет и не будет — см. `scripts/geocode-store.js`). */
function storePoint(settings) {
  const raw = String((settings || {}).storeGeo || '').trim();
...
/* Карта показывается, когда известны И адрес, И точка. Адрес без точки рисовать
 * нечем, а точка без адреса ничего не говорит: кружок посреди города — это не
 * ответ на вопрос «где вы». */
/* Открыт ли магазин ПРЯМО СЕЙЧАС — по строке часов из настроек.
 *
 * Строку владелец пишет руками («09:00–22:00 МСК», «10:00-20:00»), поэтому
 * разбираем осторожно: нашли две пары «часы:минуты» — считаем, не нашли —
 * молчим. Придумать часы работы за владельца хуже, чем не показать их вовсе:
 * покупатель поедет в закрытый магазин.
 *
 * Часы московские, как и всё время в проекте (`mskNow`): магазин работает по
 * Москве, и «сейчас» для него — московское, а не то, что на часах у покупателя
 * во Владивостоке.
 */
/* Сколько снимков магазина принимаем. Предел смысловой: карточка показывает
 * место, а не фотоальбом, и шестая плитка в ней уже не помещается. */
const STORE_PHOTOS_MAX = 6;
// Снимки магазина из настроек: только имена загруженных файлов, мусор отсеян —
// значение уезжает в адрес картинки.
function storePhotos(settings) {
  const raw = (settings || {}).storePhotos;
  const list = Array.isArray(raw) ? raw : [];
  return list.map(v => String(v || '')).filter(v => /^[A-Za-z0-9._-]{1,120}$/.test(v)).slice(0, STORE_PHOTOS_MAX);
}

function openNow(hours, now) {
  const raw = String(hours == null ? '' : hours);
  const m = raw.match(/(\d{1,2})[:.](\d{2})\s*[–—−-]\s*(\d{1,2})[:.](\d{2})/);
  if (!m) return null;
  const open = Number(m[1]) * 60 + Number(m[2]);
  const close = Number(m[3]) * 60 + Number(m[4]);
  if (!(open >= 0 && open < 24 * 60 && close > 0 && close <= 24 * 60)) return null;
  const d = mskDate(now || Date.now());
  const minutes = d.getHours() * 60 + d.getMinutes();
  // Ночная смена («22:00–06:00») перекрывает полночь: там открыто, когда время
  // ЗА пределами промежутка, а не внутри него.
  ...
  return { open: isOpen, until: hhmm(isOpen ? close : open) };
}

function storeMap(settings) {
  const address = String((settings || {}).storeAddress || '').trim();
  const point = storePoint(settings);
  if (!address || !point) return null;
  return { point, address };
}
/* Метка. Своя, а не общая булавка подвала: там глиф в 16 px подписывает строку
 * текста, а здесь он лежит НА КАРТЕ и обязан читаться поверх домов и подписей —
 * отсюда белая обводка и белая точка внутри. Красится акцентом магазина: цвет у
 * витрины один, и заводить карте свой незачем.
 *
 * Остриё приходится ровно на точку магазина: `translate(-50%,-100%)` в стилях
 * ставит его в центр блока, а центр блока — это и есть центр слоя тайлов. */
function mapPin() {
  return '<svg class="map-pin" viewBox="0 0 32 44" aria-hidden="true">'
    + '<path d="M16 1.8c-7.3 0-13.2 5.9-13.2 13.2 0 9.6 11.8 26.3 12.3 27a1.1 1.1 0 0 0 1.8 0c.5-.7 12.3-17.4 12.3-27C29.2 7.7 23.3 1.8 16 1.8Z" fill="var(--accent)" stroke="#fff" stroke-width="2.6" stroke-linejoin="round"/>'
    + '<circle cx="16" cy="15" r="4.7" fill="#fff"/></svg>';
}
/* Разметка карты: слой тайлов, метка, наша кнопка маршрута и строка лицензии.
 *
 * Слой ставится центром на точку магазина отрицательными полями от середины
 * блока — числа считает `MAP.layer()`, поэтому одна и та же разметка годится и
 * широкому экрану, и телефону: лишнее обрезает рамка блока.
 *
 * СТРОКУ «© OpenStreetMap» СНИМАТЬ НЕЛЬЗЯ — этого требует лицензия данных, и это
 * ровно та плата, ради отказа от которой ушёл чужой виджет: здесь она одна
 * мелкая ссылка, а не ряд чужих кнопок поверх карты.
 *
 * Кнопка маршрута — НАША: своё слово, свой вид, и ведёт она наружу только по
 * нажатию покупателя. Адрес уезжает в неё строкой, а не координатами: маршрут
 * до «ТЦ Ноябрьский» человек узнаёт по названию, и подпись на чужой карте будет
 * та же, что у нас. */
/* Карта: одна готовая картинка, а при её отсутствии — прежняя плитка тайлов.
 *
 * ПОЧЕМУ КАРТИНКА. Тайл 256×256 показывался ровно в 256 CSS-пикселей, то есть
 * на любом экране плотнее обычного (а это все телефоны и все ноутбуки с Retina)
 * карта была растянута в два-три раза и выглядела мутной рядом с текстом,
 * нарисованным чётко. Постер собран из тайлов вдвое ближнего масштаба и
 * показывается вдвое мельче — плотность двойная, карта резкая (`buildPoster` в
 * lib/map-tiles.js). Заодно это ОДИН запрос вместо трёх десятков.
 *
 * Кнопки «Построить маршрут» здесь больше нет. Она уводила покупателя на
 * чужие карты с нашей же страницы — и была единственной ссылкой на Яндекс,
 * оставшейся после того, как оттуда убрали виджет. Адрес рядом, в карточке
 * магазина: кому нужен маршрут, тот проложит его сам, скопировав строку.
 */
function storeMapBlock(map, poster) {
  const inner = poster
    // Ширина и высота — CSS-размер: картинка вдвое плотнее, и без явных
    // размеров браузер показал бы её вдвое крупнее, чем нужно.
    ? `<img class="map-poster" src="/map/store/${esc(poster)}" alt="" width="${MAP.POSTER_COLS * MAP.TILE / 2}" height="${MAP.POSTER_ROWS * MAP.TILE / 2}" loading="lazy" decoding="async"
        style="margin-left:${-Math.round(MAP.posterLayer(map.point).cx)}px;margin-top:${-Math.round(MAP.posterLayer(map.point).cy)}px">`
    : (() => {
      const view = MAP.layer(map.point);
      const tiles = view.tiles.map(t =>
        `<img class="map-tile" src="${esc(t.src)}" alt="" width="${MAP.TILE}" height="${MAP.TILE}" loading="lazy" style="left:${t.left}px;top:${t.top}px">`
      ).join('');
      return `<div class="map-layer" style="width:${view.width}px;height:${view.height}px;margin-left:${-Math.round(view.cx)}px;margin-top:${-Math.round(view.cy)}px">${tiles}</div>`;
    })();
  return `<div class="about-map">
    ${inner}
    ${mapPin()}
...
/* Плитки на «Гарантию» и «Возврат». Строчной ссылкой их сюда не поставить:
 * страницу открывают перед предоплатой, и «на каких условиях» — второй вопрос
 * после «кто продаёт», а не сноска в конце абзаца.
 *
 * Глифы — те же, что в бегущей строке главной и в блоке доверия под кнопкой
 * покупки (BENEFIT_GLYPHS): щит гарантии и стрелка возврата уже нарисованы, и
 * второй набор значков для тех же понятий разошёлся бы с первым по весу штриха.
 * Стрелка справа своя и волосяная тем же весом — это не глиф понятия, а знак
 * «здесь переходят».
 *
 * Подпись под названием говорит, ЧТО внутри документа, а не пересказывает его:
 * покупатель выбирает между двумя страницами, а читать будет уже на них. */
const ABOUT_LINKS = [
  ['warranty', '/warranty', 'Гарантия', 'Сроки, возврат денег при заводском браке и порядок обращения'],
  ['refund', '/returns', 'Возврат и обмен', 'Сроки отказа, отправка и возврат денег']
];
const ABOUT_LINKS_BLOCK = `<div class="about-links">${ABOUT_LINKS.map(([g, href, name, note]) => `<a class="about-link" href="${href}">`
  + `<svg class="about-link-ico" viewBox="0 0 35 35" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true">${glyphBody(g)}</svg>`
  + `<span class="about-link-text"><b>${name}</b><span>${note}</span></span>`
  + '<svg class="about-link-go" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9.5 5.5 16 12l-6.5 6.5"/></svg>'
  + '</a>').join('')}</div>`;

/* Карточка магазина рядом с картой — та самая, что показывают карты-справочники:
 * снимки места, название, оценка со звёздами, «открыто сейчас», адрес и связь.
 *
 * Зачем она здесь. Карта отвечает на вопрос «где это», но не отвечает на
 * главный — «что это за место и живое ли оно». Покупатель платит вперёд, и
 * витрина с вывеской на снимке говорит ему больше, чем строка адреса под точкой.
 *
 * ОЦЕНКА ЧЕСТНО ПОДПИСАНА. Это средняя по отзывам О ТОВАРАХ (`db.shopRating`), а
 * не отдельный опрос про магазин, поэтому рядом стоит «по отзывам покупателей»,
 * а не «рейтинг организации». Отзывов нет вовсе — строки нет: пустые звёзды
 * читались бы как ноль из пяти.
 *
 * «Открыто» считается по часам из настроек (`openNow`) и молчит, когда строку
 * разобрать не вышло: отправить покупателя в закрытый магазин хуже, чем не
 * сказать ничего.
 */
function storeCard(settings, map, opts) {
  opts = opts || {};
  const shots = storePhotos(settings);
  const rating = opts.rating && opts.rating.count ? opts.rating : null;
  const state = openNow(settings.contactHours);
  const phone = String(settings.contactPhone || '').trim();
  const hours = String(settings.contactHours || '').trim();
  const tg = tgUser(settings);
  const waUrl = whatsAppHref(settings);

  /* ОБЛОЖКА СВЕРХУ — то, с чего карточка начинается в справочниках, и то, чего
   * у нас не было вовсе. Первый снимок крупный, остальные узкой лентой рядом,
   * поверх последней — счётчик «ещё N»: так место узнают за один взгляд, а
   * список текста внизу отвечает уже на вопросы. Снимков нет — обложки нет
   * тоже: пустая серая рамка сказала бы о магазине ровно ничего. */
  const cover = shots.length
    ? `<div class="place-cover" data-media="store">
        <a class="place-cover-main" href="/uploads/${esc(shots[0])}" data-kind="photo" target="_blank" rel="noopener">
          <img src="/uploads/${esc(shots[0])}" alt="${esc(settings.storeName)} — фото магазина" loading="lazy" decoding="async">
        </a>
        ${shots.length > 1 ? `<div class="place-cover-rest">${shots.slice(1, 4).map((name, i) => {
          const more = shots.length > 4 && i === 2 ? shots.length - 4 + 1 : 0;
          return `<a class="place-cover-thumb" href="/uploads/${esc(name)}" data-kind="photo" target="_blank" rel="noopener">
            <img src="/uploads/${esc(name)}" alt="" loading="lazy" decoding="async">
            ${more ? `<span class="place-cover-more">+${more}</span>` : ''}
          </a>`;
        }).join('')}${shots.slice(4).map(name =>
          `<a href="/uploads/${esc(name)}" data-kind="photo" hidden></a>`).join('')}</div>` : ''}
      </div>`
    : '';

  /* Оценка — строкой, как в справочнике: звёзды, крупное число, серым сколько
   * оценок. Приписка «по отзывам о товарах» обязательна и стоит тут же: это
   * средняя по отзывам О ТОВАРАХ, а не отдельный опрос про магазин, и выдавать
   * одно за другое мы не будем. Отзывов нет — строки нет вовсе. */
  const rate = rating
    ? `<div class="place-rate">${stars(rating.avg)}<b>${esc(String(rating.avg).replace('.', ','))}</b>
        <span class="place-rate-note">${esc(reviewsCountLong(rating.count))} о товарах</span></div>`
    : '';

  /* Статус — ЦВЕТНОЙ ТЕКСТ, а не серая плашка: в справочниках «Закрыто» видно
   * первым, и видно именно цветом. Зелёный — наш цвет наличия, красный — тот
   * же, которым подписано «осталось несколько штук». */
  const open = state
    ? `<div class="place-state${state.open ? ' is-open' : ''}">${state.open
      ? `Открыто · до ${esc(state.until)}`
      : `Закрыто · откроется в ${esc(state.until)}`}</div>`
    : '';

  /* Ряд действий. Без него карточка читалась списком текста: справочник в этом
   * месте даёт крупные кнопки, потому что с карточки звонят и пишут, а не
   * читают её. Кнопки «Маршрут» здесь нет намеренно — она уводила на чужие
   * карты, и её убрали. */
  /* ГЛАВНАЯ КНОПКА — ТА СВЯЗЬ, КОТОРАЯ У МАГАЗИНА ЕСТЬ, а не непременно телефон.
   * На боевой витрине `contactPhone` не заполнен вовсе, и ряд оставался из двух
   * круглых значков без единого крупного действия — то есть выглядел ровно тем,
   * от чего карточку и переделывали. Поэтому первой идёт «Позвонить», а если
   * телефона нет — «Написать» в мессенджер; круглыми остаются остальные. */
  const tgLink = tg ? { href: `https://t.me/${esc(tg)}`, mark: tgMark(), cls: 'msg-tg', name: 'Telegram' } : null;
  const waLink = waUrl ? { href: esc(waUrl), mark: waMark(), cls: 'msg-wa', name: 'WhatsApp' } : null;
  const round = link => link
    ? `<a class="place-act place-act-round ${link.cls}" href="${link.href}" target="_blank" rel="noopener" title="${link.name}" aria-label="Написать в ${link.name}">${link.mark}</a>`
    : '';
  const chat = waLink || tgLink;                       // чем писать, если звонить некуда
  const main = phone
    ? `<a class="place-act place-act-main" href="tel:${esc(phoneHref(phone))}">${phoneIcon()}<span>Позвонить</span></a>`
    : (chat ? `<a class="place-act place-act-main ${chat.cls}" href="${chat.href}" target="_blank" rel="noopener">${chat.mark}<span>Написать</span></a>` : '');
  const acts = [
    main,
    // Тот мессенджер, что стал главной кнопкой, круглым уже не повторяем.
    phone || chat !== tgLink ? round(tgLink) : '',
    phone || chat !== waLink ? round(waLink) : ''
  ].filter(Boolean).join('');

  /* Разделы с заголовками — «Адрес», «Время работы», — как в карточке
   * справочника: значок и подпись слева говорят, ЧТО это за строка, а значение
   * идёт крупнее под ней. Прежние строки «значок + значение» читались одним
   * списком, в котором адрес и часы выглядели одинаково важными. */
  const row = (icon, title, value) => value
    ? `<div class="place-row">${icon}<div><span class="place-row-title">${esc(title)}</span><span class="place-row-value">${esc(value)}</span></div></div>`
    : '';

  return `<div class="place-card">
    ${cover}
    <div class="place-body">
      <b class="place-name">${esc(settings.storeName)}</b>
      <span class="place-kind">Магазин техники Apple</span>
      ${rate}
      ${open}
      ${acts ? `<div class="place-acts">${acts}</div>` : ''}
      <div class="place-rows">
        ${row(pinIcon(), 'Адрес', map.address)}
        ${row(clockIcon(), 'Время работы', hours)}
      </div>
    </div>
  </div>`;
}

function aboutPage(settings, opts) {
  opts = opts || {};
  const map = storeMap(settings);
  const hours = String(settings.contactHours || '').trim();
  const phone = String(settings.contactPhone || '').trim();
  const email = String(settings.contactEmail || '').trim();
  const tg = tgUser(settings);
  const waUrl = whatsAppHref(settings);
  const waPhone = PHONE.store(settings.contactWhatsApp);
  /* Контакты — строки со ЗНАЧКОМ, как в подвале, а не пары «подпись → значение».
   * Подписи («Телефон», «Почта», «WhatsApp») повторяли то, что говорят сам
   * значок и само значение, и превращали четыре контакта в таблицу с пустой
   * левой колонкой. Знаки мессенджеров те же, что в подвале, — второй рисунок
   * для одного и того же сервиса заводить незачем; WhatsApp ведёт по той же
   * ссылке `wa.me` с готовой репликой, то есть нажатие на номер открывает ровно
   * тот же диалог, что и строка в подвале. */
  const line = (icon, value, href, brand) => {
    if (!value) return '';
    const inner = `${icon}<span>${esc(value)}</span>`;
    if (!href) return `<li>${inner}</li>`;
    const blank = /^https?:/i.test(href) ? ' target="_blank" rel="noopener"' : '';
    return `<li><a class="msg-link${brand ? ' ' + brand : ''}" href="${esc(href)}"${blank}>${inner}</a></li>`;
  };
  const contacts = [
    line(phoneIcon(), phoneText(phone) || phone, phone ? `tel:${phoneHref(phone)}` : ''),
    line(mailIcon(), email, email ? `mailto:${email}` : ''),
    line(tgMark(), tg ? `@${tg}` : '', tg ? `https://t.me/${tg}` : '', 'msg-tg'),
    line(waMark(), phoneText(waPhone) || waPhone || '', waUrl, 'msg-wa'),
    line(clockIcon(), hours, '')
  ].join('');

  const body = `<div class="container legal-page about-page">
    <nav class="breadcrumb"><a href="/">Главная</a> / <span>О компании</span></nav>
    <header class="legal-hero">
      <span class="section-kicker">О компании</span>
      <h1>${esc(settings.storeName)}</h1>
      ${settings.tagline ? `<p>${esc(settings.tagline)}</p>` : ''}
    </header>

    <div class="legal-card">
      ${/* Реквизиты стоят ПЕРВЫМИ: страницу открывают, чтобы проверить продавца
           перед предоплатой, и ради этого не должно приходиться листать. Это
           единственное место витрины с банковскими реквизитами — раздел прямо
           отвечает на вопрос «кто продаёт и куда идут деньги». */''}
      <section class="legal-section" id="seller">
        <h2>Реквизиты продавца</h2>
        ${operatorCard(settings, 'Продавец', 'Обращения покупателей', { bank: true })}
      </section>

      ${/* Условия покупки идут вторыми: «кто продаёт» и «на каких условиях» —
           это одно решение, доверять ли продавцу деньги вперёд. Контакты и
           адрес отвечают на другой вопрос, «как достучаться», и стоят ниже. */''}
      <section class="legal-section" id="terms">
        <h2>Гарантия и возврат</h2>
        ${ABOUT_LINKS_BLOCK}
      </section>

      ${contacts ? `<section class="legal-section" id="contacts">
        <h2>Контакты</h2>
        <ul class="about-contacts">${contacts}</ul>
      </section>` : ''}

      ${/* Карта — и БОЛЬШЕ НИЧЕГО: ни абзаца «магазин работает и офлайн», ни
           списка с адресом под ней. Адрес стоит на самой карте меткой и строкой
           в контактах подвала. Рисует её сервер (`storeMapBlock`), поэтому она
           видна сразу и работает без скриптов. */''}
      ${map ? `<section class="legal-section" id="address">
        <h2>Как нас найти</h2>
        <div class="place">
          ${storeCard(settings, map, opts)}
          ${storeMapBlock(map, opts.mapPoster)}
        </div>
      </section>` : ''}
    </div>
  </div>`;
  ...
  const origin = opts.origin || '';
  /* Карточка организации для поисковика. Адрес и телефон в ней — то, из чего
   * собирается ответ «магазин рядом с вами»: без них строка в выдаче остаётся
   * голой ссылкой. Пустые поля не подставляем вовсе — выдуманный адрес хуже
   * отсутствующего. */
  const ld = origin ? jsonLd({
    '@context': 'https://schema.org', '@type': 'Store',
    name: settings.storeName, url: origin + '/about',
    description: shopDescription(settings) || undefined,
    telephone: phone ? (phoneText(phone) || phone) : undefined,
    email: email || undefined,
    openingHours: hours || undefined,
    address: map ? { '@type': 'PostalAddress', streetAddress: map.address, addressCountry: 'RU' } : undefined,
    geo: map && map.point ? { '@type': 'GeoCoordinates', latitude: map.point.lat, longitude: map.point.lon } : undefined
  }) : '';
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym,
    title: 'О компании', origin, canonicalPath: '/about', ogType: 'website', jsonLd: ld,
    description: `${settings.storeName}: оригинальная техника Apple, доставка по России, гарантия, контакты и адрес магазина`
  });
}

/* ===================== Личный кабинет покупателя =====================
 *
 * Вход по e-mail и паролю, свои заказы, отправления и корзина. Разметку рисует
 * сервер целиком: формы обычные (POST и страница заново), скрипта у кабинета
 * нет — кроме корзины, которая живёт в localStorage и ничем, кроме скрипта, не
 * читается (`initAccountCart` в public/app.js).
 *
 * Страницы служебные, как оплата и чек: в метрике не считаются, в карту сайта
 * не идут, `noindex`. Счётчика Яндекса здесь нет (`pageAllowed` в
 * lib/yandex-metrika.js) — в кабинете почта и заказы, и в чужую статистику им
 * не место.
 */
const ACCOUNT_ICON = '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8.2" r="4.1"/><path d="M4.6 20.4c.7-3.9 3.8-6.3 7.4-6.3s6.7 2.4 7.4 6.3"/></svg>';

// Отметка о кабинете на экране «заказ оформлен» и на странице оплаты: кабинет
// завёлся сам, пароль ушёл письмом — покупатель обязан об этом узнать сразу, а
// не найти письмо через неделю. Слова одни на оба места.
function accountNoteText(note) {
  if (!note || !(note.email || note.phone)) return '';
  if (note.created && note.email) return `Мы создали для вас личный кабинет: пароль отправили на ${note.email}. Там будут все ваши заказы и отправления.`;
  if (note.exists && note.email) return `На ${note.email} уже есть личный кабинет — войдите, и этот заказ появится там.`;
  // Кабинет нашёлся по номеру заказа: войти в него можно по этому же номеру.
  if (note.exists && note.phone) return `К номеру ${phoneText(note.phone) || note.phone} уже привязан личный кабинет — войдите по нему, и этот заказ появится там.`;
  return '';
}
function accountNote(note) {
  const text = accountNoteText(note);
  if (!text) return '';
  return `<p class="acc-note" role="status">${ACCOUNT_ICON}<span>${esc(text)}${note.exists ? ` <a href="/account/login">Войти</a>` : ''}</span></p>`;
}

/* Состояние заказа СЛОВАМИ ПОКУПАТЕЛЯ. Тон берётся у той же `payView`, что
 * красит строку в панели, — второй разбор оплаты разошёлся бы с ней, — а слова
 * свои: «ждём оплату» и «отменил покупатель» написаны менеджеру, а здесь
 * читает тот самый покупатель. */
function accountOrderState(order, now) {
  const at = now || Date.now();
  if (order && order.payment && order.payment.status === 'refunded') return { tone: 'off', label: 'Возврат' };
  const view = payView(order, at);
  if (!view) return { tone: 'none', label: 'Принят' };
  if (order.manualVoid) return { tone: 'off', label: 'Отменён' };
  switch (view.tone) {
    case 'ok': return { tone: 'ok', label: 'Оплачен' };
    case 'wait': return { tone: 'wait', label: 'Ждёт оплаты' };
    case 'draft': return { tone: 'draft', label: 'Не завершён' };
    case 'warn': return { tone: 'warn', label: 'Проверяем оплату' };
    default: return { tone: 'off', label: 'Не оплачен' };
  }
}

// Одна строка заказа в кабинете: номер, дата, состав, сумма, состояние и
// действия — оплатить, чек, отследить. Действий ровно те, что сейчас возможны.
function accountOrderRow(order, settings, now) {
  const at = now || Date.now();
  const state = accountOrderState(order, at);
  const items = Array.isArray(order.items) ? order.items : [];
  const first = items[0] ? String(items[0].name || '') : '';
  const rest = items.length - 1;
  const composition = first
    ? esc(first) + (rest > 0 ? ` <span class="acc-more">и ещё ${rest} ${pluralRu(rest, 'товар', 'товара', 'товаров')}</span>` : '')
    : '<span class="muted">—</span>';
  const paid = state.tone === 'ok';
  const payable = !paid && !payClosed(order, at, false) && !!(order.draft || order.payment || order.payMode === 'own');
  const acts = [
    payable ? `<a class="btn btn-primary" href="/pay/${esc(order.id)}">Оплатить</a>` : '',
    paid ? `<a class="btn" href="/receipt/${esc(order.id)}">Товарный чек</a>` : '',
    TRACK.shownToBuyer(order.shipment) ? `<a class="btn" href="${esc(TRACK.trackPath(order.shipment))}">Отследить</a>` : ''
  ].filter(Boolean).join('');
  return `<li class="acc-order acc-tone-${esc(state.tone)}">
    <div class="acc-order-head">
      <b>${esc(orderNo(order.number))}</b>
      <span class="acc-order-date">${esc(formatDate(order.createdAt))}</span>
      <span class="acc-state"><i></i>${esc(state.label)}</span>
    </div>
    <div class="acc-order-body">
      <span class="acc-order-items">${composition}</span>
      <b class="acc-order-sum">${money(order.total, settings)}</b>
    </div>
    ${acts ? `<div class="acc-order-acts">${acts}</div>` : ''}
  </li>`;
}

function accountShipmentRow(order) {
  const shipment = order.shipment;
  const carrier = DELIVERY.nameOf(shipment.carrier);
  return `<li><a class="trk-mine-item trk-${esc(shipment.carrier)}" href="${esc(TRACK.trackPath(shipment))}">
      <span class="trk-mine-cap"><b>${esc(orderNo(order.number))}</b>
        <span>${esc([carrier, shipment.to].filter(Boolean).join(' → '))}</span></span>
      <span class="trk-mine-state">${esc(TRACK.stateText(shipment))}</span>
      <span class="trk-mine-go" aria-hidden="true">›</span>
    </a></li>`;
}

// Уведомление над формой: исходное время показа SmoothUI для всех типов.
function accountFlash(opts) {
  if (opts.error) {
    const type = /^(?:укажите|выберите|введите|подтвердите)(?:\s|$)/i.test(String(opts.error)) ? 'info' : 'error';
    return `<p class="form-msg err acc-flash" role="${type === 'error' ? 'alert' : 'status'}" data-store-toast data-toast-type="${type}" data-toast-id="account-result">${esc(opts.error)}</p>`;
  }
  if (opts.flash) return `<p class="form-msg ok acc-flash" role="status" data-store-toast data-toast-type="success" data-toast-id="account-result">${esc(opts.flash)}</p>`;
  return '';
}

/* Поле адреса — одно на оформление и кабинет: растущий `<textarea>` в одну
 * строку, который вытягивается под адрес, и список подсказок под ним.
 *
 * ПОЧЕМУ НЕ `<input>`. Полный адрес — «Свердловская обл, г Екатеринбург, ул
 * Малышева, д 51, кв 12» — в однострочное поле шириной с телефон не
 * помещается: начало уезжает за левый край, и покупатель видит только хвост,
 * а сверяет он именно начало — город и улицу. Растущее поле показывает адрес
 * целиком, а ведёт себя как строка: Enter не переносит строку, а закрывает
 * ввод (см. `initAddressField` в public/app.js), перевод строки из вставки
 * схлопывается, сервер хранит адрес одной строкой (`ADDRESS.normalize`).
 *
 * `rows="1"` — стартовая высота; дальше высоту считает скрипт. Без скрипта
 * остаётся обычный textarea в одну строку с прокруткой — адрес всё равно
 * набирается и проверяется сервером. Оформление рисует своё поле в
 * public/app.js той же разметкой — там форма собирается скриптом. */
function addressField(id, value, opts) {
  const o = opts || {};
  return `<div class="suggest-box">`
    + `<textarea id="${esc(id)}" class="addr-input" rows="1" maxlength="${ADDRESS.MAX}"${o.name ? ` name="${esc(o.name)}"` : ''}`
    + ` placeholder="Город, улица, дом" autocomplete="street-address" enterkeyhint="done"`
    + ` role="combobox" aria-expanded="false" aria-autocomplete="list" aria-controls="${esc(id)}-list"${o.required ? ' required' : ''}>${esc(value || '')}</textarea>`
    + `<div class="suggest-list" id="${esc(id)}-list" role="listbox" hidden></div>`
    + `</div>`;
}

/* Вход, регистрация, «забыли пароль» и новый пароль по ссылке — одна страница
 * с одной карточкой: они отличаются полями и подписью, а вид у них один.
 *
 * СТРАНИЦА ЗАНИМАЕТ ЭКРАН ЦЕЛИКОМ, а не стоит карточкой под крошками
 * (20 сентября 2026, по просьбе владельца). Вход — экран одного действия, как
 * в приложении: заголовок, два поля и кнопка по центру свободного места,
 * подвал уезжает за сгиб. Крошек «Главная / Личный кабинет» здесь нет — на
 * телефоне у кабинета своя вкладка нижней панели, а на экране входа путь
 * читался служебной строкой над формой. Надписи по центру (см. `.acc-auth` в
 * styles.css), поле ввода при этом остаётся набранным слева. */
function accountAuthPage(settings, opts) {
  opts = opts || {};
  const mode = ['login', 'register', 'forgot', 'reset', 'sent'].includes(opts.mode) ? opts.mode : 'login';
  // Логин — e-mail или телефон; `email` принимается ради прежних вызовов.
  const login = String(opts.login != null ? opts.login : (opts.email || ''));
  const mailOn = !!opts.mailOn;
  /* Поле одно на оба логина и текстовое нарочно: под адрес и под номер
   * клавиатуры разные, и угадывать здесь нечего — к хранимой форме введённое
   * приводит сервер (`CUSTOMERS.loginKey`). */
  const loginField = (auto) => `<div class="field"><label for="acc-login">E-mail или телефон</label>
      <input type="text" id="acc-login" name="login" value="${esc(login)}" maxlength="120" autocomplete="${auto}" autocapitalize="none" spellcheck="false" required autofocus></div>`;
  const passwordField = (id, name, label, auto, extra) => `<div class="field"><label for="${id}">${esc(label)}</label>
      <input type="password" id="${id}" name="${name}" minlength="${CUSTOMER_PASSWORD_MIN}" maxlength="200" autocomplete="${auto}" required>${extra || ''}</div>`;
  const hint = `<p class="field-hint">Не короче ${CUSTOMER_PASSWORD_MIN} знаков.</p>`;
  let title = 'Вход в личный кабинет', body = '', foot = '';
  if (mode === 'login') {
    body = `<form method="post" action="/account/login" class="acc-form">
      ${loginField('username')}
      ${passwordField('acc-pass', 'password', 'Пароль', 'current-password')}
      <button class="btn btn-primary btn-block btn-lg" type="submit">Войти</button>
    </form>`;
    foot = `<p class="acc-links"><a href="/account/register">Зарегистрироваться</a>${mailOn ? ` · <a href="/account/forgot">Забыли пароль?</a>` : ''}</p>`
      + (mailOn ? `<p class="acc-auth-hint">Оформили заказ и указали почту? Кабинет уже есть — пароль в письме.</p>` : '');
  } else if (mode === 'register') {
    title = 'Регистрация';
    // Регистрация по телефону законна, но пароль восстанавливается только
    // письмом — об этом честно сказано под кнопкой, когда почта настроена.
    body = `<form method="post" action="/account/register" class="acc-form">
      ${loginField('username')}
      ${passwordField('acc-pass', 'password', 'Пароль', 'new-password', hint)}
      <button class="btn btn-primary btn-block btn-lg" type="submit">Создать кабинет</button>
    </form>${mailOn ? `<p class="acc-auth-hint">Телефон можно будет добавить в кабинете, e-mail — тоже; ссылка для восстановления пароля приходит только на e-mail.</p>` : ''}`;
    foot = `<p class="acc-links">Уже есть кабинет? <a href="/account/login">Войти</a></p>`;
  } else if (mode === 'forgot') {
    title = 'Восстановление пароля';
    body = mailOn ? `<p class="acc-auth-copy">Пришлём на почту кабинета ссылку — по ней зададите новый пароль. Ссылка действует час.</p>
    <form method="post" action="/account/forgot" class="acc-form">
      ${loginField('username')}
      <button class="btn btn-primary btn-block btn-lg" type="submit">Отправить ссылку</button>
    </form>` : `<p class="acc-auth-copy">Восстановление по почте сейчас недоступно — напишите нам, и менеджер поможет войти.</p>`;
    foot = `<p class="acc-links"><a href="/account/login">Вернуться ко входу</a></p>`;
  } else if (mode === 'sent') {
    title = 'Письмо отправлено';
    body = opts.via === 'phone'
      ? `<p class="acc-auth-copy">Если к номеру ${esc(login)} привязан кабинет с e-mail, ссылка для нового пароля уже в пути на эту почту. Проверьте и папку «Спам».</p>`
      : `<p class="acc-auth-copy">Если на ${esc(login)} есть кабинет, ссылка для нового пароля уже в пути. Проверьте и папку «Спам».</p>`;
    foot = `<p class="acc-links"><a href="/account/login">Вернуться ко входу</a></p>`;
  } else {
    title = 'Новый пароль';
    body = `<form method="post" action="/account/reset/${esc(opts.token || '')}" class="acc-form">
      ${passwordField('acc-pass', 'password', 'Новый пароль', 'new-password', hint)}
      <button class="btn btn-primary btn-block btn-lg" type="submit">Сохранить и войти</button>
    </form>`;
  }
  const html = `<div class="container acc-auth">
    <section class="acc-card acc-auth-card">
      <h1 class="acc-auth-title">${esc(title)}</h1>
      ${accountFlash(opts)}
      ${body}
      ${foot}
    </section>
  ...
  return layout(settings, Object.assign({}, opts, {
    body: html, title, canonicalPath: '/account/login', noindex: true, ym: null, tab: 'account',
    description: `Личный кабинет покупателя ${settings.storeName}`
  }));
}

/* Сам кабинет: заказы, отправления, профиль и пароль. Разделов столько,
 * сколько вопросов у покупателя, и первыми стоят те, ради которых он зашёл, —
 * «где мой заказ» и «где посылка».
 *
 * КОРЗИНЫ В КАБИНЕТЕ НЕТ (снята 20 сентября 2026 по просьбе владельца): на
 * телефоне у неё своя вкладка нижней панели, на компьютере — шторка в шапке,
 * и третья копия того же списка в кабинете отвечала на вопрос, который здесь
 * не задают. Крошек нет по той же причине, что на входе. «Выйти» стоит ПОСЛЕ
 * всех разделов маленькой кнопкой: это действие раз в жизни, а в шапке рядом
 * с заголовком оно было самой заметной кнопкой страницы. */
function accountPage(settings, opts) {
  opts = opts || {};
  const c = opts.customer || {};
  const now = opts.now || Date.now();
  const orders = Array.isArray(opts.orders) ? opts.orders : [];
  const shipments = orders.filter(o => TRACK.shownToBuyer(o && o.shipment));
  const ordersHtml = orders.length
    ? `<ul class="acc-orders">${orders.map(o => accountOrderRow(o, settings, now)).join('')}</ul>`
    : `<p class="acc-empty">Заказов пока нет. <a href="/catalog">Перейти в каталог</a></p>`;
  const shipHtml = shipments.length
    ? `<ul class="trk-mine acc-ships">${shipments.map(accountShipmentRow).join('')}</ul>`
    : `<p class="acc-empty">Посылок в пути нет. Ссылка на отслеживание появится здесь, как только заказ уедет со склада.</p>`;
  const openPass = opts.open === 'password';
  // Пароль, присланный письмом, покупатель не выбирал: пока он им не вошёл,
  // сменить пароль можно без старого (см. lib/customers.js).
  const passwordForm = `<form method="post" action="/account/password" class="acc-form">
      ${c.autoPassword
        ? `<p class="acc-copy">Пароль от кабинета мы отправили на ${esc(c.email)}. Не пришло или неудобно — задайте свой.</p>`
        : `<div class="field"><label for="acc-cur">Текущий пароль</label><input type="password" id="acc-cur" name="current" maxlength="200" autocomplete="current-password" required></div>`}
      <div class="field"><label for="acc-new">Новый пароль</label><input type="password" id="acc-new" name="password" minlength="${CUSTOMER_PASSWORD_MIN}" maxlength="200" autocomplete="new-password" required>
        <p class="field-hint">Не короче ${CUSTOMER_PASSWORD_MIN} знаков.</p></div>
      <button class="btn btn-primary" type="submit">Сменить пароль</button>
    </form>`;
  const html = `<div class="container acc-page">
    <header class="acc-head">
      <h1>Личный кабинет</h1><p class="acc-sub">${esc(CUSTOMERS.loginText(c))}</p>
    </header>
    ${accountFlash(opts)}
    <div class="acc-grid">
      <section class="acc-card" id="orders" aria-labelledby="acc-orders-title">
        <h2 id="acc-orders-title">Заказы${orders.length ? ` <span class="acc-count">${orders.length}</span>` : ''}</h2>
        ${ordersHtml}
      </section>
      <section class="acc-card" id="shipments" aria-labelledby="acc-ships-title">
        <h2 id="acc-ships-title">Отправления${shipments.length ? ` <span class="acc-count">${shipments.length}</span>` : ''}</h2>
        ${shipHtml}
      </section>
      ${/* Адрес доставки — то же растущее поле с подсказками, что на оформлении
           (`addressField()`): один адрес у покупателя, и выглядеть он обязан
           одинаково в обоих местах. Проверяется той же полнотой, что и заказ. */''}
      <section class="acc-card" id="profile" aria-labelledby="acc-profile-title">
        <h2 id="acc-profile-title">Профиль</h2>
        <form method="post" action="/account/profile" class="acc-form">
          <div class="field"><label for="acc-name">Имя и фамилия</label><input type="text" id="acc-name" name="name" value="${esc(c.name || '')}" maxlength="120" autocomplete="name"></div>
          ${/* E-mail и телефон — логины кабинета (войти можно по любому), поэтому
               оба правятся здесь, с проверкой, что не заняты другим кабинетом
               и что хоть один остался (lib/customers.js). На e-mail приходит
               ссылка восстановления пароля. */''}
          <div class="field"><label for="acc-email">E-mail</label><input type="email" id="acc-email" name="email" value="${esc(c.email || '')}" maxlength="120" inputmode="email" autocomplete="email" placeholder="mail@example.ru"></div>
          <div class="field"><label for="acc-phone">Телефон</label><input type="tel" id="acc-phone" name="phone" value="${esc(phoneText(c.phone) || c.phone || '')}" maxlength="24" inputmode="tel" autocomplete="tel" placeholder="+7 900 000-00-00"></div>
          <div class="field"><label for="acc-address">Адрес доставки</label>${addressField('acc-address', c.address || '', { name: 'address' })}</div>
          <p class="field-hint">По e-mail или телефону вы входите в кабинет, на e-mail приходит ссылка для восстановления пароля. Имя, телефон и адрес подставляются в оформление заказа, чтобы не набирать заново.</p>
          <button class="btn btn-primary" type="submit">Сохранить</button>
        </form>
      </section>
      <section class="acc-card" id="password" aria-labelledby="acc-pass-title">
        <h2 id="acc-pass-title">Пароль</h2>
        ${openPass || c.autoPassword ? passwordForm : `<details class="acc-fold"><summary>Сменить пароль</summary>${passwordForm}</details>`}
      </section>
    </div>
    <form method="post" action="/account/logout" class="acc-logout"><button class="btn btn-sm" type="submit">Выйти</button></form>
  </div>`;
  return layout(settings, Object.assign({}, opts, {
    body: html, title: 'Личный кабинет', canonicalPath: '/account', noindex: true, ym: null, tab: 'account',
    description: `Личный кабинет покупателя ${settings.storeName}`
  }));
}

module.exports = {
  CRITICAL_KINDS, criticalKind, stylesTag,
  esc, money, formatDate, mskDate, mskNow, mskTime, mskDateTime, mskSameDay, mskDayStart, mskInputValue, parseMskInput, MSK_ZONE, orderStatus, orderPayMethod, orderBankMark, orderTone, orderRowClass, orderStats, orderStatsBar, ORDER_TONES,
  ORDER_PERIODS, orderPeriod, orderPeriodLabel, orderPeriodSince, ordersInPeriod, ordersTotal,
  providerStats, providerStatsBar,
  payLive, payUntil, payDisplay, payView, payLeftText, requisiteView, orderRequisites,
  PAY_WINDOW, orderPayUntil, payExpired, payClosed,
  moneyText, orderPaymentDiscount, orderMessage, orderWaHref, orderTelHref, telegramUsername, orderTelegramHref, tgMark, waMark,
  editSwitch, adminIcon, adminIconPath, orderNo, orderDelivery, orderClient, orderItems, orderPromo, phoneText, phoneFlag, clientMarks, visitorHref, stars, startPrice, sellable, priceHidden, layout, homePage, catalogPage, categoryTiles, notFoundPage, blockedPage, productPage, checkoutPage, payPage, receiptPage, privacyPage,
  personalDataConsentPage, publicationConsentPage, warrantyPage, returnsPage, aboutPage, storeMap, storePoint, imageMarkup, placeholderSvg,
  trackingPage, trackingBoard, trackWhen, openNow, storePhotos, STORE_PHOTOS_MAX,
  ...
  cardName, cardNameStorageOn, startStorageLabel, homeCatalogOn,
  // Потолок одной покупки: его спрашивает ещё и таблица цен для консультанта
  // (`lib/chat-prompt.js`) — сборка дороже потолка на витрине погашена, и
  // называть её цену нельзя. Свой расчёт потолка там стал бы вторым.
  orderMax, fitsOrderLimit,
  // Реквизиты продавца: `reqDigits` — одна форма номера на форму настроек и
  // витрину, `operatorDetails` отдаёт разобранные значения панели и тестам.
  reqDigits, ogrnLabel, operatorDetails, footerRequisites,
  ...
  // Личный кабинет покупателя: страницы и отметка о созданном кабинете.
  accountPage, accountAuthPage, accountNote, accountNoteText, accountOrderState, ACCOUNT_ICON
};
