...
/* Карта — ЯНДЕКС, и она показывается СРАЗУ, решением владельца.
 *
 * Цена этого названа честно: фрейм на странице — запрос к чужому серверу у
 * КАЖДОГО, кто её открыл, то есть Яндекс получает адрес и время визита всякого
 * посетителя «О компании». Раньше здесь стояла кнопка «Показать карту», и
 * наружу не уходило ничего, пока покупатель сам не попросил карту, — но карта,
 * которую надо сперва вызвать нажатием, покупателю на этой странице не помогает:
 * он пришёл посмотреть, где магазин.
 *
 * Что от прежней осторожности осталось:
 *  - `frame-src` в CSP открыт узко — только яндексовый хост и только для фрейма:
 *    ни скриптов, ни картинок, ни соединений оттуда мы не разрешаем;
 *  - `referrerpolicy="no-referrer"` — Яндекс не узнаёт, С КАКОЙ страницы фрейм;
 *  - `loading="lazy"` — запрос уходит, когда карта подъезжает к экрану, а не на
 *    каждое открытие страницы вслепую.
 *
 * Скрипта у карты больше нет вовсе: фрейм рисует сервер, поэтому карта работает
 * и там, где скрипты витрины не загрузились.
 */
...
/* Адрес виджета. Нет адреса магазина — нет и карты: точка посреди города, не
 * названная адресом, ничего не говорит.
 *
 * НА КАРТЕ ОБЯЗАНА БЫТЬ МЕТКА — ради неё карту и смотрят. Ставится она двумя
 * способами, и первый надёжнее:
 *  - заданы координаты — рисуем метку сами (`pt=<долгота>,<широта>,pm2rdm`,
 *    красная булавка Яндекса) и центрируем карту на ней же. Промахнуться
 *    нечем: точка ровно та, что вписал владелец;
 *  - координат нет — виджет ищет по самому адресу (`text=`) и метку ставит на
 *    найденное. Это ровно то, что покупатель сделал бы руками, но попадание
 *    зависит от того, как Яндекс разобрал строку: в ТЦ, в промзоне и в новом
 *    доме он промахивается. Поэтому у поля «Координаты на карте» в настройках
 *    и есть смысл. */
function storeMap(settings) {
  ...
  return { frame: `${MAP_HOST}/map-widget/v1/?${q}`, point: pt, address };
}
  ...
  const body = `<div class="container legal-page about-page">
    ...
    <div class="legal-card">
      ...
      ${/* Карта — и БОЛЬШЕ НИЧЕГО: ни абзаца «магазин работает и офлайн», ни
           ...
           в контактах подвала. Фрейм рисует СЕРВЕР, поэтому карта видна сразу и
           работает без скриптов; чем это оплачено — в комментарии к MAP_HOST.
      ...
      ${map ? `<section class="legal-section" id="address">
        <h2>Как нас найти</h2>
        <div class="about-map">
          <iframe class="map-frame" src="${esc(map.frame)}" title="Карта: ${esc(map.address)}"
