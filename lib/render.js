...
function checkoutPage(settings, opts) {
  ...
  const fee = opts.payOnline ? PAYMENTS.checkoutFee(settings, 0) : null;
  const paymentMethods = opts.payOnline ? PAYMENTS.offeredMethods(settings) : [];
  const roundedOnly = paymentMethods.length === 1 && paymentMethods[0].provider === 'platega';
  const feeAttr = fee ? ` data-payment-fee-percent="${esc(fee.percent)}"${roundedOnly ? ' data-payment-rounded-only="1"' : ''}` : '';
...
function orderPaymentDiscount(order) {
  ...
  return fee && fee.rounding === 'rubles' && fee.discount > 0 && fee.discount <= 1
    && order.total === fee.paymentTotal
    && Math.round((fee.originalTotal - order.total) * 100) === Math.round(fee.discount * 100)
    ? fee.discount : 0;
}
...
module.exports = {
  ...
  moneyText, orderMessage, orderWaHref, orderTelHref, telegramUsername, orderTelegramHref, tgMark, waMark,
