...
/* ------------------------- Вордмарк: надпись КОНТУРАМИ -------------------------
 *
 * Знак магазина рисуется геометрией, а не набирается шрифтом. Причина не в
 * красоте, а в том, что шрифтом эту задачу решить нельзя вовсе:
 *
 * 1. НУЖНОГО ВЕСА НЕТ НИ В ОДНОМ СИСТЕМНОМ НАЧЕРТАНИИ. У вордмарков такого рода
 *    штрих около 27% от роста строчной; самое жирное, что стоит в системах, —
 *    Futura Bold, и она заметно легче.
 * 2. FIREFOX НЕ ОТДАЁТ САЙТУ СИСТЕМНЫЕ ШРИФТЫ. Набор установленных шрифтов —
 *    признак для отслеживания, и браузер ограничивает страницу базовым списком,
 *    в котором никакой геометрической антиквы нет. На витрине это выглядело
 *    так, будто правка не доехала: в одном браузере знак геометрический, в
 *    соседнем — обычный системный с двухэтажной «a». Своего файла шрифта ради
 *    трёх букв не заводим — 40 КБ на первом экране у каждого покупателя дороже
 *    полукилобайта контуров, которые вдобавок не мигают подменой начертания.
 *
 * Буквы строятся из КРУГА и ПРЯМОЙ СТОЙКИ — так же, как рисовали геометрические
 * гротески: кольцо даёт очко, стойка — прямой бок. Все размеры считаются от
 * роста строчной (`XH`), поэтому знак остаётся собой на любом кегле.
 */
const MARK = {
  XH: 100,   // рост строчной
  ASC: 138,  // высота выносной вверх («d») от базовой линии
  W: 27,     // толщина штриха — 27% от роста строчной
  GAP: 4     // просвет между буквами: у такого знака они почти касаются
};
MARK.R = MARK.XH / 2;          // внешний радиус очка
MARK.RI = MARK.R - MARK.W;     // внутренний
MARK.TOP = MARK.ASC - MARK.XH; // верх строчной буквы

// Кольцо: внешняя окружность по часовой, внутренняя против — дырку пробивает
// правило `nonzero`. `evenodd` здесь брать НЕЛЬЗЯ: стойка буквы накладывается на
// штрих кольца, и совпавшие области выворачивались бы в пустоту.
function markRing(cx, cy, R, r) {
  return `M${cx - R} ${cy}A${R} ${R} 0 1 1 ${cx + R} ${cy}A${R} ${R} 0 1 1 ${cx - R} ${cy}Z`
    + `M${cx - r} ${cy}A${r} ${r} 0 1 0 ${cx + r} ${cy}A${r} ${r} 0 1 0 ${cx - r} ${cy}Z`;
}
function markBar(x, y, w, h) { return `M${x} ${y}H${x + w}V${y + h}H${x}Z`; }
// Незамкнутое кольцо («c»): раствор задаётся половиной угла от горизонтали.
function markOpenRing(cx, cy, R, r, deg) {
  const a1 = -deg * Math.PI / 180, a2 = deg * Math.PI / 180;
  const at = (rad, ang) => `${(cx + rad * Math.cos(ang)).toFixed(1)} ${(cy + rad * Math.sin(ang)).toFixed(1)}`;
  return `M${at(R, a1)}A${R} ${R} 0 1 0 ${at(R, a2)}L${at(r, a2)}A${r} ${r} 0 1 1 ${at(r, a1)}Z`;
}

/* Глифы. Нарисованы ровно те буквы, что нужны знаку магазина; любая другая
 * надпись остаётся текстом (см. `wordmark`) — выдуманная на ходу буква хуже
 * честно набранной. Добавить букву — дописать сюда строку.
 *
 * `top` у каждого глифа свой: у «d» выносная достаёт до самого верха, у
 * строчных без выносных ink начинается ниже, и общая рамка знака считается по
 * тем буквам, которые в надписи ЕСТЬ, — иначе у «adc» сверху висела бы пустая
 * полоса в треть высоты.
 */
const MARK_GLYPHS = {
  // Кольцо и прямая стойка справа во весь рост строчной.
  a: { w: MARK.XH, top: MARK.TOP, d: x => markRing(x + MARK.R, MARK.ASC - MARK.R, MARK.R, MARK.RI)
    + markBar(x + MARK.XH - MARK.W, MARK.TOP, MARK.W, MARK.XH) },
  // То же, но стойка уходит вверх выносной.
  d: { w: MARK.XH, top: 0, d: x => markRing(x + MARK.R, MARK.ASC - MARK.R, MARK.R, MARK.RI)
    + markBar(x + MARK.XH - MARK.W, 0, MARK.W, MARK.ASC) },
  // Кольцо с вырезом справа; концы срезаны вертикально, как у геометрических гротесков.
  c: { w: 96, top: MARK.TOP, d: x => markOpenRing(x + MARK.R, MARK.ASC - MARK.R, MARK.R, MARK.RI, 40) }
};

/* Собирает знак. Возвращает пустую строку, когда рисовать нечем, — и тогда
 * надпись остаётся текстом, как была. Отказываемся в трёх случаях:
 * буква без глифа, пусто, и {фигурные скобки} — акцентная буква у нарисованного
 * знака означала бы отдельный контур на цвет, а знак такого рода одноцветный.
 */
function wordmark(text, label) {
  ...
  const glyphs = [];
  for (const ch of src) {
    const g = MARK_GLYPHS[ch];
    if (!g) return '';
    glyphs.push(g);
  }
  let x = 0, top = MARK.ASC, path = '';
  ...
  const w = x - MARK.GAP;
  return `<svg class="logo-mark" viewBox="0 ${top} ${w} ${MARK.ASC - top}" role="img"`
    + ` aria-label="${esc(label || text)}"><path d="${path}" fill="currentColor"/></svg>`;
}
