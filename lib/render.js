 ...
 *
 * Покупатель вернулся с оплаты, деньги дошли — и на этом месте у него до сих пор
 * стояли три строки: «Платёж получен», сумма и номер заказа. ЧТО именно он
 * купил, куда это поедет и кому он заплатил, страница не говорила вовсе, а
 * письма магазин не шлёт: единственным следом покупки оставалась строка в
 * банковском приложении. Это ровно тот момент, когда документ на руках нужнее
 * всего — им сверяют состав, по нему пишут в чат и по нему же проверяют
 * продавца.
 *
 * ЭТО ПОДТВЕРЖДЕНИЕ ЗАКАЗА, А НЕ КАССОВЫЙ ЧЕК, и разница не в словах. Фискальный
 * чек выдаёт онлайн-касса через ОФД, у него есть номер ФД, фискальный признак и
 * QR для проверки в ФНС; ничего этого магазин не выдаёт — значит и вид такого
 * документа подделывать нельзя. Поэтому здесь нет ни слова «чек», ни QR, ни
 * полей фискального документа: только то, что магазин правда знает о заказе.
 *
 * Печатается оно обычной печатью браузера (`@media print` в styles.css):
 * «сохранить в PDF» — это она же, и заводить ради одной страницы свой
 * генератор PDF или отправку письмом незачем.
 */
function receiptRow(label, value) {
  return value ? `<div class="rc-row"><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>` : '';
}

/* Чем оплачено — словами покупателя, а не кассы.
 *
 * Имя платёжки здесь не появляется никогда (см. «Покупатель не знает, что касс
 * несколько»): способ он выбирал сам, а какая касса выдала реквизиты — наше
 * внутреннее дело. У оплаты по своим реквизитам способа в заказе нет вовсе, и
 * тогда это просто перевод.
 */
function receiptPayLine(order, pay) {
  if (order && order.manualPaid) return 'Перевод по реквизитам магазина';
  const method = pay && pay.method ? PAY.describe(pay.method) : null;
  return method ? method.name : 'Онлайн-оплата';
}

function payReceipt(settings, order, opts) {
  opts = opts || {};
  const items = Array.isArray(order.items) ? order.items : [];
  // Суммы собираются ровно так, как их видел покупатель на оформлении: товары
  // до скидки, отдельной строкой сама скидка, доставка и итог. Иначе столбик не
  // сходится — цена позиции хранится уже со скидкой, и «товары» без неё
  // читались бы как ошибка в подсчёте.
  const goods = Number(order.itemsTotal) > 0 ? Number(order.itemsTotal)
  ...
  const rows = items.map(it => {
    ...
    return `<tr>
      ...
      <td class="rc-qty">${esc(String(qty))}</td>
      <td class="rc-one">${money(price, settings)}</td>
      <td class="rc-sum">${money(price * qty, settings)}</td>
    </tr>`;
  }).join('');

  const sum = (label, value, cls) => `<div class="rc-sum-row${cls ? ' ' + cls : ''}">
    <span>${esc(label)}</span><b>${value}</b></div>`;

  const info = operatorDetails(settings);
  const seller = [
    receiptRow(info.kind, info.name),
    receiptRow('ИНН', info.inn),
    receiptRow(info.ogrnLabel, info.ogrn),
    receiptRow('Реквизиты', info.registration)
  ].filter(Boolean).join('');

  // Куда едет посылка — адрес пункта выдачи, а у курьерского заказа адрес
  // покупателя: это разные поля, и путать их нельзя (см. «Два адреса»).
  const way = [DELIVERY.nameOf(order.delivery), DELIVERY.shortModeOf(order.delivery, order.deliveryMode)]
    .filter(Boolean).join(', ');
  const to = order.pickupAddress || order.address || '';

  return `<section class="receipt" id="receipt" aria-labelledby="receipt-title">
    <header class="rc-head">
      <div class="rc-store">
        <b>${esc(settings.storeName || '')}</b>
        <span id="receipt-title">Подтверждение оплаты</span>
      </div>
      <div class="rc-num">
        <span>Заказ ${esc(orderNo(order.number))}</span>
        <b>${esc(mskDateTime(opts.paidAt || order.createdAt))}</b>
      </div>
    </header>
    ...
    <div class="rc-sums">
      ${sum('Товары', money(goods + saved, settings))}
      ${saved > 0 ? sum('Скидка' + (order.promoCode ? ' по промокоду ' + order.promoCode : ''), '−' + money(saved, settings), 'is-save') : ''}
      ${shipping > 0 ? sum('Доставка', money(shipping, settings)) : ''}
      ${sum('Оплачено', money(order.total, settings), 'is-total')}
    </div>

    <div class="rc-grid">
      <dl class="rc-block">
        <div class="rc-row rc-row-head"><dt>Оплата</dt><dd></dd></div>
        ${receiptRow('Способ', receiptPayLine(order, opts.pay))}
        ${receiptRow('Когда', mskDateTime(opts.paidAt || Date.now()))}
        ${receiptRow('Статус', 'оплачено полностью')}
      </dl>
      <dl class="rc-block">
        <div class="rc-row rc-row-head"><dt>Доставка</dt><dd></dd></div>
        ${receiptRow('Способ', way)}
        ${receiptRow('Пункт выдачи', order.pickupCode)}
        ${receiptRow('Адрес', to)}
        ${receiptRow('Получатель', order.customerName)}
        ${receiptRow('Телефон', phoneText(order.phone))}
      </dl>
      <dl class="rc-block">
        <div class="rc-row rc-row-head"><dt>Продавец</dt><dd></dd></div>
        ${seller}
      </dl>
    </div>

    <div class="rc-actions">
      <button type="button" class="btn" data-print>Распечатать или сохранить в PDF</button>
    </div>
    <p class="rc-note">Сохраните подтверждение: по номеру заказа менеджер найдёт покупку, а по реквизитам продавца — кому она оплачена.</p>
  </section>`;
}
...
function payPage(settings, order, opts) {
  ...
  if (ownPaid && state !== 'paid' && !orderArchived) {
    ...
    ${payReceipt(settings, order, { paidAt: order.manualPaid && order.manualPaid.at, pay })}`;
    ...
    ${payReceipt(settings, order, { paidAt: pay && pay.paidAt, pay })}`;
...
function storeMapBlock(map) {
  ...
  const route = `https://yandex.ru/maps/?rtext=~${encodeURIComponent(map.address)}&rtt=auto`;
  return `<div class="about-map">
    <div class="map-layer" style="width:${view.width}px;height:${view.height}px;margin-left:${-Math.round(view.cx)}px;margin-top:${-Math.round(view.cy)}px">${tiles}</div>
    ${mapPin()}
    <a class="map-route" href="${esc(route)}" target="_blank" rel="noopener">Построить маршрут</a>
