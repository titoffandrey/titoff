 ...
 */
function orderRequisites(pay) {
  if (!pay || !pay.requisite) return '';
...
function providerStatsBar(rows) {
  const list = rows || [];
  ...
  return `<div class="a-panel"><h2>Кассы</h2>
...
const ORDER_TONES = [
  ...
  { key: 'warn', label: 'Проверить', note: 'сумма не сошлась или сбой', hideEmpty: true },
...
function orderStats(list, now) {
  ...
  const avg = tones.ok.n ? Math.round(tones.ok.sum / tones.ok.n) : 0;
  return { count, sum, revenue: tones.ok.sum, paid: tones.ok.n, avg, today, tones };
}
...
function orderStatsBar(stats, settings) {
  ...
  const paidPct = s.count ? Math.round((s.paid / s.count) * 100) : 0;
  ...
  const facts = [['Оплачено', s.count ? `${statNumber(s.paid)} из ${orders(s.count)} · ${paidPct}%` : '']];
  ...
  const moneyCard = `<section class="o-money">
      ...
      ${s.count ? `<div class="o-money-bar" role="img" aria-label="Оплачено ${paidPct}% заказов"><i class="o-stat-ok" style="width:${paidPct}%"></i></div>` : ''}
...
function editSwitch(id, opts) {
  ...
  return `<input type="checkbox" id="${esc(id)}" class="edit-switch sr-only"${o.on ? ' checked' : ''}>
    <div class="a-panel-head a-panel-edit">
      <label class="edit-btn" for="${esc(id)}" title="${esc(o.label || 'Показать удаление')}">
...
function chatWidget(settings) {
  ...
  return `<div class="chat-widget" id="chat-widget" data-mode="ai">
  ...
  <section class="chat-panel" id="chat-panel" role="dialog" aria-label="Чат с магазином" aria-hidden="true" inert>
    <header class="chat-head">
      ...
      <span class="chat-brand"><b>${esc(settings.storeName || 'Магазин')}</b><span class="chat-status" id="chat-status">Обычно отвечаем сразу</span></span>
    ...
    <div class="chat-log" id="chat-log" role="log" aria-live="polite" aria-atomic="false">
      <div class="chat-msg chat-them"><span class="chat-who">Консультант</span><span class="chat-text">${esc(greeting)}</span></div>
    </div>
    <div class="chat-foot">
      <button class="chat-call" id="chat-call" type="button" data-chat-act="call">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M16 19.2v-1.1a3.1 3.1 0 0 0-3.1-3.1H6.9A3.1 3.1 0 0 0 3.8 18.1v1.1"/><circle cx="9.9" cy="8.1" r="3.1"/><path d="M17 8.4h4M19 6.4v4"/></svg>
        Позвать менеджера
      </button>
      <form class="chat-form" id="chat-form">
        <textarea id="chat-input" rows="1" placeholder="Спросите о товаре, доставке, оплате" aria-label="Сообщение" maxlength="${CHAT.MAX_TEXT}"></textarea>
...
<footer class="site-footer">
  <div class="container footer-grid">
    ...
    <div class="footer-bottom">
      ${PAYMENTS.enabled(settings) ? PAY_METHODS : ''}
...
${CHAT.visible(settings) ? `<script src="/static/chat.js?v=${assetV('chat.js')}" defer></script>` : ''}
