...
function layout(settings, opts) {
  ...
  /* Telegram — обычная строка меню под волосяной линией, как «Справка» и
   * «Отправить отзыв» у Trends. Кнопкой фирменного цвета он стоял в шапке и
   * тянул внимание на себя на каждой странице, хотя ведёт в канал, куда ходят
   * один раз. Подпись та же, что у кнопки в подвале: адрес у них один, и
   * обещать по нему разное нельзя. */
  const navTg = settings.contactTelegram
    ? `<span class="nav-sep" aria-hidden="true"></span>`
      + `<a class="nav-item" href="https://t.me/${esc(tgUser(settings))}" target="_blank" rel="noopener">Канал в Telegram</a>`
    : '';
