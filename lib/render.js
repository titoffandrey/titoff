...
// Версии публичных юридических документов. Версия записывается вместе с
// согласием в заказ/отзыв, чтобы можно было подтвердить, какой текст принял
// посетитель.
...
<aside class="cookie-notice" id="cookie-notice" role="status" aria-live="polite" aria-label="Уведомление об использовании cookie" hidden>
  <div class="cookie-notice-copy">
    <strong>Необходимые cookie</strong>
    <span>Используем cookie для защищённого входа и локальное хранилище для корзины. Рекламных cookie нет. <a href="/privacy#cookies">Подробнее</a></span>
  </div>
  <button class="btn btn-primary btn-sm" type="button" id="cookie-ok">ОК</button>
</aside>
    ...
    <section class="store-hero">
      <div class="container store-hero-inner">
        <div class="store-hero-copy">
          ...
          <p>${esc(settings.tagline || 'Выбирайте нужную модель — мы поможем с остальным.')}</p>
        </div>
        <div class="store-benefits" aria-label="Преимущества магазина">
          <span>Проверенная техника</span><span>Гарантия</span><span>Быстрая связь</span>
        </div>
      </div>
    </section>` : '';
...
function productPage(settings, db, p, site, opts) {
  ...
  const mainImg = imageMarkup(p, 0, { eager: true });
  ...
  const slidesData = (p.images && p.images.length)
    ? esc(JSON.stringify(p.images.map(s => ({ src: '/uploads/' + s, color: imgColorOf(s) }))))
    : '';
  ...
  const reviewItems = reviews.length ? reviews.map(rv => {
    const chips = rv.aspects ? ['delivery', 'service', 'price'].filter(k => Number(rv.aspects[k]) >= 1 && Number(rv.aspects[k]) <= 5).map(k => `<span class="asp-chip">${aspLabels[k]} ${Number(rv.aspects[k])}</span>`).join('') : '';
    const reviewRating = Math.max(1, Math.min(5, Number(rv.rating) || 1));
    const reviewTime = Number.isFinite(Number(rv.createdAt)) ? Number(rv.createdAt) : 0;
    return `
    <article class="review" data-rating="${reviewRating}" data-ts="${reviewTime}" data-len="${String(rv.text || '').length}">
      ...
      <div class="product-info">
        ...
        <div class="variant-group">
          ...
          <div class="swatches" id="colors">
            ${p.colors.map((c, i) => `<button type="button" class="swatch${i === 0 ? ' active' : ''}" data-color="${esc(c.name)}" aria-label="Цвет: ${esc(c.name)}" aria-pressed="${i === 0 ? 'true' : 'false'}" title="${esc(c.name)}" style="--sw:${cssColor(c.hex, '#cccccc')}"></button>`).join('')}
          </div>
        </div>` : ''}
        ...
        <div class="variant-group">
          ...
          <div class="storage-opts" id="storages">
            ${p.storages.map((s, i) => `<button type="button" class="storage-opt${i === 0 ? ' active' : ''}" data-add="${Number(s.add) || 0}" data-label="${esc(s.label)}" aria-pressed="${i === 0 ? 'true' : 'false'}">${esc(s.label)}</button>`).join('')}
          </div>
        </div>` : ''}
        <div class="buy-row">
      ...
      <div class="review-form-wrap">
        ...
        <form id="review-form" enctype="multipart/form-data">
          ...
          <label class="consent-check">
            <input type="checkbox" name="privacyAccepted" value="1" required>
            <span>Даю <a href="/personal-data-consent" target="_blank" rel="noopener">согласие на обработку персональных данных</a> и ознакомлен(а) с <a href="/privacy" target="_blank" rel="noopener">Политикой конфиденциальности</a>.</span>
          </label>
          <label class="consent-check">
            <input type="checkbox" name="publicationAccepted" value="1" required>
            <span>Даю отдельное <a href="/personal-data-publication-consent" target="_blank" rel="noopener">согласие на публикацию отзыва</a>.</span>
          </label>
  ...
  const body = `<div class="container legal-page">
    ...
    <div class="legal-card">
      <section class="legal-section" id="general">
        ...
        <p>Оператор обрабатывает только данные, необходимые для ответа на обращение, оформления заказа, публикации и модерации отзывов, работы корзины, защиты сайта и исполнения требований закона.</p>
      </section>
      ...
      <section class="legal-section" id="data">
        ...
        <ul>
          <li><b>Заказы и обращения:</b> имя (если указано), телефон, Telegram или e-mail, комментарий, состав и параметры заказа.</li>
          <li><b>Отзывы:</b> имя или псевдоним, оценки, текст, приложенные фотографии и сведения о товаре.</li>
          <li><b>Технические данные:</b> IP-адрес, дата и время запроса, адрес страницы, сведения о браузере и устройстве — в объёме, необходимом для работы сервера, безопасности и ограничения спама.</li>
          <li><b>Данные браузера:</b> содержимое корзины и отметка о прочтении уведомления хранятся локально на устройстве посетителя.</li>
        </ul>
      ...
      <section class="legal-section" id="purposes">
        ...
        <div class="legal-table-wrap"><table class="legal-table purposes-table"><thead><tr><th>Цель</th><th>Данные</th><th>Основание</th></tr></thead><tbody>
          <tr><td>Обработка заявки и связь с покупателем</td><td>Имя, контакт, комментарий, заказ</td><td>Согласие посетителя; действия по его инициативе до заключения договора и исполнение договора</td></tr>
      ...
      <section class="legal-section" id="processing">
        ...
        <p>Если для сайта настроены уведомления в Telegram, сведения из заказа или отзыва могут быть направлены менеджеру через этот сервис. Переход посетителя по внешней ссылке Telegram регулируется также документами самого Telegram.</p>
        <p>Оператор не продаёт персональные данные и не использует на сайте рекламные или аналитические cookie. Передача государственным органам допускается только в случаях, предусмотренных законом.</p>
      </section>

      <section class="legal-section" id="storage">
        <h2>6. Сроки хранения и защита</h2>
        <p>Данные заказа хранятся до завершения работы с заявкой и далее в течение срока, необходимого для исполнения договора, разрешения споров и выполнения требований закона. Данные отзыва хранятся до удаления отзыва, отзыва согласия либо прекращения работы сайта. Технические сведения хранятся не дольше, чем требуется для безопасности и диагностики.</p>
        <p>Оператор применяет разграничение доступа, защищённые сессии, ограничение частоты запросов, резервное копирование и иные разумные организационные и технические меры. При сборе данных граждан Российской Федерации Оператор обеспечивает выполнение требований о локализации баз данных.</p>
      </section>
      ...
      <section class="legal-section" id="cookies">
        ...
        <p>На сайте используются только технологии, необходимые для работы функций и защищённого доступа. Уведомление с кнопкой «ОК» подтверждает ознакомление; необязательные рекламные или аналитические технологии не запускаются.</p>
        <div class="legal-table-wrap"><table class="legal-table cookies-table"><thead><tr><th>Название</th><th>Тип</th><th>Назначение</th><th>Срок</th></tr></thead><tbody>
          ...
          <tr><td><code>cookie_notice_v1</code></td><td>localStorage</td><td>Запоминает нажатие кнопки «ОК», чтобы не показывать уведомление повторно.</td><td>До очистки данных сайта в браузере</td></tr>
        </tbody></table></div>
        <p>Удалить эти данные можно в настройках браузера. После удаления корзина очистится, а уведомление появится снова.</p>
      </section>

      <section class="legal-section" id="changes">
        <h2>9. Изменение Политики</h2>
        <p>Оператор может обновлять Политику при изменении функций сайта или законодательства. Новая редакция действует с момента публикации по этому адресу; дата редакции указана в начале страницы.</p>
...
function personalDataConsentPage(settings, opts) {
  ...
  const body = `<div class="container legal-page">
    ...
    <header class="legal-hero"><span class="section-kicker">Отдельный документ</span><h1>Согласие на обработку персональных данных</h1><p>Редакция от 26 июля 2026 года</p></header>
    <div class="legal-card">
      <section class="legal-section"><h2>Оператор</h2>${operatorCard(settings)}</section>
      <section class="legal-section">
        <h2>Условия согласия</h2>
        <p>Устанавливая отметку в форме заказа или отзыва и нажимая кнопку отправки, я свободно, своей волей и в своём интересе даю Оператору конкретное, информированное и однозначное согласие на обработку переданных мной персональных данных.</p>
        <h3>Для заказа или обращения</h3>
        <p>Состав данных: имя (если указано), контакт для связи, комментарий, состав и параметры заказа. Цели: обработать заявку, связаться со мной, согласовать и исполнить заказ, рассмотреть претензии.</p>
        <h3>Для отзыва</h3>
...
function publicationConsentPage(settings, opts) {
  ...
  const body = `<div class="container legal-page">
    ...
    <header class="legal-hero"><span class="section-kicker">Отдельный документ</span><h1>Согласие на обработку персональных данных, разрешённых для распространения</h1><p>Редакция от 26 июля 2026 года</p></header>
    <div class="legal-card">
      <section class="legal-section"><h2>Оператор</h2>${operatorCard(settings)}</section>
      <section class="legal-section">
        <h2>Что разрешается публиковать</h2>
        <p>Устанавливая отдельную отметку «Согласие на публикацию отзыва» и отправляя форму, я разрешаю Оператору после модерации разместить на сайте указанные мной имя или псевдоним, оценку, текст отзыва и приложенные фотографии в связи с выбранным товаром.</p>
...
module.exports = {
  esc, money, formatDate, stars, layout, homePage, productPage, privacyPage,
  personalDataConsentPage, publicationConsentPage, imageMarkup, placeholderSvg,
  assetV, brandFields, cssColor, PRIVACY_VERSION
};
