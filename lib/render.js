...
// и — у часов — хотя бы один доступный ремешок.
function sellable(p) {
  if (!p || !p.inStock) return false;
  const colors = p.colors || [], storages = p.storages || [], bands = p.bands || [];
  const bandsOk = !bands.length || bands.some(g => (g.options || []).some(variantInStock));
  return (!colors.length || colors.some(variantInStock)) && (!storages.length || storages.some(variantInStock)) && bandsOk;
}

// Первая коллекция ремешков, где есть хоть один доступный вариант.
function firstBandGroup(bands) {
  const i = (bands || []).findIndex(g => (g.options || []).some(variantInStock));
  return i === -1 ? 0 : i;
}
  ...
  <article class="card${hot ? ' card-hot' : ''}">
    ...
    <div class="card-add">
      <button type="button" class="btn btn-primary btn-block add-to-cart"
        data-id="${p.id}" data-name="${esc(p.name)}" data-price="${eff}" data-img="${esc((p.images || [])[0] || '')}"
        ${inStock ? '' : 'disabled'}>${inStock ? 'В корзину' : 'Нет в наличии'}</button>
    </div>
  </article>`;
}
...
function productPage(settings, db, p, site, opts) {
  ...
  const slidesData = (p.images && p.images.length)
    ? esc(JSON.stringify(p.images.map(s => ({ src: '/uploads/' + s, color: imgColorOf(s), band: (p.imageBands || {})[s] || '' }))))
    : '';
    ...
    <div class="product">
      ...
      <div class="product-info">
        ...
        ${(p.colors && p.colors.length) ? (() => {
          const sel = firstAvailable(p.colors);
          return `
        <div class="variant-group">
          <div class="variant-label">Цвет: <b id="sel-color">${esc(p.colors[sel].name)}</b></div>
          <div class="swatches" id="colors">
            ${p.colors.map((c, i) => { const ok = variantInStock(c); return `<button type="button" class="swatch${i === sel ? ' active' : ''}${ok ? '' : ' out'}" data-color="${esc(c.name)}" aria-label="Цвет: ${esc(c.name)}${ok ? '' : ' — нет в наличии'}" aria-pressed="${i === sel ? 'true' : 'false'}" title="${esc(c.name)}${ok ? '' : ' — нет в наличии'}" style="--sw:${cssColor(c.hex, '#cccccc')}"${ok ? '' : ' disabled'}></button>`; }).join('')}
          </div>
        </div>`; })() : ''}
        ...
        ${(p.bands && p.bands.length) ? (() => {
          const gi = firstBandGroup(p.bands);
          const group = p.bands[gi];
          const oi = firstAvailable(group.options);
          const tabs = p.bands.length > 1 ? `<div class="band-tabs" id="band-tabs" role="tablist">
            ${p.bands.map((g, i) => `<button type="button" class="band-tab${i === gi ? ' active' : ''}" data-group="${i}" role="tab" aria-selected="${i === gi ? 'true' : 'false'}">${esc(g.name)}</button>`).join('')}
          </div>` : '';
          ...
          const colorRows = p.bands.map((g, i) => `<div class="band-colors" data-group="${i}"${i === gi ? '' : ' hidden'}>
            ${(g.options || []).map((o, j) => { const ok = variantInStock(o); const selected = i === gi && j === oi; return `<button type="button" class="swatch${selected ? ' active' : ''}${ok ? '' : ' out'}" data-band="${esc(g.name)}" data-option="${esc(o.name)}" data-add="${Number(o.add) || 0}" title="${esc(o.name)}${ok ? '' : ' — нет в наличии'}" aria-label="Ремешок ${esc(g.name)}: ${esc(o.name)}" aria-pressed="${selected ? 'true' : 'false'}" style="--sw:${cssColor(o.hex, '#cccccc')}"${ok ? '' : ' disabled'}></button>`; }).join('')}
          </div>`).join('');
...
function checkoutPage(settings, opts) {
  ...
  return layout(settings, {
    ...
    origin: opts.origin || '', canonicalPath: '/checkout',
...
module.exports = {
  esc, money, formatDate, stars, layout, homePage, productPage, checkoutPage, privacyPage,
