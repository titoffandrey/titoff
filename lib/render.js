...
/* Ширина снимка в плитке категории на главной (`.cat-grid`): три колонки на
 * телефоне с полями плитки 10 px, четыре — в контейнере 1200 px с полями 18.
 * Без `sizes` у плитки была одна мелкая копия (320) без srcset: на телефоне
 * это верно, а на экране с DPR 2 плитка в 240 px растягивала 320 до мыла.
 * Числа обязаны идти рядом с `.cat-grid` в styles.css. */
const TILE_SIZES_ATTR = '(min-width:1248px) 240px, (min-width:801px) 22vw, 30vw';
...
function imageMarkup(product, index, opts) {
  ...
  if (imgs.length) {
    ...
    // `fetchpriority="high"` — только у того, что и правда LCP: у первой плитки
    // категорий на главной их девять в первом экране, и девять «высоких»
    // приоритетов разом означали бы, что высокого нет ни у одной.
...
function categoryTiles(db, settings, opts) {
  ...
  const tiles = cats.map((c, i) => {
     ...
     * LCP там — снимок ПЕРВОЙ плитки: PageSpeed 19 сентября 2026 показал его с
     * `loading="lazy"` и без приоритета, то есть браузер откладывал ровно ту
     * картинку, по которой считается скорость. Первый ряд грузится сразу,
     * приоритет — у первой; ряд каталога (`row`) остаётся мелкими копиями. */
    const photo = withPhoto
      ? imageMarkup(withPhoto, 0, opts.row
        ? { small: true, card: true, dir: db && db.UPLOAD_DIR }
        : { tile: true, dir: db && db.UPLOAD_DIR, eager: i < 3, priority: i === 0 })
...
function homePage(settings, db, opts) {
  ...
  return layout(settings, {
    ...
    payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, customer: opts.customer,
    tab: 'home',
    title: '',
