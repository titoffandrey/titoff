...
function reviewCard(rv) {
  ...
  return `
    <article class="review" data-rating="${rating}" data-ts="${ts}" data-len="${String(rv.text || '').length}"${videos.length ? ' data-video="1"' : ''}>
      ...
      ${videos.length ? `<div class="review-videos">${videos.map(v =>
        `<video class="review-video" src="/uploads/${esc(v)}" controls preload="none"${rv.poster ? ` poster="/uploads/${esc(rv.poster)}"` : ''}></video>`
      ).join('')}</div>` : ''}
      ${photos.length ? `<div class="review-photos">${photos.map(ph =>
