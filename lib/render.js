...
function brandFields(s) {
  ...
  return `${brandPreview(s)}
    ...
    <div class="field">
      <label for="brand-text">Надпись логотипа — буквы в {фигурных скобках} красятся акцентом</label>
    ...
    ${cur}
    <div class="field">
      <label for="brand-logo">Логотип-картинка вместо надписи</label>
    ...
    <div class="field field-check"><label><input type="checkbox" name="cardNameStorage"${s.cardNameStorage ? ' checked' : ''}> В названии карточки каталога — объём памяти: «iPhone 15 Pro Max 256 ГБ»</label></div>
    ${/* Весь каталог на главной — см. `homeCatalogOn()` и `homePage()`. */''}
    <div class="field field-check"><label><input type="checkbox" name="homeCatalog"${s.homeCatalog ? ' checked' : ''}> На главной — сразу весь каталог, без плиток категорий и «Популярного»</label></div>`;
}
