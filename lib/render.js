...
function storeCard(settings, map, opts) {
  ...
  const gallery = shots.length
    ? `<div class="place-shots" data-media="store">${shots.map((name, i) => `
        <a class="place-shot${i ? '' : ' is-lead'}" href="/uploads/${esc(name)}" data-kind="photo" target="_blank" rel="noopener">
          <img src="/uploads/${esc(name)}" alt="${esc(settings.storeName)} — фото магазина" loading="lazy" decoding="async">
        </a>`).join('')}</div>`
    : '';
  // Звёзды рисует общая `stars()` — та же, что в карточке отзыва и в панели:
  // второй ряд звёзд со своим округлением разъехался бы с ней молча.
  const rate = rating
    ? `<div class="place-rate">${stars(rating.avg)}
        <b>${esc(String(rating.avg).replace('.', ','))}</b>
        <span class="muted">${esc(reviewsCountLong(rating.count))} · по отзывам покупателей</span></div>`
    : '';
  const open = state
    ? `<div class="place-open${state.open ? ' is-open' : ''}">${state.open
  ...
  return `<div class="place-card">
    ...
    <div class="place-body">
      ...
      <span class="place-kind muted">Магазин техники Apple</span>
      ${rate}
      ${open}
      <div class="place-line">${pinIcon()}<span>${esc(map.address)}</span></div>
      ${settings.contactHours ? `<div class="place-line">${clockIcon()}<span>${esc(settings.contactHours)}</span></div>` : ''}
      ${phone ? `<a class="place-line place-call" href="tel:${esc(phoneHref(phone))}">${phoneIcon()}<span>${esc(phoneText(phone) || phone)}</span></a>` : ''}
    </div>
  </div>`;
}
