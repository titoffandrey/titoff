...
// Отзывы с вложениями — вперёд, остальные следом; внутри каждой группы порядок
// не меняется, то есть остаётся по дате. Снимок или ролик убедительнее любого
// текста, и прятать их на пятой странице незачем.
//
// Раньше здесь стояла раскладка «видео на каждое третье место»: она разносила
// ролики по передним страницам, но ломала главное — при сортировке «Новые»
// самый свежий отзыв мог оказаться где угодно, потому что раскладка тасовала
// ленту целиком. Сортировка, которая не показывает свежее сверху, называется
// «Новые» зря.
//
// Видео от фото здесь НЕ отделяется: иначе свежий отзыв с фотографиями встал бы
// позади годовалых роликов — и «Новые» опять перестали бы быть новыми.
function mediaFirst(list) {
  const withMedia = list.filter(hasMedia);
  if (!withMedia.length || withMedia.length === list.length) return list;
  return withMedia.concat(list.filter(rv => !hasMedia(rv)));
}
