...
function storefrontMark() {
  return `<img class="apple-logo-mark" src="/static/apple-logo.svg?v=${assetV('apple-logo.svg')}" alt="">`;
}
...
function categoryTiles(db, settings, opts) {
  ...
  const tiles = cats.map(c => {
    ...
    const photo = withPhoto
      ? imageMarkup(withPhoto, 0, { small: true, card: true, dir: db && db.UPLOAD_DIR })
