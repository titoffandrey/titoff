...
function accountFlash(opts) {
  if (opts.error) return `<p class="form-msg err acc-flash" role="alert">${esc(opts.error)}</p>`;
  if (opts.flash) return `<p class="form-msg ok acc-flash" role="status">${esc(opts.flash)}</p>`;
  return '';
}
