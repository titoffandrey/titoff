...
function payMethodChoice(methods, current) {
  ...
  return `<div class="co-choice" role="radiogroup" aria-label="Способ оплаты">${methods.map((m, i) => {
    const on = current ? m.id === current : i === 0;
    return `<label class="co-choice-opt"><input type="radio" name="pay-method" value="${esc(m.id)}"${on ? ' checked' : ''}>
      <span class="co-choice-text"><span class="co-choice-mark"><b>${esc(m.name)}</b></span>${m.hint ? `<i>${esc(m.hint)}</i>` : ''}</span></label>`;
  }).join('')}</div>`;
}
