...
function hasMedia(rv) { return hasVideo(rv) || hasPhoto(rv); }

// Отзывы с вложениями поднимаются наверх — но В ПРЕДЕЛАХ СТРАНИЦЫ, а не всей
// ленты. Список уже отсортирован по дате, и окно ровно в страницу означает:
// какие отзывы попадут на страницу N, решает только дата, а в каком порядке они
// там встанут — вложения. Снимок или ролик убедительнее текста, и наверху
// страницы им и место.
//
// Почему не подъём по всей ленте: на боевых данных у 17 Pro Max 614 отзывов с
// вложениями и 611 без. Строгий подъём отправил бы все текстовые на 77-ю
// страницу — вместе с самым свежим отзывом за сегодня. Сортировка, которая не
// показывает свежее сверху, называется «Новые» зря; ровно это и делала прежняя
// раскладка «видео на каждое третье место», тасовавшая ленту целиком.
//
// Видео от фото не отделяется: иначе свежий отзыв с фотографиями встал бы
// позади роликов постарше уже внутри страницы.
function mediaFirst(list) {
  const out = [];
  for (let i = 0; i < list.length; i += REVIEWS_PER_PAGE) {
    const page = list.slice(i, i + REVIEWS_PER_PAGE);
    const withMedia = page.filter(hasMedia);
    if (!withMedia.length || withMedia.length === page.length) { out.push.apply(out, page); continue; }
    out.push.apply(out, withMedia.concat(page.filter(rv => !hasMedia(rv))));
  }
  return out;
}
...
function sortReviews(list, sort) {
  ...
  // Вложения поднимаются только в порядке по умолчанию: выбрав «с высокой
  // оценкой», покупатель просил именно оценки, и переставлять их нельзя.
  return mediaFirst(arr.sort(fresh));
}
      ...
      <div class="review-form-wrap">
        ...
        <form id="review-form" enctype="multipart/form-data">
          ...
          ${REVIEW_PHOTOS ? `<div class="field">
            ...
            <span class="field-note">До ${REVIEW_PHOTOS_MAX} снимков. Отзыв с фотографиями показывается в ленте первым.</span>
          </div>` : ''}
...
module.exports = {
  ...
  reviewsSlice, reviewCard, reviewsPager, reviewsRangeText, reviewCountText, REVIEWS_PER_PAGE, REVIEW_PHOTOS_MAX,
  sortReviews, mediaFirst,
