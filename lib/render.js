...
// Общая на обе панели, чтобы подпись не разъехалась между ними.
function orderDelivery(order) {
  const name = DELIVERY.nameOf(order && order.delivery);
  return (name ? `<div class="muted small">🚚 ${esc(name)}</div>` : '')
    + (order && order.address ? `<div class="muted small">📍 ${esc(order.address)}</div>` : '')
    + (order && order.comment ? `<div class="muted small">«${esc(order.comment)}»</div>` : '');
}
...
function payPage(settings, order, opts) {
  ...
  if (state === 'paid') {
    card = `<section class="order-success" role="status" aria-live="polite">
      ...
      <div class="order-success-number"><span>Номер заказа</span><strong>${esc(order.number)}</strong></div>
  ...
  const body = `<div class="container checkout-page pay-page" id="pay-page"
    ...
    <header class="checkout-head">
      ...
      <p class="checkout-sub">Заказ ${esc(order.number)} на ${esc(total)}</p>
    </header>
...
module.exports = {
  esc, money, formatDate, paymentBadge, orderDelivery, stars, layout, homePage, notFoundPage, productPage, checkoutPage, payPage, privacyPage,
