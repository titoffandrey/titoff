  ...
  if (ownPaid && state !== 'paid' && !orderArchived) {
    ...
    card = `<div class="co-block pay-invoice">
      ...
      <p class="pay-hint">${hosted
        ? 'Оплата картой пройдёт на защищённой странице банка — она откроется в новой вкладке, сумма там уже указана. Эту страницу не закрывайте: как только банк подтвердит оплату, она обновится сама.'
...
function receiptPage(settings, order, opts) {
  ...
  const method = paidByProvider && pay.method ? PAY.describe(pay.method) : null;
