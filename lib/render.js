...
function layout(settings, opts) {
  ...
  const navItems = navItem('/', 'Все', !opts.activeCategory)
    + cats.map(c => navItem('/?category=' + encodeURIComponent(c), esc(c), opts.activeCategory === c)).join('');
  ...
  const searchForm = (cls, id) => `<form class="${cls}" action="/" method="get" role="search">
  ...
  const footCats = cats.map(c =>
    `<li><a href="/?category=${encodeURIComponent(c)}">${esc(c)}</a></li>`
  ).join('');
...
function homePage(settings, db, opts) {
   ...
   *
   * И он же — ЕДИНСТВЕННЫЙ заголовок такой страницы, поэтому пишется `<h1>`, а
   * не `<h2>`. Первый экран со слоганом там не показывается вовсе (`intro`
   * ниже), а вместе с ним пропадал и `<h1>` — то есть страница категории
   * оставалась без главного заголовка совсем, начинаясь сразу со второго
   * уровня. Для читающего с экрана это сломанный план документа (по заголовкам
   * он и перемещается), а страницы категорий вдобавок ИНДЕКСИРУЮТСЯ и стоят в
   * карте сайта: `<h1>` — самый сильный признак того, о чём страница, и отдавать
   * выдаче страницу без него значит терять её позиции на ровном месте.
   *
   * На каждой странице ровно один `<h1>`: на главной это слоган в первом экране,
   * здесь — название категории или строка поиска, у товара — его имя. Заголовок
   * отзывов ниже остаётся `<h2>` — он подраздел страницы товара, а не её тема. */
  ...
  const body = `
    ...
    <section class="container section${heading ? ' section-top' : ''}">
      ${heading ? `<div class="section-head section-head-center"><h1>${heading}</h1></div>` : ''}
  ...
  return layout(settings, {
    ...
    payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym,
...
function checkoutPage(settings, opts) {
  ...
  return layout(settings, {
    body, categories: opts.categories || [], payRemind: opts.payRemind, chatWaiting: opts.chatWaiting, ym: opts.ym, title: 'Оформление заказа',
    paymentFee: !!fee,
...
function payPage(settings, order, opts) {
  ...
  if (state === 'refunded') {
    card = `<section class="order-success order-success-neutral" role="status" aria-live="polite">
      ...
      <a class="btn btn-lg" href="/">Вернуться в каталог</a>
    </section>`;
    ...
    card = `<section class="order-success" role="status" aria-live="polite">
      ...
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      ${receiptLink(order)}${trackLink(order)}<a class="btn btn-lg" href="/">Продолжить покупки</a>
    </section>`;
    ...
    card = `<section class="order-success" role="status" aria-live="polite">
      ...
      <div class="order-success-number"><span>Заказ</span><strong>${esc(orderNo(order.number))}</strong></div>
      ${receiptLink(order)}${trackLink(order)}<a class="btn btn-primary btn-lg" href="/">Продолжить покупки</a>
    </section>`;
...
function accountAuthPage(settings, opts) {
  ...
  return layout(settings, Object.assign({}, opts, {
    body: html, title, canonicalPath: '/account/login', noindex: true, ym: null,
...
function accountPage(settings, opts) {
  ...
  const ordersHtml = orders.length
    ...
    : `<p class="acc-empty">Заказов пока нет. <a href="/">Перейти в каталог</a></p>`;
  ...
  const html = `<div class="container acc-page">
    ...
    <div class="acc-grid">
      ...
      <section class="acc-card" id="cart" aria-labelledby="acc-cart-title">
        ...
        <div id="account-cart" class="acc-cart"><p class="acc-empty">Корзина пуста. <a href="/">Перейти в каталог</a></p></div>
      </section>
  ...
  return layout(settings, Object.assign({}, opts, {
    body: html, title: 'Личный кабинет', canonicalPath: '/account', noindex: true, ym: null,
    description: `Личный кабинет покупателя ${settings.storeName}`
  }));
}
...
module.exports = {
  ...
  editSwitch, adminIcon, adminIconPath, orderNo, orderDelivery, orderClient, orderItems, orderPromo, phoneText, phoneFlag, clientMarks, visitorHref, stars, startPrice, sellable, priceHidden, layout, homePage, notFoundPage, blockedPage, productPage, checkoutPage, payPage, receiptPage, privacyPage,
