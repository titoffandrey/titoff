...
function orderDelivery(order) {
  const name = DELIVERY.nameOf(order && order.delivery);
  const line = [name, order && order.address].filter(Boolean).map(esc).join(' · ');
  return (line ? `<div class="o-line">${line}</div>` : '')
    + (order && order.comment ? `<div class="o-line">«${esc(order.comment)}»</div>` : '');
}

// Технические поля посетителя: город, устройство, IP, источник перехода. Нужны
// редко — при разборе спорного заказа, — а места занимали четыре строки в
// каждой строке таблицы. Поэтому свёрнуты: браузер сам покажет их по клику,
// без единой строчки скрипта.
function orderTech(order) {
  const o = order || {};
  ...
  const rows = [place, device, o.clientIp, source].filter(Boolean);
  if (!rows.length) return '';
  return `<details class="o-tech"><summary>Откуда зашёл</summary>${rows.map(r => `<div>${esc(r)}</div>`).join('')}</details>`;
}
...
function checkoutPage(settings, opts) {
  ...
  // рисует название текстом, как раньше.
...
// названию. Остальным рисуем волосяные глифы на холсте 35×35 — тот же вес
// штриха, что у характеристик товара и блока доверия, чтобы карточки не
// выглядели собранными из разных наборов.
const PAY_GLYPHS = {
  // Карта с полосой и чипом
  card: '<path d="M4.4 8.6h26.2a1.9 1.9 0 0 1 1.9 1.9v14a1.9 1.9 0 0 1-1.9 1.9H4.4a1.9 1.9 0 0 1-1.9-1.9v-14A1.9 1.9 0 0 1 4.4 8.6z"/><path d="M2.5 14.2h30"/><path d="M6.6 21.2h6" stroke-linecap="round"/>',
  // QR: три «глаза» и точка
  qr: '<rect x="4.5" y="4.5" width="9.4" height="9.4" rx="1.4"/><rect x="21.1" y="4.5" width="9.4" height="9.4" rx="1.4"/><rect x="4.5" y="21.1" width="9.4" height="9.4" rx="1.4"/><path d="M21.1 21.1h4v4h-4zM26.6 26.6h3.9v3.9h-3.9z"/>',
  // Глобус — трансграничные способы
  globe: '<circle cx="17.5" cy="17.5" r="13"/><path d="M4.5 17.5h26"/><path d="M17.5 4.5c3.5 3.7 5.3 8.2 5.3 13s-1.8 9.3-5.3 13c-3.5-3.7-5.3-8.2-5.3-13s1.8-9.3 5.3-13z"/>'
};
function payMark(mark) {
  ...
  return `<svg class="pay-opt-glyph" viewBox="0 0 35 35" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true">${glyph}</svg>`;
}
...
function payPage(settings, order, opts) {
  ...
  if (state === 'paid') {
    ...
    card = `<div class="co-block pay-invoice">
      ...
      <p class="pay-hint">Переведите точную сумму по этим реквизитам из приложения своего банка. Как только касса увидит перевод, страница обновится сама — закрывать её не нужно.</p>
      ...
      <div class="pay-actions">
        <button type="button" class="btn" id="pay-recheck">Я оплатил — проверить</button>
...
function warrantyPage(settings, opts) {
  ...
  return layout(settings, {
    body, categories: opts.categories || [], title: 'Гарантия',
...
function returnsPage(settings, opts) {
  ...
  return layout(settings, {
    body, categories: opts.categories || [], title: 'Возврат и обмен',
...
module.exports = {
  esc, money, formatDate, paymentBadge, orderNo, orderDelivery, orderClient, orderItems, stars, layout, homePage, notFoundPage, productPage, checkoutPage, payPage, privacyPage,
