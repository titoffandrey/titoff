...
function aboutPage(settings, opts) {
  ...
  const body = `<div class="container legal-page about-page">
    ...
    <div class="legal-card">
      ...
      ${/* Карта — и БОЛЬШЕ НИЧЕГО. Адрес стоит на самой карте булавкой и в
           контактах подвала, а абзац «магазин работает и офлайн» рассказывал то,
           что видно по карте. Грузится она только по нажатию (см. комментарий
           к MAP_HOST): до него наружу не уходит ни одного запроса, а адрес
           виджета уезжает атрибутом — второй сборщик того же адреса разошёлся
           бы с этим на первой правке. Ссылка внутри — путь к карте там, где
           скрипты не загрузились; со скриптом её место занимает кнопка. */''}
