...
function accountAuthPage(settings, opts) {
  ...
  const hint = `<p class="field-hint">Не короче ${CUSTOMER_PASSWORD_MIN} знаков — можно и просто цифры.</p>`;
  ...
  const html = `<div class="container acc-auth">
    <nav class="breadcrumb"><a href="/">Главная</a> / <span>Личный кабинет</span></nav>
...
/* Сам кабинет: заказы, отправления, корзина, профиль и пароль. Разделов
 * столько, сколько вопросов у покупателя, и первыми стоят те, ради которых он
 * зашёл, — «где мой заказ» и «где посылка». */
function accountPage(settings, opts) {
  ...
  const passwordForm = `<form method="post" action="/account/password" class="acc-form">
      ...
      <div class="field"><label for="acc-new">Новый пароль</label><input type="password" id="acc-new" name="password" minlength="${CUSTOMER_PASSWORD_MIN}" maxlength="200" autocomplete="new-password" required>
        <p class="field-hint">Не короче ${CUSTOMER_PASSWORD_MIN} знаков — можно и просто цифры.</p></div>
      <button class="btn btn-primary" type="submit">Сменить пароль</button>
    </form>`;
  const html = `<div class="container acc-page">
    <nav class="breadcrumb"><a href="/">Главная</a> / <span>Личный кабинет</span></nav>
    <header class="acc-head">
      <div><h1>Личный кабинет</h1><p class="acc-sub">${esc(c.email)}</p></div>
      <form method="post" action="/account/logout"><button class="btn" type="submit">Выйти</button></form>
    </header>
    ${accountFlash(opts)}
    <div class="acc-grid">
      ...
      ${/* Корзина живёт в браузере и на сервере не видна: список рисует
           public/app.js по тому же хранилищу, что и шторка корзины. */''}
      <section class="acc-card" id="cart" aria-labelledby="acc-cart-title">
        <h2 id="acc-cart-title">Корзина</h2>
        <div id="account-cart" class="acc-cart"><p class="acc-empty">Корзина пуста. <a href="/catalog">Перейти в каталог</a></p></div>
      </section>
