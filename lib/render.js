...
function trackLink(order) {
  ...
  if (!ship || !Array.isArray(ship.steps) || !ship.steps.length) return '';
...
function trackingBoard(order, opts) {
   ...
   * затянулась (`holdHours` у отправки): ночная пересортировка задержкой не
...
function trackingPage(settings, opts) {
  ...
  const found = opts.order && opts.order.shipment ? trackingBoard(opts.order, { own: opts.own, now: opts.now }) : '';
