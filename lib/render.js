...
function brandFields(s) {
  ...
  return `<div class="a-form-grid">
      ...
      ${/* «Как акцентный» — отдельная галочка, а не пустое поле.
           `<input type="color">` пустым не бывает вовсе, поэтому до неё
           настройка «пусто — берём акцентный» жила ровно до первого сохранения
           формы: круговая отправка штамповала в неё текущий акцент, и дальше
           вторичный цвет за акцентом уже не следовал никогда. */''}
      <div class="field"><label>Вторичный цвет (для выделенных букв)</label>
        <input name="secondaryColor" type="color" value="${cssColor(s.secondaryColor || s.accentColor)}">
        <label class="switch-row"><input type="checkbox" name="secondaryAuto"${s.secondaryColor ? '' : ' checked'}><span>Как акцентный</span></label>
      </div>
    </div>
...
<style>:root{--accent:${accent};--accent-deep:${deepAccent(accent)};--secondary:${cssColor(settings.secondaryColor, accent)};--brand-font:${brandFont(settings)}}</style>
</head>
...
function checkoutPage(settings, opts) {
  ...
  const body = `<div class="container checkout-page" id="checkout-page"${pay}${delivery}${promo}>
    ${DLOGO.sprite()}
