...
function payPage(settings, order, opts) {
  ...
  if (ownPaid && state !== 'paid') {
    ...
    const reqCard = (mark, title, value, copy) => `<div class="pay-own-card">
        ...
        <p class="pay-own-value">${esc(value)}</p>
        <button type="button" class="pay-copy" data-copy="${esc(copy)}" aria-label="Скопировать: ${esc(title)}">Копировать</button>
      </div>`;
    card = `<div class="co-block pay-invoice pay-own">
      ...
      <p class="pay-own-note">Переведите по СБП на номер телефона или на карту — как вам удобнее.</p>
      <div class="pay-own-grid">
        ${phoneText ? reqCard(`<span class="pay-own-mark pay-own-sbp">${PAY_MARKS.sbp}${PAY_MARKS.sbpWord}</span>`,
    'По номеру телефона', phoneText, own.phone.replace(/\D+/g, '')) : ''}
        ${cardText ? reqCard(brand
      ? `<span class="pay-own-mark pay-own-${esc(brand)}">${PAY_MARKS[brand]}</span>`
      : '<span class="pay-own-mark pay-own-plain">Карта</span>', 'На карту', cardText, own.card) : ''}
      </div>
      <div class="pay-own-who">
        <div><span>Получатель</span><b>${esc(own.owner)}</b></div>
        ${own.bank ? `<div><span>Банк получателя</span><b class="pay-own-bank">${bankMark(own.bank)}</b></div>` : ''}
      </div>
      <p class="pay-state" role="status">После перевода менеджер подтвердит оплату и свяжется с вами — страницу можно закрыть.</p>
    </div>`;
  } else if (state === 'paid') {
    card = `<section class="order-success" role="status" aria-live="polite">
      <div class="order-success-check" aria-hidden="true">✓</div>
