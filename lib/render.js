...
function payView(order, now) {
  ...
  /* Менеджер отменил оплату руками: покупатель передумал, ошибся суммой или
   * оформил заказ дважды. Касса тут ни при чём — её счёт, если он был, живёт
   * своей жизнью и продолжает сверяться. */
  if (order && order.manualVoid) {
    return {
      tone: 'off', label: 'оплата отменена',
      when: payMoment(order.manualVoid.at), whenLabel: 'отменил менеджер'
    };
  }
...
function brandFields(s) {
  ...
  return `<div class="a-form-grid">
      <div class="field"><label>Шрифт названия и заголовков</label><select name="logoFont">${LOGO_FONTS.map(([v, l]) => `<option value="${v}" ${s.logoFont === v ? 'selected' : ''}>${l}</option>`).join('')}</select></div>
    ...
    <div class="chat-foot">
      ...
      <form class="chat-form" id="chat-form">
        ...
        <button class="chat-send" type="submit" aria-label="Отправить">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4.5 12h13M12 6.5l5.5 5.5L12 17.5"/></svg>
        </button>
      </form>
    </div>
  </section>
  ...
  <div class="container footer-grid">
    ...
    <div class="footer-contacts">
      ${settings.contactTelegram ? `<div class="foot-tg">
        <a class="tg-cta" href="https://t.me/${esc(tgUser(settings))}" target="_blank" rel="noopener">${tgIcon()}<span>Написать в Telegram</span></a>
      </div>` : ''}
...
function trackLink(order) {
  ...
  if (!number) return '';
  return `<a class="btn btn-lg" href="/track/${encodeURIComponent(number)}">Отследить заказ</a>`;
}
...
/* Страница отслеживания: форма с номером заказа и результат под ней.
 *
 * Форма остаётся на месте и когда посылка найдена: с одного устройства
 * отслеживают несколько заказов, и заставлять возвращаться на пустую страницу
 * ради второго номера незачем.
 */
function trackingPage(settings, opts) {
  opts = opts || {};
  const number = String(opts.number || '');
  /* Скрытое отправление покупателю не показывается ВООБЩЕ, и ответ у него тот
   * же, что у чужого номера: маршрут собирают заранее, до того как посылку
   * правда отдали перевозчику, и «Принят на склад» у коробки, которая лежит на
   * столе, — обещание, которого никто не давал. */
  const found = TRACK.shownToBuyer(opts.order && opts.order.shipment)
    ? trackingBoard(opts.order, { own: opts.own, now: opts.now }) : '';
  /* Ответ на «такого заказа нет» и на «отправление ещё не сформировано» ОДИН и
   * тот же. Разные ответы превратили бы страницу в проверялку существования
   * заказов: перебором шестизначных номеров можно было бы узнать, сколько их у
   * магазина и в какие дни они идут.
   */
  const miss = !found && number ? `<div class="trk-miss">
      <b>Отправление не найдено</b>
      <p>Проверьте номер заказа — он есть в письме и на странице оплаты. Если заказ оформлен только что, отправление ещё не сформировано: мы соберём посылку и статус появится здесь.</p>
    </div>` : '';
  const body = `<div class="container trk-page">
    ...
    <header class="trk-page-head">
      ...
      <p class="trk-page-sub">Введите номер заказа — покажем, где сейчас посылка и когда её ждать.</p>
    </header>
    <form class="trk-lookup" method="get" action="/track">
      <label for="trk-number">Номер заказа</label>
      <input id="trk-number" name="order" inputmode="numeric" autocomplete="off" spellcheck="false"
        value="${esc(number)}" placeholder="например, 482913" maxlength="12">
      <button class="btn btn-primary" type="submit">Отследить</button>
    </form>
    ${miss}
    ${found}
  </div>`;
...
module.exports = {
  ...
  editSwitch, adminIcon, adminIconPath, orderNo, orderDelivery, orderClient, orderItems, phoneText, phoneFlag, clientMarks, visitorHref, stars, startPrice, sellable, layout, homePage, notFoundPage, productPage, checkoutPage, payPage, privacyPage,
  ...
  assetV, brandFields, logoFontName, cssColor, sellable, colorAvailable, defaultOption, accessibleFields, PRIVACY_VERSION,
  ...
  reviewsPageNumbers, adminSlice, adminPager, ADMIN_PER_PAGE, pluralRu,
