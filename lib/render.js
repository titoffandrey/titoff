...
function checkoutPage(settings, opts) {
  ...
  // Пределы платежа тоже приходят от сервера, а не зашиты в app.js: по ним
  // витрина только подписывает кнопку («Оформить заказ» вместо «Перейти к
  // оплате»), а решает всё равно сервер — свой список разъехался бы с его.
  const pay = opts.payOnline ? ` data-pay="1" data-pay-min="${CROCO.MIN_TOTAL}" data-pay-max="${CROCO.MAX_TOTAL}"` : '';
