...
function orderMessage(order, settings) {
   ...
   * ровно оно же. */
  const name = String(o.firstName || String(o.customerName || '').trim().split(/\s+/)[0] || '').trim();
  ...
  if (o.pickupAddress) {
    ...
    // одном ряду с ними код читался бы его частью. Разделитель тот же, что в
    // уведомлении менеджеру.
 ...
 */
function orderWaHref(order, settings) {
  ...
  return `https://wa.me/${stored.slice(1)}?text=${encodeURIComponent(orderMessage(order, settings))}`;
}
...
function orderClient(order, opts) {
  ...
  const waHref = orderWaHref(o, (opts && opts.money) || null);
  const wa = waHref
...
function orderPeriod(raw) {
  ...
  return ORDER_PERIODS.some(([d]) => d === days) ? days : 0;
}
function orderPeriodLabel(raw) {
  const days = orderPeriod(raw);
  return (ORDER_PERIODS.find(([d]) => d === days) || ORDER_PERIODS[0])[1];
}
// Граница периода настоящей меткой времени. Ноль — «за всё время», то есть
// границы нет вовсе.
function orderPeriodSince(raw, now) {
  const days = orderPeriod(raw);
  if (!days) return 0;
  return mskDayStart(now) - (days - 1) * 86400000;
}
function ordersInPeriod(list, raw, now) {
  const all = Array.isArray(list) ? list : [];
  const since = orderPeriodSince(raw, now);
  return since ? all.filter(o => Number(o && o.createdAt) >= since) : all;
}
...
module.exports = {
  esc, money, formatDate, mskDate, mskNow, mskTime, mskDateTime, mskSameDay, mskDayStart, mskInputValue, parseMskInput, MSK_ZONE, orderStatus, orderPayMethod, orderTone, orderRowClass, orderStats, orderStatsBar, ORDER_TONES,
  ...
  moneyText, orderMessage, orderWaHref,
