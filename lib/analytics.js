...
function deviceFromUa(raw) {
  ...
  if (/bot|crawler|spider|slurp/i.test(ua)) device = 'Робот';
  ...
  return { device, model, os, browser };
}
...
function emptyData() { return { version: 1, visitors: [], daily: {}, geoCache: {}, geoUsage: { date: '', count: 0 } }; }
...
function bumpLimited(object, key, maxKeys) {
  ...
  if (!(safeKey in object) && Object.keys(object).length >= maxKeys) safeKey = 'Другие';
  object[safeKey] = (Number(object[safeKey]) || 0) + 1;
}
...
class Analytics {
  ...
  ensureVisitor(id, siteId, ctx, now) {
    ...
    if (!v) {
      v = {
        ...
        orderCount: 0, lastOrderAt: null, policyVersion: POLICY_VERSION, startedAt: now, activeSeconds: 0
      };
    ...
    for (const key of ['device', 'model', 'os', 'browser', 'screen', 'viewport', 'language', 'timezone', 'platform', 'connection', 'utmSource', 'utmMedium', 'utmCampaign']) if (ctx[key]) v[key] = ctx[key];
  ...
  heartbeat(input) {
    ...
    if (!v || v.siteId !== input.siteId) return false;
    const now = Date.now();
  ...
  snapshot(options) {
    ...
    for (const d of rows) {
      ...
      for (const [k, n] of Object.entries(d.pages || {})) pages[k] = (pages[k] || 0) + n;
      for (const [k, n] of Object.entries(d.sources || {})) sources[k] = (sources[k] || 0) + n;
      for (const [k, n] of Object.entries(d.devices || {})) devices[k] = (devices[k] || 0) + n;
      for (const [k, n] of Object.entries(d.browsers || {})) browsers[k] = (browsers[k] || 0) + n;
      for (const [k, n] of Object.entries(d.systems || {})) systems[k] = (systems[k] || 0) + n;
      for (const [k, n] of Object.entries(d.campaigns || {})) campaigns[k] = (campaigns[k] || 0) + n;
    }
    const matching = this.data.visitors.filter(v => (!siteId || v.siteId === siteId) && (unique.has(v.id) || now - v.lastSeen <= ONLINE_MS));
