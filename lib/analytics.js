...
/* Субъект РФ для хороплет-карты.
 *
 * Карта намеренно российская, а не «весь мир мелкими пятнами»: магазин работает
 * по России, и владельцу важна разница между областями. Старые карточки иногда
 * не содержат кода страны, поэтому одно русское имя страны тоже принимаем.
 * Москва и Петербург — самостоятельные субъекты; если источник отдал город, но
 * забыл область, карта всё равно может честно поставить посетителя на место.
 */
function russianRegion(v) {
  const code = String(v && v.countryCode || '').toUpperCase();
  const country = countryOf(v).toLowerCase();
  if (code !== 'RU' && !/^росси/.test(country)) return '';
...
class Analytics {
  ...
  snapshot(options) {
    ...
    const cached = this.cachedSnapshot(days, options, now);
    ...
    if (days !== 1 && !(Number(options.visitors) > 0)) this.snapshotCache.set(days, { at: now, value: report });
    return report;
  }
