...
class Analytics {
    ...
    try {
      ...
      if (Number(parsed.version) < 3) {
        parsed.daily = mergeDaily(parsed.daily, emptyDay, ['visits', 'pageViews', 'orders', 'activeSeconds', 'bounces'],
          ['visitors', 'orderVisitors', 'pages', 'sources', 'devices', 'browsers', 'systems', 'campaigns', 'funnel', 'scrollSum', 'scrollViews']);
  ...
  daily(now) {
    ...
    for (const name of ['pages', 'sources', 'devices', 'browsers', 'systems', 'campaigns', 'hours', 'hourVisitors', 'funnel', 'scrollSum', 'scrollViews']) if (!d[name] || typeof d[name] !== 'object') d[name] = {};
  ...
  snapshot(options) {
    ...
    const scrollSum = {}; const scrollViews = {};
    for (const d of rows) {
...
module.exports = {
  ...
  cookieValue, normalizeIp, isPrivateIp, deviceFromUa, clientDetails, safePath, sourceFromReferrer, significantSource, dayKey, hourKey, familyOf, sessionsOf
};
