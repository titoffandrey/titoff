...
class Analytics {
  ...
  pulse() {
    ...
    return {
      online: this.data.visitors.filter(v => v.clientConfirmed && now - Number(v.lastSeen || 0) <= ONLINE_MS).length,
  ...
  /* Посетители по часам одного дня, посчитанные по хронологии карточек.
   *
   * Запасной путь для дня, записанного до появления `hourVisitors`: у карточки
   * последние 50 просмотров, поэтому за вчера он уже неверен — и зовётся только
   * для текущих суток, где хронология ещё цела. Считается тем же правилом, что и
   * сам счётчик: посетитель попадает в час своего ПЕРВОГО просмотра за день.
   */
  ...
  snapshot(options) {
    ...
    const days = [1, 7, 30].includes(Number(options.days)) ? Number(options.days) : 1;
    const wantedDates = [];
    ...
    const locations = {}; const sources = {}; const devices = {}; const browsers = {}; const systems = {};
    for (const v of matching) {
      if (!unique.has(v.id)) continue;
      bumpLimited(locations, [v.city, v.region, v.country].filter(Boolean).filter((x, i, a) => a.indexOf(x) === i).join(', ') || 'Не определено', 200);
     ...
     *
     * У дня, записанного до появления `hourVisitors`, поля нет вовсе — тогда
     * час восстанавливается по хронологии карточек. Нужно это ровно один раз, в
     * день выкатки: без этого утренние часы показали бы ноль, то есть соврали
     * бы про то, что до обеда никто не заходил. */
    const todayDate = wantedDates[wantedDates.length - 1];
    const todayRow = this.data.daily[todayDate];
    const byHour = todayRow && todayRow.hourVisitors && Object.keys(todayRow.hourVisitors).length
      ? todayRow.hourVisitors
      : this.hourVisitorsFrom(todayRow, todayDate);
    const hourly = [];
    ...
    return {
      generatedAt: now, days, online, unique: unique.size, hourly, prev,
