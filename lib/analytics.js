...
function emptyDay() {
  return {
    ...
    // Просмотры по часам московских суток — 24 числа на день. Нужны отчёту
    // «Сегодня»: там график по дням состоял бы из одной точки, а по часам видно,
    // как день шёл. Дня без этого поля (записан до его появления) график по
    // часам просто не строит — считать его задним числом неоткуда.
    hours: {}
  };
}
...
class Analytics {
  ...
  daily(now) {
    ...
    for (const name of ['pages', 'sources', 'devices', 'browsers', 'systems', 'campaigns', 'hours']) if (!d[name] || typeof d[name] !== 'object') d[name] = {};
  ...
  snapshot(options) {
    ...
    const todayRow = this.data.daily[wantedDates[wantedDates.length - 1]];
    const hourly = [];
    for (let h = 0; h < 24; h++) hourly.push({ hour: h, pageViews: Number((todayRow && todayRow.hours || {})[String(h)]) || 0 });
    ...
    return {
      ...
      hasHours: hourly.some(x => x.pageViews > 0),
