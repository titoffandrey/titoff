...
/* Сколько секунд визит обязан прожить, чтобы перестать быть отказом. Пятнадцать —
 * та же мерка, что у Метрики: отказ это заход, где открыли одну страницу и ушли,
 * не задержавшись. Число измеримо только потому, что первый ping витрины идёт
 * через те же 15 секунд (см. `startAnalyticsHeartbeat` в `public/app.js`): с одним
 * минутным тактом любой визит короче минуты записывался бы нулём секунд. */
const BOUNCE_SECONDS = 15;
/* Через сколько «ушёл» становится «не вернулся». Сутки, потому что раньше это
 * не факт, а догадка: человек отложил телефон, доехал до дома и открыл витрину
 * снова. Следствие названо честно и видно в самом блоке: за сегодняшний день
 * таких почти нет — про сегодняшних ещё рано судить, и они считаются отдельной
 * строкой, а не молча записываются в вернувшихся. */
const GONE_AFTER_MS = 24 * 60 * 60 * 1000;
/* Стадии пути к заказу. Считаются РАЗ ЗА ВИЗИТ, а знаменателем воронки стоит
 * число визитов: стадия, посчитанная дважды, дала бы больше ста процентов.
 * Код — одна буква: он копится строкой в карточке посетителя и повторяется на
 * каждой из 10 000 карточек в одном файле. */
const STAGES = [
  ['p', 'product', path => path.startsWith('/product/')],
  ['c', 'checkout', path => path === '/checkout']
];
/* Куда уходят с витрины. Витрина ведёт наружу ровно в мессенджеры и к контактам
 * (Telegram и WhatsApp стоят и в меню, и в подвале, и на «О компании»), и
 * сколько по ним нажимают, не знал никто. У Метрики это `trackLinks`.
 *
 * Браузер присылает ОДИН ХОСТ, а не адрес: у ссылки WhatsApp в query лежит
 * готовая реплика, и тащить её на сервер незачем. Знакомые сервисы получают имя,
 * остальные остаются хостом — «пришли с какого-то сайта» это и есть правда о
 * нём, то же правило, что у источников перехода. */
const OUTBOUND = [
  [/^tel$/, 'Звонок'],
  [/^mailto$/, 'Почта'],
...
const MAX_CLICKS = 10;
/* Сколько подписей ухода помещается в ОДИН просмотр хронологии. Предел свой и
 * маленький, потому что платит за него файл метрики: просмотров у человека до
 * MAX_HITS, а карточек до MAX_VISITORS. Трёх хватает: больше трёх разных
 * мессенджеров с одной страницы не нажимают, а повторы не пишутся вовсе. */
const HIT_CLICKS = 3;
/* Скорость у живых покупателей. Весь проект вылизан под неё — спрайты,
 * снятие комментариев на отдаче, уменьшенные копии фото, — а измерений с
 * настоящих телефонов не было ни одного: PageSpeed меряет лабораторию, а не
 * покупателя из Ноябрьска на 4G.
 *
 * Пороги — официальные Web Vitals: «хорошо», «терпимо» и «плохо». CLS приходит в
 * тысячных долях: он дробный, а суточные счётчики целочисленные, и хранить
 * 0,0374 в общем счётчике значило бы копить ошибку округления. */
const SPEED = {
  lcp: { good: 2500, poor: 4000, max: 600000 },
  cls: { good: 100, poor: 250, max: 10000 },
  ttfb: { good: 800, poor: 1800, max: 600000 }
};

// Хост или схема, присланные браузером, — в подпись отчёта. Незнакомое остаётся
// хостом, пустое и мусор не считаются вовсе.
function outboundLabel(raw) {
  const value = clean(raw, 120).toLowerCase().replace(/^www\./, '');
  if (!value || !/^[a-z0-9.:-]+$/.test(value)) return '';
  for (const [re, label] of OUTBOUND) if (re.test(value)) return label;
...
function emptyDay() {
  return {
    ...
    /* Отказы за сутки. Считаются «наоборот»: новый визит сразу записывается в
     * отказы, а второй просмотр или прожитые 15 секунд его оттуда снимают.
     * Иначе пришлось бы ждать конца визита — а конца у визита нет, есть только
     * тишина, и наступает она уже следующими сутками. */
    bounces: 0,
    // Стадии пути к заказу, по разу за визит: {product, checkout}. Заявки уже
    // лежат в `orders`, а заходы — в `visits`, и вместе это вся воронка.
    funnel: {},
    /* Глубина прокрутки по страницам: сумма достигнутых максимумов и число
     * замеров. Среднее из них отвечает на вопрос, ради которого чаще всего и
     * включают вебвизор, — долистывают ли до кнопки покупки. */
    scrollSum: {}, scrollViews: {},
    // Нажатия по ссылкам, ведущим с витрины наружу: Telegram, WhatsApp, звонок,
    // почта. Считаются В НАЖАТИЯХ, а не в посетителях, — это действие, как и
    // открытие страницы, а не свойство человека.
    outbound: {},
    /* Скорость страниц у покупателей — плоские счётчики вида `lcp:good`,
     * `lcp:sum`, `lcp:n`. Плоские потому, что суточная сводка складывается
     * `addCount`, а он умеет только числа; вложенные объекты пришлось бы
     * обходить своим сложением в каждом месте, где сводка суммируется. */
    speed: {},
...
// `amount` не задан — прибавляем единицу: так этот счётчик и звали везде, пока
// у глубины прокрутки не появилась нужда прибавлять к странице сразу несколько.
function bumpLimited(object, key, maxKeys, amount) {
  ...
  addCount(object, safeKey, Number.isFinite(step) ? step : 1);
}

/* Значимый переход — тот, что вправе перебить источник посетителя.
 *
 * Источник записывался ОДИН РАЗ, при создании карточки, и жил так весь год:
 * человек, впервые пришедший из поиска и вернувшийся через неделю по рекламной
 * ссылке, до конца срока хранения числился «из поиска». Отчёт «Откуда приходят»
 * отвечал на вопрос «откуда его привели ВПЕРВЫЕ», а читают его как «откуда идёт
 * трафик». У Метрики на этот случай три модели атрибуции, и по умолчанию там
 * ровно это — последний значимый переход.
 *
 * Значим переход с чужого сайта. Внутренние переходы и прямые заходы источник не
 * перебивают: иначе первый же клик по своему меню стирал бы рекламу, из которой
 * человек пришёл, и вся витрина оказалась бы «прямыми заходами».
 */
function significantSource(value) {
  const s = clean(value, 200);
  return s && s !== 'Прямой заход' && s !== 'Внутренний переход' ? s : '';
}
...
class Analytics {
   ...
   *
   * ВТОРЫМ проходом переводятся уже записанные ЛАТИНСКИЕ названия. Перевод
   * применяется при записи карточки, поэтому всё, что попало в файл до того, как
   * словарь выучил название, остаётся английским навсегда: на боевых данных так
   * жила каждая пятая карточка, и «Sysert'» стоял в списке рядом с «Щёлково».
   * Второй проход этим и отличается от первого — ему не нужны ни база, ни IP,
   * ни сеть: он переводит строку, которая уже лежит в карточке.
   */
  backfillGeo() {
    if (!this.geoEnabled) return 0;
    let fixed = 0;
    if (GEOIP.info(this.dataDir)) {
    ...
    fixed += this.translateGeo();
    if (fixed) this.dirty = true;
    return fixed;
  }

  /* Перевод уже записанных названий. Идёт и по карточкам, и по КЭШУ адресов:
   * вернувшийся посетитель берёт город оттуда, и непереведённая запись вернула
   * бы латиницу обратно в свежую карточку.
   *
   * Код страны у старых карточек бывает пустым (их писал прежний внешний
   * сервис), а без него словарь молчит: «Moscow» бывает и в Айдахо. Поэтому код
   * добирается по названию страны — той же таблицей, что рисует флаги.
   */
  translateGeo() {
    let fixed = 0;
    const ru = (place) => {
      const city = String((place && place.city) || '').trim();
      if (!city || /[А-Яа-яЁё]/.test(city)) return false;
      const name = GEOIP.cityRu(city, CI.countryCode(place.country, place.countryCode));
      if (!name || name === city) return false;
      place.city = name;
      return true;
    };
    for (const v of this.data.visitors) if (ru(v)) fixed++;
    for (const key of Object.keys(this.data.geoCache || {})) if (ru(this.data.geoCache[key])) fixed++;
    return fixed;
  }
  ...
  load() {
    try {
      ...
      if (Number(parsed.version) < 3) {
        parsed.daily = mergeDaily(parsed.daily, emptyDay, ['visits', 'pageViews', 'orders', 'activeSeconds', 'bounces'],
          ['visitors', 'orderVisitors', 'pages', 'sources', 'devices', 'browsers', 'systems', 'campaigns', 'funnel', 'scrollSum', 'scrollViews', 'outbound', 'speed']);
  ...
  ensureVisitor(id, ctx, now) {
    ...
    // Источник — последний ЗНАЧИМЫЙ переход, а не первый в жизни карточки
    // (см. `significantSource`). В списке выше его быть не может: там правило
    // «непустое перебивает», а «Прямой заход» непустой и стирал бы рекламу.
    const significant = significantSource(ctx.source);
    if (significant) v.source = significant;
  ...
  daily(now) {
    ...
    for (const name of ['pages', 'sources', 'devices', 'browsers', 'systems', 'campaigns', 'hours', 'hourVisitors', 'funnel', 'scrollSum', 'scrollViews', 'outbound', 'speed']) if (!d[name] || typeof d[name] !== 'object') d[name] = {};
    d.activeSeconds = Number(d.activeSeconds) || 0;
    d.bounces = Number(d.bounces) || 0;
    return d;
  }
  ...
  recordPageView(input) {
    ...
    // Метку читаем ДО `ensureVisitor` — он её и перезапишет, а счётчик кампании
    // должен знать, была ли она у человека раньше (см. ниже).
    const known = this.findVisitor(input.id);
    const previousCampaign = (known && known.utmCampaign) || '';
    ...
    if (newSession) {
      v.visits++; v.lastSessionAt = now; d.visits++;
      // Новый заход начинается отказом и своими стадиями воронки. Снимут его
      // либо второй просмотр (строкой ниже), либо прожитые 15 секунд (heartbeat).
      d.bounces++; v.bounceDay = d.date; v.visitSeconds = 0; v.stages = '';
    } else this.unbounce(v);
    ...
    /* Стадии воронки — по разу за визит. Строка кодов лежит в карточке и
     * обнуляется вместе с началом захода: без неё десять открытых карточек
     * товара дали бы десять «смотрели товар» на один заход, и доля перевалила бы
     * за сто процентов. */
    for (const [code, stage, match] of STAGES) {
      if (!match(p) || String(v.stages || '').includes(code)) continue;
      v.stages = String(v.stages || '') + code;
      addCount(d.funnel, stage, 1);
    }
    ...
    /* Переход по UTM-метке засчитывается РАЗ ЗА ВИЗИТ и только когда метка правда
     * пришла в адресе (`ctx.utmCampaign`), а не когда она просто лежит в карточке.
     *
     * Прежняя строка била счётчик кампании на КАЖДЫЙ просмотр любой страницы, а
     * метка липнет к карточке на год: пришедший по кампании в январе в июне
     * продолжал добавлять ей переходы, не нажав ни одной ссылки. Кампании росли
     * сами по себе, и это был единственный блок отчёта, считавшийся по
     * просмотрам, когда все соседние считаются по посетителям. */
    if (ctx.utmCampaign && (newSession || ctx.utmCampaign !== previousCampaign)) {
      bumpLimited(d.campaigns, [v.utmSource, v.utmMedium, v.utmCampaign].filter(Boolean).join(' · '), 200);
    }
    this.dirty = true;
  ...
  /* Снять визит с отказов.
   *
   * Отметка `bounceDay` помнит, В КАКИЕ СУТКИ визит был записан в отказы, а не
   * просто «был»: заход, начавшийся в 23:58, доживает до следующего дня, и
   * вычесть его надо из вчерашней строки, а не из сегодняшней. Строки могло уже
   * не быть вовсе — её вымело сроком хранения; заводить её заново ради вычитания
   * незачем.
   */
  unbounce(v) {
    const date = v && v.bounceDay;
    if (!date) return false;
    delete v.bounceDay;
    const row = this.data.daily[date];
    if (row && Number(row.bounces) > 0) row.bounces--;
    this.dirty = true;
    return true;
  }

  /* Глубина прокрутки последнего просмотра.
   *
   * Пишется ДОСТИГНУТЫЙ МАКСИМУМ, а не каждый замер: ping приходит на одну
   * страницу по нескольку раз, и складывать их значило бы посчитать её пять раз
   * и завысить среднее. Поэтому в самом просмотре лежит уже достигнутая глубина
   * (`hit.d`), а в суточную сводку уходит только прирост — сумма остаётся суммой
   * максимумов, а счётчик замеров считает просмотры, а не пинги.
   *
   * Ключ берётся у ПРОСМОТРА, а не из тела запроса: `input.path` у heartbeat не
   * проходит через `metricPublicPath`, и по нему в сводку уехал бы любой адрес,
   * который вздумается прислать.
   */
  recordScroll(v, raw, now) {
    ...
    const was = Number(last.d) || 0;
    const reached = Math.min(100, depth);
    if (reached <= was) return false;
    last.d = reached;
    const d = this.daily(now);
    bumpLimited(d.scrollSum, last.p, 500, reached - was);
    if (!was) bumpLimited(d.scrollViews, last.p, 500);
    return true;
  }

  /* Нажатия по ссылкам, ведущим наружу.
   *
   * Приходят пачкой в уже уходящем ping'е — своего маршрута у них нет: витрина
   * шлёт их тем же запросом, что и глубину прокрутки. Подпись собирает
   * `outboundLabel()` из ХОСТА, а не из адреса, поэтому ни query, ни номер
   * телефона в счётчик не попадают.
   */
  recordClicks(v, list, now) {
    if (!Array.isArray(list) || !list.length) return 0;
    const d = this.daily(now);
    /* Подпись ложится ещё и в САМ ПРОСМОТР (`hit.o`) — тем же способом и по той
     * же причине, что глубина прокрутки: суточный счётчик отвечает «сколько раз
     * нажали», а хронология одного человека — «этот дошёл до контактов и ушёл в
     * Telegram». Второй вопрос задают, когда разбирают конкретную заявку, и
     * общий счётчик на него не отвечает никак.
     *
     * Просмотр берётся последний: нажатие уходит своим ping'ом сразу же (см.
     * `initOutboundClicks` на витрине), то есть с той страницы, что открыта. */
    const last = Array.isArray(v && v.hits) && v.hits.length ? v.hits[v.hits.length - 1] : null;
    let counted = 0;
    for (const raw of list.slice(0, MAX_CLICKS)) {
      const label = outboundLabel(raw);
      if (!label) continue;
      bumpLimited(d.outbound, label, 40);
      /* Предел на просмотр свой и маленький: подписи лежат в карточке
       * посетителя, а их до MAX_HITS штук на человека и до 10 000 человек в
       * файле. Повтор не пишется — «нажал Telegram» дважды подряд означает то
       * же самое, а строку в хронологии заняло бы две. */
      if (last && last.p) {
        if (!Array.isArray(last.o)) last.o = [];
        if (last.o.length < HIT_CLICKS && last.o[last.o.length - 1] !== label) last.o.push(label);
      }
      counted++;
    }
    if (counted) this.dirty = true;
    return counted;
  }

  /* Скорость страницы у живого покупателя.
   *
   * Пишутся три числа сразу: и оценка («хорошо / терпимо / плохо» по официальным
   * порогам Web Vitals), и сумма со счётчиком для среднего. Одного среднего мало:
   * оно прячет хвост, а именно хвост и есть та часть покупателей, у которых
   * витрина открывается медленно. Одних долей тоже мало — по ним не сказать,
   * насколько именно медленно.
   *
   * Невозможные значения отбрасываются: замер приходит из браузера, а часы и
   * `PerformanceObserver` там бывают какими угодно, и одна запись в миллион
   * секунд испортила бы среднее за сутки.
   */
  recordSpeed(raw, now) {
    if (!raw || typeof raw !== 'object') return false;
    const d = this.daily(now);
    let wrote = false;
    for (const [name, limits] of Object.entries(SPEED)) {
      const value = Math.round(Number(raw[name]));
      if (!Number.isFinite(value) || value < 0 || value > limits.max) continue;
      addCount(d.speed, name + ':' + (value <= limits.good ? 'good' : value <= limits.poor ? 'ok' : 'poor'), 1);
      addCount(d.speed, name + ':sum', value);
      addCount(d.speed, name + ':n', 1);
      wrote = true;
    }
  ...
  heartbeat(input) {
    ...
    if (elapsed >= 5000 && elapsed <= 90000) {
      ...
      /* Секунды этого захода — отдельно от общих: отказ снимает не сам факт
       * ping'а, а прожитые в визите BOUNCE_SECONDS. Разница настоящая: ping
       * уходит и при уходе со вкладки, то есть может прийти на пятой секунде, —
       * и без этого счёта пятисекундный заход перестал бы быть отказом. */
      v.visitSeconds = (Number(v.visitSeconds) || 0) + seconds;
      if (v.visitSeconds >= BOUNCE_SECONDS) this.unbounce(v);
    }
    this.recordScroll(v, input.scroll, now);
    this.recordClicks(v, input.clicks, now);
    this.recordSpeed(input.speed, now);
    v.lastSeen = now;
  ...
  totalsFor(dates) {
    ...
    const out = { visits: 0, pageViews: 0, orders: 0, activeSeconds: 0, bounces: 0, visitors: 0 };
    const unique = new Set();
    for (const d of Object.values(this.data.daily)) {
      if (!wanted.has(d.date)) continue;
      for (const key of ['visits', 'pageViews', 'orders', 'activeSeconds', 'bounces']) out[key] += Number(d[key]) || 0;
  ...
  pulse() {
    ...
    const seen = new Set(today ? today.visitors || [] : []);
    const countries = {};
    for (const visitor of this.data.visitors) {
      if (visitor.clientConfirmed && seen.has(visitor.id)) addCount(countries, countryCodeOf(visitor), 1);
    }
    return {
      online: this.onlineCount(),
      countries: Object.entries(countries).map(([code, value]) => ({ code, value })).sort((a, b) => b.value - a.value),
  ...
  /* ГДЕ ОБРЫВАЕТСЯ ВИЗИТ: последняя страница захода и «вернулся ли он потом».
   *
   * Воронка говорит, на каком ЭТАПЕ теряются покупатели, и не говорит, на какой
   * странице. Разница денежная: «387 открыли товар, до оформления дошли 63»
   * не отвечает, оборвалось это на карточке конкретного Mac, на доставке или на
   * странице оплаты.
   *
   * СЧИТАЕТСЯ ПО ХРОНОЛОГИИ КАРТОЧЕК, А НЕ СВОИМ СЧЁТЧИКОМ, и это осознанно.
   * Счётчик отвечал бы только про будущее — со дня выкатки, — а хронология уже
   * лежит на диске за всё время хранения, то есть ответ есть сразу. Заодно не
   * появляется ни нового поля, ни новой записи на горячем пути: конец визита
   * вообще нельзя записать в момент ухода, его видно только задним числом.
   *
   * Просмотры считаются В ТЕХ ЖЕ ВИЗИТАХ, а не берутся из суточной сводки:
   * два источника с разными потолками дали бы долю выходов больше ста
   * процентов на редкой странице. Цена честности названа: у карточки последние
   * MAX_HITS просмотров, поэтому у самых ходовых посетителей начало истории
   * вытеснено, и числа здесь — про визиты, а не про все открытия страницы.
   */
  /* Проход по карточкам запоминается на SNAPSHOT_TTL — тот же приём и та же
   * причина, что у пересчёта часов (`_hourlyMemo`). Замер на боевых данных: у
   * «Сегодня» это 21 мс из 62, а он КЭШИРОВАНИЮ НЕ ПОДЛЕЖИТ и перезапрашивается
   * живым обновлением каждые 1,2 с, пока на витрине хоть кто-то есть, — то есть
   * пятая часть отчёта пересчитывалась бы по кругу за счёт покупателей.
   *
   * Десять секунд свежести здесь — с запасом: «где обрывается визит» смотрят,
   * разбирая неделю, а не секунду. Ключ несёт границы периода и длину списка
   * карточек; за десять секунд у уже известного человека мог добавиться
   * просмотр, и его этот ключ не заметит — цена названа и она того стоит. */
  exitPointsCached(wanted, oldestDate, now) {
    const key = oldestDate + '|' + wanted.size + '|' + this.data.visitors.length;
    const memo = this._exitsMemo;
    if (memo && memo.key === key && now - memo.at < SNAPSHOT_TTL) return memo.value;
    const value = this.exitPoints(wanted, oldestDate, now);
    this._exitsMemo = { key, at: now, value };
    return value;
  }

  exitPoints(wanted, oldestDate, now) {
    const views = {}; const exits = {}; const gone = {};
    let visits = 0; let lost = 0; let fresh = 0;
    /* Начало периода — ЧИСЛОМ, а не ключом даты, и это не микрооптимизация.
     * Первый заход отсеивал карточки сравнением `dayKey(v.lastSeen) < oldest`,
     * то есть звал `new Date().toISOString()` восемь с половиной тысяч раз на
     * каждый показ отчёта: «Сегодня» стоило 6,1 мс вместо 0,6. А он не
     * кэшируется вовсе (см. `cachedSnapshot`) и перезапрашивается живым
     * обновлением каждые 1,2 с, пока на витрине хоть кто-то есть, — эти
     * миллисекунды платил бы покупатель. Смещение то же, что у `dayKey`:
     * сутки магазина московские. */
    const fromMs = Date.parse(oldestDate + 'T00:00:00Z') - 3 * 60 * 60 * 1000;
    for (const v of this.data.visitors) {
      if (!v || !Array.isArray(v.hits) || !v.hits.length) continue;
      // Визит не может кончиться позже последней отметки человека, поэтому
      // карточки, у которых её нет в периоде, не разбираются вовсе. На
      // «Сегодня» это отсекает почти всё: девять сотен карточек из восьми тысяч.
      if (!(Number(v.lastSeen || 0) >= fromMs)) continue;
      const sessions = sessionsOf(v);           // свежие сверху
      const quiet = now - Number(v.lastSeen || 0) >= GONE_AFTER_MS;
      for (let i = 0; i < sessions.length; i++) {
        const s = sessions[i];
        if (!wanted.has(dayKey(s.lastAt))) continue;
        visits++;
        for (const h of s.hits) if (h && h.p) addCount(views, h.p, 1);
        const last = s.hits[s.hits.length - 1];
        if (!last || !last.p) continue;
        addCount(exits, last.p, 1);
        /* «Не вернулся» — только про ПОСЛЕДНИЙ визит человека (он же первый в
         * списке) и только когда с тех пор прошли сутки. Раньше это не факт, а
         * догадка: человек вышел покурить и вернётся через десять минут.
         * Слишком свежие считаются отдельно — иначе их молчаливо записали бы в
         * «вернулись», и за сегодняшний день блок врал бы уверенно. */
        if (i !== 0) continue;
        if (quiet) { addCount(gone, last.p, 1); lost++; } else fresh++;
      }
    }
    const rows = Object.entries(exits).sort((a, b) => b[1] - a[1]).slice(0, 12).map(([label, value]) => {
      const seen = Number(views[label]) || 0;
      return {
        label, value,
        gone: Number(gone[label]) || 0,
        // Доля выходов: из скольких просмотров страницы она стала последней.
        rate: seen ? Math.round((value / seen) * 1000) / 10 : null
      };
    });
    return { rows, visits, lost, fresh };
  }
  ...
  snapshot(options) {
    ...
    const total = { visits: 0, pageViews: 0, orders: 0, activeSeconds: 0, bounces: 0 };
    const unique = new Set(); const orderVisitors = new Set();
    const pages = {}; const campaigns = {}; const funnel = {};
    const scrollSum = {}; const scrollViews = {}; const outbound = {}; const speed = {};
    for (const d of rows) {
      ...
      for (const key of ['visits', 'pageViews', 'orders', 'activeSeconds', 'bounces']) { const n = Number(d[key]) || 0; x[key] += n; total[key] += n; }
      ...
      for (const [k, n] of Object.entries(d.funnel || {})) addCount(funnel, k, n);
      for (const [k, n] of Object.entries(d.scrollSum || {})) addCount(scrollSum, k, n);
      for (const [k, n] of Object.entries(d.scrollViews || {})) addCount(scrollViews, k, n);
      for (const [k, n] of Object.entries(d.outbound || {})) addCount(outbound, k, n);
      for (const [k, n] of Object.entries(d.speed || {})) addCount(speed, k, n);
    }
    ...
    /* Путь к заказу. Знаменателем стоят ЗАХОДЫ, а не посетители: стадии
     * считаются раз за визит, и делить их на число людей значило бы мерить одно
     * другим — доля вылезла бы за сто процентов у всякого, кто заходил дважды.
     * Последняя ступень — заявки: они и есть то, ради чего витрина работает.
     *
     * `step` — доля от ПРЕДЫДУЩЕЙ ступени: воронку читают именно так («из тех,
     * кто открыл карточку, до оформления дошёл каждый пятый»), а доля от заходов
     * у нижних ступеней быстро становится неразличимо малой. Считается число, а
     * не строка: проценты в этом проекте печатает панель, и вторая их запись
     * разошлась бы с первой. */
    /* СТУПЕНИ МОЛОЖЕ САМОГО ОТЧЁТА, и молчать об этом нельзя.
     *
     * Счётчик стадий появился позже счётчика заходов, поэтому на длинном
     * периоде в числителе стоят двое суток, а в знаменателе тридцать. На бою
     * это выглядело так: «Заходы 9730 → Смотрели товар 497 → ушли здесь 9233»,
     * то есть панель уверенно называла потерю, которой не было. Само оно
     * пройдёт, когда счётчик догонит срок хранения, а до тех пор дата
     * приписывается прямо в панели — числа тогда читаются правильно.
     *
     * Признак — первая дата периода с непустыми ступенями: ключ `funnel` при
     * загрузке дописывается всем суткам подряд (`normalizeDay`), и отличить по
     * его наличию «счётчика не было» от «за день никто не дошёл» уже нельзя. */
    let funnelSince = '';
    for (const d of rows) {
      if (!Object.keys(d.funnel || {}).length) continue;
      if (!funnelSince || d.date < funnelSince) funnelSince = d.date;
    }
    if (funnelSince === wantedDates[0]) funnelSince = '';
    const funnelRows = [
      { label: 'Заходы', value: total.visits },
      { label: 'Смотрели товар', value: Number(funnel.product) || 0 },
      { label: 'Дошли до оформления', value: Number(funnel.checkout) || 0 },
      { label: 'Оформили заявку', value: total.orders }
    ];
    /* `lost` — сколько человек НЕ перешло на эту ступень с предыдущей. Число
     * выводится из соседей и своего счётчика не требует, но без него воронку
     * приходится вычитать в уме, а вопрос к ней задают именно про потерю: не
     * «сколько дошло», а «сколько отвалилось и где». Отрицательным оно не
     * бывает по построению (ступень считается раз за визит), но `Math.max`
     * стоит: у ступеней разные потолки ключей, и порченый файл не должен
     * рисовать в отчёте минус. */
    for (let i = 1; i < funnelRows.length; i++) {
      const before = funnelRows[i - 1].value;
      funnelRows[i].step = before ? Math.round((funnelRows[i].value / before) * 1000) / 10 : null;
      funnelRows[i].lost = Math.max(0, before - funnelRows[i].value);
    }
    // Средняя глубина прокрутки страницы — рядом с числом её открытий. Замеров
    // может не быть вовсе (страницу открыли до появления счётчика), и тогда
    // строка остаётся без подписи, а не получает честный на вид ноль.
    const withScroll = row => {
      const views = Number(scrollViews[row.label]) || 0;
      return views ? Object.assign(row, { scroll: Math.round((Number(scrollSum[row.label]) || 0) / views) }) : row;
    };
    ...
    const report = {
      ...
      /* Отказ — заход, где открыли одну страницу и ушли, не прожив 15 секунд:
       * та же мерка, что у Метрики. Считается от ЗАХОДОВ, а не от посетителей —
       * отказывается визит, а не человек. */
      bounces: total.bounces, bounceRate: total.visits ? Math.round((total.bounces / total.visits) * 1000) / 10 : 0,
      funnel: funnelRows,
      funnelSince,
      exits: this.exitPointsCached(wanted, wantedDates[0], now),
      ...
      /* Скорость отдаётся ЧИСЛАМИ: среднее и три доли по каждому показателю.
       * Как это назвать по-русски, в какой единице показать и где поставить
       * порог «хорошо», решает панель — единицы измерения в этом проекте печатает
       * она, и вторая их запись разошлась бы с первой. Показателя нет в отчёте
       * вовсе, пока по нему не пришло ни одного замера: строка «0 мс» у
       * браузера, который так не умеет, была бы неправдой. */
      speed: Object.keys(SPEED).map(name => {
        const count = Number(speed[name + ':n']) || 0;
        if (!count) return null;
        return {
          name, count,
          average: Math.round((Number(speed[name + ':sum']) || 0) / count),
          good: Number(speed[name + ':good']) || 0,
          ok: Number(speed[name + ':ok']) || 0,
          poor: Number(speed[name + ':poor']) || 0
        };
      }).filter(Boolean),
      daily, pages: top(pages).map(withScroll), sources: top(sources), devices: top(devices), browsers: top(browsers), systems: top(systems), campaigns: top(campaigns), locations: top(locations), regions: ranked(regions),
...
module.exports = {
  Analytics, COOKIE_NAME, OPT_OUT_COOKIE, POLICY_VERSION, ONLINE_MS, SESSION_MS, MAX_HITS, RETENTION_DAYS, VISITORS_PER_PAGE, BOUNCE_SECONDS,
  cookieValue, normalizeIp, isPrivateIp, deviceFromUa, clientDetails, safePath, sourceFromReferrer, significantSource, outboundLabel, dayKey, hourKey, familyOf, sessionsOf
};
