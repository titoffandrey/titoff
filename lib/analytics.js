...
class Analytics {
  ...
  findByIp(rawIp, siteId) {
    const ip = normalizeIp(rawIp);
    if (!ip) return [];
    return this.data.visitors
      .filter(v => v.ip === ip && (!siteId || v.siteId === siteId))
...
module.exports = {
  Analytics, COOKIE_NAME, OPT_OUT_COOKIE, POLICY_VERSION, ONLINE_MS, SESSION_MS, MAX_HITS,
  cookieValue, normalizeIp, isPrivateIp, deviceFromUa, clientDetails, safePath, sourceFromReferrer, dayKey, sessionsOf
};
