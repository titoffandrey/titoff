 ...
 */
async function cleanVideo(dir, file) {
  const bin = await ffmpeg();
  if (!bin) return false;
  const input = path.join(dir, file);
  ...
  try {
    // -map_metadata -1 снимает теги контейнера, но имена дорожек живут отдельно
    // и переживают его: без явной очистки в файле остаётся «VideoHandler»
    // с vendor_id, то есть подпись того, чем файл собран.
...
module.exports = { buildPreviews, makePoster, cleanVideo, posterName, isVideo, ffmpeg };
