...
const INITIALS = 'АБВГДЕЖЗИКЛМНОПРСТУФХШЮЯ'.split('');
const CITIES = [
...
function personaFor(random) {
  ...
  return { gender, age, author: `${pick(names, random)}${initial}` };
}
...
function addHumanNoise(text, random) {
  ...
  if (random() < 0.28) out = out.charAt(0).toLowerCase() + out.slice(1);
  if (random() < 0.05) out = out.toLowerCase();
  ...
  if (random() < 0.18) {
    ...
    const words = out.split(' ').map((word, idx) => ({ word, idx })).filter(x => x.word.length >= 7);
    if (words.length) {
