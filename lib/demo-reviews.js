...
const CASUAL_TWEAKS = [
  ...
  ['магазин', 'магаз'], ['телефон', 'тел'], ['компьютер', 'комп'],
  ['ноутбук', 'ноут']
];
...
function alignPhrase(phrase, persona) {
  ...
  const pairs = [
    ...
    // формы из коротких отзывов
    ['Ждал', 'Ждала'], ['ждал', 'ждала'], ['Взял', 'Взяла'], ['брал', 'брала'],
    ['заказал', 'заказала'], ['забрал', 'забрала'], ['проверил', 'проверила'],
    ['намучился', 'намучилась'], ['хотел', 'хотела'], ['Написал', 'Написала'],
    ['Оформил', 'Оформила'], ['Спросил', 'Спросила']
  ];
  let out = phrase;
...
function addTail(text, rating, random) {
  ...
  const chance = rating === 5 ? 0.22 : rating === 4 ? 0.12 : 0.08;
  if (random() < chance) out += ' ' + pick(rating >= 4 ? EMOJI_GOOD : EMOJI_BAD, random);
  return out;
}

function shortText(rating, persona, random) {
  const phrase = alignPhrase(pick(SHORT_TEXTS[rating] || SHORT_TEXTS[5], random), persona);
  return addHumanNoise(phrase, random);
}
