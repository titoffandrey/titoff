...
class App {
  ...
  async handle(req, res) {
    ...
    const originOk = sameOriginPost(req, isHttps, forwardedHost || req.headers.host);
    // Некоторые privacy-браузеры скрывают Origin именно у отправки формы. Входы
    // безопасно пропускаем: они защищены отдельным rate limit и не используют
    // существующую авторизацию. Внутри панели разрешаем скрытый Origin только с
    // подписанной авторизованной SameSite-cookie; явный cross-site не пропускаем.
    const loginPost = req.method === 'POST' && pathname === '/admin/login';
    const fetchSite = String(req.headers['sec-fetch-site'] || '').toLowerCase();
    const origin = String(req.headers.origin || '').trim();
    // Формам входа разрешаем только действительно скрытый Origin. Явный чужой
    // Origin раньше проходил из-за общего исключения loginPost и позволял login
    // CSRF: сторонняя страница могла попытаться заменить сессию пользователя.
    const loginPostWithoutOrigin = loginPost && fetchSite !== 'cross-site' && (!origin || origin === 'null');
    const privateSession = typeof session.admin === 'string';
    const privatePostWithoutOrigin = privateSession && fetchSite !== 'cross-site' && (!origin || origin === 'null');
    if (!originOk && !loginPostWithoutOrigin && !privatePostWithoutOrigin) {
      req.resume();
