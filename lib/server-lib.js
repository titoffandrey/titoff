...
// CSP разрешает собственные ресурсы и один чужой хост — счётчик Яндекс Метрики (см. ниже); 'unsafe-inline'
// нужен для встроенных обработчиков и стилей самого приложения. Внешний запрос из
// браузера один — счётчик Яндекс Метрики, и только у тех, кому сервер его отдал.
const SECURITY_HEADERS = {
   ...
   */
  /* ЧУЖОЙ ХОСТ В ПОЛИТИКЕ РОВНО ОДИН — `mc.yandex.ru`, и только в трёх
   * директивах: скрипт счётчика Яндекс Метрики, его запросы и пиксель. Стоит он
   * здесь ради Директа (цели конверсий), а КОМУ отдаётся сам скрипт, решает
   * сервер (lib/yandex-metrika.js): без номера счётчика в настройках и для
   * всех, кому он не положен, в разметке нет ни строки про Яндекс, и открытая
   * директива ничего не загружает. `frame-src` и `child-src` не открыты
   * намеренно: они нужны вебвизору, а он выключен — содержимое полей оформления
   * Яндексу не уходит.
   *
   * Прежний `frame-src` с яндексовым доменом стоял ради карты на «О компании»;
   * карта теперь своя — тайлы отдаёт наш сервер, и они проходят обычным
   * `img-src 'self'`. Любой другой чужой хост сюда — это снова показывать
   * посетителя стороннему сервису, и решаться так же осознанно. */
  'Content-Security-Policy': "default-src 'self'; img-src 'self' data: blob: https://mc.yandex.ru; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' https://mc.yandex.ru; connect-src 'self' https://mc.yandex.ru; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'",
...
function cachedStatic(file, mtime, size) {
  ...
  // Кэшируем саму подготовку: одновременные первые запросы делят одну работу.
  // Сжатие выполняется вне event loop — каталог и оформление не ждут Brotli
  // большого app.js. При переполнении убираем самый давно запрошенный файл,
  // а не весь кэш, вызывавший повторную обработку каждой страницы.
  e = fs.promises.readFile(file).then(async source => {
    const raw = MINIFY.forFile(file, source);
    if (raw.length <= COMPRESS_MIN) return { raw, gz: null, br: null };
    const pack = (fn, opts) => new Promise(resolve => fn(raw, opts, (error, data) => resolve(error ? null : data)));
    const [gz, br] = await Promise.all([
      pack(zlib.gzip, { level: 9 }),
      pack(zlib.brotliCompress, { params: { [zlib.constants.BROTLI_PARAM_QUALITY]: 6 } })
    ]);
    return { raw, gz, br };
  }).catch(error => {
    if (staticCache.get(key) === e) staticCache.delete(key);
    throw error;
  });
...
// If-None-Match допускает список и слабое сравнение для GET/HEAD.
...
// При изменении файла нельзя склеивать сохранённое начало со свежим хвостом.
// Наш ETag слабый и для If-Range непригоден; дату принимаем по Last-Modified.
function rangeIsCurrent(value, mtime) {
  if (!value) return true;
  if (/^(?:W\/)?"/.test(String(value).trim())) return false;
  const date = Date.parse(String(value));
  return Number.isFinite(date) && Math.floor(mtime / 1000) * 1000 === date;
}
...
function streamFile(file, res, options) {
  ...
  // Файл могли удалить/заменить между stat и open. Необработанный error
  // раньше завершал весь Node-процесс, а обрыв клиента оставлял чтение файла.
...
/* Ключи JSON-тела, которые ломают приведение к строке, и потому до маршрута не
 * доезжают.
 *
 * `String(x)` у объекта с полем `toString: 1` не возвращает «[object Object]»,
 * а БРОСАЕТ TypeError: собственное свойство перебивает Object.prototype, а
 * числом объект не приводится и через valueOf. Любой маршрут, печатающий чужое
 * значение в строку, падал на этом 500-й вместо честного отказа — проверено на
 * `/api/cart` и `/api/order`, то есть на самом денежном пути витрины:
 * `{"items":[{"id":{"toString":1}}]}` роняло оба.
 *
 * Ловить это в каждом маршруте по отдельности нельзя — забытое место и было бы
 * дырой, а мест таких десятки. Поэтому чистим на входе, один раз: законного
 * поля с такими именами в JSON нет и быть не может — ключи там всегда наши
 * (`items`, `id`, `qty`, `address`…), а не пользовательские. `__proto__` заодно:
 * собственным свойством он прототип не меняет, но сбивает с толку любое слияние
 * объектов ниже по течению.
 *
 * ФОРМ ЭТО НЕ КАСАЕТСЯ, и это не забывчивость: у urlencoded и multipart
 * значение поля ВСЕГДА строка, а строка `String()` не ломает. Поле формы с
 * именем `toString` там остаётся обычной строкой (см. `own()` выше и тест
 * «поле формы с именем из прототипа остаётся строкой»).
 */
const POISON_KEYS = new Set(['__proto__', 'toString', 'valueOf']);
const dropPoison = (key, value) => POISON_KEYS.has(key) ? undefined : value;

/* Reviver стоит вшестеро дороже обычного разбора (замер: 8 → 48 мкс на теле
 * корзины в 2 КБ), а сработать ему почти никогда не на чем. Поэтому сперва
 * смотрим на сам текст: нет в нём такого ключа — разбираем как раньше.
 *
 * Обратный слэш в условии обязателен и он же делает проверку герметичной: имя
 * ключа в JSON можно записать не буквами, а escape-последовательностями (первая
 * буква `toString` — это ещё и «t»), и поиск по литералу такой ключ бы не
 * увидел. Нет во всём теле ни одного слэша, значит все имена записаны как есть,
 * и литерального совпадения достаточно. Есть — идём длинным путём, то есть
 * безопасным.
 *
 * Заодно это снимает усиление нагрузки: тело здесь бывает до MAX_BODY, и
 * платить за reviver шестикратным разбором мегабайтов ради ключа, которого там
 * нет, не за что.
 */
const POISON_RX = /\\|"(?:__proto__|toString|valueOf)"/;
function parseJsonBody(text) {
  return POISON_RX.test(text) ? JSON.parse(text, dropPoison) : JSON.parse(text);
}
...
function parseMultipart(buf, boundary) {
  // Отсутствующий/повреждённый разделитель нельзя превращать в пустую форму:
  // частичный POST настроек иначе способен стереть поля, которых «не было».
  if (!/^[0-9A-Za-z'()+_,.\/:=? -]{1,70}$/.test(boundary) || boundary.endsWith(' ')) {
    throw new Error('invalid_multipart_boundary');
  }
  const body = {}, files = [];
  const delim = Buffer.from('--' + boundary);
  const findBoundary = from => {
    let at = buf.indexOf(delim, from);
    while (at !== -1) {
      // Разделитель — отдельная строка, а не произвольная последовательность
      // внутри фото/текста. Проверяем и начало, и хвост: --boundary-extra
      // остаётся содержимым поля даже после CRLF.
      if (at === 0 || (buf[at - 2] === 0x0d && buf[at - 1] === 0x0a)) {
        let end = at + delim.length;
        const closing = buf[end] === 0x2d && buf[end + 1] === 0x2d;
        if (closing) end += 2;
        while (buf[end] === 0x20 || buf[end] === 0x09) end++;
        if (buf[end] === 0x0d && buf[end + 1] === 0x0a) return { at, after: end + 2, closing };
        if (closing && end === buf.length) return { at, after: end, closing };
      }
      at = buf.indexOf(delim, at + delim.length);
    }
    return null;
  };
  let current = findBoundary(0);
  if (!current) throw new Error('invalid_multipart_body');
  while (!current.closing) {
    const next = findBoundary(current.after);
    if (!next) throw new Error('incomplete_multipart_body');
    processPart(buf.subarray(current.after, next.at - 2), body, files);
    current = next;
  }
  return { body, files };
}
...
function processPart(part, body, files) {
  ...
  // Параметры могут идти в любом порядке. Поиск просто name= попадал внутрь
  // filename=, когда имя файла было указано раньше имени поля.
  const nameM = /(?:^|;)[ \t]*name="([^"]*)"/i.exec(disposition[1]);
  const fileM = /(?:^|;)[ \t]*filename="([^"]*)"/i.exec(disposition[1]);
...
class App {
  ...
  /* Проверка ДО статики и маршрутов: вернула `true` — запрос закрыт, дальше не
   * идём. Ради одного заблокированного посетителя (см. lib/visitor-rules.js):
   * «тотальный блок» означает, что ему не отдаётся ничего — ни страница, ни
   * картинка, ни ответ API, — а на каждый маршрут по отдельности такую проверку
   * не навесить: забытый маршрут и был бы дырой в блоке.
   *
   * Стоит она ПОСЛЕ разбора сессии: живая сессия панели блок не ловит (иначе
   * владелец, заблокировавший свой же адрес, остался бы без входа), и ПЕРЕД
   * чтением тела: с заблокированным нам и разговаривать незачем. */
  before(fn) { this.beforeRoutes = typeof fn === 'function' ? fn : null; }
  ...
  async handle(req, res) {
    ...
    // Закрыт ли этот посетитель вовсе (см. `before`). Раньше статики: у блока
    // нет исключений даже для картинок.
    if (this.beforeRoutes && this.beforeRoutes(req, res)) return;
    ...
    for (const s of this.statics) {
      if (pathname.startsWith(s.prefix + '/') || pathname === s.prefix) {
        ...
        if (st && st.isFile()) {
          ...
          if (matchesEtag(req.headers['if-none-match'], etag)) { res.writeHead(304, headers); return res.end(); }
          if (compressible) {
            const c = await cachedStatic(file, st.mtimeMs, st.size);
            if (res.destroyed || res.writableEnded) return;
          ...
          const range = rangeIsCurrent(req.headers['if-range'], st.mtimeMs)
            ? parseRange(req.headers.range, st.size) : null;
          if (range === 'invalid') {
          ...
          if (range) {
            ...
            return streamFile(file, res, { start: range.start, end: range.end });
          }
    ...
    if (req.method === 'POST') {
      try {
        ...
        if (ctype.includes('application/json')) {
          req.body = raw.length ? parseJsonBody(raw.toString('utf8')) : {};
