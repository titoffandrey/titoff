...
class App {
  ...
  async handle(req, res) {
    ...
    const loginPost = req.method === 'POST' && (pathname === '/owner/login' || pathname === '/admin/login');
    const fetchSite = String(req.headers['sec-fetch-site'] || '').toLowerCase();
    const origin = String(req.headers.origin || '').trim();
    const privateSession = typeof session.owner === 'string'
      || (typeof session.siteAdmin === 'string' && typeof session.siteAdminAuth === 'string');
    const privatePostWithoutOrigin = privateSession && fetchSite !== 'cross-site' && (!origin || origin === 'null');
    if (!originOk && !loginPost && !privatePostWithoutOrigin) {
      req.resume();
      ...
      if (pathname.startsWith(s.prefix + '/') || pathname === s.prefix) {
        ...
        if (st && st.isFile()) {
          ...
          if (/text|javascript|json|svg/.test(type)) {
            ...
            if (packed) {
              if (req.method === 'HEAD') { res.writeHead(200, Object.assign({ 'Content-Type': type, 'Content-Encoding': encoding, Vary: 'Accept-Encoding', 'Content-Length': packed.length }, headers)); return res.end(); }
              return sendRaw(res, 200, Object.assign({ 'Content-Type': type, 'Content-Encoding': encoding, Vary: 'Accept-Encoding', 'Content-Length': packed.length }, headers), packed);
            }
