...
// Браузер всегда присылает Origin для fetch и межсайтовой отправки формы. Сверка
// хоста защищает все POST-маршруты (включая админские) от CSRF с соседнего
// поддомена; запросы сервер-сервер без Origin остаются совместимыми.
function sameOriginPost(req, isHttps) {
  ...
  try {
    ...
    return source.protocol === (isHttps ? 'https:' : 'http:')
      && source.host.toLowerCase() === String(req.headers.host || '').trim().toLowerCase();
...
class App {
  ...
  async handle(req, res) {
    ...
    const isHttps = forwardedProto === 'https' || !!(req.socket && req.socket.encrypted);

    if (!sameOriginPost(req, isHttps)) {
      req.resume();
