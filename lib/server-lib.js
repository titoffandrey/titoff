...
const SECURITY_HEADERS = {
   ...
   */
  /* Чужих хостов в политике нет ВОВСЕ. Строка `frame-src` с яндексовым доменом
   * стояла здесь ради карты на «О компании»; карта теперь своя — тайлы отдаёт
   * наш сервер, и они проходят обычным `img-src 'self'`. Возвращать чужой хост
   * сюда — значит снова показывать посетителя стороннему сервису. */
  'Content-Security-Policy': "default-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'",
