...
function posterFile(dir, name) {
  /* Имя чистится от всего, что могло бы увести из каталога, — но БУКВЫ здесь
   * законны: в имени стоит расширение `.webp`. Прежний фильтр разрешал только
   * шестнадцатеричные `a-f`, то есть выгрызал из расширения `w` и `p`, и файл
   * оказывался с именем `….eb`. Хуже того, по такому расширению ImageMagick
   * писал PNG вместо WebP — карта весила втрое больше положенного.
   *
   * Поэтому здесь не чистка, а СВЕРКА С ОБРАЗЦОМ: имя постера мы формируем сами
   * (`posterName`), и всё, что на него не похоже, именем не считается вовсе —
   * путь тогда указывает на сам каталог, чтение падает, маршрут отвечает 502. */
  const safe = /^[a-f0-9]{8,32}\.webp$/.test(String(name || '')) ? String(name) : '';
  return path.join(dir, 'map-tiles', 'poster', safe);
}
