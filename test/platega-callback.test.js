...
function fixture(t, patch = {}) {
  ...
  fs.writeFileSync(path.join(dir, 'orders.json'), JSON.stringify([{ id: 'synthetic-order', total: 1000, createdAt: Date.now(),
...
function checkoutHarness(t) {
  ...
  const s = { ...settings, plategaEnabled: true };
  const source = fs.readFileSync(path.join(__dirname, '../server.js'), 'utf8');
  ...
  const start = vm.runInNewContext(helpers + route + '\nstartPaymentRoute', {
    ...
    payContext: async (_, o) => ({ live: null, amount: o.total, currency: 'RUB', methods: [PAY.describe('ONLINE_PAYMENT')] }),
  ...
  const request = async (key = 'a'.repeat(32)) => {
    let response;
    await start({ body: { orderId, method: 'ONLINE_PAYMENT', requestId: key } }, {
      json: (body, status = 200) => { response = { body, status }; }
    });
    return response;
  };
  return { db, orderId, request };
}
...
test('Platega: одновременные и повторные нажатия создают один счёт', async t => {
  ...
  stubFetch(t, async (url, init) => {
    ...
    assert.equal(data.paymentDetails.amount, 1000);
    ...
    return new Response(JSON.stringify({ transactionId: invoiceId, status: 'PENDING', url: link, expiresIn: '00:15:00', rate: 1 }));
  });
...
test('Platega: частичный ответ с ID и плохой ссылкой не разрешает второй POST', async t => {
  ...
  stubFetch(t, async () => {
    calls++;
    return new Response(JSON.stringify({ transactionId: invoiceId, status: 'PENDING', url: 'javascript:alert(1)', expiresIn: '00:15:00', rate: 1 }));
  });
  assert.equal((await request()).status, 502);
