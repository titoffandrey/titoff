...
test('срезанный краем кадр отличается от целого снимка квадратного товара', () => {
  ...
  // Требование «заполняет кадр по обеим сторонам» такие кадры пропускало.
  assert.equal(images.looksCropped({ w: 802, h: 1104 }, [0.48, 0.55, 0.97, 0.97], S), true, 'вертикальная макро-панель');
