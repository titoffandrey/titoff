...
(function () {
  ...
  /* ПРИВЕТСТВИЕ — НЕПРОЧИТАННОЕ СООБЩЕНИЕ. У того, кто окно ни разу не
   * открывал, на кнопке чата и на вкладке «Чат» нижней панели стоит значок
   * «1»: в окне его и правда ждёт приветствие консультанта, оно лежит в
   * разметке. Первое открытие гасит значок как любое непрочитанное и
   * запоминается в браузере (`chat_seen_v1`) — в следующий заход приветствие
   * уже прочитано, и звать некуда. Покупатель с начатым диалогом окно видел,
   * ему значок ставит сервер по настоящим непрочитанным. Анимаций у вкладки
   * нет вовсе — только это число; ни таймера, ни запроса. */
  ...
  function bubble(role, text, by, at, photos, local, reply) {
    ...
    // Обычная реплика всегда получит единый узел времени/галочки. Класс нужен
    // раскладке: приветствие из серверной разметки метаданных не имеет и не
    // должно зря резервировать под них место в конце текста.
    row.classList.add('chat-has-meta');
    ...
    /* Узел текста создаётся ВСЕГДА, даже когда текста нет.
     *
     * Он нужен пустым: ответ консультанта печатается прямо в него
     * (`delta()` ниже), и рождается такая реплика без единого слова. Когда узел
     * заводился только при непустом тексте, дописывать было некуда — пузырь
     * оставался с одним именем и временем, а ответ не появлялся вовсе.
     *
     * Реплике из одних снимков он не мешает: пустой `.chat-text` убирается
     * стилем (`:empty`), поэтому лишнего отступа под фото не остаётся. */
    var body = document.createElement('span');
    body.className = 'chat-text';
    if (text) fillText(body, text);
    row.appendChild(body);
    /* Время и галочка — ОДНИМ узлом в правом нижнем углу пузыря, как в
     * WhatsApp.
     *
     * Порознь они и разъезжались: это были два соседних inline-элемента, и
     * строка рвалась между ними — время оставалось в конце текста, а галочка
     * уезжала на следующую строку одна. В обёртке с `nowrap` разорвать их
     * нечем.
     *
     * Узел позиционируется от пузыря, а место под него оставляет псевдоэлемент
     * в конце текста. `float` здесь не годится: его фактическая позиция зависит
     * от места появления в потоке, и на некоторых длинах он проваливал время
     * на отдельную строку при пустом месте справа.
     *
     * Обёртка создаётся ВСЕГДА, даже пустая: у растущей реплики времени ещё
     * нет — оно приедет вместе с готовой (событие `done`), и дописывать его
     * надо в уже готовое место. Покупатель возвращается в чат через час и через
     * день, и без времени непонятно, ответили ему только что или вчера. */
    var meta = document.createElement('span');
    meta.className = 'chat-meta';
    if (at) meta.appendChild(stampNode(at));
    // Галочка стоит только у своих реплик: у чужих отвечать ею не на что.
    if (role === 'user') meta.appendChild(tickNode());
  ...
  function endDelta(message) {
    ...
    // Время дописывается в готовую обёртку — ту, что завёл `bubble()`: рядом с
    // ним живёт галочка, и вдвоём они держатся одной строкой.
    var meta = state.live.querySelector('.chat-meta');
    if (meta && !meta.querySelector('.chat-at')) {
  ...
  /* Реплика магазина дошла до покупателя. ЕДИНСТВЕННОЕ место, куда сходятся оба
   * её пути — обычный `append` и дописанный ответ консультанта (`endDelta`), —
   * и потому единственное, где решается, что при этом происходит.
   *
   * ЗВУК ЗВУЧИТ В ЛЮБОМ СЛУЧАЕ, открыто окно или нет. Сначала он стоял только у
   * закрытого («покупатель и так смотрит в ленту»), но это неправда: вкладка с
   * открытым чатом чаще всего не та, в которой человек сидит, — он ушёл читать
   * карточку товара или вовсе в другое окно. Так ведут себя и мессенджеры:
   * звук у входящего есть всегда, а от того, открыт ли разговор, зависит
   ...
   *
   * БЕЗ ОГОВОРКИ ПРО СЕНСОРНЫЙ ЭКРАН, и это осознанная замена прежнего правила.
   * Раньше на `pointer:coarse` Enter оставался обычным переводом строки: расчёт
   * был на то, что экранная клавиатура телефона и так показывает «ввод», а не
   * «отправить». На деле сенсорным считается и ноутбук с тач-экраном, и окно
   * браузера в режиме телефона — то есть Enter молча переставал работать там,
   * где его нажимают клавишей. Отправка кнопкой рядом никуда не делась, поэтому
