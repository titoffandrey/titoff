...
(function (root, factory) {
   ...
   *
   * `track` здесь обязателен: ссылку на отслеживание консультанту дают сами
   * факты о заказе (`[посылка](/track/<ключ>)` в lib/chat-prompt.js), и пока
   * пути не было в списке, покупатель и менеджер видели в ленте голую
   * разметку со скобками — ровно то, что этот файл и прячет (14 сентября
   * 2026, боевой диалог «когда отправят?»). `about` — страница магазина,
   * на неё консультант отправляет за реквизитами и адресом точки. */
  var PATHS = 'product|checkout|warranty|returns|privacy|track|about';
