...
(function () {
  ...
  function noteShow(html) {
    ...
    /* ЗВУК — ВСЕГДА, даже когда карточку показывать незачем.
     *
     * Это две разные вещи, и путать их не надо. Карточка отвечает на вопрос
     * «что случилось» — про открытый диалог она бесполезна, реплика и так
     * приезжает в ленту у менеджера на глазах. Звук отвечает на другой вопрос:
     * «случилось хоть что-то» — а вкладка с открытым диалогом чаще всего не та,
     * в которой он сейчас сидит. Молчать в ней значило бы пропускать ровно те
     * сообщения, ради которых её и держат открытой.
     *
     * Файл общий с витриной (`public/chat-sound.js`): двух представлений о том,
