...
(function (root, factory) {
  ...
  /* Сколько номеров вытаскиваем из одной фразы (`find`) и какая группа цифр
   * вообще считается кандидатом на номер. Десять — чтобы шестизначный номер
   * заказа, стоящий в той же реплике, номером телефона не притворялся. */
  var FIND_MAX = 3;
  var FIND_MIN_DIGITS = 10;
  ...
  /* Номера, названные внутри произвольного текста: «мой номер 8 996 996-29-15».
   *
   * Разбирает их тот же `check`, что и поле оформления, и это здесь главное:
   * номер, который витрина записала в заказ, обязан узнаваться и в реплике
   * чата — в любой привычной записи, с «+7», с «8» и без разделителей вовсе.
   * Своей таблицы кодов и своей проверки длины у чата поэтому нет.
   *
   * Кандидатов набираем ДВУМЯ проходами, и второй обязателен: пробел — законный
   * разделитель внутри номера, поэтому в одной фразе «8 996 996-29-15 заказ
   * 149021» кусок захватывает и номер заказа, а такой кандидат отбраковывается
   * по лишним цифрам. Поэтому кроме куска целиком пробуем каждую непрерывную
   * группу цифр из него.
   *
   * Перевод строки в кандидат не входит (`[  ]`, а не `\s`): номер и
   * следующая за ним фраза стоят на разных строках чаще, чем в одной.
   */
  function find(text) {
    ...
    var seen = {};
    var m;
    while (out.length < FIND_MAX && (m = re.exec(s))) {
      var tries = [m[0]];
      var parts = m[0].split(/[^\d+]+/);
      for (var i = 0; i < parts.length; i++) {
        if (onlyDigits(parts[i]).length >= FIND_MIN_DIGITS) tries.push(parts[i]);
      }
      for (var j = 0; j < tries.length && out.length < FIND_MAX; j++) {
        var res = check(tries[j]);
        if (!res.ok || seen[res.e164]) continue;
        seen[res.e164] = 1;
        out.push(res.e164);
      }
    }
  ...
  return {
    ...
    check: check, store: store, find: find, attach: attach
  };
});
