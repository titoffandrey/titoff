...
(function () {
  ...
  /* ====== Просмотр медиа отзыва ======
     ...
     без скрипта клик открывает файл, как и раньше. */
  var lightbox = null;
  ...
  function initReviewLightbox() {
    // Список отзывов перерисовывается при листании и смене сортировки, поэтому
    // слушаем документ, а не сами карточки.
    document.addEventListener('click', function (e) {
      var link = e.target.closest && e.target.closest('.rv-item');
      if (!link) return;
      // Открыть в новой вкладке средней кнопкой или с Cmd/Ctrl — обычное дело.
      if (e.metaKey || e.ctrlKey || e.shiftKey || e.button !== 0) return;
      var box = link.closest('.review-media');
      if (!box) return;
      e.preventDefault();
      var nodes = box.querySelectorAll('.rv-item');
      var items = [], index = 0;
    ...
    // Листание пальцем. Порог заметно больше вертикального сдвига, иначе
    // обычная прокрутка страницы перелистывала бы кадры.
    var x0 = null, y0 = null;
