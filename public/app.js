...
(function () {
   ...
   * неоплаченный заказ покупатель вправе отменить, и тогда он
    ...
    items.innerHTML = '<div class="co-block">'
      ...
      + Cart.items.map(function (i) {
        ...
        return '<article class="co-item' + (out ? ' co-item-out' : '') + '">'
          /* Плашки «Промоакция» здесь нет намеренно, хотя на карточке каталога
  ...
  function startPayment(orderId, payNow, total) {
    ...
    fetch('/api/pay/start', {
      ...
      .then(function (d) {
        if (d && d.terminal === 'order_cancelled') { location.href = fallback; return; }
        ...
        /* На ссылку кассы уводит НАША страница оплаты (`?go=1`, см. autoOpen в
         * pay.js), а не оформление напрямую: иначе вернувшийся из банка
         * покупатель попадал «назад» на оформление с пустой корзиной, а
         * страницы оплаты с «платёж получен» у него в истории не было вовсе. */
        var go = function () { location.href = hosted ? fallback + '?go=1' : fallback; };
   ...
   * товары или поля формы.
   *
   * ЖИВЁТ ОН РОВНО СТОЛЬКО, СКОЛЬКО САМ ЗАКАЗ — полчаса (`R.PAY_WINDOW` на
   * сервере). Раньше стояли сутки, и это переживало заказ в сорок восемь раз:
   * покупатель, вернувшийся через час к той же корзине, повторял ключ мёртвого
   * заказа. Сервер такой повтор больше не принимает (`R.payExpired` в
   * `/api/order`), но и присылать его незачем: восстанавливать потерянный ответ
   * дольше, чем по заказу можно заплатить, нечего.
   */
  var ORDER_REQUEST_KEY = 'checkout_order_request_v1';
  var ORDER_REQUEST_TTL = 30 * 60 * 1000;
