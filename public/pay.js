...
(function () {
  ...
  /* Печать подтверждения оплаты. Всё, что нужно, делает сам браузер: «сохранить
   * в PDF» у него — это та же печать, а как документ ляжет на лист, задаёт
   * `@media print` в styles.css. Кнопка рисуется сервером и без скрипта просто
   * ничего не делает — распечатать страницу в этом случае можно из меню
   * браузера, поэтому прятать её незачем. */
  page.addEventListener('click', function (e) {
    var btn = e.target.closest ? e.target.closest('[data-print]') : null;
    if (btn) window.print();
  });
