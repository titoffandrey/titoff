/* УВЕДОМЛЕНИЕ О СВОЁМ ДЕЙСТВИИ ПОКАЗЫВАЕТСЯ ОДИН РАЗ.
 *
 * «Сохранено», «Товар удалён», «Касса ещё ждёт оплату» — это ответ на то, что
 * человек только что сделал. Приезжает он параметром `?flash=`: действие идёт
 * POST'ом, а дальше редирект на ту же страницу (иначе обновление повторяло бы
 * само действие), и сообщение передать больше нечем.
 *
 * Но параметр остаётся в адресной строке, и сервер честно рисует по нему
 * карточку КАЖДЫЙ раз, когда страницу открывают заново. Поэтому F5 на списке
 * заказов доставал «Сохранено» спустя часы после самого сохранения — а
 * уведомление о действии, которого сейчас не было, читается как сбой панели.
 * Достаточно вернуться на такую страницу из истории, чтобы увидеть его снова.
 *
 * Убираем параметр сразу после показа — тем же приёмом, каким гасится повтор
 * звука отправки в чате (`?sent=1` ниже). Остальные параметры остаются: номер
 * страницы, вкладка, режим правки и сортировка описывают, ЧТО показано, и
 * обновление переживать обязаны.
 *
 * ПОЧЕМУ НЕ ОДНОРАЗОВОЕ СООБЩЕНИЕ В СЕССИИ, как это обычно делают. Оно
 * читается и стирается на сервере, то есть первым же запросом — а панель
 * обновляет себя живьём и перезапрашивает страницы каждые пару секунд, из
 * ЛЮБОЙ открытой вкладки. Фоновый запрос соседней вкладки съедал бы чужое
 * «Сохранено» раньше, чем его увидит человек, и цена ошибки там выше: не
 * лишняя карточка, а неизвестность, сохранилось ли вообще.
 *
 * Без скриптов повтор остаётся; текст по-прежнему доступен в HTML.
 */
(function () {
  'use strict';
  ...
  // `history.state` сохраняем: живое обновление кладёт туда своё при переходах
  // внутри отчёта метрики, и обнулить его значило бы сломать «назад».
...
(function () {
  ...
  if (field) {
    ...
    /* ENTER ОТПРАВЛЯЕТ, SHIFT+ENTER ПЕРЕНОСИТ СТРОКУ — ровно так же, как в окне
     * покупателя (`public/chat.js`) и во всех мессенджерах. Правило одно на обе
     * стороны разговора; тут его пришлось написать отдельно только потому, что
     * у покупателя реплика уходит своим каналом, а у менеджера — формой.
     *
     * Без оговорки про сенсорный экран, по той же причине, что и на витрине:
     * сенсорным считается и ноутбук с тач-экраном, и окно браузера в режиме
     * телефона, — то есть Enter молча переставал бы работать там, где его
     * нажимают клавишей. Кнопка отправки рядом никуда не делась.
     *
     * Просим отправиться САМУ ФОРМУ, а не шлём её мимо обработчика: в нём
     * лежит и разбор пустого поля («Напишите сообщение или приложите фото»), и
     * отправка выбранных снимков через fetch. `requestSubmit()` знают не все
     * версии Safari, доходящие до панели, — там нажимаем кнопку, она тоже
     * `type="submit"` и поднимает то же событие. */
    field.addEventListener('keydown', function (e) {
      if (e.key !== 'Enter' || e.shiftKey || e.ctrlKey || e.metaKey || e.altKey) return;
      // Пока открыт список автодополнения IME, Enter выбирает подсказку в нём,
      // а не отправляет реплику: иначе половина слова уезжала бы покупателю.
      if (e.isComposing || e.keyCode === 229) return;
      e.preventDefault();
      if (!dock) return;
      if (dock.requestSubmit) dock.requestSubmit();
      else if (sendButton) sendButton.click();
    });
  }
...
(function () {
  function say(btn) {
    if (window.StoreToast) {
      window.StoreToast.show('Скопировано', { type: 'success', id: 'admin-copy' });
      return;
    }
